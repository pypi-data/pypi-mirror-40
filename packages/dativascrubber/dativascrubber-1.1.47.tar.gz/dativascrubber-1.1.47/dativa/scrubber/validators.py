        ...
        if is_numeric_dtype(df[dirty_col]):
            ...
            if not self.fix_decimal_places:
                ...
                write_report_dp = df.loc[
                    (df[clean_col] == df[dirty_col].round(self.decimal_places))
                    # & (df[clean_col] != df[dirty_col]), # uncomment for entries which lose information upon rounding
                    , [dirty_col, clean_col]]
        else:
...
class SessionValidator(BaseValidator):
    ...
    def _remove_zero_length_sessions(self, df, cols):
        ...
        if rows.shape[0] > 0:
            self.report_writer.log_history(
                self.rule_type,
                cols["original_start"],
                rows[[cols["key"], cols["original_start"], cols["original_end"]]],
                HistoryType.REMOVED,
                'Removed zero length')
    ...
    def run(self, df):
        ...
        if self.overlaps_option == SessionOverlapsType.TRUNCATE_START or self.overlaps_option == SessionOverlapsType.TRUNCATE_END:
            self._run_truncation(df, cols)

        if self.gaps_option == SessionGapsType.EXTEND_START or self.gaps_option == SessionGapsType.EXTEND_END:
