...
class Instrument():
    ...
    def __init__(self, port, slaveaddress, mode=MODE_RTU, close_after_each_call=CLOSE_PORT_AFTER_EACH_CALL):
        ...
        self.serial = serial.Serial(port=None, baudrate=BAUDRATE, parity=PARITY, bytesize=BYTESIZE, stopbits=STOPBITS, timeout=TIMEOUT)
        ...
        self.close_port_after_each_call = close_after_each_call
    ...
    ######################################
    def custom_command(self, subfunc, value):
        self.serial.reset_input_buffer()
        self.serial.reset_output_buffer()
        functioncode = CMD_CUSTOM
        # _checkInt(subfunc, minvalue=0, maxvalue=1, description='Invalid subfunction received ({})'.format(subfunc))
        # Build request to send to device
        request = bytearray()
        request.append(_numToOneByteNum(self.address))
        request.append(_numToOneByteNum(functioncode))
        request.extend(_numToTwoByteNum(subfunc))
        request.extend(_numToTwoByteNum(value))
        # request.extend(_calculateCrcNum(request.decode('utf-8')))
        request.extend(_calculateCrcNum("".join(map(chr, request))))
        response = ''
    ...
    def read_flash(self, address, bytesToRead):
        self.serial.reset_input_buffer()
        self.serial.reset_output_buffer()
        functioncode = CMD_FLASH
        # Build request to send to device
        request = bytearray()
        request.append(_numToOneByteNum(self.address))
        request.append(_numToOneByteNum(functioncode))
        request.extend(_numToTwoByteNum( (address >> 16) ))
        request.extend(_numToTwoByteNum( (int('0x0000FFFF', 16) & address) ))
        # request.extend(_numToTwoByteNum(address))
        request.extend(_numToTwoByteNum(bytesToRead))
        request.extend(_calculateCrcNum("".join(map(chr, request))))
        rqstring = ''.join(map(chr,request))
        # Send request to device
        # print(request, rqstring)
        # Communicate
        response = self._communicate(rqstring, bytesToRead+6)
        # Verify CRC
        NUMBER_OF_CRC_BYTES                    = 2
        receivedChecksum = response[-NUMBER_OF_CRC_BYTES:]
        responseWithoutChecksum = response[0 : len(response) - NUMBER_OF_CRC_BYTES]
        calculatedChecksum = _calculateCrcString(responseWithoutChecksum)
        if receivedChecksum != calculatedChecksum:
            template = 'Checksum error: {!r} instead of {!r} . The response is: {!r}{}'
            text = template.format(
                    receivedChecksum,
                    calculatedChecksum,
                    response[0:20],
                    ('...' if len(response) > 20 else ''))
    ...
    def synchronize(self, timestamp):
        self.serial.reset_input_buffer()
        self.serial.reset_output_buffer()
        functioncode = CMD_SYNC
        # Build request to send to device
        request = bytearray()
        request.append(_numToOneByteNum(self.address))
        request.append(_numToOneByteNum(functioncode))
        request.extend(struct.pack('<d', timestamp))
        request.extend(_calculateCrcNum("".join(map(chr, request))))
        rqstring = ''.join(map(chr,request))
        # Communicate
        response = self._communicate(rqstring, 12)
        return response
    ...
    def _communicate(self, request, number_of_bytes_to_read):
        ...
        if sys.version_info[0] > 2:
            request = bytes(request, 'latin1')  # Convert types to make it Python3 compatible
...
####################################
# Num to formatted num conversions #
####################################

def _numToOneByteNum(inputvalue):
    _checkInt(inputvalue, minvalue=0, maxvalue=0xFF)
    return inputvalue

def _numToTwoByteNum(inputvalue):
    _checkInt(inputvalue, minvalue=0, maxvalue=0xFFFF)
    return [ (0xFF00 & inputvalue) >> 8, 0x00FF & inputvalue ]
...
def _unpack(formatstring, packed):
    ...
    if sys.version_info[0] > 2:
        packed = bytes(packed, 'latin1')  # Convert types to make it Python3 compatible
...
def _calculateCrcNum(inputstring):
    ...
    return [_numToOneByteNum(0x00FF & register), _numToOneByteNum((0xFF00 & register) >> 8)]
