...
class Config(object):
    def __init__(self, config_map):
        ...
        self.manager = ConfigManager(
            [ConfigOSEnv(),
             ...
             ConfigIniEnv([
                 os.environ.get("COMET_INI"),
                 "./.comet.ini",
                 "~/.comet.ini",
             ]),
            ],
            doc=('See https://comet.ml/docs/python-sdk/config/ for more ' +
    ...
    def display(self, display_all=False):
        ...
        n += 1
        for path in ["./.env", "~/.comet.ini", "./.comet.ini"]:
        ...
        print("Settings:\n")
        last_section = None
        for section, setting in sorted([key.rsplit(".", 1)
            ...
            if value is None:
                ...
                value = repr(value)
            default_value = repr(self.config_map[key].get("default", None))
    ...
    def save(self, directory="./", save_all=False, **kwargs):
        """
        Save the settings to .comet.ini (default) or
        other path/filename. Defaults are commented out.

        Args:
            directory: the path to save the .comet.ini config settings.
            save_all: save unset variables with defaults too
            kwargs: key=value pairs to save
        """
        directory = os.path.expanduser(directory)
        filename = os.path.abspath(os.path.join(directory, ".comet.ini"))
        ...
        with io.open(filename, "w", encoding='utf-8') as ini_file:
            ...
            ini_file.write(six.u("## For help see https://www.comet.ml/docs/python-sdk/config/\n"))
            last_section = None
            for section, setting in sorted([key.rsplit(".", 1)
                                            for key in self.config_map.keys()]):
                key = "%s.%s" % (section, setting)
                ...
                if key_arg in kwargs:
                    ...
                    value = self[key]
                if len(kwargs) != 0:
                    raise ValueError("'%s' is not a valid config key" % list(kwargs.keys())[0])
                if "." in section:
                    section = section.replace(".", "_")
                if value is None:
                    value = "..."
