...
FASTAI_ALREADY_IMPORTED_MSG = (
    "Please import comet before importing any fastai modules"
)
...
def learner_constructor(experiment, original, *args, **kwargs):
    ...
    try:
        ...
        LOGGER.info("Failed to run Learner.__init__ logger")
...
def callback_on_epoch_begin(experiment, original, retval, *args, **kwargs):
    if experiment.auto_metric_logging:
        try:
            ...
            LOGGER.info("Failed to run CallbackHander.on_epoch_begin logger")
    return retval
...
def callback_on_epoch_end(experiment, original, retval, *args, **kwargs):
    if experiment.auto_metric_logging:
        try:
            ...
            cbhandler = args[0]
            metric_names = ["val_loss"] + [f.__name__ for f
                                           in cbhandler.state_dict["metrics"]]
            metric_values = cbhandler.state_dict["last_metrics"]
            for name, value in zip(metric_names, metric_values):
                experiment.log_metric(name, value, step=experiment.curr_step)
        except Exception:
            LOGGER.info("Failed to run CallbackHander.on_epoch_end logger")
    return retval

def patch(module_finder):
    ## For testing:
    if "fastai" in sys.modules:
        LOGGER.warning(FASTAI_ALREADY_IMPORTED_MSG)
    ## For Learner constructor:
    module_finder.register_after("fastai.basic_train",
                                 "Learner.__init__",
                                  learner_constructor)
    module_finder.register_after("fastai.callback",
                                 "CallbackHandler.on_epoch_end",
                                 callback_on_epoch_end)
    module_finder.register_after("fastai.callback",
                                 "CallbackHandler.on_epoch_begin",
                                 callback_on_epoch_begin)

check_module("fastai", FASTAI_ALREADY_IMPORTED_MSG)
