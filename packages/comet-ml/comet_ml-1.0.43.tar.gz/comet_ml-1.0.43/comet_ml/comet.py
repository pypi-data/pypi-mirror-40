...
from comet_ml._logging import EXPERIMENT_LIVE, STREAMER_WAIT_FOR_FINISH_FAILED
...
class Streamer(threading.Thread):
    ...
    def __init__(
        ...
        self.beat_duration = beat_duration / 1000.0  # We get milliseconds
    ...
    def _check_heartbeat(self):
        ...
        if next_beat < now:
            ...
            self.beat_duration = new_beat_duration / 1000.0  # We get milliseconds
            self.last_beat = time.time()
    ...
    def wait_for_finish(self):
        ...
        if not self._is_msg_queue_empty() or not self._is_file_upload_done():
            ...
            LOGGER.error(
                STREAMER_WAIT_FOR_FINISH_FAILED, remaining, len(remaining_upload)
            )
            return False
...
class OfflineStreamer(threading.Thread):
    """
    This class extends threading.Thread and provides a simple concurrent queue
    and an async service that pulls data from the queue and sends it to the server.
    """
    ...
    def getn(self, n):
        ...
        """
        try:
            msg = self.messages.get(timeout=1)  # block until at least 1
        except Empty:
            LOGGER.debug("No message in queue, timeout")
            return None
