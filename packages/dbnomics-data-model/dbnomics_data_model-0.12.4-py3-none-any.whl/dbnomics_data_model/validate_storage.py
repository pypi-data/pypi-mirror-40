...
def main():
    ...
    for dataset_index, dataset_dir in enumerate(storage.iter_datasets_dirs(sort_by_dir_name=True), start=1):
        ...
        _, series_errors = validate_series(dataset_dir, dataset_series)
...
def format_error(error):
    ...
    sio.write("  Error message: " + error["message"] + "\n")
...
def validate_dataset(dataset_dir):
    ...
    if error_code not in args.ignore_errors and dataset_json["code"] != dataset_code:
        errors.append({
            "error_code": error_code,
            "message": "Dataset code from dataset.json ({}) is different than the directory name ({})".format(dataset_json["code"], dataset_code),
            "provider_code": provider_code,
...
def validate_provider_directory_name(error_code, provider_code=None):
    ...
    if args.storage_dir.absolute().name not in valid_directories_names:
        return {
            "error_code": error_code,
            "message": "Directory name must be named one of {}".format(valid_directories_names),
            "provider_code": provider_code,
            "location": ".",
        }
    return None
...
def validate_observations(provider_code, dataset_code, series_code, location, observations):
    ...
    if error_code not in args.ignore_errors:
        if get(0, header, default=None) != "PERIOD":
            errors.append({
                "error_code": error_code,
                "message": "First column of header is not 'PERIOD': {!r}".format(header),
                "provider_code": provider_code,
                "dataset_code": dataset_code,
        ...
        if get(1, header, default=None) != "VALUE":
            errors.append({
                "error_code": error_code,
                "message": "Second column of header is not 'VALUE': {!r}".format(header),
                "provider_code": provider_code,
                "dataset_code": dataset_code,
                "series_code": series_code,
                "location": location,
    ...
    for line_number, observation in enumerate(rows, start=2):
        ...
        if error_code_invalid not in args.ignore_errors and period_format is None:
            errors.append({
                ...
                "message": "Period format is invalid ({})".format(period),
                "provider_code": provider_code,
                "dataset_code": dataset_code,
                "series_code": series_code,
                "location": location,
            ...
            errors.append({
                ...
                "message": "Period format ({}) is different than previous row ({})".format(period_format, previous_period_format),
                "provider_code": provider_code,
                "dataset_code": dataset_code,
                "series_code": series_code,
                "location": location,
        ...
        if error_code not in args.ignore_errors:
            ...
            if value_float != NOT_AVAILABLE and not isinstance(value_float, float):
                errors.append({
                    "error_code": error_code,
                    "message": "Observation value is invalid ({})".format(value),
                    "provider_code": provider_code,
                    "dataset_code": dataset_code,
                    "series_code": series_code,
                    "location": location,
...
def validate_series(dataset_dir, dataset_series):
    ...
    for index, series_json in enumerate(series_iterator):
        ...
        error_code = "duplicate-series-code"
