...
class DataHelper(S3DataHelper):
    ...
    def get_demand_prediction_result_s3folder(self, dp_job_key, dt_hour_str, update_time_str=''):
        '''
        get latest (before 'update_time_str') demand prediction result in given 'dt_hour_str'
        '''
        job_path = JobPathHelper().get_dirs_by_demand_prediction_key(dp_job_key, dt_hour_str)
    ...
    def download_demand_prediction_result_by_job_key(
        self, 
        dp_job_key, 
        dt_hour_str, 
        update_time_str=''):
        ...
        if s3folder is None:
            if update_time_str=='':
                self.logger.warn('No demand_prediction_result in %s for %s' % (dt_hour_str, dp_job_key))
            else:
                self.logger.warn('No demand_prediction_result in %s for %s (before %s)' % (dt_hour_str, dp_job_key, update_time_str))
            return []

        job_path = JobPathHelper().get_dirs_by_demand_prediction_key(
            dp_job_key,
            dt_hour_str, 
            update_time_str=s3folder.split('/')[-1])
    ...
    def upload_demand_prediction_result_by_job_key(
        self, 
        dp_job_key,
        dt_hour_str, 
        local_dir):
