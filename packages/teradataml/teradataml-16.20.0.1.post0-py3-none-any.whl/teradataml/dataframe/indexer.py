...
class _LocationIndexer():
    ...
    def _get_sort_index(self, key, df):
        ...
        if isinstance(key, tdmldf.sql._SQLColumnExpression):
            ...
            if key.start is None and key.stop is None:
                ...
                return tdmldf.dataframe.DataFrame._from_node(sel_nodeid, df._metaexpr, df._index_label)
    ...
    def _get_integer_sort_index(self, key, df):
        ...
        if isinstance(key, slice):
            ...
            return tdmldf.dataframe.DataFrame._from_node(sel2_nodeid, df._metaexpr, df._index_label)
        else:
            key_list = key
            if not isinstance(key, list):
                key_list = [key_list]
