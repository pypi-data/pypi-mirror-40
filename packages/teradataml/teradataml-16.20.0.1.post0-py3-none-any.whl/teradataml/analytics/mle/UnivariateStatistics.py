...
class UnivariateStatistics:
    ...
    def __init__(self,
        ...
        self.__arg_info_matrix.append(["target_columns", self.target_columns, True, "str"])
        self.__arg_info_matrix.append(["exclude_columns", self.exclude_columns, True, "str"])
        self.__arg_info_matrix.append(["statistics", self.statistics, True, "str"])
    ...
    def __validate(self) :
        ...
        self.__awu._validate_dataframe_has_argument_columns(self.target_columns, "target_columns", self.data, "data")
        
        self.__awu._validate_input_columns_not_empty(self.exclude_columns, "exclude_columns")
        self.__awu._validate_dataframe_has_argument_columns(self.exclude_columns, "exclude_columns", self.data, "data")
    ...
    def __execute(self) :
        ...
        if self.statistics is None:
            ...
            self.moments_table   = self.__awu._create_data_set_object(df_input=UtilFuncs._extract_table_name(self.__moments_table_temp_tablename),source_type="table",database_name=UtilFuncs._extract_db_name(self.__moments_table_temp_tablename))
            self.basic_table     = "INFO: 'basic_table' output DataFrame is not created, when 'statistics' is set to 'MOMENTS'."
            self.quantiles_table = "INFO: 'quantiles_table' output DataFrame is not created, when 'statistics' is set to 'MOMENTS'."
        elif self.statistics.upper() == "BASIC":
            self.moments_table   = "INFO: 'moments_table' output DataFrame is not created, when 'statistics' is set to 'BASIC'."
            self.basic_table     = self.__awu._create_data_set_object(df_input=UtilFuncs._extract_table_name(self.__basic_table_temp_tablename),source_type="table",database_name=UtilFuncs._extract_db_name(self.__basic_table_temp_tablename))
            self.quantiles_table = "INFO: 'quantiles_table' output DataFrame is not created, when 'statistics' is set to 'BASIC'."
        else: # if self.statistics.upper() == "QUANTILES"
            self.moments_table   = "INFO: 'moments_table' output DataFrame is not created, when 'statistics' is set to 'QUANTILES'."
            self.basic_table     = "INFO: 'basic_table' output DataFrame is not created, when 'statistics' is set to 'QUANTILES'."
            self.quantiles_table = self.__awu._create_data_set_object(df_input=UtilFuncs._extract_table_name(self.__quantiles_table_temp_tablename),source_type="table",database_name=UtilFuncs._extract_db_name(self.__quantiles_table_temp_tablename))
