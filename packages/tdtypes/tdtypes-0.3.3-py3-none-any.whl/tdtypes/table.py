...
class DBObj:
	...
	def _xmldef(self):
		...
		try:
			fn = self.get_xmldef or (sqlcsr.csr.get_xmldef if sqlcsr.csr else None)
			if not fn:
			...
			raise NotFoundError('{} {!s} does not exist'.format(self._xml_type, self)) from err
...
class DBObjPat:
	...
	def search_cond(self, db='DatabaseName', tb='TableName'):
		...
		return self.search_predicate(db, self.sch, defval='DATABASE') + " AND " + self.search_predicate(tb, self.name)
...
class DBObjFinder:
	...
	def match_col(self):
		...
		return """CASE {}
            ELSE NULL
       END AS MATCHED_PAT""".format('\n            '.join('WHEN {} THEN {}'.format(pat, e) for e, pat in enumerate(self.patterns)))
