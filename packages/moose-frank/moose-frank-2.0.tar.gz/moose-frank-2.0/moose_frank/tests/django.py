...
def get_local_time_with_delta(delta):
    return timezone.localtime(timezone.now() + delta).time().replace(
        microsecond=0)
...
class LoginRequiredTestCaseMixin:
    ...
    def setUp(self):
        ...
        assert self.client.login(
            username='user@mediamoose.nl', password='test')
...
class SuperuserRequiredTestCaseMixin(LoginRequiredTestCaseMixin):
    ...
    def setUp(self):
        super().setUp()
        assert self.client.login(
            username='superuser@mediamoose.nl', password='test')
...
class TemplateViewTestCaseMixin:
    ...
    def test_url_name(self):
        ...
        assert reverse(self.url_name) == self.get_url()
...
class DetailViewTestCaseMixin(URLTestCaseMixin):
    ...
    def setUp(self):
        ...
        url = self.get_url()
        if not url:
            self.fail("URL is not defined in {}.".format(
...
class ListViewTestCaseMixin(URLTestCaseMixin):
    ...
    def test_list_view(self):
        ...
        if not hasattr(self, 'items'):
            self.fail('`{}` has no items, because `setUpTestData` was '
    ...
    def test_pages(self):
        ...
        for index, page_nr in enumerate(page_range, start=1):
            ...
            assert all(
                page_object in self.items for page_object in
                page_objects_set), \
                "Page '{}' contains unexpected objects.\n " \
                "{!r} \n not in \n {!r}".format(
                    page_nr, page_objects_set, self.items)

            if page_objects_set.intersection(seen):
                self.fail("Page '{}' contains objects already seen on a"
...
class CreateViewTestCaseMixin(URLTestCaseMixin):
    ...
    def get_save_and_back_url(self):
        if not self.save_and_back_url:
            return None
        return self.save_and_back_url.format(
            **self.get_url_formatting_kwargs())
    ...
    def test_invalid_submit(self):
        ...
        if self.test_invalid_message:
            assert 'messages' in resp.context
            assert len(resp.context['messages']) == 1
            assert any(message.level_tag == 'error' for message in
                       resp.context['messages'])
...
class UpdateViewTestCaseMixin(DetailViewTestCaseMixin):
    ...
    def test_invalid_submit(self):
        ...
        assert 'form' in resp.context
        assert not resp.context['form'].is_valid(), \
            'Form {!r} should be invalid.'.format(resp.context['form'])
...
class DeleteViewTestCaseMixin(DetailViewTestCaseMixin):
    ...
    def check_deleted_object(self):
        pk = self.item.pk
        assert not self.item.__class__.objects.filter(pk=pk).exists(), \
            "Object `{!r}` was not deleted.".format(self.item)
