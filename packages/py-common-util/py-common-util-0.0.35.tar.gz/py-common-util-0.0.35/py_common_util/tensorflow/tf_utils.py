...
class TFUtils:
    ...
    def device(device="gpu", device_index=0):
        ...
        if TFUtils.cuda_is_available(show_info=False) and device == 'gpu':
            return tf.device(tf.DeviceSpec(device_type="GPU", device_index=device_index))
        else:
            return tf.device(tf.DeviceSpec(device_type="CPU", device_index=device_index))
    ...
    def tf_get_length1(sequence):
        ...
        length = tf.reduce_sum(used, axis=0, keepdims=False)
        length = tf.cast(length, tf.int32)
        return length
