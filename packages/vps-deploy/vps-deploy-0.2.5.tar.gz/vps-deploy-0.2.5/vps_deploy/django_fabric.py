...
def prepare_django(fail_level='WARNING'):
    with cd(env.project_dir):
        ...
        run('{env.virtualenv}/bin/python manage.py check --deploy {fail_level_arg} --settings={env.settings}'.format(
            env=env, fail_level_arg=fail_level_arg))
...
@task
def reload_uwsgi():
    sudo("ln -s --force {env.project_dir}/{env.uwsgi_conf} /etc/uwsgi-emperor/vassals/{env.site_name}.ini".format(
            env=env))
    with cd(env.project_dir):
...
def transfer_files_git():
    ...
    run('mkdir -p {env.project_dir}'.format(env=env))
...
def download_postgres_db():
    ...
    sudo('sudo -u postgres pg_dump -Fc {env.db_name} > {tempfile}'.format(
