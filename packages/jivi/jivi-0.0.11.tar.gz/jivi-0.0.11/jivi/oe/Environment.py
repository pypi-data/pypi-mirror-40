...
class Environment:
	def __init__(self,fp,file_types=['ininame','pf'],sections=['Startup','Colors','fonts'],temp_dir="C:\\temp\\"):
	...
	def set_fields_and_tables(self):
		...
		tabs = {}
		for a in ['ttField','ttIndex','ttIndexField','ttFile']:
		...
		for t in tabs['file']:
			...
			file_by_recid[tab['rrecid']] = tab['name']
		...
		for t in tabs['field']:
			...
			tab['name'] = tab['name'].lower()
			table = self.tables[file_by_recid[tab['file_recid']]] if tab['file_recid'] in file_by_recid else 0
			if not table: continue
			tab['table'] = table['name']
			self.fields[table['name'] + '.' + tab['name']] = tab
