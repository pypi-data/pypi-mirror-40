"""
 * $Id: 0.8+9 model_data_service.py 4da1658fcc91 2019-01-07 od $
 *
...
class Service(object):
    ...
    def __get_param(self, param, def_value=None, typ=None):
        ...
        :return: string
        """
        v = self.csip.get_data_value(param, def_value)
        if typ:
            try:
                v = typ(v)
            except TypeError:
    ...
    def put_result(self, name, value, unit=None, desc=None):
        """
        ...
        if desc:
            self.response[name][Client.KEY_DESCRIPTION] = desc
