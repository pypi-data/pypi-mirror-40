...
def get_quote(code, days=20, dt=None,
              fields=['pt', 'turnover', 'close', 'open', 'high', 'low', 'money',
    ...
    if rs is None or len(rs[0]) == 0:
        df = pd.DataFrame([], columns=['date', 'pt', 'turnover', 'money', 'close', 'open', 'high', 'low'])
        df.set_index('date', inplace=True)
        df = df.astype(float)
        df.index.name = None
        return df[fields]

    df = pd.DataFrame(rs[0], columns=['date', 'open', 'high', 'low', 'pt', 'turnover', 'money', 'factor'])
    ...
    if factor == 0:
        print('data error: get_quote(' + code + ') factor is 0')
        factor = 1.0
...
def get_quotes(dt=None, fields=['close', 'open', 'high', 'low',
    ...
    df = pd.DataFrame(rs, columns=['code', 'close', 'open', 'high', 'low', 'pt', 'turnover', 'money', 'factor'])
