...
def write_file(path, data, append=False, byte=False):
...
class EsFileObject:
    ...
    JSON Capabilities: On creation the onbject will try to create a json version,
    ...
    Save the EsFileObject with var.save(), by default this will try to save to the location it came from, unless dirs is set. if json is avalible, it will use json unless json=False is in the parameters
    """
