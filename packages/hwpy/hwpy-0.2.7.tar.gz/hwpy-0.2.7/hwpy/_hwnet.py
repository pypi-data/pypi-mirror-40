...
def getnet():
    ...
    for k, v in net_if_addrs().items():
        if not (k.startswith('tap') or k.startswith('vir') or k.startswith('br') or k.startswith('lo')):
            for item in v:
                mac = item[1]
                if ':' in mac and len(mac)==17:
                    netdir = os.readlink(os.path.join("/sys/class/net", k))
                    m = re.match(".+/\d+:(\w+:\w+\.\w)/[a-z]+\d+/\s*", netdir)
                    if m:
                        pciid = m.group(1)
                        host = _find_device(pcidata, pciid)
                    else:
                        host = ""
                    isup = net_if_stats()[k].isup
                    if isup == True and net_if_stats()[k].speed != 65535:
                        speed = str(net_if_stats()[k].speed) + 'Mbps'
                    elif net_if_stats()[k].speed == 65535:
                        speed = 'Unknown'
                    else:
                        speed = False
