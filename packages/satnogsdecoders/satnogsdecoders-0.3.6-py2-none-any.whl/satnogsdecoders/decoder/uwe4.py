...
class Uwe4(KaitaiStruct):
    ...
    class Beacon(KaitaiStruct):
        ...
        def _read(self):
            self._raw_beacon_header = self._io.read_bytes(10)
            io = KaitaiStream(BytesIO(self._raw_beacon_header))
            self.beacon_header = self._root.BeaconHeader(io, self, self._root)
        ...
        def beacon_payload_timestamp(self):
            ...
            self._m_beacon_payload_timestamp = (((((self.beacon_payload_timestamp_raw[5] | (self.beacon_payload_timestamp_raw[4] << 8)) | (self.beacon_payload_timestamp_raw[3] << 16)) | (self.beacon_payload_timestamp_raw[2] << 24)) | (self.beacon_payload_timestamp_raw[1] << 32)) | (self.beacon_payload_timestamp_raw[0] << 48))
            return self._m_beacon_payload_timestamp if hasattr(self, '_m_beacon_payload_timestamp') else None
