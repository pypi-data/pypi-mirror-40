...
class BaseModel(models.Model):
    ...
    def find_many(cls, **kwargs):
        try:
            item = cls.objects.filter(**kwargs)
        except:
            item = None
        return item
    ...
    def find_perm_by_path(cls, request_url):
        ...
        for perm in perms:
            if re.search(perm.rule, request_url):
                exclude_path_list = perm.exclude_path.split(',') if perm.exclude_path and perm.exclude_path != '' else []
...
class UserTokens(BaseModel):
    ...
    last_token = models.CharField(max_length=64, null=True)
    system = models.CharField(max_length=64, null=True)
    login_time = models.DateTimeField(default=datetime.datetime.now())
    expired_time = models.DateTimeField(default=datetime.datetime.now()+datetime.timedelta(hours=48))
