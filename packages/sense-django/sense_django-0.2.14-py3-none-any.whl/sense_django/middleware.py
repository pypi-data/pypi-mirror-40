...
from .utils import decode_token, str2int, encode_token
...
class UserTokenCheckMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        try:
            ...
            if not user_token:
                user_token = UserTokens.find_one_by(last_token=lis[0])
                if user_token:
...
class RequestLogMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        info = '[start][' + request.META['REMOTE_ADDR'] + '][' + request.method + '][' + path + ']'
        sd.log_info(info)
