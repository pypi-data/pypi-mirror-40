...
class Config(object):
    ...
    def auth_post(self, url, data=None, json=None, params={}, headers={}):
        debug("In auth_post url", url)
        debug("IN auth_post settings", self.settings)
        debug("self.settings.region", self.settings["REGION"])
        def make_request():
            if self.settings["REGION"] == 'local':
                return requests.post(url, data=(js.dumps(data) if data else data), json=json, params=params, headers=headers, verify=False)
            else:
                return requests.post(url, data=(js.dumps(data) if data else data), json=json, params=params, headers=headers)
        return self.make_auth_call(make_request, params, headers, data)
    ...
    # JWT token).
    def make_auth_call(self, function, params={}, headers={}, data={}):
        ...
        # Use JWT token if cached:
            ...
            debug("HAS JWT TOKEN")
            if("blackfin_request" in params):
                data["api_key"] = self.get_jwt_token()
            else:
    ...
    def get_organization(self):
        org_json = self.auth_get(self.get("MANTA_URL") + "organization.json").json()
        return org_json["name"] if "name" in org_json else "Unknown organization"
