...
cdef class _Terrain(CoordSyst):
	...
	def set_material_from_image(self, _Image image, materials):
		...
		if len(materials) < m :
			...
			return
		for i from 0 <= i < m :
			self._add_material(<_Material> (<void*> (materials[i])))
			pack[i] =  (<_Material> (<void*> (materials[i])))._pack(FACE_TRIANGLE)
	...
	def get_material(self, int x, int z):
		...
		return <_Material> (<void*> (self._get_vertex(x, z).pack.material_id))
...
(e.g. with Terrain.set_height)."""
	...
	cdef void _vertex_render_special(self, TerrainVertex* vertex):
		...
		if (self._option & TERRAIN_VERTEX_OPTIONS) and (self._vertex_options[index] & TERRAIN_VERTEX_ALPHA) and (not (<_Material> (<void*> (vertex.pack.material_id))) is renderer.current_material):
			ccc[0] = color[0]
			ccc[1] = color[1]
			ccc[2] = color[2]
			ccc[3] = 0.0
	...
	cdef void _tri_render_secondpass(self, TerrainTri* tri):
		...
		if ((not(self._option & TERRAIN_VERTEX_OPTIONS) or
			...
			if  (((<_Material> (<void*> (tri.v2.pack.material_id))) is renderer.current_material) and
			...
			elif (((<_Material> (<void*> (tri.v3.pack.material_id))) is renderer.current_material) and
		...
		if (<_Material> (<void*> (tri.v1.pack.material_id))) is renderer.current_material: SET_COLOR_OPAQUE(self, index)
		...
		if (<_Material> (<void*> (tri.v2.pack.material_id))) is renderer.current_material: SET_COLOR_OPAQUE(self, index)
		else:                                                                    SET_COLOR_ALPHA (self, index)
		...
		if (<_Material> (<void*> (tri.v3.pack.material_id))) is renderer.current_material: SET_COLOR_OPAQUE(self, index)
		else:                                                                    SET_COLOR_ALPHA (self, index)
	...
	cdef void _render(self, CoordSyst coordsyst):
		...
		if renderer.state == RENDERER_STATE_OPAQUE:
			while pack:
				(<_Material> (<void*> (pack.material_id)))._activate()
	...
	cdef __getcstate__(self):
		...
		for i from 0 <= i < nb:
			...
			chunk_add_int_endian_safe  (chunk, material_id2index[(<_Material> (<void*> (v.pack.material_id)))])
	...
	cdef void __setcstate__(self, cstate):
		...
		if (self._nb_vertex_width != 0) and (self._nb_vertex_depth != 0):
			...
			for i from 0 <= i < nb:
				...
				v.pack = (<_Material> (<void*> (self._materials[temp])))._pack(FACE_TRIANGLE)
			self._normals = <float*> malloc((self._nb_vertex_width - 1) * (self._nb_vertex_depth - 1) * 6 * sizeof(float))
