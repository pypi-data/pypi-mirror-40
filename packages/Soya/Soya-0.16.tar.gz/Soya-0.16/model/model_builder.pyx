...
cdef class VertexBufferModelBuilder(ModelBuilder):
...
Attributes are :
 ...
 - max_face_angle (default 80.0) : if the angle (in degree) between 2 faces is less than
	...
	def __init__(self, float max_face_angle = 80.0):
		"""SimpleModelBuilder(**attributes) -> SimpleModelBuilder
...
See SimpleModelBuilder.__doc__ for more info."""
	...
	cdef void __setcstate__(self, cstate):
		(self._max_face_angle,)  = cstate
		
	cdef _Model _to_model(self, _World world):
		cdef _VertexBufferedModel model
		cdef int                  option
		cdef                      ligths
		option = 0
		model = VertexBufferedModel(world, self._max_face_angle, option, world.search_all(_is_static_light))
		model._build_sphere()
		return model
...
cdef class CellShadingVertexBufferModelBuilder(VertexBufferModelBuilder):
	"""CellShadingVertexBufferModelBuilder
...
"""
	...
	def __init__(self, float max_face_angle = 80.0, _Material shader = None, outline_color = BLACK, float outline_width = 4.0, float outline_attenuation = 0.3):
		"""CellShadingModelBuilder(**attributes) -> CellShadingModelBuilder

See CellShadingModelBuilder.__doc__ for more info."""
		VertexBufferModelBuilder.__init__(self, max_face_angle)
		self._shader              = shader or _SHADER_DEFAULT_MATERIAL
		self._outline_color       = outline_color
		self._outline_width       = outline_width
		self._outline_attenuation = outline_attenuation
	...
	cdef __getcstate__(self):
		return self._max_face_angle, self._shader, self._outline_color, self._outline_width, self._outline_attenuation
	
	cdef void __setcstate__(self, cstate):
		self._max_face_angle, self._shader, self._outline_color, self._outline_width, self._outline_attenuation = cstate
		
	cdef _Model _to_model(self, _World world):
		cdef _CellShadingVertexBufferedModel model
		cdef int                             option
		option = 0
		#if self._shadow: option = option | MODEL_PLANE_EQUATION | MODEL_SIMPLE_NEIGHBORS | MODEL_SHADOW
		model = CellShadingVertexBufferedModel(world, self._max_face_angle, option | MODEL_PLANE_EQUATION | MODEL_NEIGHBORS | MODEL_CELLSHADING, world.search_all(_is_static_light))
		model._build_sphere()
		model._build_cellshading(self._shader, self._outline_color, self._outline_width, self._outline_attenuation)
		return model
