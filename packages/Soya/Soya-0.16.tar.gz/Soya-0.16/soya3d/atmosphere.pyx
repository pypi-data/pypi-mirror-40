...
cdef _ARBShaderProgram _texture_on_depth_buffer_shader
_texture_on_depth_buffer_shader = None

cdef class _FixedBackgroundAtmosphere(_Atmosphere):
	def __init__(self):
		_Atmosphere.__init__(self)
		glPushAttrib(GL_TEXTURE_BIT)
		glGenTextures(1, &(self._color_tex_id))
		glGenTextures(1, &(self._depth_tex_id))
		
		cdef int width
		cdef int height
		width  = power_of_2(renderer.screen_width )
		height = power_of_2(renderer.screen_height)
		...
		glBindTexture(GL_TEXTURE_2D, self._depth_tex_id)
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR)
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
		...
		if _texture_on_depth_buffer_shader is None:
			_texture_on_depth_buffer_shader = _ARBShaderProgram(SHADER_TYPE_FRAGMENT, """!!ARBfp1.0
TEMP color;
TEX color, fragment.texcoord[0], texture[0], 2D;
MOV result.depth, color;
END
""")
			
	def __dealloc__(self):
		glDeleteTextures(1, &(self._color_tex_id))
		glDeleteTextures(1, &(self._depth_tex_id))
		
	def capture(self, int x = 0, int y = 0, int width = 0, int height = 0, int color = 1, int depth = 1):
		self.x = x
		self.y = y
	...
	def restore(self, int color = 1, int depth = 1, int draw_depth_on_color = 0):
		glClear(GL_DEPTH_BUFFER_BIT)
		...
		if depth:
			_texture_on_depth_buffer_shader.activate()
			if draw_depth_on_color == 0:
			...
			glTexCoord2f(1.0, 0.0); glVertex2i(self.x + self.width, self.y)
			glEnd()
			_texture_on_depth_buffer_shader.inactivate()
