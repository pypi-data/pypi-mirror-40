import numpy as np
from pyecharts import EffectScatter, Scatter, Line
...
def plot_trades(trades,
                ...
                line_type='dashed',
                scatter_type='ef'):
    dates_str = all_dates.astype(str).tolist()
    [entry.update({'x': dates_str[(all_dates - entry['dt']).abs().idxmin()]}) for trade in trades for entry in trade.get('entrys', [])]
    [cover.update({'x': dates_str[(all_dates - cover['dt']).abs().idxmin()]}) for trade in trades for cover in trade.get('covers', [])]
    long_trades = [trade for trade in trades if trade.get('side', '') == 'long']
    short_trades = [
        trade for trade in trades if trade.get('side', '') == 'short'
    ]
    es = EffectScatter() if scatter_type == 'ef' else Scatter()
