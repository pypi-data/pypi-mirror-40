...
def add_pair_efs(es, side, trades, entry_color, cover_coler):
    es = add_efs(es, '{}_entry'.format(side),
                 [[p['x'], p['y'], p['value']]
                     for trade in trades for p in trade.get('entrys', [])],
                 entry_color, rotate=180 if side == 'long' else 0)
    es = add_efs(es, '{}_cover'.format(side),
                 [[p['x'], p['y'], p['value']]
                     for trade in trades for p in trade.get('covers', [])],
...
def trade_line(name='esline', trades=[], all_dates=[], win_color='rgba(0,255,255,0.8)', loss_color='rgba(255,0,255,0.8)'):
    ...
    for trade in trades:
        for entry_ind, cover_ind in trade.get('links', []):
            ...
            cover = trade.get('covers', [])[entry_ind]
            side = trade.get('side', '')
            ...
            esline.add(
                f'{entry_date}->{cover_date}: {side}', [], [], is_symbol_show=False)
