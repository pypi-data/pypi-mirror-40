...
from augustus.system.models.calendar import Calendar


class MarketMaker(augustusEnvBase):
    ...
    @classmethod
    def initialize(cls):
        cls.env.logger.critical(f"正在初始化augustus")
        cls._initialize_calendar()
        cls._initialize_feeds()
        cls._initialize_cleaners()
        cls.env.logger.critical(f"{'='*15} augustus初始化成功！ {'='*15}")
        cls.env.logger.critical("开始寻找Gaius之旅~~~")
