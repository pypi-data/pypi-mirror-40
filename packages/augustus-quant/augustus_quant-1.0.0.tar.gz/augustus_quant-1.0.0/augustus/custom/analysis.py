...
from augustus.constants import ActionType, OrderType
...
class AmazingAnalysis(augustusEnvBase):
    ...
    def trade_analysis(self, full_trade_log: pd.DataFrame=None):
        if full_trade_log:
            augustusEnvBase.full_trade_log = full_trade_log
        else:
            augustusEnvBase.full_trade_log = self.get_full_trade_log()

        from augustus.custom.trade_log_analysis import APP
