...
class data_box(object):
    ...
    def factor_size_neutral(self,ln=False,ind_neutral_size=False):
        ...
        cap_grouped=size.stack().groupby('dt') if not ind_neutral_size else pd.concat([size.stack(),self.Ind.stack().rename('_ind')],axis=1).groupby(['dt','_ind'])
        cap_zscore=(size-cap_grouped.transform(np.mean).unstack())/cap_grouped.transform(np.std,ddof=1).unstack()
        new_factor_list=[]
