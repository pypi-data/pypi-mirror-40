...
class data_box(object):
    ...
    def factor_ind_neutral(self):
        ...
        return self
    @timeit    
    def factor_size_neutral(self,ln=False):
        cap_grouped=self.Cap.stack().groupby('dt') if not ln else np.log(self.Cap).stack().groupby('dt')        
        cap_zscore=(self.Cap-cap_grouped.transform(np.mean).unstack())/cap_grouped.transform(np.std,ddof=1).unstack()
        new_factor_list=[]
