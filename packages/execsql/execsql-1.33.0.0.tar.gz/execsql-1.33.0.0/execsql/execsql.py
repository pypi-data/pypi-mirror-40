...
class SubVarSet(object):
	...
	def __init__(self):
		self.substitutions = []
		self.var_rx_str = r'^[\$\&\@\~]?\w+$'
		# Don't compile on init because deepcopy() can't handle compiled regexes.
		self.var_rx = None
	def var_name_ok(self, varname):
		if self.var_rx is None:
			self.var_rx = re.compile(r'^[\$\&\@\~]?\w+$', re.I)
...
class CommandList(object):
	...
	def set_paramvals(self, paramvals):
		...
		if self.paranames is not None:
			# Check that all parameters are provided.
			if not all([p in self.paramvals.keys() for p in self.paramnames]):
...
def x_executescript(**kwargs):
	...
	commandliststack.append(copy.deepcopy(savedscripts[script_id]))
...
def xf_sub_defined(**kwargs):
	...
	invert = False if invert is None else True
	subvarset = subvars if varname[0] != '~' else commandliststack[-1].localvars
	s_exists = subvarset.sub_exists(varname)
	if invert:
...
def substitute_vars(command_str, localvars = None):
	...
	while substitutions_made:
		cmdstr, strsubs = subs.substitute(cmdstr)
		cmdstr, ctrsubs = counters.substitute(cmdstr)
		substitutions_made = strsubs or ctrsubs
	return cmdstr
			...
			if not is_comment_line:
				if metacommand_match:
					...
					if begs:
						...
						subscript_stack.append(CommandList([], scriptname))
					elif ends:
						# This is an END SCRIPT metacommand.
						if len(subscript_stack) == 0:
							raise ErrInfo(type="cmd", command_text=line, other_msg="Unmatched END SCRIPT metacommand.")
