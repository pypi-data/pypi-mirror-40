...
class cutter():
    ...
    def __init__(self):
        ...
        # com_etypes的库
        path_types = os.path.join(self.module_path,"user_dict","user_dict_com_etypes_all.dat")
        with open(path_types, "r", encoding="utf8") as f:
            ETYPES = f.readlines()
        self.ETYPES = [x.strip() for x in ETYPES]
        ...
        jieba.load_userdict(os.path.join(self.module_path,"user_dict","user_dict_com_etypes_extra01.dat"))
        jieba.load_userdict(os.path.join(self.module_path,"user_dict","user_dict_com_etypes_extra02.dat"))
        jieba.load_userdict(os.path.join(self.module_path,"user_dict","user_dict_com_etypes_extra03.dat"))
        jieba.load_userdict(os.path.join(self.module_path,"user_dict","user_dict_com_etypes_extra04.dat"))
        ...
        #对公司名片段进行映射的映射表
        name_rmap = {}
        name_rmap0 = jsonfiler.load(os.path.join(self.module_path,"user_dict","user_dict_com_etypes.json"))   
        name_rmap1 = jsonfiler.load(os.path.join(self.module_path,"user_dict","user_dict_com_etypes_extra01.json"))   
        name_rmap2 = jsonfiler.load(os.path.join(self.module_path,"user_dict","user_dict_com_etypes_extra02.json"))   
        name_rmap3 = jsonfiler.load(os.path.join(self.module_path,"user_dict","user_dict_com_etypes_extra03.json"))
        name_rmap4 = jsonfiler.load(os.path.join(self.module_path,"user_dict","user_dict_com_etypes_extra04.json"))
    ...
    def docut1(self,name_segs):
        ...
        if set(name_segs) & self.ETYPES_SET == ():
            return None
        ...
        for seg in reversed(name_segs):
            if seg in self.TAILS or seg in self.SUBTAILS or seg in self.ETYPES:
        ...
        for seg in name_segs:
            if cur_state is Lookingfor.LOC:
                if seg in self.LOCS:
                    ...
                    com_etype = seg
                    cur_state = Lookingfor.ETYPE
                #先判断在不在ETYPES里,如果不在才去行业里看
                ...
                #先判断在不在ETYPES里,如果不在才去行业里看
                    ...
                    com_feature +=seg
                continue
            #能进这个状态,说明实现已经找到type了,etype只能有一次
            elif cur_state is  Lookingfor.ETYPE:
                if seg in self.SUBS:
                    com_sub += seg
        ...
        if com_etype != "" and com_etype in self.name_rmap:
            name_rmap = self.name_rmap[com_etype]
            com_industry = name_rmap[0]
            com_type  = name_rmap[-1]
...
def main():
    ...
    test_names = jsonfiler.load("json/com_names5w.json")
    mycutter = cutter()
    error_count = 0
