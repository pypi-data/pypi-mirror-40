...
class cutter():
    ...
    #从企业名中获取它的名字tags
    def get_tags(self,com_name):
        name_cuts = self.cut(com_name)
        if not name_cuts:
            return []
        if not name_cuts["ok"]:
            return []

        if name_cuts["com_industry"] != "":
            #调用1w多个com_types_new.json 翻译成 tags
...
def contain_ch(string):
    for c in string:
        if ('\u4e00' <= c <= '\u9fa5'):
            return True
    return False

def pre_test(test):         #字符串预处理
    test_new=test
    test_new=test_new.replace("0", "零")
    test_new=test_new.replace("1", "一")
    test_new=test_new.replace("2", "二")
    test_new=test_new.replace("3", "三")
    test_new=test_new.replace("4", "四")
    test_new=test_new.replace("5", "五")
    test_new=test_new.replace("6", "六")
    test_new=test_new.replace("7", "七")
    test_new=test_new.replace("8", "八")
    test_new=test_new.replace("9", "九")
    test_new=test_new.replace("O", "零")
    test_new=test_new.replace("１", "一")
    test_new=test_new.replace("２", "二")
    test_new=test_new.replace("３", "三")
    test_new=test_new.replace("４", "四")
    test_new=test_new.replace("５", "五")
    test_new=test_new.replace("６", "六")
    test_new=test_new.replace("７", "七")
    test_new=test_new.replace("８", "八")
    test_new=test_new.replace("９", "九")
...
def main():
    ...
    test_names = jsonfiler.load("json/com_names_5w.json")[:5000]
    mycutter = cutter()
    error_count = 0
    ...
    path_types = os.path.join("","first_names.dat")   #姓
    with open(path_types, "r", encoding="utf8") as f:
        SUBTAILS = f.readlines()
    FNames = [x.strip() for x in SUBTAILS]

    with open("car_names.dat", "r", encoding="utf8") as f:  #车牌
        cname = f.readlines()
    CNames = [x.strip() for x in cname]
    ...
    for test_name in tqdm(test_names):
        ...
        test_name=test_name.replace("(", "")
        test_name=test_name.replace("〇", "零")
        test_name=test_name.replace(" ", "")
        if len(test_name)>0:
            if len(test_name) <=3:                                 #过滤人名
                if test_name[0] in FNames:
                    people_name.append(test_name)
                else:
                    no_means.append(test_name)
            elif test_name[0] in CNames and (is_alphabet(test_name[1]) or is_number(test_name[1]) or contain_ch(test_name[1:5])==False):
                car_id.append(test_name)                          #过滤车牌号
            elif "·" in test_name and len(test_name) <=8:     #过滤带·人名  如：买买提·提力瓦德
                people_name.append(test_name)
            elif "--" in test_name and test_name[0] in FNames:    #过滤通讯地址
                address.append(test_name)
            elif contain_ch(test_name) == False:
                no_means.append(test_name)
            else:
    ...
    print("纯人名:"+str(len(people_name)))
    print("车牌号:"+str(len(car_id)))
    print("通讯地址:"+str(len(address)))
    print("无意义:"+str(len(no_means)))

    jsonfiler.dump(no_means, "error_no_means.json", indent=4)
    jsonfiler.dump(people_name, "error_people_name.json", indent=4)
    jsonfiler.dump(address, "error_address.json", indent=4)
    jsonfiler.dump(car_id, "error_car_id.json", indent=4)
