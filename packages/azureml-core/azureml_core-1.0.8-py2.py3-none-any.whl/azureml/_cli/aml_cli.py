...
AZUREML_CLI_IN_USE = "AZUREML_CLI_IN_USE"
...
def _add_arguments_to_command_subparser(command_subparser, argument_list):
    ...
    for argument_obj in argument_list:
        ...
        if len(argument_obj.long_form) > 0:
            positional_argument_list.append(argument_obj.long_form)
...
def _register_subgroups_and_commands(root_parser):
    """Performs a bread-first search over the subgroups package structure and registers corresponding subparsers
    and commands."""
    aml_cli = AmlCli()
...
def load_command_table_azurecli(az_command_loader, _):
    ...
    while len(subgroups_list) > 0:
        ...
        for subgroup_object in subgroups_list:
            ...
            commands_at_current_subgroup = subgroup_object.subgroup.get_commands()
                ...
                with az_command_loader.command_group(subgroup_object.partial_command_name, cli_command_type) as g:
                    for command_obj in commands_at_current_subgroup:
                        ...
                        assert curr_parsed[0] == function_handler_module, \
                            ...
                            "module = {}".format(subgroup_object.partial_command_name, function_handler_module)
...
def _process_single_return_object(return_object):
    ...
    if isinstance(return_object, Model):
        ...
        sys.exit("CLIError: Unsupported output type.")


def start_cli():
    """ This function is called by __main__.py to start the azureml._cli"""

    os.environ[AZUREML_CLI_IN_USE] = "True"

    parser = argparse.ArgumentParser(prog="azureml._cli")
            ...
            if cmd_output:
                if isinstance(cmd_output, list):
                    ...
                    for element in cmd_output:
                        list_output.append(_process_single_return_object(element))
                    print(str(list_output))
                elif isinstance(cmd_output, Iterator):
                    list_output = []
                    for element in cmd_output:
                        list_output.append(_process_single_return_object(element))
                    print(str(list_output))
