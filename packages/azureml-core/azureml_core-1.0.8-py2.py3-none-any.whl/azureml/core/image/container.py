...
class ContainerImage(Image):
    ...
    .. remarks::
        A ContainerImage is retrieved using the Image class constructor by passing the name or ID of a
        previously created ContainerImage.
        Ex: container_image = Image(<workspace>, name=<image name>) or
        container_image = Image(<workspace>, id=<image id>)
    """
    ...
    def image_configuration(execution_script, runtime, conda_file=None, docker_file=None, schema_file=None,
        ...
        """
        Create and return a Container image configuration.
    ...
    def run(self, input_data):
        """
        Run the image locally and test with the given input data.

        Must have Docker installed and running to work. This method will only work on CPU, as the GPU image must be
        used on Microsoft Azure Services only such as ACI, AML Compute, Azure VMs, and AKS.
