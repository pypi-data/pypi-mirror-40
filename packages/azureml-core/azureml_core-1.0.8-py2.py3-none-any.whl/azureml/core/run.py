...
class Run(_RunBase):
    ...
    def get_context(cls, allow_offline=True, **kwargs):
        """
        Use this method to retrieve the current service context for logging metrics and uploading files.
        If allow_offline is set to True (and is by default), actions against the Run object will be printed to
        standard out.
    ...
    def add_type_provider(runtype, run_factory):
        """
        This is an extensibility hook for custom Run types stored in Run History.
    ...
    def get_status(self):
        """
        Get the run's current status. Common values returned include "Running", "Completed", and "Failed".
    ...
    def get_metrics(self, name=None, recursive=False, run_type=None, populate=False):
        """
        Retrieve the metrics logged to the run. If recursive=True (False by default), fetch metrics for
        runs in the given run's subtree.
    ...
    def start(self):
        """
        Mark the run as started. This is typically used in advanced scenarios when the
        run has been created by another actor.
        """
        self._client.start()

    def complete(self):
        """
        Wait for task queue to be processed. Then run is marked as completed.
        This is typically used in interactive notebook scenarios.
        """
    ...
    def _download_artifact_contents_to_string(self, name):
        """Downloads the contents of an artifact saved to the run"""
        container = self._run_dto.get("data_container_id", None)
        if container is None:
            raise UserErrorException(
                "DataContainerId is set to null for run with ID {0}, so file cannot be downloaded.".format(self.id))
