...
# ---------------------------------------------------------
"""Manages AKS Compute Targets in Azure ML"""
...
class AksCompute(ComputeTarget):
    ...
    def provisioning_configuration(agent_count=None, vm_size=None, ssl_cname=None, ssl_cert_pem_file=None,
                                   ...
                                   dns_service_ip=None, docker_bridge_cidr=None):
        """
    ...
    def _build_create_payload(config, location, subscription_id):
        ...
        if not config.agent_count and not config.vm_size and not config.ssl_cname and not config.vnet_name and \
                ...
                not config.dns_service_ip and not config.docker_bridge_cidr:
    ...
    @staticmethod
    def attach_configuration(resource_group=None, cluster_name=None, resource_id=None):
        """
        ...
        """
        config = AksAttachConfiguration(resource_group, cluster_name, resource_id)
        return config
    ...
    def delete(self):
        """
        Removes the AksCompute object from its associated workspace. If this object was created through Azure ML,
    ...
    def detach(self):
        """
        Detaches the AksCompute object from its associated workspace. No underlying cloud object will be deleted, the
...
class AksProvisioningConfiguration(ComputeTargetProvisioningConfiguration):
    """
    Provisioning configuration object for AKS compute targets. This object is used to
    ...
    def __init__(self, agent_count, vm_size, ssl_cname, ssl_cert_pem_file, ssl_key_pem_file, location,
                 vnet_resourcegroup_name, vnet_name, subnet_name, service_cidr, dns_service_ip, docker_bridge_cidr):
        """
        Initialize a configuration object for provisioning an AKS compute target.  Must provide all three
            CName, cert file, and key file to enable SSL validation
...
class AksAttachConfiguration(ComputeTargetAttachConfiguration):
    """
    Attach configuration object for AKS compute targets. This objects is used to define the configuration
    parameters for attaching AksCompute objects.
    """
    def __init__(self, resource_group=None, cluster_name=None, resource_id=None):
        """
