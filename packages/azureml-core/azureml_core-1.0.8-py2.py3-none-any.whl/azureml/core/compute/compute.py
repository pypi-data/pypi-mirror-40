...
# ---------------------------------------------------------
"""Manages the abstract parent class for Compute Targets in Azure ML"""
...
class ComputeTarget(ABC):
    """
    Abstract parent class for all compute targets managed by AzureML.
    ...
    def _initialize(self, compute_resource_id, name, location, compute_type, tags, description, created_on,
        ...
        :type provisioning_errors: list[dict]
    ...
    def get_status(self):
        """
        Retrieves the current provisioning state of the Compute object
    ...
    def delete(self):
        """
        Removes the Compute object from its associated workspace.

        ..remarks::
            If this object was created through Azure ML, the
    ...
    def detach(self):
        """
        Detaches the Compute object from its associated workspace.
    ...
    def _delete_or_detach(self, underlying_resource_action):
        """
        Removes the Compute object from its associated workspace. If underlying_resource_action is 'delete', the
        corresponding cloud based objects will also be deleted. If underlying_resource_action is 'detach', no
        underlying cloud object will be deleted, the association will just be removed.
    ...
    def deserialize(workspace, object_dict):
        """
        Convert a json object into a Compute object. Will fail if the provided workspace is not the workspace the
...
class ComputeTargetProvisioningConfiguration(ABC):
    """
    Abstract parent class for all ComputeTarget provisioning configuration objects. These objects are used to
    define the configuration parameters for provisioning Compute objects.

    :param type: The type of ComputeTarget this object is associated with
    :type type: class[ComputeTarget]
...
class ComputeTargetAttachConfiguration(ABC):
    """
    Abstract parent class for all ComputeTarget attach configuration objects. These objects are used to define the
    configuration parameters for attaching Compute objects.
    """
