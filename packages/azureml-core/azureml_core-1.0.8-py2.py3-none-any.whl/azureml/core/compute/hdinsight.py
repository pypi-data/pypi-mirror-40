...
# ---------------------------------------------------------
"""Manages HDInsight Compute Targets in Azure ML"""
...
class HDInsightCompute(ComputeTarget):
    """
    Manages HD Insight Cluster target objects.
    """
    ...
    def deserialize(workspace, object_dict):
        """
        Convert a json object into a HDICompute object. Will fail if the provided workspace is not the
...
class HDInsightAttachConfiguration(ComputeTargetAttachConfiguration):
    """
    Attach configuration object for HDI compute targets. This object is used to define the configuration
    parameters for attaching HDInsightCompute objects.
