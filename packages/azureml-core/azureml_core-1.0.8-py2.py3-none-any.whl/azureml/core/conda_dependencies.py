...
""" conda_dependencies.py for handling conda dependencies."""
import os
import ruamel.yaml
import re
from azureml._base_sdk_common.common import AML_CONFIG_DIR
...
class CondaDependencies(object):
    ...
    def __init__(self, conda_dependencies_file_path=None, _underlying_structure=None):
        """
        Initializes a new object to manage dependencies. The object will only contain the AzureML packages
        by default, if conda_dependencies_file_path is not used.
    ...
    def create(pip_indexurl=None, pip_packages=None, conda_packages=None,
        ...
        """
        Initializes a new CondaDependencies object.
        ...
        for package in pip_packages:
            ...
            if package.startswith('azureml-') and pin_sdk_version:
    ...
    def conda_channels(self):
        """
        :return: Returns the channel dependencies. The returned dependencies
        are a copy, and any changes to the returned channels won't update the conda channels
        in this object.
        :rtype: iter
        """
        conda_channels = []
    ...
    def save_to_file(self, base_directory, conda_file_path=None):
        ...
        if not conda_file_path:
            conda_file_path = AML_CONFIG_DIR + "/" + CONDA_FILE_NAME
    ...
    def _remove_from_list(self, element_list, element):
        """
        Removes element from element_list considering both cases if
        type(element_list)==ruamel.yaml.comments.CommentedSeq or  type(element_list)==list
    ...
    def _has_python_package(self):
        """
        Checks if the conda file, dict, hsa python package or not.
