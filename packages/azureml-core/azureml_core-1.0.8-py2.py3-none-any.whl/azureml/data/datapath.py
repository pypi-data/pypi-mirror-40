...
class DataPath(object):
    """
    Represents a path in a datastore. The path can point to a directory or a data artifact (blob, file)
    """
    ...
    def __init__(self, datastore=None, path_on_datastore=None):
        ...
        if None in [datastore]:
            raise ValueError('name and datastor are required parameters')
...
class DataPathComputeBinding(object):
    """
    Represents configuration for where a data represented by DataPath will be available in a
    compute target and how (mount or download, whether data can be overwritten)
    """
    def __init__(self, mode='mount', path_on_compute=None, overwrite=False):
        ...
        self._name = '{0}'.format(str(uuid.uuid4().hex)[0:8])
