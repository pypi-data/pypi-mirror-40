...
from azureml.core.util import _parse_date, _parse_expiry_time_from_token
...
class DatastoreContextManager(object):
    ...
    def __exit__(self, *exc_details):
        """
        uploads files for datastore
        :param exc_details:
        :return:
        """
        module_logger.debug("Enter __exist__ function of datastore cmgr")
        for key, value in self._config.items():
            df_config = self._to_data_reference_config(value)
    ...
    def _get_datastore_workspace(self):
        ...
        try:
            ...
            token_expiry_time = os.environ.get('AZUREML_RUN_TOKEN_EXPIRY',
                                               _parse_expiry_time_from_token(token))
