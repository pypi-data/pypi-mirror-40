...
from azureml._base_sdk_common.service_discovery import get_service_url, \
    load_history_service_url_from_env, load_mms_service_url_from_env
...
class ServiceContext(ChainedIdentity):
    ...
    def _fetch_endpoints(self):
        ...
        if isinstance(self._authentication, AzureMLTokenAuthentication):
            return self._fetch_endpoints_from_env()
        else:
            return self._fetch_endpoints_from_discovery()

    def _fetch_endpoints_from_env(self):
        history_endpoint = load_history_service_url_from_env()
        model_management_endpoint = load_mms_service_url_from_env()
        endpoints = {_ServiceKeys.HISTORY_DISCOVERY_KEY: history_endpoint,
                     _ServiceKeys.MMS_DISCOVERY_KEY: model_management_endpoint}

        self._logger.debug("Loaded endpoints {} from environment.".format(endpoints))

        return endpoints

    def _fetch_endpoints_from_discovery(self):
