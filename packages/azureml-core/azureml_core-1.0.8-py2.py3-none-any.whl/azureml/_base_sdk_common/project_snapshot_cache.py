...
class ContentSnapshotCache(object):
    ...
    def get_arm_scope_hash(self):
        '''TODO: Use projectId instead of arm scope since arm scope can change with move.
            Also revisit this once we get rid of workspaces and projects in ARM'''
        h = hashlib.sha256()
        h.update(self.project_context.get_workspace_uri_path().encode('utf-8'))
        return h.hexdigest()
    ...
    def get_cache_directory(self):
        return os.path.join(file_utilities.get_home_settings_directory(),
                            PROJECT_CONTENT_CACHE_DIR_NAME, self.get_arm_scope_hash())
