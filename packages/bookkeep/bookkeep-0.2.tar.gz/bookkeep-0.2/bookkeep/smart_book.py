...
class SmartBook(dict):
    """Create a dictionary that represents values with units of measure and alerts when setting an item out of bounds.
    
    **Parameters**
    
        **units:** [UnitManager or dict] Dictionary with units of measure.
        
        **bounds:** [dict] Dictionary with bounds.
        
        **source:** Should be one of the following [-]:
            * [str] Short description of SmartBook.
    ...
    **Examples**
    ...
    SmartBook objects provide an easy way to keep track of units of measure and enforce bounds. The units attribute is a UnitManager object which can change the units of the SmartBook object.
    
        Create a SmartBook object:
         
            >>> sb = SmartBook(units={'Duty': 'kJ/hr', 'T': 'K'},
            ...                bounds={'T': (0, 1000)},
        ...
        The *units* attribute becomes a UnitManager object with a reference to all dictionaries (*clients*) it controls. These include the SmartBook and its bounds.
            ...
            >>> sb.units
            UnitManager: {'T': 'degC', 'P': 'Pa'}
            >>> sb.units.clients
            [{'T': 350 (K), 'P': 101325 (Pa)}, {'T': (0, 1000)}]
    ...
    def __init__(self, units={}, bounds={}, source=None, *args, **kwargs):
    ...
    def unitscheck(self, key, value):
        """Adjust Quantity objects to correct units."""
        if isinstance(value, self.Quantity):
            units = self._units.get(key, '')
            value.ito(units)
    
    def boundscheck(self, key, value, units=None):
        """Issue Warning if value is out of bounds and class attribute, 'enforced_bounds', is True."""
        Q_ = self.Quantity
        ...
        if bounds is not None:
            ...
            try:
                ...
                if value_is_Q and not bounds_is_Q:           
                    # Make bounds a quantity
                    lb, ub = self._bounds[key] = Q_(bounds, units)
                    is_Q = 'value'
                    not_Q = 'bounds'                        
                elif bounds_is_Q and not value_is_Q:
                    # Use magnitude of bounds
                    bounds.ito(units)
                    lb, ub = bounds.magnitude
                    is_Q = 'bounds'
                    not_Q = 'value'                        
                else:
                    raise VE
    ...
    def table(self, with_units=True):
        ...
        def add_row(key, val, N_recursive):
    ...
    def _info(self, previous_dicts=[], N_recursive=0, depth=1):
        ...
        if self:
            # Log self to prevent infinit recursion later
    ...
    def _info_item(self, key, value, units, previous_dicts, N_recursive, depth):
        ...
        if isinstance(value, dict):
            ...
            if units:
                if isinstance(value, Q_):
                    # Convert Quantity objects to the right units
                    value.ito(units)
                    value = value.magnitude
                    units = ' ' + units
                else:
                    # Dim units for normal values
                    units = dim(' (' + units + ')')
