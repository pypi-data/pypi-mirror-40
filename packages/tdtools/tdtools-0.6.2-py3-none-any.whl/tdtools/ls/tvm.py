...
class TVM(Listing):
	...
	def user_args(self):
		yield lambda p: p.add_argument("names", metavar='TVM', nargs='*', type=DBObjPat, help=DBObjPat.__doc__)
		yield args_filter
		yield args_display
		yield args_order
...
def args_filter(p):
	...
	def rel_time(val: str) -> str:
		...
		val = int(val.lstrip())
		return ('< ' if val < 0 else '>= ') + (dt.date.today() - dt.timedelta(days=val)).isoformat()
	...
	def rel_size(val: str):
		...
		return ('< ' if val < 0 else '>= ') + compact(abs(val))
