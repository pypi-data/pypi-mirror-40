...
def args_size(p: ArgumentParser) -> None:
	...
	x.add_argument("-h", "--human-readable", dest='sizefmt', action="store_const", const='.1h', default=',d',
		help="print human readable sizes (e.g., 1K 234M 2G)")
	x.add_argument("--si", dest='sizefmt', action="store_const", const='.1s', help="likewise, but use powers of 1000 not 1024")

def args_misc(p: ArgumentParser) -> None:
	"add misc args"
	g = p.add_argument_group("Miscellaneous")
	g.add_argument('-v', '-l', '--verbose', default=0, action='count', help='show verbose output. Use -vv for more details')
