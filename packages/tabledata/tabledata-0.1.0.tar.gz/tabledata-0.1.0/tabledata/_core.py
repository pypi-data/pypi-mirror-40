...
class TableData(object):
    ...
    def __equals_raw(self, other):
        if not self.__equals_base(other):
            print("!!! d1", file=sys.stderr, flush=True)
            return False

        if self.header_list != other.header_list:
            print("!!! d2", file=sys.stderr, flush=True)
            return False

        for lhs_row, rhs_row in zip(self.row_list, other.row_list):
            if len(lhs_row) != len(rhs_row):
                print("!!! d3", file=sys.stderr, flush=True)
                return False

            print("!3!", lhs_row, rhs_row, file=sys.stderr, flush=True)
            for lhs, rhs in zip(lhs_row, rhs_row):
                print("!!! d: '{}' '{}' ".format(lhs, rhs), file=sys.stderr, flush=True)
            ...
            if not all(
                ...
                print("!!! d4", file=sys.stderr, flush=True)
                return False
        print("!!! d5", file=sys.stderr, flush=True)
    ...
    def __equals_dp(self, other):
        ...
        for lhs_list, rhs_list in zip(self.value_dp_matrix, other.value_dp_matrix):
            if len(lhs_list) != len(rhs_list):
                print("!! aa", file=sys.stderr, flush=True)
                return False
