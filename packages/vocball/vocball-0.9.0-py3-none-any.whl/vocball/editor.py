#!/usr/bin/env python3
#
# editor.py - Vocabulary Football (VocBall)
...
def save_new_vocabulary_list(self):
    ...
    global voc_file_path, voc_unit_orig
    ...
    if response == Gtk.ResponseType.ACCEPT:
        voc_file_path = dialog.get_filename()
        csv_file = ('.csv', '.cSv', '.csV', '.cSV', '.Csv', '.CSv', '.CsV', '.CSV')
        if not voc_file_path[-4:] in csv_file:
...
class Editor(Gtk.Window):
    ...
    def on_btnEditorOpen_clicked(self, widget):
        ...
        if voc_changed:
            self.lblDialogEditor.set_text(_("Do you want to save the current vocabulary list before opening a new one?"))
    ...
    def on_btnEditorNew_clicked(self, widget):
        ...
        if voc_changed:
            self.lblDialogEditor.set_text(_("Do you want to save the current vocabulary list before creating a new one?"))
    ...
    def on_btnEditorSave_clicked(self, widget):
        ...
        if voc_file_path == '':
            ...
            write_vocabulary(self, voc_file_path)
    ...
    def on_winEditor_delete_event(self, widget, event):
        ...
        if voc_changed:
            self.lblDialogEditor.set_text(_("Do you want to save the current vocabulary list before closing?"))
