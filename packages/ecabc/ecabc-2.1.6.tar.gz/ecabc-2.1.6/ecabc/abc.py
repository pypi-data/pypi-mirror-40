...
class ABC:
    ...
    def __init__(self, fitness_fxn, value_ranges=[], print_level='info', file_logging='disable', processes=4, args={}):
    ...
    def _calc_new_positions(self):
        ...
        if self._processes > 1:
            ...
            for i, bee in enumerate(self._to_modify):
                if self.__is_picked(bee, probability):
    ...
    def __is_picked(self, bee, probability):
        '''
        Returns true if the bee should be picked based on the probability given 
        and the bee's probablity, False if not.
        '''
        return (self._minimize == True and bee.probability  < probability) or\
               (self._minimize == False and bee.probability > probability)
    ...
    def _calc_average(self):
        ...
        self._total_score = self._average_score
