...
class CSVHandler:
    ...
    def _process_kwargs(self, **kwargs):
        ...
        if self.aes_key and self.detect_parameters:
            raise NotImplementedError("Cannot yet specify detect_parameters and encryption simultaneously - must "
    ...
    def _get_df_from_raw(self, file, force_dtype):
        ...
        if self.aes_key:
            ...
            if self._is_s3_file(file_or_buffer):
                ...
                with fs.open(file_or_buffer, "rb") as f:
                    buffer = f.read()
