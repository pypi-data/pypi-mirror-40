...
def calc_ranges(range_lengths, sample_pitches=[], center_offsets=[]):
    ...
    sample_pitches = extend_to_length(sample_pitches, nb_dims, 1)
...
def pad_to_length(V, length, value=0):
    V = np.array(V)
    result = value * np.ones(length, dtype=V.dtype)
    result[:V.size] = V.ravel()
    return result


def extend_to_length(V, length, value=0):
    V = np.array(V)
    result = np.ones(length, dtype=V.dtype)
    result[:V.size] = V.ravel()
    result[V.size:] = result[V.size-1]
