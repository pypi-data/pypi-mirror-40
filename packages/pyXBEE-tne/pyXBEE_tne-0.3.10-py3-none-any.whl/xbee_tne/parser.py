...
DEVICE_CLASS_FAN = 8
DEVICE_CLASS_TEST = 16
DEVICE_CLASS_TEST1 = 32
    ...
    if (frame[-1] + sum(frame[3:-1]) & 0xff) == 0xff:
        ...
        if (frame[15] == MESSAGE_STATE):
            if (frame[16] == DEVICE_CLASS_WEATHER):
                ...
                return None
            elif (frame[16] == DEVICE_CLASS_TEST):
                obj = TestSensor()
            elif (frame[16] == DEVICE_CLASS_TEST1):
                obj = TestSensor2()
            else:
                return None
        else:
...
class WeatherStation(SensorFrame):
    ...
    def load_receive(self, frame):
        ...
        self.DHT22_temp = round(struct.unpack('<f',self.data[4:8])[0],1)
        self.DHT22_hum = round(struct.unpack('<f',self.data[8:12])[0],0)
        self.BMP280_pres = round(struct.unpack('<f',self.data[12:16])[0],2)
        self.BMP280_temp = round(struct.unpack('<f',self.data[16:20])[0],1)
        self.BME280_pres = round(struct.unpack('<f',self.data[20:24])[0],2)
        self.BME280_temp = round(struct.unpack('<f',self.data[24:28])[0],1)
        self.BME280_hum = round(struct.unpack('<f',self.data[28:32])[0],0)
...
class TestSensor2(SensorFrame):
    """ Data class for testing """

    def __init__(self):
        """Constructor"""
        super(TestSensor2, self).__init__()
        self.DHT22_temp = None
        self.DHT22_hum = None
