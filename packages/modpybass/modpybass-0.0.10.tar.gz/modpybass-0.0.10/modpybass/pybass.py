...
def LOBYTE(a):
    return (ctypes.c_byte)(a)


def HIBYTE(a):
    return (ctypes.c_byte)((a) >> 8)


def LOWORD(a):
    return (ctypes.c_ushort)(a)


def HIWORD(a):
    return (ctypes.c_ushort)((a) >> 16)


def MAKEWORD(a, b):
    return (ctypes.c_ushort)(((a) & 0xff) | ((b) << 8))


def MAKELONG(a, b):
    return (ctypes.c_ulong)(((a) & 0xffff) | ((b) << 16))
...
class BASS_DEVICEINFO(ctypes.Structure):
    _fields_ = [
        ('name', ctypes.c_char_p),  # description
...
if platform.system().lower() == 'windows':
    if sys.getwindowsversion()[3] == 3:  # VER_PLATFORM_WIN32_CE
        BASS_DEVICEINFO._fields_ = [
            ('name', ctypes.c_wchar_p),  # description
...
class BASS_INFO(ctypes.Structure):
    _fields_ = [
        ('flags', ctypes.c_ulong),  # device capabilities (DSCAPS_xxx flags)
...
class BASS_RECORDINFO(ctypes.Structure):
    _fields_ = [
        ('flags', ctypes.c_ulong),  # DWORD flags; // device capabilities (DSCCAPS_xxx flags)
...
class BASS_SAMPLE(ctypes.Structure):
    _fields_ = [
        ('freq', ctypes.c_ulong),  # DWORD freq; // default playback rate
...
def BASS_SPEAKER_N(n):
    return ((n) << 24)  # n'th pair of speakers (max 15)
...
class BASS_PLUGINFORM(ctypes.Structure):
    _fields_ = [
        ('ctype', ctypes.c_ulong),  # DWORD ctype; // channel type
...
class BASS_PLUGININFO(ctypes.Structure):
    _fields_ = [
        ('version', ctypes.c_ulong),  # DWORD version; // version (same form as BASS_GetVersion)
...
class BASS_3DVECTOR(ctypes.Structure):
    _fields_ = [
        ('x', ctypes.c_float),  # float x; // +=right, -=left
...
STREAMPROC = func_type(
    ctypes.c_ulong, HSTREAM, ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p
)
...
# special STREAMPROCs
def streamproc_dummy(handle, buffer, length, user):
    return 0


def streamproc_push(handle, buffer, length, user):
    return -1
...
FILELENPROC = func_type(
    QWORD, ctypes.c_void_p
)
# typedef DWORD (CALLBACK FILEREADPROC)(
...
FILESEEKPROC = func_type(
    ctypes.c_bool, QWORD, ctypes.c_void_p
)


class BASS_FILEPROCS(ctypes.Structure):
    _fields_ = [
        ('close', FILECLOSEPROC),  # FILECLOSEPROC *close;
...
# typedef void (CALLBACK DOWNLOADPROC)(
# const void *buffer, DWORD length, void *user);
...
# typedef void (CALLBACK SYNCPROC)(
# HSYNC handle, DWORD channel, DWORD data, void *user);
SYNCPROC = func_type(
    None, HSYNC, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p
)
...
# typedef void (CALLBACK DSPPROC)(
# HDSP handle, DWORD channel, void *buffer, DWORD length, void *user);
DSPPROC = func_type(
    None, HDSP, ctypes.c_ulong, ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p
)
...
# typedef BOOL (CALLBACK RECORDPROC)(
# HRECORD handle, const void *buffer, DWORD length, void *user);
RECORDPROC = func_type(
    ctypes.c_bool, HRECORD, ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p
)
...
class TAG_BEXT(ctypes.Structure):
    if platform.system().lower() != 'windows':
        _pack_ = 1
    _fields_ = [
        ('Description', ctypes.c_char * 256),  # char Description[256]; // description
...
class TAG_CART_TIMER(ctypes.Structure):
    _fields_ = [
        ('dwUsage', ctypes.c_ulong),  # DWORD dwUsage; // FOURCC timer usage ID
...
class TAG_CART(ctypes.Structure):
    _fields_ = [
        ('Version', ctypes.c_char * 4),  # char Version[4]; // version of the data structure
...
class TAG_CA_CODEC(ctypes.Structure):
    _fields_ = [
        ('ftype', ctypes.c_ulong),  # DWORD ftype; // file format
...
class BASS_DX8_GARGLE(ctypes.Structure):
    _fields_ = [
        ('dwRateHz', ctypes.c_ulong),  # DWORD dwRateHz; // Rate of modulation in hz
...
class BASS_DX8_I3DL2REVERB(ctypes.Structure):
    _fields_ = [
        ('lRoom', ctypes.c_int),  # int lRoom; // [-10000, 0] default: -1000 mB
...
class BASS_DX8_PARAMEQ(ctypes.Structure):
    _fields_ = [
        ('fCenter', ctypes.c_float),  # float fCenter;
...
class BASS_DX8_REVERB(ctypes.Structure):
    _fields_ = [
        ('fInGain', ctypes.c_float),  # float fInGain; // [-96.0,0.0] default: 0.0 dB
...
# typedef void (CALLBACK IOSNOTIFYPROC)(
# DWORD status);
...
# BOOL BASSDEF(BASS_GetDeviceInfo)(
# DWORD device, BASS_DEVICEINFO *info);
BASS_GetDeviceInfo = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.POINTER(
        BASS_DEVICEINFO)
)(('BASS_GetDeviceInfo', bass_module))


# BOOL BASSDEF(BASS_Init)(
# int device, DWORD freq, DWORD flags, void *win, void *dsguid);
BASS_Init = func_type(
    ctypes.c_bool, ctypes.c_int, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p, ctypes.c_void_p
)(('BASS_Init', bass_module))


# BOOL BASSDEF(BASS_SetDevice)(
# DWORD device);
...
# BOOL BASSDEF(BASS_GetInfo)(
# BASS_INFO *info);
BASS_GetInfo = func_type(
    ctypes.c_bool, ctypes.POINTER(BASS_INFO)
...
# HPLUGIN BASSDEF(BASS_PluginLoad)(
# const char *file, DWORD flags);
BASS_PluginLoad = func_type(
    HPLUGIN, ctypes.c_char_p, ctypes.c_ulong
)(('BASS_PluginLoad', bass_module))


# BOOL BASSDEF(BASS_PluginFree)(
# HPLUGIN handle);
BASS_PluginFree = func_type(
    ctypes.c_bool, HPLUGIN
)(('BASS_PluginFree', bass_module))


# const BASS_PLUGININFO *BASSDEF(
# BASS_PluginGetInfo)(HPLUGIN handle);
BASS_PluginGetInfo = func_type(
    ctypes.POINTER(BASS_PLUGININFO), HPLUGIN
)(('BASS_PluginGetInfo', bass_module))


# BOOL BASSDEF(BASS_Set3DFactors)(
# float distf, float rollf, float doppf);
BASS_Set3DFactors = func_type(
    ctypes.c_bool, ctypes.c_float, ctypes.c_float, ctypes.c_float
...
BASS_Get3DFactors = func_type(
    ctypes.c_bool, ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float)
)(('BASS_Get3DFactors', bass_module))


# BOOL BASSDEF(BASS_Set3DPosition)(
# const BASS_3DVECTOR *pos, const BASS_3DVECTOR *vel, const BASS_3DVECTOR *front, const BASS_3DVECTOR
# *top);
BASS_Set3DPosition = func_type(
    ctypes.c_bool, ctypes.POINTER(BASS_3DVECTOR), ctypes.POINTER(BASS_3DVECTOR),
    ctypes.POINTER(BASS_3DVECTOR), ctypes.POINTER(BASS_3DVECTOR)
)(('BASS_Set3DPosition', bass_module))


# BOOL BASSDEF(BASS_Get3DPosition)(
# BASS_3DVECTOR *pos, BASS_3DVECTOR *vel, BASS_3DVECTOR *front, BASS_3DVECTOR *top);
...
# HMUSIC BASSDEF(BASS_MusicLoad)(
# BOOL mem, const void *file, QWORD offset, DWORD length, DWORD flags, DWORD freq);
BASS_MusicLoad = func_type(
    HMUSIC, ctypes.c_bool, ctypes.c_void_p, QWORD, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong
)(('BASS_MusicLoad', bass_module))


# BOOL BASSDEF(BASS_MusicFree)(
# HMUSIC handle);
BASS_MusicFree = func_type(
    ctypes.c_bool, HMUSIC
)(('BASS_MusicFree', bass_module))


# HSAMPLE BASSDEF(BASS_SampleLoad)(
# BOOL mem, const void *file, QWORD offset, DWORD length, DWORD max, DWORD flags);
BASS_SampleLoad = func_type(
    HSAMPLE, ctypes.c_bool, ctypes.c_void_p, QWORD, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong
)(('BASS_SampleLoad', bass_module))


# HSAMPLE BASSDEF(BASS_SampleCreate)(
# DWORD length, DWORD freq, DWORD chans, DWORD max, DWORD flags);
BASS_SampleCreate = func_type(
    HSAMPLE, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong
)(('BASS_SampleCreate', bass_module))


# BOOL BASSDEF(BASS_SampleFree)(
# HSAMPLE handle);
BASS_SampleFree = func_type(
    ctypes.c_bool, HSAMPLE
)(('BASS_SampleFree', bass_module))


# BOOL BASSDEF(BASS_SampleSetData)(
# HSAMPLE handle, const void *buffer);
BASS_SampleSetData = func_type(
    ctypes.c_bool, HSAMPLE, ctypes.c_void_p
)(('BASS_SampleSetData', bass_module))


# BOOL BASSDEF(BASS_SampleGetData)(
# HSAMPLE handle, void *buffer);
BASS_SampleGetData = func_type(
    ctypes.c_bool, HSAMPLE, ctypes.c_void_p
)(('BASS_SampleGetData', bass_module))


# BOOL BASSDEF(BASS_SampleGetInfo)(
# HSAMPLE handle, BASS_SAMPLE *info);
BASS_SampleGetInfo = func_type(
    ctypes.c_bool, HSAMPLE, ctypes.POINTER(BASS_SAMPLE)
)(('BASS_SampleGetInfo', bass_module))


# BOOL BASSDEF(BASS_SampleSetInfo)(
# HSAMPLE handle, const BASS_SAMPLE *info);
BASS_SampleSetInfo = func_type(
    ctypes.c_bool, HSAMPLE, ctypes.POINTER(BASS_SAMPLE)
)(('BASS_SampleSetInfo', bass_module))


# HCHANNEL BASSDEF(BASS_SampleGetChannel)(
# HSAMPLE handle, BOOL onlynew);
BASS_SampleGetChannel = func_type(
    HCHANNEL, HSAMPLE, ctypes.c_bool
)(('BASS_SampleGetChannel', bass_module))


# DWORD BASSDEF(BASS_SampleGetChannels)(
# HSAMPLE handle, HCHANNEL *channels);
BASS_SampleGetChannels = func_type(
    ctypes.c_ulong, HSAMPLE, ctypes.POINTER(HCHANNEL)
)(('BASS_SampleGetChannels', bass_module))


# BOOL BASSDEF(BASS_SampleStop)(
# HSAMPLE handle);
...
# HSTREAM BASSDEF(BASS_StreamCreate)(
# DWORD freq, DWORD chans, DWORD flags, STREAMPROC *proc, void *user);
BASS_StreamCreate = func_type(
    HSTREAM, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, STREAMPROC, ctypes.c_void_p
)(('BASS_StreamCreate', bass_module))


# HSTREAM BASSDEF(BASS_StreamCreateFile)(
# BOOL mem, const void *file, QWORD offset, QWORD length, DWORD flags);
BASS_StreamCreateFile = func_type(
    HSTREAM, ctypes.c_bool, ctypes.c_void_p, QWORD, QWORD, ctypes.c_ulong
)(('BASS_StreamCreateFile', bass_module))


# HSTREAM BASSDEF(BASS_StreamCreateURL)(
...
# HSTREAM BASSDEF(BASS_StreamCreateFileUser)(
...
BASS_RecordGetDeviceInfo = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.POINTER(BASS_DEVICEINFO)
)(('BASS_RecordGetDeviceInfo', bass_module))


# BOOL BASSDEF(BASS_RecordInit)(
# int device);
...
# BOOL BASSDEF(BASS_RecordSetDevice)(
# DWORD device);
...
# BOOL BASSDEF(BASS_RecordGetInfo)(
# BASS_RECORDINFO *info);
BASS_RecordGetInfo = func_type(
    ctypes.c_bool, ctypes.POINTER(BASS_RECORDINFO)
...
# BOOL BASSDEF(BASS_RecordSetInput)(
# int input, DWORD flags, float volume);
BASS_RecordSetInput = func_type(
    ctypes.c_bool, ctypes.c_int, ctypes.c_ulong, ctypes.c_float
)(('BASS_RecordSetInput', bass_module))


# DWORD BASSDEF(BASS_RecordGetInput)(
# int input, float *volume);
BASS_RecordGetInput = func_type(
    ctypes.c_ulong, ctypes.c_int, ctypes.c_float
)(('BASS_RecordGetInput', bass_module))


# HRECORD BASSDEF(BASS_RecordStart)(
# DWORD freq, DWORD chans, DWORD flags, RECORDPROC *proc, void *user);
BASS_RecordStart = func_type(
    HRECORD, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, RECORDPROC, ctypes.c_void_p
)(('BASS_RecordStart', bass_module))


# double BASSDEF(BASS_ChannelBytes2Seconds)(
...
# BOOL BASSDEF(BASS_ChannelSetDevice)(
# DWORD handle, DWORD device);
BASS_ChannelSetDevice = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_ulong
)(('BASS_ChannelSetDevice', bass_module))


# DWORD BASSDEF(BASS_ChannelIsActive)(
# DWORD handle);
...
# BOOL BASSDEF(BASS_ChannelGetInfo)(
# DWORD handle, BASS_CHANNELINFO *info);
BASS_ChannelGetInfo = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.POINTER(BASS_CHANNELINFO)
)(('BASS_ChannelGetInfo', bass_module))


# const char *BASSDEF(BASS_ChannelGetTags)(
# DWORD handle, DWORD tags);
...
# DWORD BASSDEF(BASS_ChannelFlags)(
...
# BOOL BASSDEF(BASS_ChannelUpdate)(
...
# BOOL BASSDEF(BASS_ChannelLock)(
# DWORD handle, BOOL lock);
BASS_ChannelLock = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_bool
)(('BASS_ChannelLock', bass_module))


# BOOL BASSDEF(BASS_ChannelPlay)(
# DWORD handle, BOOL restart);
BASS_ChannelPlay = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_bool
)(('BASS_ChannelPlay', bass_module))


# BOOL BASSDEF(BASS_ChannelStop)(
# DWORD handle);
...
# BOOL BASSDEF(BASS_ChannelPause)(
# DWORD handle);
...
# BOOL BASSDEF(BASS_ChannelSetAttribute)(
# DWORD handle, DWORD attrib, float value);
BASS_ChannelSetAttribute = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_float
)(('BASS_ChannelSetAttribute', bass_module))


# BOOL BASSDEF(BASS_ChannelGetAttribute)(
# DWORD handle, DWORD attrib, float *value);
BASS_ChannelGetAttribute = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_ulong, ctypes.POINTER(ctypes.c_float)
)(('BASS_ChannelGetAttribute', bass_module))


# BOOL BASSDEF(BASS_ChannelSlideAttribute)(
# DWORD handle, DWORD attrib, float value, DWORD time);
BASS_ChannelSlideAttribute = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_float, ctypes.c_ulong
)(('BASS_ChannelSlideAttribute', bass_module))


# BOOL BASSDEF(BASS_ChannelIsSliding)(
# DWORD handle, DWORD attrib);
BASS_ChannelIsSliding = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_ulong
)(('BASS_ChannelIsSliding', bass_module))


# BOOL BASSDEF(BASS_ChannelSet3DAttributes)(
# DWORD handle, int mode, float min, float max, int iangle, int oangle, float outvol);
BASS_ChannelSet3DAttributes = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.c_int, ctypes.c_float,
    ctypes.c_float, ctypes.c_int, ctypes.c_int, ctypes.c_float
)(('BASS_ChannelSet3DAttributes', bass_module))


# BOOL BASSDEF(BASS_ChannelGet3DAttributes)(
# DWORD handle, DWORD *mode, float *min, float *max, DWORD *iangle, DWORD *oangle, float *outvol);
BASS_ChannelGet3DAttributes = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.POINTER(ctypes.c_ulong),
    ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float),
    ctypes.POINTER(ctypes.c_ulong), ctypes.POINTER(ctypes.c_ulong), ctypes.POINTER(ctypes.c_float)
)(('BASS_ChannelGet3DAttributes', bass_module))


# BOOL BASSDEF(BASS_ChannelSet3DPosition)(
...
BASS_ChannelSet3DPosition = func_type(
    ctypes.c_bool, ctypes.c_ulong, ctypes.POINTER(BASS_3DVECTOR),
    ctypes.POINTER(BASS_3DVECTOR), ctypes.POINTER(BASS_3DVECTOR)
)(('BASS_ChannelSet3DPosition', bass_module))


# BOOL BASSDEF(BASS_ChannelGet3DPosition)(
# DWORD handle, BASS_3DVECTOR *pos, BASS_3DVECTOR *orient, BASS_3DVECTOR *vel);
...
# QWORD BASSDEF(BASS_ChannelGetLength)(
# DWORD handle, DWORD mode);
BASS_ChannelGetLength = func_type(
    QWORD, ctypes.c_ulong, ctypes.c_ulong
)(('BASS_ChannelGetLength', bass_module))


# BOOL BASSDEF(BASS_ChannelSetPosition)(
# DWORD handle, QWORD pos, DWORD mode);
BASS_ChannelSetPosition = func_type(
    ctypes.c_bool, ctypes.c_ulong, QWORD, ctypes.c_ulong
)(('BASS_ChannelSetPosition', bass_module))


# QWORD BASSDEF(BASS_ChannelGetPosition)(
# DWORD handle, DWORD mode);
...
# DWORD BASSDEF(BASS_ChannelGetLevel)(
# DWORD handle);
BASS_ChannelGetLevel = func_type(
    ctypes.c_ulong, ctypes.c_ulong
)(('BASS_ChannelGetLevel', bass_module))

# DWORD BASSDEF(BASS_ChannelGetData)(
# DWORD handle, void *buffer, DWORD length);
BASS_ChannelGetData = func_type(
    ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p, ctypes.c_ulong
)(('BASS_ChannelGetData', bass_module))


# HSYNC BASSDEF(BASS_ChannelSetSync)(
# DWORD handle, DWORD type, QWORD param, SYNCPROC *proc, void *user);
...
if platform.system().lower() == 'windows':
    ...
    BASS_Init = func_type(
        ctypes.c_bool, ctypes.c_int, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p
    )(('BASS_Init', bass_module))

    # void *BASSDEF(BASS_GetDSoundObject)(
    # DWORD object);
    ...
    # int env, float vol, float decay, float damp);
    BASS_SetEAXParameters = func_type(
        ctypes.c_bool, ctypes.c_int, ctypes.c_float, ctypes.c_float, ctypes.c_float
    )(('BASS_SetEAXParameters', bass_module))

    # BOOL BASSDEF(BASS_GetEAXParameters)(
    # DWORD *env, float *vol, float *decay, float *damp);
    BASS_GetEAXParameters = func_type(
        ctypes.c_bool, ctypes.POINTER(ctypes.c_ulong), ctypes.POINTER(ctypes.c_float),
        ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float)
...
def play_handle(handle, show_tags=True):
    if handle == 0:
        ...
        if show_tags:
            ...
            try:
                import pytags
                print(pytags.TAGS_Read(handle, (
                    '%IFV1(%ITRM(%TRCK),%ITRM(%TRCK). )%IFV2(%ITRM(%ARTI),%ICAP(%ITRM(%ARTI)),no artist) - '
