...
def print_error():
    ...
    print(exc, traceback.tb_frame.f_code.co_filename,
          'ERROR ON LINE', traceback.tb_lineno, '\n', err)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if pybassmidi:
            ...
            if self.sound_font == 0:
                print('BASS_MIDI_FontInit error',
    ...
    def method_load_data(self, stream, name_stream='memory_stream'):
        if stream is not None:
            ...
            if isinstance(self.stream, memory_stream):
                ...
                if new_bass_handle == 0:
                    print('BASS_StreamCreateFileUser error',
                          pybass.get_error_description(pybass.BASS_ErrorGetCode()))
                else:
                    self.method_stop_audio()
                    self.bass_handle = new_bass_handle
    ...
    def __del__(self):
        ...
        if self.sound_font != 0 and pybassmidi:
            if pybassmidi.BASS_MIDI_FontFree(self.sound_font):
                print('BASS_MIDI_FontFree error',
                      pybass.get_error_description(pybass.BASS_ErrorGetCode()))
        for plugin in self.plugins.itervalues():
            if plugin[0] > 0:
                if pybass.BASS_PluginFree(plugin[0]):
                    print('BASS_PluginFree error',
                          pybass.get_error_description(pybass.BASS_ErrorGetCode()))
            ...
            if os.path.exists('lang/%s' % lang_catalog):
                for dir_name in os.listdir('lang'):
                    if os.path.exists('lang/%s/%s_%s.mo' % (dir_name, self.app_name, self.wx_coding)):
                        ...
                        list_trans.append(gettext.GNUTranslations(open('lang/%s/%s_%s.mo' %
                                                                       (dir_name, self.app_name, self.wx_coding), 'rb')))
                        i += 1
                if len(list_trans) > 0:
                    try:
