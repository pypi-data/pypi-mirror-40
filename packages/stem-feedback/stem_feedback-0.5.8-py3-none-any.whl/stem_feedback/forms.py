...
class MoneyBackForm(forms.Form):
    ...
    description = forms.CharField(max_length=255, required=False, label=_("Наименование"))
    invoice_qty = forms.IntegerField(min_value=1, label='Количество товара в накладной')
    return_qty = forms.IntegerField(min_value=0, label='Количество товара, которое пользователь хочет вернуть')
