    ...
    for row in rows:
        ...
        if "labels_sim2dData" in row and row["labels_sim2dData"] and not row["labels_sim2dData"] == "{}":
            ...
            for i, label in enumerate(label_data):
                if not label_data[label]["textPosition"]:
                    raise ValueError("Invalid text position for label on step {} of {}".format(row["stepId"],
                                                                                               phase_code))
        ...
        if "gallery_sim2dData" in row and row["gallery_sim2dData"] and not row["gallery_sim2dData"] == "{}":
            ...
            for i, gallery_item in enumerate(gallery_data):
                ...
                if not main_file:
                    raise ValueError("Gallery item is missing Main Graphic on step {} of {}".format(row["stepId"],
                                                                                                    phase_code))

                thumb_file = os.path.splitext(os.path.basename(gallery_data[str(i)]["thumbnail"]))
                if not thumb_file:
                    raise ValueError("Gallery item is missing Thumbnail on step {} of ".format(row["stepId"],
                                                                                               phase_code))
                ...
                if main_file[1] in video_ext:
                    ...
                    raise TypeError("Unable to create gallery item for step {} of {} \n"
                                    "Unrecognised file type: {}{}".format(row["stepId"], phase_code, main_file[0],
                                                                          main_file[1]))
        ...
        if not row["testable"].lower() == "false":
            ...
            if "interaction_sim2dData" in row and row["interaction_sim2dData"] and not \
               row["interaction_sim2dData"] == "{}":
...
def get_csv_info(phase_code, csv_version):
    if os.path.isdir(settings.BASE_DATA_DIR):
        ...
        csv_file = os.path.join(settings.PROCEDURE_CHECKOUT_DIRECTORY, procedure_code, "build_assets", "csv",
                                "%s.csv" % phase_code)
