...
class UpdateConfig():
    ...
    def update(self, cfg, debug=False):
        ...
        try:
            ...
            if bool_assignment(response_q0):
                ...
                question1 = ('\n\tEnable Logging? ' + rst + '[True] ').expandtabs(8)
                ...
                if self.parameters['enable_logging']:
                    ...
                    question2 = (
                            ...
                            'FILE' + rst + ACCENT + ', or the system logger, ' +
                ...
                sys.stdout.write('\n')
                self.title_header = 'Backup Keysets'
                self.padding = 45 - len(self.title_header)
                x = VeryPrettyTable(border=True, header=False, padding_width=self.padding)
                x.align = 'c'
                x.field_names = [bd + self.title_header + frame]
                x.add_row([bd + 'Backup Keysets' + frame])
                display_table(x, tabspaces=8)
                q4a = self.print_header('q4')
                print(q4a)
                default = self.parameters['enable_keybackup'] or False
                q4b = ('\n\tWrite Backup keyset? ' + rst + '[%s] ').expandtabs(8) % default
                sys.stdout.write(ACCENT)
                ...
                if self.parameters['enable_keybackup']:
                    ...
                    msg = ('\n\t' + ACCENT + 'Backup directory location?' +
                           rst + ' [%s] ' % default)
                    # test location to ensure write compatible
                ...
                self.title_header = 'Key Expiration'
                self.padding = 45 - len(self.title_header)
                x = VeryPrettyTable(border=True, header=False, padding_width=self.padding)
                x.align = 'c'
                x.field_names = [bd + self.title_header + frame]
                x.add_row([bd + 'Key Expiration' + frame])
                display_table(x, tabspaces=8)
                ...
                while True:
                    try:
                        r_q6 = input('\tEnter the number of days (1 - {} days). {}[{}]: '.format(keyage_limit, rst, default)) or default
                        sys.stdout.write(ACCENT)
    ...
    def print_header(self, header):
        ...
        title = (
                '|    ' + Colors.BLUE + PACKAGE + ' Local Configuration Setup' + Colors.BOLD + Colors.BRIGHTWHITE + '      |'
            )
        ...
        self.q1 = ACCENT + """
        Do you want to turn on logging of status messages?  Filesystem logging is
        strongly recommended if you wish to automate access key rotation via a
        scheduler.

        """ + rst
        q2_title1 = Colors.BOLD + Colors.BRIGHTWHITE + 'Type 1' + rst + ACCENT
        q2_title2 = Colors.BOLD + Colors.BRIGHTWHITE + 'Type 2' + rst + ACCENT
        self.q2 = ACCENT + """
            2 types of logging are supported:

                """ + q2_title1 + """: syslog for logging to the system logger (/var/log/syslog)

                    OR

                """ + q2_title2 + """: logging to a user-specified file.  Hit return or type 'FILE'
                        to enable this option to send log output to a location on
        ...
        self.q4 = ACCENT + """
        When access keys are rotated, would you like a backup set of new keys written
        to disk for safekeeping?  By default, newly created access keys only exist in
        your local awscli configuration unless you choose to create a backup set.
        """ + rst

        self.q5 = ACCENT + """
        When a new keyset is created, provide a writeable location on the local file
        system where you would like the backup copy of the keyset placed ?
        """ + rst

        #tab = '\t'.expandtabs(int((self.width - len(self.header)) / 2))
        #section_header = bdwt + tab + self.header + tab + rst + ACCENT
        self.q6 = ACCENT + """
        How many days before an IAM keyset expires?  Keyup will show the age of the
        keyset in """ + rd + 'red' + ACCENT + """ after this duration passes.
        """ + rst

        closing_title = (Colors.WHITE + PACKAGE + ' configuration end ' + Colors.END)
        self.closing_footer = """

        ______________________  """ + closing_title + """  ________________________

        """ + ACCENT + """
                  This concludes configuration of local setup options.
...
def display_table(table, tabspaces=4, color=frame):
...
def set_writeable_location(default_location, header, message, confirmation_msg):
    ...
    while loop:
        ...
        print(header); sys.stdout.write(ACCENT)
        ...
        if r:
            print(rst + confirmation_msg + ' ' + fs_location)
            loop = False
        else:
            msg = 'Location is not writeable. Retry.'
