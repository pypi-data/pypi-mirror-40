...
def validity_check(idnum):
    ...
    jsonfile = json.load(codecs.open('content.json', 'r', 'utf-8-sig'))
    area = jsonfile["two_digit_number"]
    ...
    if l==18:
        result = len18check(idnum)
    elif l==15:
        result = len15check(idnum)
    else:
        result = "身份证长度不合法"
