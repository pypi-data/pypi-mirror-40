...
class Sharer(DhisAccess):
    ...
    def get_usergroup_uids(self, filter_list, access, delimiter):
        ...
        if len(response['userGroups']) > 0:
            ...
            raise PKClientException("No userGroup(s) found. Check your filter / DHIS2")
...
def filter_delimiter(argument, dhis_version):
    if '||' in argument:
        if dhis_version < 25:
            raise PKClientException("rootJunction 'OR' is only supported 2.25 onwards. Nothing shared.")
        return '||'
    else:
        return '&&'
...
def main():
    args = parse_args()
    init(args.logging_to_file, args.debug)
