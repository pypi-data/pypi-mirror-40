...
class RiskEnv(gym.Env):
    ...
    def step(self, action, troop=1, exit=False):
        if self.done:
            self.reset()
        ...
        if self.graph.move(source_id, target_id, troop, self.graph.players[0]):
            reward = 1.0
        if exit:
        ...
        self.graph.move(source_id, target_id, 2, self.graph.players[1])
        """
        ----------------------------------------------------------------------------------
        """
