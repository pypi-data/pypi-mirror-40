...
class Graph(object):
    ...
    def fortify(self, start, finish, troop_count):
        if not (troop_count >= 1 and finish.troops < 5 and start.troops >= 1):
            return False

        while troop_count >= 1 and finish.troops <= 5 and start.troops >= 1:
    ...
    def attack(self, start, finish, troop_count):
        ...
        if troop_count == 1 and finish.troops == 1:
            start.troops = start.troops - 1
            return True
