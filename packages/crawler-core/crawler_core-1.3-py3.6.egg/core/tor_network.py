...
def create_controller(address, port):
    tc = TorControl()
    tc.create_controller(address=address, control_port=port)
    tc.tor_connect()
    tc.tor_authenticate()
    return tc
...
class TorControl:
    ...
    def new_identity(self, socket_port=None):
...
class TorService:
    ...
    def __init__(self):
        self.running_tors = {}
        self._maxtor = self._maxtor
        self._db_type = self._db_type
        self._db_con_type = self._db_con_type
        self._database_config = self._database_config

    def main(self, running_tors):
        self.running_tors = running_tors
        global _database
        ...
        # change identitiy for existing controllers
        print("+ Change identity for existing controllers")
        if self.running_tors is None:
            self.running_tors = {}

        print("+ total tor running {}".format(len(self.running_tors)))
        for control_port, data in self.running_tors.items():
            controller = data.get('controller')
            print(control_port,  controller.get_newnym_wait(), controller.is_alive())
            controller.new_identity(socket_port=data.get('port'))

        # create controller for those who are already in db
        print("+ Create new controller")
        db_datas = _database.select(_view_name, view=True)
        for db_data in db_datas:
            control_port = db_data.get('control_port')
            # check is that control port already in self.running_tors
            if self.running_tors.get(control_port) is None:
                try:
                    tc = create_controller(get_ipv4(), control_port)
                    filters = {'control_port': control_port,
                               'dat_pro': None}
                    full_data = _database.select(_table_name, filters=filters)[0]
                    full_data.update({'controller': tc})
                    self.running_tors.update({control_port: full_data})
                except:
                    err = "Can't create since there is no control_auth_cookie"
                    _database.callproc([('close_tor_ip', [get_ipv4(), control_port])])

        # create enough tor services
        print("+ TorBuild")
        tb = TorBuild(tormax=self._maxtor)
        self.running_tors.update(tb.tors)

        print("All done")

        return self.running_tors
