...
class write:
    ...
    def __write_szplt(filename,data):
        if 'pytecio' not in globals():
            import pytecio 
            globals()['pytecio'] = pytecio
...
if __name__=='__main__':
    ...
    # import linecache
    # s=time.time()
    # a=read('C:\\Users\\blacksong\\Desktop\\face_m.dat',read_grid=True)
    # print(time.time()-s)
    # read('/home/yxs/F/learning/CFL3D/Rotorstator/plot3d_unfmt2.g')

    # print(dir(linecache))
    # t = linecache.getlines('C:\\Users\\blacksong\\Desktop\\face_m.dat')
    # help(linecache.getlines)
    # # embed()
    # def set_mask(triang):
    #     x,y = triang.x,triang.y
    #     tx = x[triang.triangles].mean(axis = 1)
    #     tyt = y[triang.triangles]
    #     ty = tyt.mean(axis = 1)
    #     tz = tyt.min(axis = 1)*tyt.max(axis = 1)<0
    #     condition = np.logical_or(np.hypot(tx,ty)<0.0381/2,tz)
    #     triang.set_mask(condition)
    # t = a['face_m']
    # k = t['x-velocity']**2 + t['y-velocity']**2 + t['z-velocity']**2
    # t.contour_plot('x','y',k**(1/2),set_mask=set_mask)
    # print(t.plt.get_cmap('Set1'))
    # t.plt.show()
    # b = read('lumley_2.dat')
    # print(a.keys(),b.keys())
    # for zone in a.keys():
