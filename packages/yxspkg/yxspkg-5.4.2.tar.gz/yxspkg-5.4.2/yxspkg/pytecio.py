...
class read_tecio(dict):
    ...
    def write(self,filename,verbose = True):
        ...
        k.close()
class SzpltDataType(dict):
    def __init__(self,dataset):
        self.dataset = dataset
        self.nameZones = list(dataset.keys())
        self.nameVars = list(dataset[self.nameZones[0]].keys())
        self.title = 'Pytecio data'
        def cal_zone_info(name_zone):
            d = dict()
            ...
            # valueLocation: value 1 represent the data is saved on nodes, value 0 means on elements center
            d['valueLocation'] = [1] * len(self.nameVars)
            shape = self.dataset[name_zone][self.nameVars[0]].shape
            ...
            d['name'] = name_zone
            return d
        self.zone_info = [cal_zone_info(i) for i in self.nameZones]
        self.update(self.dataset)
        self.fileType = 0
    def get_varTypes(self,name_zone,name_var):
        varTypes={'int32':3,'float64':2,'float32':1}
        d = self.dataset[name_zone][name_var]
        dtype = str(d.dtype)
        if dtype == 'int64':
            d = d.astype('int32')
            self.dataset[name_zone][name_var] = d
            dtype = 'int32'
        return varTypes[dtype]

def SzpltData(dataset):
    if isinstance(dataset,read_tecio):
        return dataset 
    else:
        if isinstance(dataset,list) or isinstance(dataset,tuple):
            dataset = {str(i+1):v for i,v in enumerate(dataset)}
        return SzpltDataType(dataset)
...
class write_tecio:
    ...
    def __init__(self,filename,dataset=None ,verbose = True):
        '''
        dataset 只要是包含两层字典的数据都可以 like d[key_zone][key_var],如果是非read_tecio类型的数据，目前只支持结构化的数据
        '''
        self.filename = filename
        self.verbose = verbose
        dataset = SzpltData(dataset)
        print(dataset.zone_info)
        self.dataset = dataset
        self.dll = GLOBAL_DLL
        self.filehandle = self._get_filehandle()
...
if __name__=='__main__':
    # test = read_tecio('output.szplt')
    # print(test.zone_info)
    # print(test.fileType)
    # k = {i:test[i] for i in test}
    # print(test.fileType)
    # print(test.zone_info)
    # SzpltData(test)
    # print(test)
    ...
    #         test[i][j]
    # print(t.keys())
    # write('output2.szplt',k)
    # test[0]['new'] = test[0][0]
    # print(test[0].keys())
    # t    = write_tecio('0605_ddes_profile_yxs_terminal_sample2.szplt',test,verbose = True)
    # t.close()
    from yxspkg import tecfile 
    m = tecfile.read('/home/yxs/F/learning/CFL3D/CFL3D/cfld_test_case/plot3dg_bin2.g')
    write('/home/yxs/F/learning/CFL3D/CFL3D/cfld_test_case/plot3dg_bin2.szplt',m)
