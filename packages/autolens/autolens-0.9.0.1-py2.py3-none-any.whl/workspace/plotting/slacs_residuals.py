...
resi = array_util.numpy_array_from_fits(path=path+'../datas/slacs/slacs1430+4105/F814W_image.fits', hdu=0)
...
resi = array_util.numpy_array_from_fits(path=path+'slacs_residuals/'+lens_name+'_Decomp_Mass_x1Sersic.fits', hdu=0)
resi = scaled_array.ScaledSquarePixelArray(array=resi, pixel_scale=0.03)
resi = resi.resized_scaled_array_from_array(new_shape=(201, 201))
...
resi = array_util.numpy_array_from_fits(path=path+'slacs_residuals/'+lens_name+'_Decomp_Mass_x2SersicOffset.fits', hdu=0)
resi = scaled_array.ScaledSquarePixelArray(array=resi, pixel_scale=0.03)
resi = resi.resized_scaled_array_from_array(new_shape=(201, 201))
array_plotters.plot_array(array=resi, norm='symmetric_log', norm_min=-0.1, norm_max=0.1, linthresh=0.0005, linscale=0.01,
