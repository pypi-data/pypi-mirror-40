...
class TFDataLoader(DLPDataLoader):
  ...
  def func(self,
    ...
    data = sess.run(self._handles.resized_image_node,
                    feed_dict={
                      self._handles.jpeg_str_placeholder:data
                      })
  ...
  @staticmethod
  def add_jpeg_decoding_resize(input_height, input_width, input_depth, batch_size=64):
    '''
    ...
    decoded_image = tf.image.decode_jpeg(jpeg_str_placeholder, channels=input_depth)
    ...
    return NOD.dict_of(jpeg_str_placeholder, resized_image_node)
