...
class AlbumentationsAugmentor(DLPAugmentor):
  def func(self, data, **kwargs) -> Iterable:
    return self.augment(data)

  def __init__(self, data_iterator=None, C=None, aug_fn=None, p=.7, **kwargs):
...
def main2(images):
  augmentor = AlbumentationsAugmentor(images)

  for image in augmentor:
    visualise(image['image'])
