...
def prepare_img_size(img_path, size=224):
...
def run_models(img_path, base_path, time_path):
  saved_model_path = base_path + time_path + '/saved_model/' + time_path
  labels_path = base_path + time_path + '/output_labels.txt'
  ...
  try:
    ...
    category_result = TFClassifier.predict(sess, model_graph=graph, input_tensor=image_in)
  ...
  results = sorcery.dict_of(dlp_predictions, vgg_predictions)

  return NOD.dict_of(results, message)
