...
additional_evaluation_terms=(),
  ...
  tf.summary.histogram(f'softmax_output', softmax_node)
  ...
  if not is_training:  # If this is an eval graph, we don't need to add loss ops or an optimizer.
    return None, None, features_tensor, label_tensor, softmax_node

  with tf.name_scope('cross_entropy_loss'):
    cross_entropy_mean_node = tf.losses.sparse_softmax_cross_entropy(labels=label_tensor,
                                                                     logits=logits)

    tf.summary.scalar('cross_entropy_loss', cross_entropy_mean_node)

  with tf.name_scope('external_addtion_terms'):
    add_terms = tf.reduce_sum(additional_evaluation_terms)

  with tf.name_scope('optimiser'):
    optimizer = tf.train.GradientDescentOptimizer(learning_rate)
    loss = tf.add( cross_entropy_mean_node,add_terms)
    train_step_node = optimizer.minimize(loss)

  return NOD(train_step_node=train_step_node,
                                cross_entropy_node=cross_entropy_mean_node,
                                input_node=features_tensor,
