...
class MongoFormatter(logging.Formatter):
    ...
    def __init__(self, fmt=None, datefmt=None):
        self.fmt = fmt._fmt
        super(MongoFormatter, self).__init__(fmt._fmt, datefmt)
...
class MongoHandler(logging.Handler):
    ...
    def fix_dict_keys(self, rec):
        """
        Fix all keys with illegal names for json format
        :param rec:
        :return:
        """
        fixed_dict = {}
        for key, val in rec.items():
            if type(val) == dict:
                rec[key] = self.fix_dict_keys(val)
            if '.' in key:
                fixed_dict[key.replace('.', '_')] = rec.pop(key)
...
class BufferedMongoHandler(MongoHandler):
    def __init__(self, level=logging.NOTSET, host='localhost', port=27017,
        ...
        MongoHandler.__init__(self, level=level, host=host, port=port, database_name=database_name,
