...
class TheLogger(IndentedLoggerAdapter):
	...
	def __init__(self,name,log_folder,level=logging.DEBUG,log_format=None,to_stream=True,**kwargs):
		super(TheLogger,self).__init__(logging.Logger(name,level),**dict(dict(spaces=1,indent_char='|---'),**kwargs))
		self.log_folder: Folder=log_folder
...
log_folder=Folder('~').create('backend_logs')
logger=TheLogger('simple_logger',log_folder)
...
# 	simple_logger=TheLogger('simple_logger',Folder(path))
