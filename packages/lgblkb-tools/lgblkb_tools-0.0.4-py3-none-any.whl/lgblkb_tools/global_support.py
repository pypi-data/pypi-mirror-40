...
from .log_support import logger,with_logging,INFORM
...
def try_to_do(func,*args,check_success=None,max_attempts=5,max_wait_seconds=0,**kwargs):
	...
	logger.debug('Trying to accomplish "%s" ...',func.__name__)
	wait_time=iter(waiting_times)
	for i in range(max_attempts):
		time_to_wait=next(wait_time)
		if time_to_wait!=0: logger.info('Sleeping for %s sec.',time_to_wait)
		time.sleep(time_to_wait)
		logger.debug('Attempt #%s.',i+1)
		...
		if check_success(out):
			...
			logger.info('Attempt #%s failed.',i+1)
	return False
...
def run_until_victory(func,retry_timeout,*args,**kwargs):
	...
	while not res_dict:
		count+=1
		logger.info(f'Starting "{func.__name__}". Counter: {count}')
...
class ReactiveTask(ABC):
	...
	def create_log_file(self,info_opts=None,**kwargs):
		logger.create_log_file(self.filename,dict(process=self.__class__.__name__,**(info_opts or {})),**kwargs)
	...
	for i,row in control_desk.iterrows():
		if func==row.gen_func:
			if resultant_status==row.ret_stat or (type(row.ret_stat) is types.FunctionType and row.ret_stat(resultant_status)):
				logger.info('resultant_status: %s,data: %s',resultant_status,data)
...
def queue_processor(queue: mp.Queue,control_desk):
	...
	try:
		...
		logger.inform(str(e),exc_info=True)
...
class ScheduledTask:
	...
	def __init__(self,parent_task=None,filename=None,**log_opts):
		...
		if not self.filename is None: self.create_log_file(level=INFORM,**log_opts)
	...
	def create_log_file(self,**kwargs):
		...
		logger.create_log_file(self.filename,process=self.__class__.__name__,**kwargs)
	...
	def run(self,*tasks,sleep_time=1):
		...
		logger.info('Start of the main loop.')
		while True:
			for task in all_tasks:
				for conveyor in task.conveyors: conveyor.run_pending()
			time.sleep(sleep_time)
	
	@staticmethod
	def on_process_error(data,exception):
		logger.exception("Exception caught when processing data:\n%s\nException text:\n%s",data,str(exception))
...
def infiterate(iter_obj,max_iter_count=None,next_getter=None,inform_count=8):
	...
	for i,obj in enumerate(an_iterable):
		if i%(max_iter_count//inform_count)==0: logger.info('%d%%, i=%d',i/(max_iter_count-1)*100,i)
...
class ParallelTasker:
	...
	def set_run_params(self,**kwargs):
		...
		logger.info('self._total_proc_count: %s',self._total_proc_count)
	...
	def __join_finished_processes(active_procs,sleep_time):
		while True:
			...
			for p in active_procs:
				if not p.is_alive():
					logger.info('Finished process found. Joining it.')
					active_procs.remove(p)
					p.join()
					# p.terminate()
					logger.info('Process successfully removed.')
					return
			time.sleep(sleep_time)
	...
	def run(self,workers_count=None,sleep_time=1.0):
		...
		while True:
			...
			if len(processes)==0:
				logger.info('Waiting for the last jobs to finish.')
				for active_p in active_procs:
					active_p.join()
				break
			if len(active_procs)<workers_count:
				logger.info('Adding a process')
				proc=processes.pop()
				proc.start()
		...
		def wrapper(*args,**kwargs):
			...
			while True:
				try:
					out=func(*args,**kwargs)
					if try_count!=1:
						logger.info('Succeeded at try #%d',try_count)
					break
				except Exception as e:
					if max_tries and max_tries<=try_count:
						logger.info('Reached max. number of tries (%d).',try_count)
