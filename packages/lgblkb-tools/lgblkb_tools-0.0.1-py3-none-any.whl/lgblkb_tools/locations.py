...
class Folder:
	...
	def __init__(self,path,parent=None):
		if not os.path.isdir(path): path=get_parent_dir(path,1)
		self.path=path
		self.parent: Folder=parent
		self.is_root=parent is None
		self.child_parts=list()
	
	def get_filepath(self,*name_portions,ext='',delim='_',include_parents=True,include_datetime=False,**name_kwargs):
		parts=list()
		if include_parents:
			parent=self
			root_parts=list()
	...
	def __repr__(self):
		return f"Folder({self.path},parent={self.parent})"
...
def main():
	...
	logs_folder=project_folder['log_files']
