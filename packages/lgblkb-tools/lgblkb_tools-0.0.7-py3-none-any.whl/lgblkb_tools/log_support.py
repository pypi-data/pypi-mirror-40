...
def with_logging(log_level=logging.INFO):
	_logger=create_logger() if logger is None else logger
	...
	return _logger.with_logging(log_level)
...
def create_logger(log_path=None):
	...
	log_folder=Folder(__file__).create('backend_logs') if log_path is None else Folder(log_path)
	# if log_path is None:
	# 	log_folder=Folder('~').create('backend_logs')
	# else:
	# 	log_folder=Folder(log_path)
