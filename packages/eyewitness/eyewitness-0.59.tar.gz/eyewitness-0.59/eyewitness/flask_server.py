...
from eyewitness.config import (DRAWN_IMAGE_PATH, RAW_IMAGE_PATH, IMAGE_ID, DETECTED_OBJECTS)
...
class BboxObjectDetectionFlaskWrapper(object):
    def __init__(self, obj_detector, image_register, detection_result_handlers,
        ...
        logging.basicConfig()
        self.logger = logging.getLogger('EyeWitness Falsk')
        app = Flask(__name__)
        self.app = app
        ...
        # collect_feedback_period
        self.detection_threshold = detection_threshold  # used for de-noise
        ...
        def detect_image_bytes_objs():
            ...
            if self.drawn_image_dir and len(detection_result.detected_objects) > 0:
                self.draw_bbox_for_detection_result(image_id, image_raw, detection_result)
    ...
    def draw_bbox_for_detection_result(self, image_id, image_raw, detection_result):
        drawn_image_path = "%s/%s_%s.%s" % (
            self.drawn_image_dir, image_id.channel, image_id.timestamp,
            image_id.file_format)
        ImageHandler.draw_bbox(image_raw, detection_result.detected_objects)
        ImageHandler.save(image_raw, drawn_image_path)
    ...
    def bbox_denoise_with_false_alert_msg(
        ...
        for detected_obj in detected_objs:
            ...
            if score > detection_threshold:
                passed_objs.append((x1, y1, x2, y2, label, score, meta))
    ...
    def update_false_alert_feedback_bbox(self):
        ...
        if (now_timestamp > time_check):
            ...
            for feedback in query:
                ...
                channel = image_id.image_id.channel
                detected_objs = list(
