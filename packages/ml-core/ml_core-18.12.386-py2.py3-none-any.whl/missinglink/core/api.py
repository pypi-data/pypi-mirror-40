...
class ApiCaller(object):
    ...
    @classmethod
    def _handle_async_api(cls, config, http_session, http_method_name, method_url, data, retry, on_response=None):
        if data is not None:
            data['async'] = True
        else:
        ...
        result = cls._handle_sync_api(config, http_session, http_method_name, method_url, data, retry).json()
        ...
        while True:
            r = cls._handle_sync_api(config, http_session, 'get', 'data_volumes/tasks/' + token)
    ...
    def call(cls, ctx_or_config, http_session, http_method_name, method_url, data=None, retry=None, is_async=False, on_response=None):
        ...
        if is_async:
            return cls._handle_async_api(config, http_session, http_method_name, method_url, data, retry, on_response=on_response)

        r = cls._handle_sync_api(config, http_session, http_method_name, method_url, data, retry)
