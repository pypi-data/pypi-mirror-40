...
# Abbreviations ----------------------------------------------------
def thread (workers = 1):
    return concurrent.futures.ThreadPoolExecutor (max_workers = workers)

def process (workers = 1):
    return concurrent.futures.ProcessPoolExecutor (max_workers = workers)
    
def subprocess (cmd, *args):
    if type (cmd) in (types.FunctionType, types.MethodType, types.BuiltinFunctionType, types.BuiltinMethodType):
        with Pool (1) as p:
            return p.apply (cmd, args)
    res = subprocess_.run(cmd, shell = True, stdout = subprocess_.PIPE, stderr = subprocess_.PIPE)    
