...
def _insertOrUpdateObjects(newThings: List[ThingImportTuple],
    ...
    try:
        ...
        for importThingIndex in newThings:
            importHashSet.add(importThingIndex.importGroupHash)

            existingObject = foundObjectByKey.get(importThingIndex.key)
            importThingTypeId = thingTypeIdsByName[
                importThingIndex.thingTypeKey]

            packedJson = importThingIndex.packJson(modelSetId, importThingTypeId)
            ...
            if existingObject:
                ...
                existingObject = ThingIndex(
                    ...
                    key=importThingIndex.key,
                    importGroupHash=importThingIndex.importGroupHash,
                    chunkKey=makeChunkKey(importThingIndex.modelSetKey,
                                          importThingIndex.key),
