...
class DB:
  ...
  def __init__(self, conn_args: ConnectionArgs, logger: Logger = getLogger('apgw')) -> None:
  ...
  async def open(self) -> 'DB':
    ...
    if not self._conn:
      if isinstance(self.conn_args, str):
        self._conn = await connect(dsn=self.conn_args)
      else:
        self._conn = await connect(**self.conn_args)
    return self
