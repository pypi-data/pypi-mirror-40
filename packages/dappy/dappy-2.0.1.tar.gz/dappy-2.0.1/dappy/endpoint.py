...
from dappy.exceptions import NotSupportedException, NotJsonException


# Users use this the define and endpoint for an API
class Endpoint:
    def __init__(self, name, path, methods=[GET], defaults={}, query_map={}):
        self.name = name
...
class CompleteEndpoint:
    ...
    def __init__(self, scheme, netloc, endpoint, json=True):
        self.scheme = scheme
        self.netloc = netloc
    ...
    def _result_in_format(self, result):
        ...
        if self.json:
            try:
                ...
                raise NotJsonException(result.text)
        else:
            return result

    # Runs a couple basic tests to make sure the
    # request succeeded
