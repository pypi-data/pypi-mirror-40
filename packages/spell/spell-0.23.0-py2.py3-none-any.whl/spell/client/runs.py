...
class RunsService(object):
    ...
    def new(self, **kwargs):
        ...
        Args:
            ...
            conda_name (str, optional): the name of the conda environment to activate (default: None)
            conda_file (str, optional): the path to a conda environment file (default: None).  If not specified
                and :obj:`conda_name` is specified, ``./environment.yml`` is assumed.
        ...
        if kwargs.get("conda_name"):
            conda_env_contents = read_conda_env_contents(kwargs.pop("conda_file", "./environment.yml"))

        # set workflow id
        workflow_id = None
        if kwargs.get("workflow_id"):
            workflow_id = kwargs.pop("workflow_id")
        elif self.client.active_workflow:
    ...
    #: (i.e., :obj:`COMPLETE`, :obj:`FAILED`, :obj:`STOPPED`, and :obj:`KILLED`)
    FINAL = (COMPLETE, FAILED, STOPPED, KILLED)
...
class Run(SpellModel):
    ...
    def refresh(self):
        ...
        Example:
            ...
            'running'
        """
        self.run = self._api.get_run(self.id)
    ...
    def cp(self, source_path="", destination_directory=None):
        ...
        source_path = "runs/{}/{}".format(self.id, source_path)
