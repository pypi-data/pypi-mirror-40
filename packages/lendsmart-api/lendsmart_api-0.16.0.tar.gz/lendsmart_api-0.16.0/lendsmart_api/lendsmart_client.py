...
from lendsmart_api.objects import PredictionChunk, PredictionWorkflow, PredictionSegment, Base, LoanApplication
...
class PredictionGroup(Group):
    ...
    def segment_create(self, label=None):
        ...
        :raises UnexpectedResponseError: If the returned data from the api does
            not look as expected.
        """
        result = self.client.post('/prediction_segements', data=label)
...
class LendsmartClient:
    def __init__(self, service_account, base_url="https://192.168.2.47:6443/api/v1", user_agent=None):
        """
    ...
    def document_get(self, disk, label=None):
        """
        Get a Document
        ...
        result = self.get('/documents', data=label)
