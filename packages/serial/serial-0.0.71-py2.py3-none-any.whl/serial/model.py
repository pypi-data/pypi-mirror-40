...
from urllib.response import addbase  # noqa - `addbase` *is* used, it's just in type hints only.
...
try:
    from typing import Union, Dict, Any, AnyStr, IO, Sequence, Mapping, Callable, Tuple  # noqa
except ImportError:
    Union = Dict = Any = AnyStr = IO = Sequence = Mapping = Callable = Tuple = None
...
def marshal( 
    ...
    """
    Recursively converts instances of `serial.model.Model` into JSON/YAML/XML serializable objects.
    """
    ...
    # If data types have been provided, validate the unmarshalled data by attempting to initialize the provided type(s) with `data`
...
class _Unmarshaller(object):
    """
    This class is used to "un-marshal" data which has been deserialized from bytes or text, but is still represented by
    generic containers. This is done by attempting to cast that data into a series of `types`. Calling an instance of
    this class will return an unmarshalled representation of the data such that containers and objects are instances
    which inherit from `serial.model.Model`. This class should not be used directly, it is intended for use only by
    `serial.model.unmarshal`
    """
    ...
    def __init__(
        ...
        self.types = types  # type: Optional[Sequence[Union[type, serial.properties.Property]]]
        self.value_types = value_types   # type: Optional[Sequence[Union[type, serial.properties.Property]]]
        self.item_types = item_types   # type: Optional[Sequence[Union[type, serial.properties.Property]]]
        self.meta = None  # type: Optional[serial.meta.Meta]
    ...
    def __call__(self):
        ...
        if (self.data is not None) and (self.data is not serial.properties.NULL):

            # If the data is a serial `Model`, get it's metadata
            if isinstance(self.data, Model):
                self.meta = serial.meta.read(self.data)

            # If `types` is a function, it should be one which expects to receive marshalled data and returns a list of
            # types which are applicable
            ...
            if self.types is None:
                ...
                #     old_metadata = serial.meta.read(self.data)

                # else:

                #     old_metadata = None

                # Attempt to unmarshal as each provided type
                for type_ in (type_ for type_ in self.types if (unmarshalled_data is None)):
                ...
                if unmarshalled_data is None:
                    ...
                    if (first_error is None) or len(self.types) != 1:
                        self.raise_error()
                    else:
                        raise first_error  # noqa - pylint identifies this as raising `None`, but that can't happend, of course

                # if old_metadata is not None:  # TODO: Determine whether this is needed...
                #     # When the original data is an instance of `serial.properties.Model`, we check to see if the
                #     # unmarshalled data has the *same* metadata

                #     new_metadata = serial.meta.read(self.data)

                #     if new_metadata is not None:

                #         if old_metadata is not new_metadata:

                #             writable = False

                #             if isinstance(old_metadata, serial.meta.Object):

                #                 for property_name, value in properties_values(old_metadata):
                ...
                #                             if not writable:
                #                                 new_metadata = serial.meta.writable(self.data)
                ...
                #             elif isinstance(old_metadata, serial.meta.Array):

                #                 pass

                #             elif isinstance(old_metadata, serial.meta.Dictionary):
    ...
    def as_container_or_simple_type(self):
        ...
        if isinstance(self.data, Dictionary):

            if (self.meta is None) or (self.meta.value_types is None):  # It's a generic container
            ...
            if (self.meta is None) or (self.meta.item_types is None):  # It's a generic container
            ...
            isinstance(self.data, (collections_abc.Set, collections_abc.Sequence, Array))
        ) and (
        ...
        elif not isinstance(self.data, (str, bytes, native_str, Number, Decimal, date, datetime, bool, Model)):

            raise serial.errors.UnmarshalValueError(
    ...
    def backport_types(self):
        ...
        if (str in self.types) and (native_str is not str) and (native_str not in self.types):
            ...
            )))  # type: Tuple[Union[type, serial.properties.Property], ...]

    def as_type(
        self,
        type_,  # type: Union[type, serial.properties.Property]
    ):
        # type: (...) -> bool

        unmarshalled_data = None  # type: Union[Model, Number, str, bytes, date, datetime]
        ...
        if isinstance(
            ...
            if issubclass(type_, Object) and isinstance(self.data, (dict, collections.OrderedDict)):
                ...
                isinstance(self.data, (collections_abc.Set, collections_abc.Sequence, Generator)) and
                (not isinstance(self.data, (str, bytes, native_str))) and
                issubclass(type_, (collections_abc.Set, collections_abc.Sequence)) and
    ...
    def raise_error(self):
        # type: () -> None
        """
        Generate a comprehensible error message for data which could not be unmarshalled according to spec, and raise
        the appropriate exception, which should be an instance of one of the following:
        ...
        raise serial.errors.UnmarshalTypeError(
...
def unmarshal(
    ...
    # type: (...) -> Optional[Union[Model, str, Number, date, datetime]]
    """
    Convert `data` into an instance of `serial.model.Model`, and recursively do the same for all member data
    """
...
def serialize(data, format_='json'):
    # type: (Union[Model, str, Number], Optional[str]) -> str
    """
...
def version(data, specification, version_number):
    ...
    elif isinstance(data, (collections_abc.Set, collections_abc.Sequence)) and not isinstance(data, (str, bytes)):
        # for d in data:
