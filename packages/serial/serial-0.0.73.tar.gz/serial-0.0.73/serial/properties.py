...
from builtins import *  # noqa - the import comnes after what is above necessary in order set up python 2x compatibility
...
from .abc.properties import Property as _PropertyBase
from . import model, errors, meta
...
class Property(object):
            ...
            if callable(types_or_properties):
                ...
                if native_str is not str:
                    ...
                    def types_or_properties(d):
                        # type: (Sequence[Union[type, Property, Model]]) -> Types
    ...
    def marshal(self, data):
        # type: (Any) -> Any
        return model.marshal(data, types=self.types)  #, types=self.types, value_types=self.value_types)
...
_PropertyBase.register(Property)
    ...
    def values(self, values):
        ...
        if not ((values is None) or callable(values)):
            ...
            if values is not None:
                values = model.Array(model.unmarshal(v, types=self.types) for v in values)
...
class Array(Property):
    ...
    def unmarshal(self, data):
        # type: (Any) -> Any
        return model.unmarshal(data, types=self.types, item_types=self.item_types)

    def marshal(self, data):
        # type: (Any) -> Any
        return model.marshal(data, types=self.types, item_types=self.item_types)
        ...
        if item_types is not None:
            if callable(item_types):
                if native_str is not str:
                    ...
                    def item_types(d):
                        # type: (Sequence[Union[type, Property, model.Object]]) -> Types
