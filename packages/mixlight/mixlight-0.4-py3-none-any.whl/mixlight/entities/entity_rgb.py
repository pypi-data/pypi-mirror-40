...
PAYLOAD_CONFIG = '{"name": "%s", "unique_id": "%s.%s", "command_topic": "%s", "schema": "json", "hs": "true", "brightness":"true" }'
...
class entity_RGB():
  ...
  def __init__(self, id, addr, name, type, linker):
    ...
    self.linker = linker
    self.hsb = [270, 0, 255]
    self.topic_config = linker.buildTopic(PLATFORM, id, "config")
    self.topic_state = linker.buildTopic(PLATFORM, id, "state")
    self.topic_set = linker.buildTopic(PLATFORM, id, "set")

  def create(self):
    if self.linker.client is None:
      return
    payload = PAYLOAD_CONFIG % (self.name, PLATFORM, self.id, self.topic_set)
