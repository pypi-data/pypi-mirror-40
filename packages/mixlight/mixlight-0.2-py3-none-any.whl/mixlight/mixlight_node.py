...
class mixlight_Node():
    ...
    if 'output' in cfg:
      ...
      for type in output:
        ...
        for value in output[type]:
          ...
          name = value[addr]
          id = "MIX-O%s-E%s" % (universe, addr)
          if type == 'switch':
            self.entities[id] = entity_Switch(id, addr, name, type, self)
          elif type == 'light':
            self.entities[id] = entity_Light(id, addr, name, type, self)
          elif type == 'dimmer':
            self.entities[id] = entity_Dimmer(id, addr, name, type, self)
          elif type == 'rgb':
            self.entities[id] = entity_RGB(id, addr, name, type, self)
    # read input entities
    if 'input' in cfg:
      input = cfg['input']
      ...
      for group in input['boards']:
        ...
        for type in group:
          ...
          conf = group[type]
          if type == 'temperature':
            id = "MIX-I%s-B%s-ET" % (universe, board)
            self.entities[id] = entity_Temp(id, board, conf, type, self)
          elif type == 'humidity':
            id = "MIX-I%s-B%s-EH" % (universe, board)
            self.entities[id] = entity_Humi(id, board, conf, type, self)
          elif type == 'sensor':
            id = "MIX-I%s-B%s-ES" % (universe, board)
            self.entities[id] = entity_Sens(id, board, conf, type, self)
          else:
            for value in conf:
              nr = next(iter(value))
              name = value[nr]
              id = "MIX-I%s-B%s-E%s" % (universe, board, nr)
              if name in self.entities:
                self.entities[id] = entity_Link(id, board, nr, name, type, self)
              else:
                self.entities[id] = entity_Bina(id, board, nr, name, type, self)
  ...
  def change(self, uni, board, nr):
    self.queue.put("MIX-I%s-B%s-E%s" % (uni, board, nr))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
      print ("Please pass the config folder as a parameter")
    else:
      linker = mixlight_Node(sys.argv[1])
      linker.load_config()
      linker.start()
      try:
        while True:
