...
def main(args=None):
  ...
  parser.add_argument('action', choices=[ 'config', 'install', 'start' ], help='action to be taken')
  ...
  if parsed.action == 'config':
    ...
    if not os.path.exists(directory):
      os.makedirs(directory)
    # config files
    fileCopy(sourcePath, directory, 'mixlight.yaml')
    fileCopy(sourcePath, directory, 'entities.yaml')
  elif parsed.action == 'install':
    daemonsFolder = '/lib/systemd/system'
    fileCopy(sourcePath, daemonsFolder, 'mixlight.service', directory)
