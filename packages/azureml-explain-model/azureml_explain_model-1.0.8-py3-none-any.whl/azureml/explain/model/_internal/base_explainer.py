...
from .common import MAX_NUM_BLOCKS, BLOCK_SIZE, NUM_FEATURES, NUM_BLOCKS, module_logger
...
class BaseExplainer:
    ...
    def _get_model_summary_artifacts(self, upload_dir, name, summary):
        ...
        block_size = self.storage_policy[BLOCK_SIZE]
        storage_metadata = {
            MAX_NUM_BLOCKS + '_' + name: self.storage_policy[MAX_NUM_BLOCKS],
            BLOCK_SIZE + '_' + name: self.storage_policy[BLOCK_SIZE],
            NUM_FEATURES + '_' + name: num_columns,
            NUM_BLOCKS + '_' + name: num_blocks
        }
        ...
        for idx in range(num_blocks):
            ...
            artifacts[idx]['prefix'] = os.path.normpath('{}/{}/{}/{}'.format(RUN_ORIGIN,
                                                                             self.run.id,
                                                                             upload_dir,
                                                                             block_name))
            start += block_size
