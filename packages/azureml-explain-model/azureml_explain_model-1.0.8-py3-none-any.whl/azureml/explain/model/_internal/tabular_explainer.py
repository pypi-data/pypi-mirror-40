...
class TabularExplainer(BaseExplainer):
    ...
    def explain_model(self, model, initialization_examples, evaluation_examples=None,
        ...
        :return: tuple(run, feature_importance_values, expected_values, overall_feature_importance_values,
            overall_important_features, per_class_feature_importance_values, per_class_important_features)
            where
            ...
            descending order.
            - overall_important_features = The feature names sorted in the same order as in overall_summary
            or the indexes that would sort overall_summary.
            - per_class_feature_importance_values = The class level feature importance values sorted in
            descending order where a binary classification (this class or not) is evaluated. Only available
            for the classification case.
            - per_class_important_features = The feature names sorted in the same order as in per_class_summary
            or the indexes that would sort per_class_summary. Only available for the classification case.
        ...
        if create_scoring_model:
            ...
            self.neighbors = NearestNeighbors(n_neighbors=1, algorithm='ball_tree').fit(scaled_data)
        ...
        if classification:
            shap_values, expected_values, overall_summary, overall_imp, overall_order, \
                per_class_summary, per_class_imp, per_class_order = \
        ...
        if classification:
            ...
            l_per_class_summary = per_class_summary.tolist()
            l_per_class_imp = per_class_imp.tolist()
        else:
        ...
        l_overall_summary = overall_summary.tolist()
        l_overall_imp = overall_imp.tolist()
        if self.run is not None:
            ...
            if features is not None:
                ...
                summary_object.add_from_get_model_summary(
                    self._get_model_summary_artifacts(upload_dir,
                                                      'overall_feature_order',
                                                      np.array(_sort_feature_list_single(features, overall_order))))
                if classification:
                    summary_object.add_from_get_model_summary(
                        self._get_model_summary_artifacts(upload_dir,
                                                          'per_class_feature_order',
            ...
            if classification:
                ...
                if classes is not None:
                    ...
                    self._upload_artifact(upload_dir, 'classes', classes)
                    class_artifact_info = [{'prefix': os.path.normpath('{}/{}/{}/{}'.format(RUN_ORIGIN,
                                                                       self.run.id,
                                                                       upload_dir,
                                                                       'classes'))}]
                    class_metadata_info = {'num_classes': len(classes)}
                    summary_object.add_from_get_model_summary((class_artifact_info, class_metadata_info))
                self.assets_client.create_asset('explanation_asset',
                                                summary_object.get_artifacts(),
                                                metadata_dict=summary_object.get_metadata_dictionary(),
                                                run_id=self.run.id,
                                                properties={'type': History.EXPLANATION,
                                                            'version_type': History.EXPLANATION_ASSET_TYPE})
                return self.run, shap_values, expected_values, l_overall_summary, l_overall_imp, \
                    l_per_class_summary, l_per_class_imp
            else:
                self.assets_client.create_asset('explanation_asset',
                                                summary_object.get_artifacts(),
                                                metadata_dict=summary_object.get_metadata_dictionary(),
            ...
            if classification:
                return shap_values, expected_values, l_overall_summary, l_overall_imp, \
                    l_per_class_summary, l_per_class_imp
            else:
                return shap_values, expected_values, l_overall_summary, l_overall_imp
    ...
    def _compute_summary(self, shap_values, classification, features, top_k):
        ...
        if classification:
            ...
            overall_summary = overall_summary[overall_order]
        if features is not None:
            overall_imp = _sort_features(features, overall_order)
            if classification:
                per_class_imp = _sort_features(features, per_class_order)
        else:
        ...
        if classification:
            return shap_values, expected_values, overall_summary, overall_imp, overall_order, per_class_summary, \
                per_class_imp, per_class_order
        else:
            return shap_values, expected_values, overall_summary, overall_imp, overall_order
    ...
    def explain_from_scoring_model(self, evaluation_examples):
        ...
        :type evaluation_examples: numpy.array or pandas.DataFrame or scipy.sparse.csr_matrix
        :return feature_importance_values: For a model with a single output such as regression,
        ...
        :rtype feature_importance_values: list
        """
    ...
    def explain_from_cached_explainer(self, evaluation_examples, features=None, top_k=None, **kwargs):
        ...
        classification = isinstance(shap_values, list)
        if classification:
            shap_values, expected_values, overall_summary, overall_imp, _, \
                per_class_summary, per_class_imp, _ = \
                self._compute_summary(shap_values, classification, features, top_k)
