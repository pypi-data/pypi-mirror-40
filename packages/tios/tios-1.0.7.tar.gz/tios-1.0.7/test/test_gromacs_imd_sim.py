...
class GromacsWorkflowTest(unittest.TestCase):
    ...
    def test_simulation_startup(self):
        command_line = 'gmx mdrun -deffnm {}/bpti'.format(self.testdir)
        ...
        self.assertIsInstance(imd_job.trajectory, mdt.Trajectory)
        self.assertIsInstance(imd_job.trajectory.xyz, np.ndarray)

    def test_simulation_startup_with_checkpoint(self):
        command_line = 'gmx mdrun -deffnm {}/bpti -cpi {}/bpti.cpt'.format(self.testdir, self.testdir)
        # Attempt to start the job:
        imd_job = imd_sim.from_command(command_line)
        # Have we created an instance of an imd_sim.imd_job?
        self.assertIsInstance(imd_job, imd_sim.IMDJob)
        # Is the status 'Ready'?
        self.assertEqual(imd_job.status, 'Ready')
        self.assertEqual(imd_job.timepoint, 1.04)
    ...
    def test_simulation_start(self):
        ...
        imd_job.step()
        self.assertEqual(imd_job.timepoint, imd_job._dt)
        # Move the simulation forward again. This time it should be trate
        # further forward:
        imd_job.step()
        self.assertEqual(imd_job.timepoint, imd_job._dt + imd_job._trate)
