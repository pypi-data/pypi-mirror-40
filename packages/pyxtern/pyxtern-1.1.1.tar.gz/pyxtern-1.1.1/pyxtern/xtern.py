...
from pyxtern.utils import _temporary_directory, _log_to_streams, _check_cmd
...
class Cmd(list):
    ...
    def add_arg(
        ...
        elif not kwargs and not val and not flag:
            raise RuntimeError(
                "add_arg() requires either kwargs or val to be set if flag=False."
            )
        if flag:
            # Create arg string
            if val:
