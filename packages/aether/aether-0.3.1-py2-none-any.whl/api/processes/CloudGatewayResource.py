import aether as ae
from aether.proto.api_pb2 import SpacetimeBuilder, Spacetime, BytesTransmission
...
import time, hashlib, tempfile, json
...
class CloudGatewayResource(PostMethodResourceBase):
    ...
    def post(self):
        ...
        return api_utils.log_and_return_pb(response, 200, request, logger)
    ...
    def DownloadSpacetime(self, request):
        ...
        if len(builder.btops) != 0:
            for op_i, op_string in enumerate(builder.btops):
                try:
                    op = sky_utils.deserialize_from_bytestring(op_string.serialized_func)
                except Exception:
                    return api_utils.log_and_return_status(
        ...
        if "url_only" in args and args["url_only"]:
            ...
            self._logger.info("Writing Spacetime {} to location {}".format(built_hash, destination_stub))
