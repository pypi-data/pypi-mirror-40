...
class SkyApplicationResource(Resource):
    ...
    def _handle_boundmethod(self, object_buffer, message_i, message):
        try:
            ...
            return api_utils.log_and_return_status("SkyApplication BoundMethod on Message {} could not be deserialized".format(message_i), 400, message, logger, exc_info=True)

        try:
            func = getattr(SpacetimeMethodWrapper(object_buffer["Spacetime"]), method.method_name)
        except AttributeError:
            return api_utils.log_and_return_status("SkyApplication BoundMethod on Message {} not found on Spacetime object".format(message_i), 400, message, logger, exc_info=True)

        try:
            response = func(**parameters)
        except Exception:
            return api_utils.log_and_return_status("SkyApplication BoundMethod on Message {} failed during operation".format(message_i), 400, message, logger, exc_info=True)
    ...
    def _handle_microservice(self, client, object_buffer, polygon, message_i, message):
        try:
            ...
            return api_utils.log_and_return_status("SkyApplication Message {} had ill formed json.".format(message_i), 400, message, logger, exc_info=True)
        ...
        if message_i == 0:
            ...
            try:
                ...
                return api_utils.log_and_return_status("SkyApplication Message {} had ill formed SpacetimeBuilder.".format(message_i), 400, message, logger, exc_info=True)
