...
class MascotDatFile(object):
    def __init__(self, dat_file_path, decoyMode = False, **kwargs):
        ...
        hasSpectralHits = self.res_file.anyPeptideSummaryMatches(self.res_file.SEC_LIBRARYPEPTIDES)
        if hasSpectralHits:
            assert not self.res_file.anyPeptideSummaryMatches(self.res_file.SEC_PEPTIDES)
            #mspepsumFlags = ms.ms_peptidesummary.MSPEPSUM_SL_INTEGRATED
            mspepsumFlags = ms.ms_peptidesummary.MSPEPSUM_SL_ONLY
        else:
        ...
        if hasSpectralHits:
            if self.var_mod_dict:
                print "WARNING- Non-spectral mods will be overwritten!"
            for i in range(1, self.params.getNumberOfLibraryMods()+1):
                self.var_mod_dict[i] = self.params.getLibraryModName(i)
    ...
    def parse_mod_pos(self, sequence, var_mod_pos, queryPeptide):
        if var_mod_pos[0] not in ['0', 'Y']:
            try:
                var_mods = ['N-term: %s' % self.var_mod_dict[var_mod_pos[0]]]
            except KeyError:
                assert var_mod_pos[0] == 'X' or var_mod_pos[0] == 'Y'
        ...
        for i, aa in enumerate(sequence):
            try:
                ...
                assert seq_mods[i] == 'X' or seq_mods[i] == 'Y' # Error-tolerant imputed modification or spectral somethingorother?
