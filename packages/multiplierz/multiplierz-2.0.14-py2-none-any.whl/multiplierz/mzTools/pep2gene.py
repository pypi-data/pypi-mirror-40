...
def pep_to_gene(peptides, fastafile):
    # Don't tell Guillaume or Manor it was this simple.
    
    fasta = parse_to_generator(fastafile)
    find_all_peptides = re.compile('|'.join(set(peptides)))
    
    pep_map = defaultdict(list)
    i = 0
    for header, protseq in fasta:
        if 'rev_' in header: continue
        
        matches = find_all_peptides.finditer(protseq)
        if matches:
            if 'GN=' in header:
                gene = header.split('GN=')[1].split()[0]
            elif '_HUMAN' in header:
                gene = header.split('_HUMAN')[0].split('|')[1]
            elif '|' in header:
                gene = header.split('|')[1]
            else:
                print "(%d) No gene parse for: %s" % (len(list(matches)), header)
                continue
            
            for match in matches:
                pep_map[match.group(0)].append(gene)
