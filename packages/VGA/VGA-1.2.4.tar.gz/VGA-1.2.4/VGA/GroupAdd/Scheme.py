...
# Scheme contains the group pattern information, and can assign groups
# based on the smiles.
...
class GroupAdditivityScheme(Scheme):
    def __init__(self, patterns=[], pretreatment_rules=[], remaps={},
                 other_descriptors=[], smiles_based_descriptors=[],
                 smarts_based_descriptors=[], include=[]):
        ...
        for scheme in include:
            ...
            self.smiles_based_descriptors = scheme_object.\
                smiles_based_descriptors
            self.smarts_based_descriptors = scheme_object.\
    ...
    def GetDescriptors(self, mol, debug=0):
        if isinstance(mol, Chem.Mol):
            ...
            # sanitize false is necessary to read adsorbed hydrogen species.
            # (e.g. [Pt][H])
            mol.replace('{', '')
            mol.replace('}', '')
            # mol = Chem.MolFromSmiles(mol,sanitize=False) # Stereochemistry
            # get's erased. :/
        ...
        if hasattr(self, 'remaps'):
            for atom in mol.GetAtoms():
                if atom.GetProp('Group_name') in self.remaps:
                    atom.SetProp('Group_name',
                                 self.remaps[atom.GetProp('Group_name')][0][1])
...
def _aromatization_Benson(mol):
    ...
    for i in range(0, len(rings)):
        ...
        # bond check
        if mol.GetBondBetweenAtoms(rings[i][0],
                                   rings[i][1]).\
                GetBondType().__str__() ==\
                'SINGLE':
            if mol.GetBondBetweenAtoms(rings[i][1],
                                       rings[i][2]).\
                    GetBondType().__str__() ==\
                    'DOUBLE':
                if mol.GetBondBetweenAtoms(rings[i][2],
                                           rings[i][3]).\
                        GetBondType().__str__() ==\
                        'SINGLE':
                    if mol.GetBondBetweenAtoms(rings[i][3],
                                               rings[i][4]).\
                            GetBondType().__str__() ==\
                            'DOUBLE':
                        if mol.GetBondBetweenAtoms(rings[i][4],
                                                   rings[i][5]).\
                                GetBondType().__str__() ==\
                                'SINGLE':
                            if mol.GetBondBetweenAtoms(rings[i][5],
                                                       rings[i][0]).\
                                    GetBondType().__str__() ==\
                                    'DOUBLE':
                                pass
                ...
                continue
        elif mol.GetBondBetweenAtoms(rings[i][0],
                                     rings[i][1]).\
                GetBondType().__str__() ==\
                'DOUBLE':
            if mol.GetBondBetweenAtoms(rings[i][1],
        ...
        mol.GetBondBetweenAtoms(rings[i][0],
                                rings[i][1]).\
            SetBondType(Chem.rdchem.BondType.AROMATIC)
        mol.GetBondBetweenAtoms(rings[i][1],
                                rings[i][2]).\
            SetBondType(Chem.rdchem.BondType.AROMATIC)
        mol.GetBondBetweenAtoms(rings[i][2],
                                rings[i][3]).\
            SetBondType(Chem.rdchem.BondType.AROMATIC)
        mol.GetBondBetweenAtoms(rings[i][3],
                                rings[i][4]).\
            SetBondType(Chem.rdchem.BondType.AROMATIC)
        mol.GetBondBetweenAtoms(rings[i][4],
                                rings[i][5]).\
            SetBondType(Chem.rdchem.BondType.AROMATIC)
        mol.GetBondBetweenAtoms(rings[i][5],
                                rings[i][0]).\
            SetBondType(Chem.rdchem.BondType.AROMATIC)
