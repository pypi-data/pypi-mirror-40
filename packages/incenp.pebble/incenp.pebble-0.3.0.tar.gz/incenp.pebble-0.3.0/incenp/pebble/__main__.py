...
from incenp.pebble.cli import get_commands
...
def main(argv=None):
    ...
    parser.add_argument('-f', '--refresh', dest='age', const=0, action='store_const', default=86400,
    ...
    commands = get_commands(parser.add_subparsers(dest='command'))
    ...
    try:
        vault.load(age=args.age)
        commands[args.command].execute(vault, args)
    except Error as e:
        die(e)
    finally:
        server.close()
