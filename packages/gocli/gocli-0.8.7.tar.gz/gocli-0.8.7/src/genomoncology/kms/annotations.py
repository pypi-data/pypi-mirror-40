...
async def async_annotate(
    ...
    annotated_calls = annotate_calls(annotations_list, data, fields)
...
def annotate_calls(annotations_list, data, fields):
    # todo: determine if it makes sense to not return all annotations....
    fields = None  # resolve_fields(fields)
    func = partial(add_annotation, annotations_list, fields)
    annotated_calls = list(map(func, data))
    return annotated_calls


def add_annotation(annotations_list: AnnotationList, fields, call: dict):
