...
def updateRedisWorksHots(worksHots):
	r = redis.Redis(host='r-wz9536616f776d74.redis.rds.aliyuncs.com', port=6379, db=3, password='Idltest503')
	#r = redis.Redis(host='r-bp1f37095aef68e4.redis.rds.aliyuncs.com', port=6379, db=8, password='flrjovlOJEROIJ324')
	r.set('recommend:srv:downgrade:keykey', 'recommend:srv:downgrade:cache')
	print(r.get('recommend:srv:downgrade:keykey'))
	r.disconnect()
	#print(worksHots)
...
def searchDbConfig():
	config = {}
	db = getDb()
	#config['hotVids'] = [item[0] for item in searchDb(db, 'select aid from author_whitelist')]
	config['hotVids'] = []
	db.close()
	return config
...
def coverStatisticsDataReal(index, dataDict):
	...
	if updateStatisticsData(index, dataDict):
		#操作频繁会发生冲突
		time.sleep(20)
		return True#deleteStatisticsData(index, {"query":{"bool":{"must_not":{"term":{"updateMark": updateMark}}}}})
	else:
		return False
...
def deleteStatisticsData(index, body):
	return len(getEsclient().delete_by_query(index='statistics_' + index + '_write', doc_type='data', body=body, conflicts='proceed')['failures']) == 0
...
def scrollSearch(index, doc_type, body, searchAll=True, log=True, version=False):
	...
	hits = []
	if searchAll:
		result = esclient.search(index=index, doc_type=doc_type, scroll='1m', body=body, version=version, preference='_primary_first')
		while len(result['hits']['hits']) != 0:
			hits.extend(result['hits']['hits'])
			scroll_id = result['_scroll_id']
			result = esclient.scroll(scroll_id=scroll_id, scroll='1m')
	else:
		hits.extend(esclient.search(index=index, doc_type=doc_type, body=body, version=version, preference='_primary_first')['hits']['hits'])
	if log:
