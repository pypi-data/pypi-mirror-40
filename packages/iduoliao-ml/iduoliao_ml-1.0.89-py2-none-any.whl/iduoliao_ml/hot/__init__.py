...
def cycleUpdateHotList(configFileName, isTest):
	...
	worksList = dataDispose.getWorksList(config['aid']['white'], config['vid']['black'])
	while True:
		#try:
			start = time.time()
			ilog.info('start:' + str(start) + '(' + time.strftime(es.esTimeFormatNoMs, time.localtime(start)) + ')')
			
			endTime = onceUpdateHotList(subjectHistory, worksHistory, worksList, endTime, config, isTest)
...
def onceUpdateHotList(subjectHistory, worksHistory, worksList, startTime, config, isTest):
	endTime = updateHotList(subjectHistory, worksHistory, worksList, startTime, config)
	#10秒后通知后台更新
	time.sleep(10)
	noticeServer(isTest)
	saveHotTopList()
...
def updateHotList(subjectHistory, worksHistory, worksList, startTime, config):
	...
	for hit in newHits:
		action = hit['_source']
		worksDict[action['vid']].updateAction(action)
	...
	worksHots = calHotV2(subjectHistory, worksHistory, worksList, config['aid']['half'])
