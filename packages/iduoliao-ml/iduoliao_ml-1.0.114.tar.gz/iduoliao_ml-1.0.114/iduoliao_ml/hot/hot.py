...
def calHotV2(subjectHistory, worksHistory, worksList, dauFactor, dbConfig, halfAids):
	print("dauFactor: " + str(dauFactor))
	print("dbConfig: " + str(dbConfig))

	calWorksScore(worksList, dauFactor)
	...
	for index, works in enumerate(worksList):
		vid = works.vid
		aid = works.aid
		if vid in dbConfig['blackVids'] or (works.auditStatus == 0 and long(aid) not in dbConfig['whiteAids']):
			continue
		actionHot = (works.score + (worksHistory[vid]['score'] if worksHistory.has_key(vid) else 0)) / 2
		hotList[vid] = {'vid': vid, 'aid': aid, 'title': works.title, 'hot': (baseHotMat[index, 0] * SubjectTotalWeight + actionHot) * getDecayRatio(works.publishTime) * (0.5 if aid in halfAids else 1) * 100000, 'isNew': works.publishTime < 72}
