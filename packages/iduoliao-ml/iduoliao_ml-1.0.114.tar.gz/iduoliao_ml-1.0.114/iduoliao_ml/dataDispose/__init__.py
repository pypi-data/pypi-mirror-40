...
def getDauFactor():
	...
	zeroTime = time.mktime((now.tm_year, now.tm_mon, now.tm_mday, 0, 0, 0, 0, 0, 0)) * 1000
...
def toWorks(worksInfo):
	...
	if publishTime > 1440:
		return
	return Works(worksInfo['vid'], worksInfo['aid'], worksInfo['title'], worksInfo['publishtime'], worksInfo['duration'] if worksInfo.has_key('duration') else 0, worksInfo['audit_status'] if worksInfo.has_key('audit_status') else 0)
