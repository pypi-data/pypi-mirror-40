...
def cycleUpdateHotList(configFileName, isTest):
	...
	while True:
		try:
			...
			maxInterval = 120 if isTest else 600
			if cost < maxInterval:
...
def onceUpdateHotList(subjectHistory, worksHistory, oriWorksDict, startTime, configFileName, isTest):
	endTime = updateHotList(subjectHistory, worksHistory, oriWorksDict, startTime, readConfig(configFileName), isTest)
	#10秒后通知后台更新
	time.sleep(10)
	noticeServer(isTest)
	saveHotTopList()
...
def updateHotList(subjectHistory, worksHistory, oriWorksDict, startTime, config, isTest):
	start = time.time()

	
	dbConfig = es.searchDbConfig()
	ilog.info('dbConfig:' + str(dbConfig))
	hotVids = dbConfig['hotVids'] + getOldHotVids(config)
	#print(hotVids)
	worksDict = {}
	for vid in hotVids:
	...
	worksHots = calHotV2(subjectHistory, worksHistory, worksDict.values(), dauFactor, dbConfig, config['aid']['half'])
