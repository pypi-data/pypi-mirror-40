...
def createWorksArgsList(worksPersonas, aids):
	subjectDict, worksDict = createDict(es.scrollSearch('works_video_read', 'video', {"size":1000}), aids)
	...
	for index, hit in enumerate(personas):
		...
		for action in actions:
			...
			if len(items) > 2 and items[2].isdigit():
				...
				worksPersonas.addAction(uid, cTime, items[1], items[2], items[3:])
	...
	for hit in works:
		...
		for index, value in enumerate(DurationSections):
			if duration < value:
				durationIndex = index
				break
		worksDict[worksInfo['vid']] = {'title': worksInfo['title'], 'aid': aid, 'publishTime': publishTime, 'playTime': 0, 'timeCount': 0, 'show': 0, 'showPlay': 0, 'recShow': 0, 'recShowPlay': 0, 'durationIndex': durationIndex, 'durationAvgPlay': 0, 'statis': [0, 0, 0, 0, 0, 0, 0, 0, 0, duration, size]}
