...
class Works(object):
	...
	def getDataByVid(self, vid, startTime, endTime):
		...
		replay = 0
		hits = es.searchStatisticsDataByBody("works_players", {"query": {"term": {"vid": vid}}, "size": 10000}, True, True, True)
		for hit in hits:
			if hit['_version'] >= 2:
				replay += 1
		...
		return len(hits), counts[12], counts[13], replay, counts[16] - counts[17], counts[14] - counts[15], counts[2], timeAvg if timeAvg != None else 0
