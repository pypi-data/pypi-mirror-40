...
class Works(object):
	...
	def toSaveEsData(self):
		return {"vid": self.vid, "score": self.score, "time": self.endTime, "ratio": self.ratio, "aid": self.aid, "title": self.title, "play": self.play, "shareF": self.shareF, "shareI": self.shareI, "replay": self.replay, "sub": self.sub, "like": self.like, "longPlay": self.longPlay, "timeAvg": self.timeAvg, "resShow": self.resShow, "resPlayShow": self.resPlayShow}
	...
	def updateActions(self, endTime, startTime=0):
		...
		play, shareF, shareI, replay, like, sub, longPlay, timeAvg, resShow, resPlayShow = self.getDataByVid(self.vid, startTime, endTime)
		self.play += play
		self.shareF += shareF
		self.shareI += shareI
		self.replay += replay
