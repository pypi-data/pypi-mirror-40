...
def calHotV2(subjectHistory, worksHistory, worksList, dauFactor, dbConfig, halfAids):
	...
	for index, works in enumerate(worksList):
		...
		hotList[vid] = {'vid': vid, 'aid': aid, 'title': works.title, 'hot': (baseHotMat[index, 0] * SubjectTotalWeight + actionHot) * getDecayRatio(works.publishTime) * (0.7 if aid in halfAids else 1) * 100000, 'isNew': works.publishTime < 72}
