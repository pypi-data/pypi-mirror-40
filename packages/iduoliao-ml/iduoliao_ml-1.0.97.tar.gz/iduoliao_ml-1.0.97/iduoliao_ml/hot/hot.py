...
def calHotV2(subjectHistory, worksHistory, worksList, halfAids):
	calWorksScore(worksList, 4)
	...
	for index, works in enumerate(worksList):
		vid = works.vid
		aid = works.aid
		actionHot = (works.score * 0.3 + (worksHistory[vid]['score'] if worksHistory.has_key(vid) else 0) * 1.7) / 2
