...
def getWorksList(aids=[], blackVids=[]):
	worksList = []
	
	for hit in es.scrollSearch('works_video_read', 'video', {"size":1000}):
		works = toWorks(hit['_source'], aids, blackVids)
		if works != None:
			worksList.append(works)
	return worksList

def updateHistoryActions():
	works = Works('9c0754f99506b42fdff83ac54d32508e', 1, '132', 31, 3044)
	works.updateActions(1546617600)
	...
	es.updateStatisticsData('subject_history', subjectHistory)

	hot.calWorksScore(worksList)
	worksHistory = {}
	for works in worksList:
		worksHistory[works.vid] = works.toSaveEsData()
	es.updateStatisticsData('works_history', worksHistory)

			

def toWorks(worksInfo, aids=[], blackVids=[]):
	...
	if not worksInfo.has_key('publishtime') or worksInfo['publishtime'] == None:
		return
	#if not worksInfo.has_key('audit_status'):
	#	return
	aid = worksInfo['aid']
	vid = worksInfo['vid']
	#if vid in blackVids or (worksInfo['audit_status'] == 0 and not aid in aids):
	if vid in blackVids or (len(aids) != 0 and not aid in aids):
		return
	...
	if publishTime > 1440:
		return
	return Works(vid, aid, worksInfo['title'], worksInfo['publishtime'], worksInfo['duration'] if worksInfo.has_key('duration') else 0)
...
simpleStatisticsMap = {'4': 12, '5': 13, '10': 14, '11': 15, '2': 16, '3': 17}
