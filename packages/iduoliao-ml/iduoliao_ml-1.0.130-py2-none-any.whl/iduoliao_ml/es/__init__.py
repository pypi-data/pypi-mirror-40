...
def updateRedisWorksHots(worksHots):
	...
	if r.get(cacheKeyKey) == cacheName:
		r.zadd(backCacheName, worksHots)
...
def deleteStatisticsData(index, body):
	return len(getEsclient().delete_by_query(index='statistics_' + index + '_write', doc_type='data', body=body, conflicts='proceed', preference='_primary_first')['failures']) == 0
