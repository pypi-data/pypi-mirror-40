...
def deletePastWorksRecord():
	vids = []
	for hit in es.scrollSearch('works_video_read', 'video', {"query": {"range": {"publishtime": {"lt": (long(time.time()) - 14 * 24 * 60 * 60) * 1000}}}, "size":1000}):
		vids.append(hit['_id'])
		if len(vids) == 1000:
			es.deleteStatisticsData('works_hot', {"query": {"has_parent":{"parent_type": "data", "query": {"terms": {"vid": vids}}}}}, "recommendviewrecordsearch")
			vids = []
	es.deleteStatisticsData('works_hot', {"query": {"has_parent":{"parent_type": "data", "query": {"terms": {"vid": vids}}}}}, "recommendviewrecordsearch")
...
def getWorksList():
	...
	for hit in es.scrollSearch('works_video_read', 'video', {"query": {"bool": {"must": [{"term": {"state": {"value": 0}}}, {"term": {"authorstate": {"value": 0}}}, {"range": {"publishtime": {"gte": (long(time.time()) - 14 * 24 * 60 * 60) * 1000}}}]}}, "size":1000}):
...
def updateHistoryActions():
	...
	es.coverStatisticsDataReal('subject_history', subjectHistory)

	hot.calWorksScore(worksList)
	worksHistory = {}
	for works in worksList:
		worksHistory[works.vid] = works.toSaveEsData()
	es.coverStatisticsDataReal('works_history', worksHistory)
	...
	for index, hit in enumerate(hits):
		...
		for action in actions:
			try:
				...
				print.info('dispose personas action error: ' + str(error))
			aIndex += 1
