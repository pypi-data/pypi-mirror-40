	...
	for hit in works:
		...
		for index, value in enumerate(DurationSections):
			if duration < value:
				durationIndex = index
				break
		worksDict[worksInfo['vid']] = {'title': worksInfo['title'], 'aid': aid, 'publishTime': publishTime, 'playCount': worksInfo['playcount'] if worksInfo.has_key('playcount') else 0, 'playTime': 0, 'timeCount': 0, 'show': 0, 'showPlay': 0, 'recShow': 0, 'recShowPlay': 0, 'durationIndex': durationIndex, 'durationAvgPlay': 0, 'statis': [0, 0, 0, 0, 0, 0, 0, 0, 0, duration, size]}
