...
def calHotV2(subjectHistory, worksHistory, worksList, halfAids):
	...
	for index, works in enumerate(worksList):
		...
		hotList[vid] = {'vid': vid, 'aid': aid, 'hot': (baseHotMat[index, 0] * SubjectTotalWeight + actionHot) * getDecayRatio(works.publishTime) * (0.5 if aid in halfAids else 1), 'isNew': works.publishTime < 72}
