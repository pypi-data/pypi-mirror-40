...
attrparse(FastParserObject* self, const CHAR_T* p, const CHAR_T* end)
{
    ...
    while (p < end) {
        ...
        key = PyBytes_FromStringAndSize(q, p-q);
        if (key == NULL)
            goto err;
        ...
        if (p < end && *p == '=') {
            ...
            if (has_entity) {
                ...
                value = PyBytes_FromStringAndSize(value_start, value_length);
...
attrexpand(FastParserObject* self, const CHAR_T* p, const CHAR_T* e)
{
    ...
    while (p < e) {
        ...
        item = PyBytes_FromStringAndSize(q, p-q);
        if (!item)
            goto err;
        ...
        if (charref >= 0) {
            ...
            if (charref > 255) {
                ...
                item = PyBytes_FromStringAndSize(&ch, sizeof(CHAR_T));
            }
