/*
Hacked verion of sgmlop.c
skipping <script>, <style>, <object> tag with inner comments
*/
...
static PyObject* attrparse(FastParserObject* self, const CHAR_T *p, const CHAR_T *e);
static PyObject* attrexpand(FastParserObject* self, const CHAR_T *p, const CHAR_T *e);
static int entity(const CHAR_T* b, const CHAR_T *e);
static int generalize (FastParserObject* self, char* temp, int limit);
static CHAR_T * nestedconsume (FastParserObject* self, CHAR_T* p, CHAR_T* end, char* current_tag);
...
static int
generalize (FastParserObject* self, char* temp, int limit)
{
	int base, bufferpos, i;
	char ch;

	if (!self->databuffer) {
		if (!(self->databuffer = malloc (4096))) return -1;
		self->databuffersize = 4096;
	}
	...
	for (i = 0; i < limit; i++) {
		...
		if (self->databufferpos >= self->databuffersize - 1) { /* maximum 2 bytes append */
			base = self->databuffersize / self->resizefactor;
			self->resizefactor ++;
			if (!(self->databuffer = realloc (self->databuffer, base * self->resizefactor))) return -1;
			self->databuffersize = base * self->resizefactor;
		}

		if (isspace (ch) && isspace (self->lch)) {
			continue;
		}

		self->databuffer [self->databufferpos++] = ch;
		self->lch = ch;
	}

	bufferpos = self->databufferpos;
	self->databufferpos = 0;
	return bufferpos;
}

static CHAR_T *
nestedconsume (FastParserObject* self, CHAR_T* p, CHAR_T* end, char* current_tag)
{
	PyObject* res;
	CHAR_T* t; 
	unsigned char ch = '\0';
	char tag [30]; // /style, /script, /object
	int tagindex=0;
	int dlen;
	int end_starttag = 0;
	/* making target tag */
	...
	while (p < end) {
		...
		if (!end_starttag) {
			if (ch == '>') {
				end_starttag = 1; // find end of start tag
				t = p;
			}	
			continue;
		}
		
		if (ch == '/') { /* maybe comments */
			if (*p == '/') { /* single line cmt. */
				while (!(ch == '\n' || ch == '\r') && p < end) {
					ch = *p++;
				}

			} else if (*p == '*') { /* multi line cmt. */
				p++;
				while (p < end) {
					while (ch != '*' && p < end) {
						ch = *p++;
			...
			while (p < end) {
				...
				if (tagindex == 28) {
					while (!(isspace (ch) || ch == '>') && p < end) ch = *p++;
					break;
				}
			}

			tag [tagindex] = '\0';
			if (strcmp ((const char*)tag, (const char*)current_tag) == 0) {
				p -= strlen (tag) + 3; /* < + /tag + > + 1 pointer*/
				if (strcmp ((const char*)tag, "/script") == 0 || strcmp ((const char*)tag, "/style") == 0) {
					dlen = p - t + 1;
					if (self->handle_sdata && dlen > 0) {
						res = PyObject_CallFunction(self->handle_sdata, "s#", t, dlen);
						if (!res) return NULL;	
						Py_DECREF(res);	
					}	
				}				
...
#define GETCB(member, name)\
	...
	GETCB(handle_sdata, "handle_sdata");	
...
feed(FastParserObject* self, char* string, int stringlen, int last)
{
	...
	if (!self->buffer) {
		length = stringlen;
		if (!(self->buffer = malloc(length + 1))) return PyErr_NoMemory ();
		self->buffertotal = stringlen;

	} else {
		length = self->bufferlen + stringlen;
		if (length > self->buffertotal) {
			if (!(self->buffer = realloc(self->buffer, length + 1))) return PyErr_NoMemory ();
	...
	if (last && self->buffer) {
		...
		if (self->buffertotal > MAXMEM) {
			if (!(self->buffer = realloc (self->buffer, MAXMEM))) return PyErr_NoMemory ();
			self->buffertotal = MAXMEM;
		}

		if (self->databuffersize > MAXMEM) {
			    if (!(self->databuffer = realloc (self->databuffer, 4096))) return PyErr_NoMemory ();
			    self->resizefactor = 1;
			    self->databuffersize = 4096;
		}
	}
...
_sgmlop_feed(FastParserObject* self, PyObject* args)
{
	...
	if (!PyArg_ParseTuple(args, "O", &pystr))
		return NULL;
#if PY_MAJOR_VERSION >= 3
	string = PyUnicode_AsUTF8 (pystr);
#else
	string = PyString_AsString (pystr);
#endif
...
entity(const CHAR_T* b, const CHAR_T* e)
{
	...
	if (b < e) {
		if (b[0] == 'a') {
			...
			return '"'; /* &quot; */
		else if (b[0] == 'n' && b+4 == e && b[1] == 'b' && b[2] == 's' && b[3] == 'p')
			return ' '; /* &nbsp; */
		else if (b[0] == '#') {
			/* character entity */
			const CHAR_T *p;
	...
	while (p < end) {
	  ...
	  eot: /* end of token */
		...
		if (q != s && self->handle_data) {
			...
			if ((len = generalize (self, (char *) s, q-s)) == -1) return -1;

			//if (self->databufferpos && isspace (lch)) self->databufferpos--;
			if (len) {
				res = PyObject_CallFunction(self->handle_data, "s#", self->databuffer, len);
				if (!res)
					return -1;
				Py_DECREF(res);
			}
		...
		if (token & TAG) {
			if (token == TAG_END) {
				...
				if (self->finish_starttag) {
					...
					res = PyObject_CallFunction(self->finish_starttag, "s#O", b, len, attr);
					
					if (strncmp ("script", (const char*) b, len) == 0) {
						p = nestedconsume (self, t, end, "/script");
						if (!p) return -1;
					} else if (strncmp ("style", (const char*)b, len) == 0) {
						p = nestedconsume (self, t, end, "/style");
						if (!p) return -1;
					}
...
#if PY_MAJOR_VERSION >= 3
		key = PyUnicode_FromStringAndSize((const char *)q, p-q);
#else
		key = PyBytes_FromStringAndSize((const char *)q, p-q);
#endif
		if (key == NULL)
			goto err;
...
#if PY_MAJOR_VERSION >= 3
				value = PyUnicode_FromStringAndSize((const char*) value_start, value_length);
#else
				value = PyBytes_FromStringAndSize((const char*) value_start, value_length);
#endif				
...
attrexpand(FastParserObject* self, const CHAR_T* p, const CHAR_T* e)
{
	...
	while (p < e) {
		...
		while (p < e && *p != '&')
			p++;
#if PY_MAJOR_VERSION >= 3
		item = PyUnicode_FromStringAndSize((const char *)q, p-q);
#else
		item = PyBytes_FromStringAndSize((const char *)q, p-q);	
#endif		
		if (!item)
			goto err;
...
#if PY_MAJOR_VERSION >= 3				
				item = PyUnicode_FromStringAndSize((const char*)&ch, sizeof(CHAR_T));
#else
				item = PyBytes_FromStringAndSize((const char*)&ch, sizeof(CHAR_T));
#endif
			}
...
PyTypeObject FastParser_Type = {
	...
	(traverseproc) _sgmlop_traverse,   /* tp_traverse */
	(inquiry) _sgmlop_clear,					   /* tp_clear */
	0,					   /* tp_richcompare */
	0,					   /* tp_weaklistoffset */
