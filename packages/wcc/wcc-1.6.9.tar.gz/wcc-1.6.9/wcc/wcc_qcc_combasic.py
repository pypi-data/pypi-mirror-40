...
import pymongo
from .hands import getpage_qichacha
from .oss import uploadHtml
import random
import jsonfiler
import pmt
...
def get_gsinfo(url,resp_text):
    ...
    try:
        ...
        item['html'] = uploadHtml(resp_text,url=url_part)
        return item
...
def get_firminfo(url,resp_text):
    ...
    try:
        ...
        data = None
        try:
            data_text = getpage_qichacha(api_url,max_try=50,timeout=30)
...
def thread_main(thread_id, thread_jobs, thread_params, thread_results):
    ...
    col_firmurls = thread_params["col_firmurls"]
    ...
    for loopc in range(loop_count+1):
        try:
            ...
            """
            try:
                resp_text = getpage_qichacha(url,max_try=30)
...
def wcc_qcc_combasic(THREAD_N):
    ...
    try:
        ...
        thread_params["col_firmurls"] = col_firmurls
    except Exception as err:
        print(err)
        return None
