...
_espresso_conf_file = '/espresso_conf.py'


# Check configuration file, create one if not exists.
try:
    file = open(_espresso_conf_file, 'r')
    file.close()
except:
    file = open(_espresso_conf_file, 'w')

    import random
    chain = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890123456789'
    KEY = ''.join(random.choice(chain) for i in range(16))

    ESPRESSO_CONF = """
# --------------------------------------------
# Espresso Configuration File
# this file is very important a not optional
# --------------------------------------------


# This key is used for the handshake between the web ide and board
KEY = '{0}'
...
NETS = (
    ...
    # ['my_network_name', 'pa55w0rd'],

)


# A custom list of websokcets can be defined here
WEBSOCKETS = (

    # "ws://my_ip_address:port/ws",

)

""".format(KEY)
    file.write(ESPRESSO_CONF)
    file.close()
    pass


# Load configurations
import espresso_conf
...
def register_in_server():
    ...
    for address in espresso_conf.WEBSOCKETS:
...
def espresso_repl(t):
    ...
    """
    global sta_if, websocket, ON_REPL, _timer, _ping
