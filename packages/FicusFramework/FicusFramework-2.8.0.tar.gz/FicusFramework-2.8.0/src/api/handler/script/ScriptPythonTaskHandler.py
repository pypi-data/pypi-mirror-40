...
class ScriptPythonTaskHandler(ITaskHandler,ISimpleOutputer, ICacheAbleHandler):
    ...
    def do_script_crawl(self,dataCrawl, params):
        ...
        """
        # 这里动态的加载script源文件
        sampleScriptCrawl = ScriptPythonFactory.load_instance(f"ScriptPython{self.job_id}", self.script_source,"ISampleScriptCrawl")

        try:
            self.set_local_code(
                f"{dataCrawl.site}_{dataCrawl.projectCode}_{dataCrawl.code}")
            resultList = sampleScriptCrawl.do_crawl(params)
    ...
    def do_script_compute_execution(self, dataComputeExecution, params):
        ...
        # 这里动态的加载script源文件
        sampleScriptCE = ScriptPythonFactory.load_instance(f"ScriptPython{self.job_id}",self.script_source,"ISampleScriptCE")
