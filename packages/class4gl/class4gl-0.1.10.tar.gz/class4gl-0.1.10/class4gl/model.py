...
# END -- HW 20170606      
    ...
    def run_mixed_layer(self):
        ...
        if (self.dtheta <= 0.1) and (dthetatend_pre < 0.):
            ...
            l_entrainment = True
            warnings.warn(str(self.t)+"/"+str(self.tsteps)+\
                          " Warning! Potential temperature jump at mixed-layer height would become too low limiting timestep from "+ str(self.dtmax)+' to '+str(dtmax_new))
...
#            print(self.t, self.dtcur,self.dt,self.htend)
        ...
        if self.sw_ap:
            ...
            if itop > 1:
                    warnings.warn(str(self.t)+"/"+str(self.tsteps)+\
                          "Warning! Temperature profile was too steep. Modifying profile: "+ str(itop - 1)+ " measurements were dropped and replaced with its average Modifying profile. mean with next profile point(s).") 
