...
parser.add_argument('--experiments', help="IDs of experiments, as a space-seperated list (default: 'BASE')")
parser.add_argument('--split_by',\
                    type=int,
                    help="the maxmimum number of soundings that are contained in each output file of a station. -1 means unlimited (default). In case of arrays experiments, this is usually overwritten.")
...
parser.add_argument('--global_chunk_number',help="this is the batch number of the expected series of experiments according to split_by")
parser.add_argument('--station_chunk_number',help="this is the batch number according to split_by in case of considering one station")
parser.add_argument('--experiments_names', help="Alternative output names that are given to the experiments. By default, these are the same as --experiments") 
...
if args.experiments_names is not None:
    experiments_names = args.experiments.strip(' ').split(' ')
    if len(experiments_names) != len(experiements):
