...
class GuosenData(object):
    ...
    def load_fund_data_from_excel(self, file_name):
        ...
        sql_head_dict = {
            ...
            't_visit_record':
                "insert into t_visit_record values (0, '%s', '%s', '%s', '%s', '%s', '%s')",
        }
        ...
        if self.sql_is_connect():
            ...
            for table_name, sql_head in sql_head_dict.items():
                ...
                if table_name == 't_private_fund_basic_info':
                    ...
                    self.save_excel_visit_record(excel_row_list, sql_head)
                else:
    ...
    def save_excel_visit_record(self, excel_row_list, sql_head):
        """
        read data from excle, save in sql
        :param sql_head: table head info
...
def test():
    ...
    data_api.load_fund_data_from_excel(path)
