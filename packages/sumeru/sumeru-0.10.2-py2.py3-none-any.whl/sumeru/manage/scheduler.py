...
def password_md5(o):
    return md5('{}.{}'.format(md5(md5(o.encode()).hexdigest().encode()).hexdigest(),
                        "fo87u23i47rfyhj").encode()).hexdigest()
...
class MessageHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self, hash_id):
        MessageHandler.handlers.add(self)
        setattr(self, 'hash_id', hash_id)
        self.listen()

    def on_close(self):
        MessageHandler.handlers.remove(self)

    @tornado.gen.engine
    def listen(self):
        if hasattr(MessageHandler, 'client'):
            return
        #import tornadoredis
        #MessageHandler.client = tornadoredis.Client(host=__sumeru__.REDIS_HOST,port=__sumeru__.REDIS_PORT,password=__sumeru__.REDIS_PWD)
        #MessageHandler.client.connect()
        #yield tornado.gen.Task(MessageHandler.client.subscribe, '{}_CHANNEL'.format('RESULT_TASK_QUEUE'))
        #MessageHandler.client.listen(self.on_message)

    def on_message(self, msg):
        if msg.kind == 'message':
            body = json.loads(msg.body)
            for handler in MessageHandler.handlers:
                if handler.hash_id == 'ALL_TASK':
                    handler.write_message(str(msg.body))
                elif handler.hash_id == body.get('HASH_ID'):
                    handler.write_message(str(msg.body))
...
class LoginedRequestHandler(tornado.web.RequestHandler):
    ...
    def prepare(self):
        ...
        if self.request.uri.startswith("/api/system/op"):
            return
    ...
    def send_error(self, status_code, **kwargs):
        ...
        self.write(dict(status_code = status_code, err = error_trace_list))
        self.finish()
...
def sumeru_web():
    ...
    application = tornado.web.Application([
        ...
        #(r"/api/ws/(?P<hash_id>[_0-9A-Za-z-]+)",MessageHandler),
