...
def load_program_graphs_from_db(opt, db, map_folder=".", is_train=True, n_classes=3):
    """
    ...
    for i in range(0, len(classes)):
        ...
        conn = sqlite3.connect(db)
        c = conn.cursor()
        c = c.execute("SELECT u.graph FROM units u, classes c, learning l WHERE u.id = c.unit_id AND c.unit_id = l.unit_id AND l.mode = ? AND c.label = ?", (mode, label))
        rows = c.fetchall()
        c.close()
        conn.close()
        n_instance = len(rows)
...
class ProgramData():   
    def __init__(self, opt, map_folder, size_vocabulary, db, is_train, n_classes=3):
        all_data = load_program_graphs_from_db(opt, db, map_folder, is_train, n_classes)
        all_data = np.array(all_data)[0:len(all_data)]       
        if is_train == True:
            print("Number of all training data : " + str(len(all_data)))
        else:
