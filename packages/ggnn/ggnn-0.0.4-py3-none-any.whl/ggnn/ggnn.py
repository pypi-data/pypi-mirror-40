...
def main():
    ...
    try:
        ...
        if opt.n_classes == 0 and len(opt.argv) > 0: # adding datasets
        ...
        if len(opt.argv) == 0 and opt.n_classes > 0:  # training and validation
            print("TODO: training and validating ...")
            prepare_datasets_for_learning(opt.db)
            writer = prepare_logs_for_training(opt.log_path)
            torch.manual_seed(opt.manualSeed)
            if opt.cuda:
                torch.cuda.manual_seed_all(opt.manualSeed)

            train_dataset = ProgramData(opt, opt.map_folder, opt.size_vocabulary, opt.db, True, opt.n_classes)
            train_dataloader = bAbIDataloader(train_dataset, batch_size=opt.train_batch_size, shuffle=True, num_workers=2)
            test_dataset = ProgramData(opt, opt.map_folder, opt.size_vocabulary, opt.db, False, opt.n_classes)
        ...
        if opt.cuda:
            torch.cuda.manual_seed_all(opt.manualSeed)
        for i in range(1, len(opt.argv)):  # validating for each data 
            print("TODO: validating for the file %d " % i)
            prepare_datasets_for_learning(opt.db, False)
            test_dataset = ProgramData(opt, opt.map_folder, opt.size_vocabulary, opt.db, False, opt.n_classes)
            test_dataloader = bAbIDataloader(test_dataset, batch_size=opt.test_batch_size, shuffle=True, num_workers=2)
