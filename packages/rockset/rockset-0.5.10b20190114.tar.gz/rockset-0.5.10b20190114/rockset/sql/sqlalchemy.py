    ...
    def _get_table_columns(self, connection, table_name, schema):
        ...
        try:
            ...
            query = """WITH counts AS (
                SELECT count(*) cnt FROM {0}
            ),
            totals AS (
                SELECT field lap, SUM(occurrences) T
                FROM $info.{0}.field_types
                WHERE field[LENGTH(field)]='*' GROUP BY field
            ),
            object_totals AS (
                SELECT field lap, SUM(occurrences) T
                FROM $info.{0}.field_types
                WHERE field[LENGTH(field)]='*' AND type = 'object' GROUP BY field
            ),
            occurrences_query AS (
                SELECT field, type, sum(occurrences) occurrences
                FROM $info.{0}.field_types
                GROUP BY field, type)
            SELECT subq.field field, subq.type type, subq.occurrences occurrences, subq.total total
            FROM (
                SELECT oq.field field, oq.type type, oq.occurrences occurrences, nt.T total
                FROM occurrences_query oq INNER JOIN totals nt ON oq.field = nt.lap
                WHERE oq.field[LENGTH(oq.field)]='*' AND LENGTH(oq.field) = 1
                UNION
                SELECT oq.field field, oq.type type, oq.occurrences occurrences, coalesce(ot.T, c.cnt) total
                FROM occurrences_query oq LEFT OUTER JOIN object_totals ot
                ON SLICE(oq.field, 1, ARRAY_LAST_POSITION(oq.field, '*')) = ot.lap CROSS JOIN counts c
                WHERE NOT oq.field[LENGTH(oq.field)]='*'  AND LENGTH(oq.field) = 1) subq
                ORDER BY subq.field
            """
