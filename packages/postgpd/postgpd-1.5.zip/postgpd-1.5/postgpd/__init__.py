...
class dbase():
	...
	__adapt_types_pd = {'object': ['varchar(200)', 'text', 'smallint', 'integer', 'bigint', 'numeric', 'boolean', 'numeric',
								   'date', 'timestamp without time zone',\
								   'timestamp with time zone'],
						'int64': ['smallint', 'integer', 'bigint', 'numeric'],
	...
	# w powyzszym w 'object' zawarte zostaly wszystkie typy danych, ktorych uzywam. a to dlatego, ze pusta kolumna w pandas
	# posiada typ object. nie chce mi sie kombinować więcej, kiedyś może zrobie cały projekt od nowa a na razie powinno wystarczyc.

	__type_codes = {16: 'bool', 23: 'int', 20: 'int', 1700: 'float', 25: 'str', 701: 'float', 17: 'bytes'}
	...
	def __init__(self, db_name, user_name, user_password, db_host, db_port, default_schema='public'):
		self.db_name = db_name
		self.user_name = user_name
		self.user_password = user_password
		self.db_host = db_host
		self.db_port = db_port
		self.__default_shema = default_schema

		#connect to data base
		try:
			self.connection = p.connect(dbname=self.db_name, user=self.user_name, password=self.user_password,
	...
	def find_duplicates(self, df, table, schema=''):
		...
		self.__df_error_raiser(df)
		difference = "there are diffrences between DataFrame and table data types stored in relevant columns."
		comparison = self.compare_cols(df, table, schema)
		if comparison is False:
			raise Exception("Uneven tables")
	...
	def update_table(self, df, table,  schema='', keep_duplicates=False):
		...
		SET %s 
		FROM tmp a
		WHERE %s; DROP TABLE tmp;""" % (tmp_table_cols, values_sets, schema, table,
	...
	def insert_df_auto_pk(self, df, table, pk_name, schema=''):
		"""
		...
		if isinstance(pk_name, str):
			if pk_name in table_columns.keys():
				table_columns.pop(pk_name, None)
			else:
