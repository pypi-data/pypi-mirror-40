...
class IPCMBplus(SupervisedLearnerPrimitiveBase[Inputs, Outputs, Params, Hyperparams]):
    ...
    metadata = metadata_base.PrimitiveMetadata({
        'id': '15e19753-aee8-4d9f-8f26-0eedeb1690b2',
        'version': '2.1.5',
        'name': 'IPCMBplus feature selector',
        'keywords': ['Markov Blanket Discovery'],
        'source': {
            'name': 'RPI-ISL',
    ...
    def fit(self, *, timeout: float = None, iterations: int = None) -> None:
        ...
        model = IPCMB(Trainset, discTrainset,'classification')
        index = model.select_features(thres=self.hyperparams['threshold'])
        self._index = list(index)
        if self._index == []:
            #raise ValueError('Please decrease hyperparameter to get features selected.')
            self._index = np.arange(self._training_inputs.shape[1])
        self._fitted = True
