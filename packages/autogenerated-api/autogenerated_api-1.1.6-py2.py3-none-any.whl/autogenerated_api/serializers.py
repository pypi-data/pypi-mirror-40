...
def DeadlySerializerFactory(model, fields=None, exclude=None, nest=[], nested_fields={}, force_nesting=False, exclude_nest=[]):
    ...
    if fields == "exhaustive":
        fields = model_tools.get_all_fields(model)
    if fields == "simple":
        fields = model_tools.get_direct_fields(model)

    fields = model_tools.field_processor(fields, model)

    if exclude is not None:
        exclude = model_tools.field_processor(exclude, model)
        fields = list(fields)
        for i in exclude:
            if i in fields:
                fields.remove(i)
    ...
    nest = model_tools.field_processor(nest, model)
    ...
    class ModelDeadlySerializer(object):
        ...
        def __init__(self, queryset, *args, **kwargs):
            ...
            fields_list = model_tools.field_processor(fields_list, model)
            self.fields = set(fields_list)
            if exclude:
                exclude = model_tools.field_processor(exclude)
                for item in exclude:
            ...
            nest_tuple = model_tools.field_processor(nest_tuple, model)
            ...
            for x in self.nest:
                ...
                if (x not in self.nested_fields_dict):
                    if hasattr(RelatedModel, "default_nest_fields"):
                        self.nested_fields_dict[x] = model_tools.field_processor(RelatedModel.default_nest_fields, model)
                    else:
