...
def pathJoin(_dir, fn) :
    return formatPath(os.path.join(_dir, fn))
...
def zipFolder(folder, zipPath = None, suffixs = '.*', skipFiles = (), inZipPath=''):
    '''压缩文件夹
    folder:待压缩的文件夹
...
def copyTree(srcDir, targetDir, removeBefore=False, suffix='.*', skipDirs=()) :
    '''Copy the srcDir to targetDir.
    参数: srcDir, targetDir, removeBeforee=False, suffix='.*', skipDirs=()
    ...
    if os.path.isfile(srcDir) :
        # print('copyTree 1 ')
        suffix = os.path.splitext(srcDir)[1]
        if not targetDir[-len(suffix):] == suffix :
            # 目标位置是路径
...
def CamelCase2UnderLine(self, s):
    '''驼峰法命名转下划线命名'''
    newStr = ''
