    ...
    def __init__(self, filename):
        ...
        try:
            ...
            if filename in self.file_dic:
                ...
                self.logger.addHandler(MyFileLog.create_console_handler())
                self.file_dic[filename] = self.logger
        finally:
            self.file_dic_lock.release()

    @staticmethod
    def create_console_handler():
        """控制台處理，只新建一個"""
        if MyFileLog.console_handler is None:
            log_fmt = '%(asctime)s - %(levelname)s - %(message)s'
            MyFileLog.console_handler = logging.StreamHandler()
            MyFileLog.console_handler.setLevel(logging.DEBUG)
            MyFileLog.console_handler.setFormatter(logging.Formatter(log_fmt))
        return MyFileLog.console_handler

    @staticmethod
    def init_riqi_log(f_name):
        """日期滚动"""
        logger = logging.getLogger(f_name) #必須傳name
...
if __name__ == '__main__':
    ...
    MyFileLog('199').debug('debugtest')
    MyFileLog('188').debug('debugtest')
    MyFileLog('177').warning('warningtest')
