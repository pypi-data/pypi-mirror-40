...
class supercanvas(tkinter.Canvas):
    ...
    # gridDefault = {'activedash': '', 'activefill': '', 'activestipple': '', 'activewidth': '0.0', 'arrow': 'none', 'arrowshape': '8 10 3', 'capstyle': 'butt', 'fill': 'black', 'dash': (5,2), 'dashoffset': '0', 'disableddash': '', 'disabledfill': '', 'disabledstipple': '', 'disabledwidth': '0.0', 'joinstyle': 'round', 'offset': '0,0', 'smooth': '0', 'splinesteps': '12', 'state': '', 'stipple': '', 'tags': '', 'width': '1.0'}

    # gridUser = gridDefault
    
    # tickDefault = {'activedash': '', 'activefill': '', 'activestipple': '', 'activewidth': '0.0', 'arrow': 'none', 'arrowshape': '8 10 3', 'capstyle': 'butt', 'fill': 'black', 'dash': '', 'dashoffset': '0', 'disableddash': '', 'disabledfill': '', 'disabledstipple': '', 'disabledwidth': '0.0', 'joinstyle': 'round', 'offset': '0,0', 'smooth': '0', 'splinesteps': '12', 'state': '', 'stipple': '', 'tags': '', 'width': '2.0'}

    # ticksUser = tickDefault
    
    # xlabelDefault = {'activefill': '', 'activestipple': '', 'anchor': 'n', 'angle': '0.0', 'disabledfill': '', 'disabledstipple': '', 'fill': 'black', 'font': 'TkDefaultFont', 'justify': 'left', 'offset': '0,0', 'state': '', 'stipple': '', 'tags': '', 'text': '', 'underline': '-1', 'width': '0'}

    # xlabelUser = xlabelDefault
    
    # ylabelDefault = {'activefill': '', 'activestipple': '', 'anchor': 'e', 'angle': '0.0', 'disabledfill': '', 'disabledstipple': '', 'fill': 'black', 'font': 'TkDefaultFont', 'justify': 'left', 'offset': '0,0', 'state': '', 'stipple': '', 'tags': '', 'text': '', 'underline': '-1', 'width': '0'}
    ...
    def __init__(self, master, **options):
        ...
        # self.drawTTicks()
        # self.drawGrid()
        # 1er appel donc aucune option n'a été passée
        self.drawRepere()
        self.drawZero()
        ########################
    ...
    def __refresh(self, *args):
        ...
        for k, v in s.items():
            if v[0] != "text":
                optionsXlabel[v[0]] = v[4]
        ##
        s = self.itemconfigure("ylabel")
        optionsYlabel = {}
        for k, v in s.items():
            if v[0] != "text":
                optionsYlabel[v[0]] = v[4]

        self.drawRepere(optionsTicks, optionsGrid,
        ...
        for objet in self.find_all():
            ...
            if "axes" not in self.gettags(objet) \
               and "follow" not in self.gettags(objet) \
               and "ticks" not in self.gettags(objet) \
               and "xlabel" not in self.gettags(objet) \
               and "ylabel" not in self.gettags(objet) \
    ...
    ## !! création séparée pour gérer les options
    ## !! 
    ## def __createRepere__(self):

    # que les éléments liés au repérage numérique
    def __refreshRepere__(self, optionsTicks, optionsGrid,
                          optionsXlabel, optionsYlabel):
        #
        self.update()
        #
        ...
        #if limite < 1:
        #    id = self.create_line(0, 0, 0, 0, **optionsGrid)
        #    self.addtag_withtag(id, "grid")
        while i <= limite :
            a = self.x_origin + self.x_unit * i * dx
            if self.ticks:
                id = self.create_line(a, o1, a, o2, **optionsTicks)
                self.addtag_withtag(id, "ticks")
            if self.grid:
                id = self.create_line(a, 0, a, self.height, **optionsGrid)
                self.addtag_withtag(id, "grid")
            if self.xlabel:
                id = self.create_text(a, o3, **optionsXlabel)
                self.itemconfigure(id, text="{:>2}".format(round(dx * i, arrondiX)))
                self.addtag_withtag(id, "xlabel")
            i += 1
        ...
        while i <= limite:
            o = self.y_origin - self.y_unit * i * dy
            if self.ticks:
                id = self.create_line(a1, o, a2, o, **optionsTicks)
                self.addtag_withtag(id, "ticks")
            if self.grid:
                id = self.create_line(0, o, self.width, o, **optionsGrid)
                self.addtag_withtag(id, "grid")
            if self.ylabel:
                id = self.create_text(a3, o, **optionsYlabel)
                self.itemconfigure(id, text="{:>2}".format(round(dy * i, arrondiY)))
                self.addtag_withtag(id, "ylabel")
            i += 1
    ...
    def drawRepere(self, optionsTicks=None, optionsGrid=None,
                   optionsXlabel=None, optionsYlabel=None):
        ##
        needToRefresh = False
        ##
        if self.ticks:
        ...
        # if self.xlabel:
        #     if self.find_withtag("xlabel") == ():
        #         # CRÉATION AVEC LES OPTIONS PAR DÉFAUT
        #         self.__createTTicks__()
        #     else:

        #         needToRefresh = True
        #         # on doit capture l'option text!
        # else:
        #     self.delete("xlabel")

        # if self.ylabel:
        #     if self.find_withtag("ylabel") == ():
        #         # CRÉATION AVEC LES OPTIONS PAR DÉFAUT
        #         self.__createTTicks__()
        #     else:
        #         needToRefresh = True
        ...
        if self.xlabel or self.ylabel:
            if self.find_withtag("xlabel") == () or self.find_withtag("ylabel") == ():
                self.__createTTicks__()
            else:
                if self.xlabel:
                    needToRefresh = True
        ...
        if needToRefresh:
            # print(optionsYlabel)
            self.__refreshRepere__(optionsTicks, optionsGrid,
                                   optionsXlabel, optionsYlabel)
...
# repère bis:
# avec getView récupérer les limites. ?
# récupérer les appels à itemconfgure ?
# effacer tout le repère à chaque fois ?
# au contraire utiliser le recalcul général sur les objets
