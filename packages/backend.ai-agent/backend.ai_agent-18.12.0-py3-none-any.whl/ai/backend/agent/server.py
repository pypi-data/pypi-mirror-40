...
class AgentRPCServer(aiozmq.rpc.AttrHandler):
    ...
    async def _create_kernel(self, kernel_id, kernel_config, restarting=False):
        ...
        entrypoint_sh_path = Path(
            pkg_resources.resource_filename('ai.backend.agent',
                                            '../runner/entrypoint.sh'))
        suexec_path = Path(
            pkg_resources.resource_filename('ai.backend.agent',
                                            f'../runner/su-exec.{distro}.bin'))
        jail_path = Path(
            pkg_resources.resource_filename('ai.backend.agent',
                                            f'../runner/jail.{distro}.bin'))
        hook_path = Path(
            pkg_resources.resource_filename('ai.backend.agent',
                                            f'../runner/libbaihook.{distro}.so'))
        kernel_pkg_path = Path(
            pkg_resources.resource_filename('ai.backend.agent',
                                            '../kernel'))
        helpers_pkg_path = Path(
            pkg_resources.resource_filename('ai.backend.agent',
                                            '../helpers'))
        ...
        # TODO: append accelerator hook
        # _mount(...)
        # environ['LD_PRELOAD'] += ':' + hook
        ...
        try:
            env_container = await self.docker.containers.create(config={
                'Image': f'backendai-krunner-env:{distro}',
