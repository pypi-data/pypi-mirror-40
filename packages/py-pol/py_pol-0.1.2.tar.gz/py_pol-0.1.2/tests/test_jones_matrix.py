...
class TestJonesMatrix(object):
    ...
    def test_diattenuator_retarder_linear(self):
        ...
        M1.diattenuator_retarder_linear(delta=np.pi / 2, p1=1, p2=1, angle=0)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ...
        solution = matrix([[1, 0], [0, -1]])
        M1 = Jones_matrix('M_diat')
        M1.diattenuator_retarder_linear(delta=np.pi, p1=1, p2=1, angle=0)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ...
        M1.diattenuator_retarder_linear(
            delta=np.pi / 2, p1=1, p2=.5, angle=45 * degrees)
        proposal = M1.get()
        assert np.linalg.norm(proposal - solution) < eps, sys._getframe(
        ).f_code.co_name + ".py --> example: delta=np.pi/2, p1=1, p2=.5, angle=45*degrees"
