...
def parser_spec(ctx, param, value):
    ...
    try:
        ...
        if package:
            ...
            parser_cls = available_parsers[classname]
    except (
        IndexError,
        ImportError,
        AttributeError,
    ...
    return ".".join((parser_cls.__module__, parser_cls.__name__))
...
@click.option(
    ...
    help=_(
        "parser class to use. "
        "This can be a built-in polt parser ({native_parsers}) "
        "or a class specification to a subclass of polt.parser.Parser "
        "(like 'mypackage.MyParser')"
    ).format(
        native_parsers=polt.l10n.join(
            map(repr, sorted(available_parsers)),
            last_sep=" {} ".format(_("or")),
        )
    ),
)
...
@click.pass_context
def add_source(ctx, parser, cmd, options, name):
    if not (parser or cmd):
        raise click.UsageError(
            _("Specify at least either --parser or --cmd"), ctx=ctx
        )
    if parser is None:
        parser = polt.parser.NumberParser
