...
subplots_for_choices = {"parser", "quantity", "unit", "key", "nothing"}
...
@click.option(
    ...
    help=_("for which property ({quantities}) to create subplots for").format(
        quantities=", ".join(sorted(map(repr, subplots_for_choices)))
    ),
...
@click.option(
    "--tight/--not-tight",
    help=_(
        "Whether to try to minimize empty space around plots. "
        "Most of the time, activating this (the default) should be fine, "
        "but if plots are resized very ugly (e.g. when the Legends are huge), "
        "disabling this feature might help. "
        "Internally, this uses matplotlib's tight_layout function."
    ),
    default=None,
    is_flag=True,
)
...
def live(
    ...
    config.update_option("plot", "tight", tight)
    ...
    with multiprocessing.Manager() as manager:
        ...
        def animate_process(buf):
            ...
            animator = polt.animator.Animator(
                ...
                tight_layout=config.getboolean("plot", "tight", fallback=True),
