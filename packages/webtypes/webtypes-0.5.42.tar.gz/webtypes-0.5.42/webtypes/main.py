...
import webtypes
from webtypes import codecs
from webtypes.exceptions import ParseError, ValidationError
...
def docs(schema):
    ...
    loader = jinja2.PrefixLoader({
        'webtypes': jinja2.PackageLoader('webtypes', 'templates')
    })
    env = jinja2.Environment(autoescape=True, loader=loader)

    template = env.get_template('webtypes/docs/index.html')
    ...
    static_dir = os.path.join(os.path.dirname(webtypes.__file__), 'static')
    shutil.copytree(static_dir, os.path.join(directory, 'webtypes'))
