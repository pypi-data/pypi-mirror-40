...
class Connection(pymysql.connections.Connection):
    ...
    def __init__(self, **kwargs):
        __host = kwargs['__host'] if '__database' in kwargs else self._host
        __port = kwargs['__port'] if '__port' in kwargs else self._port
        __user = kwargs['__user'] if '__user' in kwargs else self._user
        __password = kwargs['__password'] if '__password' in kwargs else self._password
        __database = kwargs['__database'] if '__database' in kwargs else self._database
        __charset = kwargs['__charset'] if '__charset' in kwargs else self._charset
