...
ValidateField = Callable[[Any], bool]
ValidateRecord = Callable[[Record], bool]
ValidateTryParse = Callable[[Any], Tuple[bool, Any]]
...
class FieldValidator:
  ...
  async def validate(self, value: Any) -> bool:
    validate = cast(ValidateField, self.validator)
    if iscoroutinefunction(validate):
      # TODO: Get right typing for this
...
class RecordValidator:
  ...
  async def validate(self, record: Record) -> bool:
    validate = cast(ValidateRecord, self.validator)
    if iscoroutinefunction(validate):
...
class TryParseValidator:
  ...
  async def validate(self, value: Any) -> Tuple[bool, Any]:
    validate = cast(ValidateTryParse, self.validator)
    if iscoroutinefunction(validate):
      return await validate(value)
    return validate(value)
...
class ValidatorInstance:
  ...
  async def validate(self) -> ValidationResult[MutableRecord]:
    ...
    self.validation_result.set_output(self.output)
    return self.validation_result
...
class Validators:
  ...
  @staticmethod
  def optional(validator: Callable[[Any], bool]) -> Callable[[Any], bool]:
  ...
  @staticmethod
  def is_length(max: int, min: Optional[int] = None) -> Callable[[str], bool]:
