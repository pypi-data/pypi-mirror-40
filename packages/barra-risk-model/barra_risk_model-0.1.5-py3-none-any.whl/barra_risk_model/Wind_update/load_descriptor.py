# -*- coding: utf-8 -*-
"""
Created on Thu Mar 29 11:18:01 2018
...
def load_descriptor(dt_range,stk_path,des_path,load_season=False,update_season_only=False):
    log_path=des_path
...
#        load_season=check_load_season(load_season_queue)
#        if not load_season:
#            if ((int(dt[-4:])+2) in C.season_end_int):
#                season_dt= str(int(dt)+2)
#                for key in ('leverage','update_time'):#,'growth'
#                    loading_daily(stk_lst,key,season_dt,log_path,des_path)
#                load_season=True
#            elif ((int(dt[-4:])+1) in C.season_end_int):
#                season_dt= str(int(dt)+1)
#                for key in ('leverage','update_time'):#,'growth'
#                    loading_daily(stk_lst,key,season_dt,log_path,des_path)               
#                load_season=True
#            elif (int(dt[-4:]) in C.season_end_int):
#                season_dt= dt
#                for key in ('leverage','update_time'):#,'growth'
#                    loading_daily(stk_lst,key,season_dt,log_path,des_path)  
#                load_season=True
#        load_season_queue.append(load_season)