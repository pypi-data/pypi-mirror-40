...
class AmbitionEdcSeleniumMixin(
    ...
    def go_to_subject_visit_schedule_dashboard(self, visit_schedule_name=None,
                                               schedule_name=None, visit_code=None,
                                               save_only=None, screenshot=True):
    ...
    def go_to_subject_visit_dashboard(self, visit_schedule_name=None,
                                      schedule_name=None, visit_code=None,
                                      save_only=None,
    ...
    def fill_subject_screening(self, save_only=None):
        ...
        if save_only:
            ...
            model_obj = self.fill_form(
                ...
                obj=model_obj, exclude=['subject_identifier', 'report_datetime'])
            self.wait_for_edc()
        return model_obj
    ...
    def fill_subject_consent(self, model_obj, save_only=None):
        ...
        if save_only:
            ...
            model_obj = self.fill_form(
                model=self.subject_consent_model, obj=model_obj,
    ...
    def fill_appointment_in_progress(self, subject_identifier, save_only=None):
        appointment = Appointment.objects.filter(
            subject_identifier=subject_identifier,
            appt_status=NEW_APPT).order_by('timepoint')[0]
        if save_only:
    ...
    def fill_subject_visit(self, appointment, save_only=None):
        ...
        if save_only:
            ...
            model_obj = self.fill_form(
                model=self.subject_visit_model, obj=model_obj,
                verbose=False)
            self.wait_for_edc()
        return model_obj
    ...
    def complete_previous_visits(self, subject_identifier=None, visit_schedule_name=None,
                                 schedule_name=None, visit_code=None):
        ...
        """
        if visit_code:
            visit_codes = site_visit_schedules.get_visit_schedule(
                visit_schedule_name).schedules.get(schedule_name).visits
            ...
            for code in visit_codes:
                if visit_code == code:
                    ...
                    try:
                        ...
                        self.fill_subject_visit(
                            appointment, save_only=True)
