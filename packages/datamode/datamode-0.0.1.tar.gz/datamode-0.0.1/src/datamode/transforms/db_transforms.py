...
class SourceData(Transform):
  ...
  def source_sql(self, current_context):
    ...
    if self.options.get('sql_query', None):
      get_from_sql = self.options['sql_query']
...
class SourceInline(SourceData):
  def __init__(self, query, dfname='my_query_file', *args, **kwargs):
...
## Switching/Setting ##
# Table
class SetTableContext(Transform):
