import sys, os, os.path, hashlib, inspect, pprint, math, json
import numpy as np
import pandas as pd
...
def get_sha1_hash_from_int(hashed_int):
  if type(hashed_int) != int:
    raise Exception('get_sha1_hash_from_int: hashed_int must be of type int')

  return hashlib.sha1(str(hashed_int).encode('ascii')).hexdigest()
