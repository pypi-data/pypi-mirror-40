...
defaultValue = Group(Suppress(Keyword('default')) + atom)('default')
...
fromTable = Optional( Keyword('from') + table )
droprowsAction = Group(Keyword('drop rows') + fromTable)
...
wheresClause = Suppress(Keyword('where')) + delimitedList(where, _and)
...
joinsClause = Suppress(Keyword('join')) + delimitedList(join, _and)

groupby = Group(columnOrTableColumn)
groupbysClause = Suppress(Keyword('groupby') | Keyword('group by')) + delimitedList(groupby, comma)
