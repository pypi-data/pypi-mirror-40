...
class MqttClient(mqtt.Client):
    ...
    def reply(self, msg, status, data=None, qos=1, **kwargs):
        ...
        if correlation_id:
            ...
            if forward:
                payload['_context'] = forward

            self.publish(topic, json.dumps(payload), qos=qos)

    def success(self, msg, data=None, qos=1, **kwargs):
        self.reply(msg, constants.STATUS_SUCCESS, data=data, qos=qos, **kwargs)

    def failure(self, msg, data=None, qos=1, **kwargs):
        self.reply(msg, constants.STATUS_FAILURE, data=data, qos=qos, **kwargs)
