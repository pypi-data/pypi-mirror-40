...
class Raft():
    ...
    def __init__(self, cluster_id=constants.SERVICE_NAME, member_id=constants.HOSTNAME, timeout=DEFAULT_TIMEOUT_MS, num_members=DEFAULT_NUM_CLUSTER_MEMBERS):
...
if __name__ == "__main__":
    # quick testing
    raft = Raft(num_members=3, timeout=3000).start()
    @raft.leader_only
    def test():
        x = random.randint(5, 15)
        print('.'*x + ' '*(15-x), end='\r')

    def on_replicate_state(state):
        print('hohohohoho', state)
    
    raft.on_replicate_state = on_replicate_state

    print('---------------------------------')
    while True:
        time.sleep(0.5)
        # raft.replicate_state({'a':'b'})
        test()
