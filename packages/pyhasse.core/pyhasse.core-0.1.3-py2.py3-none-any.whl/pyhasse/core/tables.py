...
class Field(object):
    ...
    def __init__(self, tdata=[], title=None, subtitle=None,
                 hlabels=True, vlabels=True, cssclass="",
                 transpose=False, placeholder="na"):
        self.title = title
        self.subtitle = subtitle
        self.tdata, \
            self.rows, \
            cols = self.convert_field2matrix(tdata,
                                             placeholder)
        self.hlabels = []
        self.vlabels = []
        if self.rows > 0:
...
class HTMLTable(Field):
    ...
    def table2html(self):
        txt = []
        f2 = "{0}\n"
        table = "<table class='table table-striped'>{1}</table>"
        td = "<td>{0}</td>"
        tr = "<tr>{0}</tr>"
        cols = ""
        rows = []
