...
class StationDiY():
    ...
    def __init__(self, host="stationdiy.eu", username = "", port = 80, multidevice = True,\
    ...
    def sendMQTT(self, **kwargs):
        ...
        def on_connect(mqttc, userdata, rc):
            ...
            if "sensor" in kwargs:
                if self.multidevice or self.host == "stationdiy.eu":
                    self.actioner = ""
                    topic='StationDiy/sensor/%s/%s/%s'%(self.user_hash,kwargs["device"],  kwargs["sensor"])
                else:
                    self.actioner = ""
                    topic='StationDiy/sensor/%s'%( kwargs["sensor"])
                ...
                if self.multidevice or self.host == "stationdiy.eu":
                    ...
                    self.sensor = ""
                    topic='StationDiy/actioner/%s'%(  kwargs["actioner"])
            ...
            if "type_data" in kwargs:
                self.type_data = kwargs["type_data"]
