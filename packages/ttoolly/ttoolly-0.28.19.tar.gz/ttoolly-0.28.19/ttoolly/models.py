    ...
    def __init__(cls, name, bases, dct):
        ...
        def check_time(fn):
            ...
            def tmp(*args, **kwargs):
                start = datetime.now()
                res = fn(*args, **kwargs)
#                 print((datetime.now() - start).total_seconds() * 1000000, '\t', fn.__name__)
                return res

            decorators = getattr(tmp, 'decorators', ())
            if not 'check_time' in [getattr(d, '__name__', d.__class__.__name__) for d in decorators]:
                tmp.decorators = decorators + (check_time,)
            return tmp
...
#                     and isinstance(getattr(cls, attr), (types.FunctionType,))
#                     #                                                         types.MethodType))
#                     and 'check_time' not in [getattr(d, '__name__', d.__class__.__name__)
#                                              for d in getattr(getattr(cls, attr), 'decorators', ())]):
#                     setattr(cls, attr, check_time(getattr(cls, attr)))
