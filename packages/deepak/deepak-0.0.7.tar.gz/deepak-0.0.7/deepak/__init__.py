...
def app_deploy(server_type):
    ...
    if (server_type == 'tomcat'):
        tomcatpath=raw_input('Enter Tomcat webapp path(If you dont have tomcat Please install tomcat) : ')
        dirname = os.path.dirname(__file__)
        print dirname
        tomcat_status = subprocess.check_output("cp "+dirname+"/jenkins.war "+tomcatpath, shell=True)
