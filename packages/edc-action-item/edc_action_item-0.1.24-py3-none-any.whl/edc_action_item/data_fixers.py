...
def fix_null_action_item_fk(apps, app_label, models):
    ...
    for model in models:
        ...
        model_cls.action_name = [
            ...
            if action.reference_model and action.reference_model.split('.')[1].lower() ==
            model.lower()][0]
                ...
                if not obj.action_item:
                    try:
                        obj.action_item_id = ActionItem.objects.get(
                            action_identifier=obj.action_identifier).pk
                    except MultipleObjectsReturned as e:
                        qs = ActionItem.objects.filter(
                            action_identifier=obj.action_identifier).order_by('created')
...
def fix_null_action_items(apps):
    ...
    try:
        action_items = ActionItem.objects.filter(
            parent_action_identifier__isnull=False,
        ...
        for action_item in action_items:
            if not action_item.parent_action_item and action_item.parent_action_identifier:
                ...
                updated = ActionItem.objects.filter(
                    ...
                    parent_action_item__isnull=True,
                    linked_to_reference=True).exclude(
    ...
    try:
        action_items = ActionItem.objects.filter(
            related_action_identifier__isnull=False,
        ...
        for action_item in action_items:
            if not action_item.related_action_item and action_item.related_action_identifier:
                ...
                updated = ActionItem.objects.filter(
                    ...
                    related_action_item__isnull=True).exclude(
                        action_identifier=action_item.related_action_identifier
                ...
                try:
                    ...
                    try:
                        ...
                        if action_item.parent_action_item:
                            ...
                            related_action_item = (
                                reference_obj.parent_action_item)
                    else:
...
def fix_action_item_sequence(action_item_cls, action_identifier=None,
        ...
        for obj in RegisteredSubject.objects.all():
            try:
                ...
                try:
                    ...
                    for index, obj in enumerate(ActionItem.objects.filter(
                            subject_identifier=obj.subject_identifier,
                            action_type__name=name)):
                        if index > 0:
                            obj.delete()
