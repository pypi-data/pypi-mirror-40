...
__all__ = ['MetricFunc', 'metric_variable', 'get_metric_name', 'metrics_accuracy', 'metrics_mean_per_class_accuracy',
           'metrics_apc_np', 'metrics_auc', 'metrics_confusion_matrix', 'metrics_spearman_correlation',
           'metrics_mean_squared_error', 'metrics_mean_absolute_error', 'spearman_correlation']


class MetricFunc:
    def __init__(self, f_metric, **kwargs):
        self.f_metric = f_metric
        self.kwargs = kwargs

    def __call__(self, labels, logits):
        return self.f_metric(labels, logits, **self.kwargs)


def metric_variable(shape, dtype=tf.float32, validate_shape=True, use_resource=None, name=None):
...
def metrics_auc(labels, logits, weights=None, use_softmax=False, num_thresholds=200,
                metrics_collections=None, updates_collections=None,
                curve='ROC', name=None, summation_method='trapezoidal'):
    if use_softmax:
...
def streaming_spearman_correlation(labels, predictions, weights=None,
    ...
    with tf.variable_scope(name, 'spearman_r'):
        ...
        cy1 = tf.concat([y1, labels], axis=-1)
        cy2 = tf.concat([y2, predictions], axis=-1)
        cw = tf.concat([w, weights], axis=-1)
        update_y1 = tf.assign(y1, cy1, validate_shape=False)
        update_y2 = tf.assign(y2, cy2, validate_shape=False)
        update_w = tf.assign(w, cw, validate_shape=False)
        u_ry1 = rankdata(update_y1[1:], rank_method)
        u_ry2 = rankdata(update_y2[1:], rank_method)
        u_cur_w = update_w[1:]
        update_spearman_r = tf.identity(_weighted_pearson(u_ry1, u_ry2, u_cur_w), name='update_op')
        ...
        if updates_collections:
            tf.add_to_collection(updates_collections, update_spearman_r)
    return spearman_r, update_spearman_r


def spearman_correlation(labels, predictions, weights=None, rank_method='average', name=None):
    def _weighted_pearson(x, y, w):
        xd = x - tf.reduce_sum(x * w) / tf.reduce_sum(w)
        yd = y - tf.reduce_sum(y * w) / tf.reduce_sum(w)
        corr = ((tf.reduce_sum(w * yd * xd) / tf.reduce_sum(w)) /
    ...
    with tf.name_scope(name, 'spearman_r'):
        if weights is None:
            weights = tf.ones_like(labels, dtype=labels.dtype)
        u_ry1 = rankdata(labels, rank_method)
        u_ry2 = rankdata(predictions, rank_method)
        u_cur_w = weights
        spearman_r = tf.identity(_weighted_pearson(u_ry1, u_ry2, u_cur_w), name=name)
        return spearman_r
