...
__all__ = ['add_regularization', 'LossFunc', 'create_loss', 'create_loss_with_aux', 'masked_sigmoid_cross_entropy',
           'seq_sigmoid_cross_entropy',
           'seq_softmax_cross_entropy', 'create_init_op', 'dynamic_learning_rate', 'create_train_op', 'get_all_ckpt',
           'load_ckpt', 'load_ckpt_path', 'save_model_pb', 'load_model_pb', 'writer_graph']


def add_regularization(raw_loss, scope=None):
    with tf.name_scope(scope):
        ...
        total_loss = raw_loss + regularization_loss
        return total_loss


class LossFunc:
    def __init__(self, f_loss, **kwargs):
        self.f_loss = f_loss
        self.kwargs = kwargs

    def __call__(self, labels, logits):
        return self.f_loss(labels, logits, **self.kwargs)


def create_loss(labels, logits, is_one_hot=False, scope=None):
    with tf.name_scope(scope):
        if is_one_hot:
...
def create_loss_with_aux(labels, logits, aux_logits, aux_weight=0.4, is_one_hot=False, scope=None):
    with tf.name_scope(scope):
        if is_one_hot:
            cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=logits))
...
def writer_graph(logdir, graph=None):
    if graph is None:
