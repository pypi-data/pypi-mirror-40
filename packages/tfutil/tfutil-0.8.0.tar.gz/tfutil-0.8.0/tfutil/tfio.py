...
__all__ = ['tfrec_name', 'tfrec_pred_name', 'tfimgrec_name', 'tfimgrec_pred_name',
           'rec_names', 'parse_tfrec_name', 'read_tfrec', 'read_tfrec_func', 'read_tfrec_img', 'read_tfrec_img_func',
           'read_raw_tfimgrec', 'read_raw_tfimgrec_func',
           'read_tfimgrec', 'read_tfimgrec_func', 'read_tfrec_array', 'read_tfrec_array_func',
           'balanced_read_tfrec_array', 'balanced_read_tfrec_array_func',
           ...
           'default_image_randomize_func', 'balanced_read_tfrec', 'balanced_read_tfrec_func']


def input_func_dec(func):
    @wraps(func)
...
def read_tfrec(filenames, batch_size=None, num_epochs=-1, shuffle=True, randomize_func=None):
    if isinstance(filenames, str):
        filenames = [filenames]
    filename_parsed_lst = [parse_tfrec_name(fn) for fn in filenames]
    filename_parsed = filename_parsed_lst[0]
    ds_shape_lst = [e[1] for e in filename_parsed_lst]
    num_examples = np.sum([e[0] for e in ds_shape_lst])
    ds_shape = ds_shape_lst[0][1:]
    ...
    ds = dataset.map(_parse_tfrec(ds_shape, is_pred, is_reg))
    if randomize_func is not None:
        ds = ds.map(randomize_func)
    if shuffle:
...
@input_func_dec
def read_tfrec_func(filenames, batch_size=None, num_epochs=-1, shuffle=True, randomize_func=None):
    return read_tfrec(filenames, batch_size, num_epochs, shuffle, randomize_func)


def balanced_read_tfrec(filenames, batch_size_per_class=None, num_epochs=-1, shuffle=True, randomize_func=None):
    '''
        The format of `filenames` should be
        [[file1_c0, file2_c0,...],
        [file1_c1, file2_c1,...],
        ...
        [file1_cn, file2_cn,...]].
        For the record, `num_examples` of file_cn should be the largest which is used to calculate `steps_per_epoch`.
    '''
    bench_filename_parsed_lst = [parse_tfrec_name(fn) for fn in filenames[-1]]
    bench_filename_parsed = bench_filename_parsed_lst[-1]
    ds_shape_lst = [e[1] for e in bench_filename_parsed_lst]
    num_examples = np.sum([e[0] for e in ds_shape_lst])
    ds_shape = ds_shape_lst[0][1:]
    if batch_size_per_class is None:
        batch_size_per_class = num_examples
    steps_per_epoch = int(np.ceil(num_examples / batch_size_per_class))
    is_pred = not bench_filename_parsed[0]
    is_reg = False
    if not is_pred and bench_filename_parsed[-1] == 0:
        is_reg = True
    dataset_lst = [tf.data.TFRecordDataset(fn) for fn in filenames]
    ds_lst = [ds.map(_parse_tfrec(ds_shape, is_pred, is_reg)) for ds in dataset_lst]
    if randomize_func is not None:
        ds_lst = [ds.map(randomize_func) for ds in ds_lst]
    if shuffle:
        ds_lst = [ds.shuffle(buffer_size=batch_size_per_class * 3) for ds in ds_lst]
    ds_lst = [ds.batch(batch_size_per_class) for ds in ds_lst]
    ds = tf.data.Dataset.zip(tuple(ds_lst))

    def _concat(*line):
        x = tf.concat([line[i][0] for i in range(len(line))], axis=0)
        y = tf.concat([line[i][1] for i in range(len(line))], axis=0)
        return x, y
...
@input_func_dec
def balanced_read_tfrec_func(filenames, batch_size_per_class=None, num_epochs=-1, shuffle=True, randomize_func=None):
    return balanced_read_tfrec(filenames, batch_size_per_class, num_epochs, shuffle, randomize_func)


def read_tfrec_img(filenames, shape, batch_size=None, num_epochs=-1, shuffle=True,
                   shape_enlarge=True, normalization=True, randomize_func=None):
    if isinstance(filenames, str):
        filenames = [filenames]
    filename_parsed_lst = [parse_tfrec_name(fn) for fn in filenames]
    filename_parsed = filename_parsed_lst[0]
    ds_shape_lst = [e[1] for e in filename_parsed_lst]
    num_examples = np.sum([e[0] for e in ds_shape_lst])
    ds_shape = ds_shape_lst[0]
    _, height, width, channels = ds_shape
    if batch_size is None:
        batch_size = num_examples
    steps_per_epoch = int(np.ceil(num_examples / batch_size))
    is_pred = not filename_parsed[0]
...
@input_func_dec
def read_tfrec_img_func(filenames, shape, batch_size=None, num_epochs=-1, shuffle=True,
                        shape_enlarge=True, normalization=True, randomize_func=None):
    return read_tfrec_img(filenames, shape, batch_size, num_epochs, shuffle, shape_enlarge, normalization,
                          randomize_func)


def read_raw_tfimgrec(filenames, num_epochs=-1, randomize_func=None):
    if isinstance(filenames, str):
        filenames = [filenames]
    filename_parsed_lst = [parse_tfrec_name(fn) for fn in filenames]
    filename_parsed = filename_parsed_lst[0]
...
@input_func_dec
def read_raw_tfimgrec_func(filenames, num_epochs=-1, randomize_func=None):
    return read_raw_tfimgrec(filenames, num_epochs, randomize_func)


def read_tfimgrec(filenames, shape, batch_size=None, num_epochs=-1, shuffle=True, shape_enlarge=True,
                  normalization=True, randomize_func=None):
    if isinstance(filenames, str):
        filenames = [filenames]
    filename_parsed_lst = [parse_tfrec_name(fn) for fn in filenames]
    filename_parsed = filename_parsed_lst[0]
    num_examples = np.sum([e[1] for e in filename_parsed_lst])
    if batch_size is None:
        batch_size = num_examples
    steps_per_epoch = int(np.ceil(num_examples / batch_size))
...
@input_func_dec
def read_tfimgrec_func(filenames, shape, batch_size=None, num_epochs=-1, shuffle=True, shape_enlarge=True,
                       normalization=True, randomize_func=None):
    return read_tfimgrec(filenames, shape, batch_size, num_epochs, shuffle, shape_enlarge, normalization,
                         randomize_func)


def read_tfrec_array(arrs, batch_size=None, num_epochs=-1, shuffle=True, is_reg=False, randomize_func=None):
...
@input_func_dec
def read_tfrec_array_func(arrs, batch_size=None, num_epochs=-1, shuffle=True, is_reg=False, randomize_func=None):
    return read_tfrec_array(arrs, batch_size, num_epochs, shuffle, is_reg, randomize_func)


def balanced_read_tfrec_array(x, y, batch_size=None, num_epochs=-1):
    x = x.astype(np.float32)
    y = y.astype(np.int32)
    ...
    y_shape[0] = None
    y_shape = tf.TensorShape(y_shape)
    ...
    ds = tf.data.Dataset.from_generator(gen, output_types=(tf.float32, tf.int32), output_shapes=(x_shape, y_shape))
    ds = ds.repeat(num_epochs)
    iterator = ds.make_one_shot_iterator()
    return iterator.get_next(), steps_per_epoch
...
@input_func_dec
def balanced_read_tfrec_array_func(x, y, batch_size=None, num_epochs=-1):
    return balanced_read_tfrec_array(x, y, batch_size, num_epochs)


def default_image_randomize_func(x, y):
    x_shape = x.get_shape().as_list()
    x = tf.pad(x, [[4, 4], [4, 4], [0, 0]])
    x = tf.random_crop(x, x_shape)
    x = tf.contrib.image.rotate(x, np.random.randint(0, 360) * np.pi / 180, interpolation='BILINEAR')
    return x, y
...
def write_tfrec_from_array(arr_x, arr_y, prefix, num_classes, num_examples_per_file=None):
    ...
    if num_examples_per_file is None or num_examples_per_file >= num_examples:
        ...
        for i in range(num_splits):
            filename = tfrec_name(prefix, (num_examples_per_file, *ds_shape[1:]), num_classes, index=i)
            filenames.append(filename)
            writer = tf.python_io.TFRecordWriter(filename)
            ...
            writer.close()
        if res != 0:
            filename = tfrec_name(prefix, (res, *ds_shape[1:]), num_classes, index=num_splits)
            filenames.append(filename)
            writer = tf.python_io.TFRecordWriter(filename)
            for _ in range(res):
                idx += 1
...
def write_tfrec_pred_from_array(arr_x, prefix, num_examples_per_file=None):
    ...
    if num_examples_per_file is None or num_examples_per_file >= num_examples:
        ...
        idx = -1
        for i in range(num_splits):
            filename = tfrec_pred_name(prefix, (num_examples_per_file, *ds_shape[1:]), index=i)
            filenames.append(filename)
            writer = tf.python_io.TFRecordWriter(filename)
            for _ in range(num_examples_per_file):
                idx += 1
            ...
            writer.close()
        if res != 0:
            filename = tfrec_pred_name(prefix, (res, *ds_shape[1:]), index=num_splits)
            filenames.append(filename)
            writer = tf.python_io.TFRecordWriter(filename)
            for _ in range(res):
                idx += 1
...
def write_tfimgrec_from_lst(x_lst, arr_y, prefix, num_classes, num_examples_per_file=None):
    ...
    if num_examples_per_file is None or num_examples_per_file >= num_examples:
        ...
        idx = -1
        for i in range(num_splits):
            filename = tfimgrec_name(prefix, num_examples_per_file, channels, num_classes, index=i)
            filenames.append(filename)
            writer = tf.python_io.TFRecordWriter(filename)
            for _ in range(num_examples_per_file):
                idx += 1
            ...
            writer.close()
        if res != 0:
            filename = tfimgrec_name(prefix, res, channels, num_classes, index=num_splits)
            filenames.append(filename)
            writer = tf.python_io.TFRecordWriter(filename)
            for _ in range(res):
                idx += 1
...
def write_tfimgrec_pred_from_lst(x_lst, prefix, num_examples_per_file=None):
    ...
    if num_examples_per_file is None or num_examples_per_file >= num_examples:
        ...
        idx = -1
        for i in range(num_splits):
            filename = tfimgrec_pred_name(prefix, num_examples_per_file, channels, index=i)
            filenames.append(filename)
            writer = tf.python_io.TFRecordWriter(filename)
            for _ in range(num_examples_per_file):
                idx += 1
            ...
            writer.close()
        if res != 0:
            filename = tfimgrec_pred_name(prefix, res, channels, index=num_splits)
            filenames.append(filename)
            writer = tf.python_io.TFRecordWriter(filename)
            for _ in range(res):
                idx += 1
...
def write_tfrec_from_generator(gn, prefix, num_classes, num_examples_per_file=None):
    ...
    if num_examples_per_file is None:
        ...
        while not end_gn:
            ...
            i_split += 1
        res = num_examples % num_examples_per_file
        for j, fnm in enumerate(temp_filenames):
            filename = _convert_to_real_name(fnm, num_examples_per_file)
            if (j + 1 == len(temp_filenames)) and res != 0:
                filename = _convert_to_real_name(fnm, res)
