...
class Binomial(Distribution):
    ...
    def calculate_stdev(self):
        ...
        stdev = math.sqrt(float(self.n) * self.p * (1. - self.p))
        self.stdev = stdev
        return self.stdev
    ...
    def replace_stats_with_data(self):
        ...
        self.p = float(sum(data_list)) / float(self.n)
    ...
    def plot_bar_pdf(self):
        ...
        # k = 0 to k = n
        ks = [i for i in range(self.n + 1)]
        #   Hint: You'll need to use the pdf() method defined above to calculate the
        #   density function for every value of k.
        probs = [self.pdf(i) for i in ks]
        plt.bar(ks, probs)
        plt.xlabel('k value')
    ...
    def __add__(self, other):
        ...
        binomial_sum = Binomial()  
        binomial_sum.n = self.n + other.n
        binomial_sum.p = self.p
        return binomial_sum
