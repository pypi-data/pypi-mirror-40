...
def main():
  ...
  datestring = "".join([str(datetime.date.today().year), str(datetime.date.today().month).zfill(2), str(datetime.date.today().day).zfill(2)])
  ...
  if args.launch == True:
    os.system('blender --python {} -- "{}" "{}"'.format(stripped, folderpath, temp))
