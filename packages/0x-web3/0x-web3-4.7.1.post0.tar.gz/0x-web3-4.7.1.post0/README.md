0x-web3 is a temporary fork of web3.  It adds primitive support for ABI tuples, which is needed in order to facilitate calling [the 0x smart contracts](https://0x.org/docs/contracts#Introduction).  The fork's changes to web3.py are visible in [an open PR](https://github.com/ethereum/web3.py/pull/1147), and when that PR (or something analogous) is merged, this package will be taken down.
