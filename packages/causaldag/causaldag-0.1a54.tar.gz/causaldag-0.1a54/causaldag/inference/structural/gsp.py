...
def unknown_target_igsp(
    ...
    is_variant_dict = {iv_nodes: dict() for iv_nodes in samples if iv_nodes != frozenset()}
    pvalue_dict = {iv_nodes: dict() for iv_nodes in samples if iv_nodes != frozenset()}
    obs_samples = samples[frozenset()]
    ...
    def _get_is_variant(iv_nodes, j, cond_set):
        ...
        pvalue = pvalue_dict[iv_nodes].get((j, cond_set))
        if is_variant is None:
            test_results = invariance_test(
                obs_samples, samples[iv_nodes], j, cond_set=list(cond_set), alpha=alpha_invariance)
            is_variant = test_results['reject']
            pvalue = test_results['p_value']
            is_variant_dict[iv_nodes][(j, cond_set)] = is_variant
            pvalue_dict[iv_nodes][(j, cond_set)] = pvalue
    ...
    def _is_icovered(i, j, dag):
        ...
        for iv_nodes, iv_samples in samples.items():
            if i in iv_nodes:
                if not _get_is_variant(iv_nodes, j, parents_j)[0]:
                    return False
        return True
        ...
        for i in dag.nodes:
            ...
            for iv_nodes, iv_samples in samples.items():
                if iv_nodes != frozenset():
                    is_variant, pvalue = _get_is_variant(iv_nodes, i, parents_i)
                    pvalues[(iv_nodes, i, parents_i)] = pvalue
                    if is_variant:
                        variants.add((iv_nodes, i))
    ...
    def _reverse_arc_igsp(dag, i_covered_arcs, i, j):
        ...
        new_score = len(new_dag.arcs) + len(_get_variants(dag))
