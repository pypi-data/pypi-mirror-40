...
def get_imagelist():
    res = requests.get(url=HOST+"/image/images")
...
def getgpu():
    ...
    if parsed_info is not None:
        if parsed_info['status'] == 'Success':
            ...
            image_data = [["#", "Repository", "Tag", "Description"]]

            for i, img in enumerate(images):
                image_data.append((i + 1, img['repository'], img['tag'], img['description']))
            ...
            while True:
                try:
                    image_idx = int(input('Select docker image (1 ~ {}): '.format(len(images))))

                    if image_idx < 1 or image_idx > image_count:
                        raise ValueError
                    else:
                        image_id = images[image_idx - 1]['id']
                        print()
                        break
                except ValueError:
                    print("Invalid input.\n")
    ...
    if parsed_info is not None:
        if parsed_info['status'] == 'Success':
            ...
            if pod_list:
                ...
                check = input('Proceed to delete contaier [{}]? (yes/no): '.format(pod_list[pod_index-1][1]))
