...
from qbase.bitmex import bitmexService
...
class Transation:
    ...
    def bitmexErrorFrozen(self, position, tradeType, num):
        if tradeType == 1 or tradeType == 2:
            position.baseCoinFrozen += num
        elif tradeType == 3:
            position.tradeCoinFrozen += num
    ...
    def bitmexFuturesServer(self, account, position, num, tradeType):
        bms = bitmexService.BitmexService(account.accessKey, account.secretKey)
        contractName = position.openContract
        ...
        if tradeType == 1 or tradeType == 2:
            if tradeType == 1:
                position.tradeContract = contractName
                trade = '开多'
                bitmexSide = bitmexService.BitMexSide.Buy
            else:
                position.nullContract = contractName
                trade = '开空'
                bitmexSide = bitmexService.BitMexSide.Sell
            amountNum = position.baseCoin - position.baseCoinFrozen
            self.log.info('买入 %s  数量 %s 交易类型 : %s ' % (position.symbol, amountNum, trade))
        elif tradeType == 3 or tradeType == 4:
            if tradeType == 3:
        ...
        clOrdID = str(account.id) + "_" + str(position.id) + "_" + str(int(time.time() * 10000))

        bitmexResultMapper.insertBitmexResult(clOrdID, position.id, contractName, bitmexSide.value, num)

        try:
            result = bms.createOrder(contractName, bitmexSide, orderQty=num, clOrdID=clOrdID)
            bitmexResultMapper.updateBitmexResultByClOrderId(position.id, result)
            if result[u'ordStatus'] != "Filled":
                return enum.TransactionResult.Fail.value
        except Exception as e:
            errorCode = str(e)[0:3]
            if errorCode == "400" or errorCode == "401" or errorCode == "403" or errorCode == "404":
                self.log.error('bitmex交易失败  %s' % str(e))
                return enum.TransactionResult.Fail.value
            #冻结
            self.bitmexErrorFrozen(position, tradeType, num)
            self.log.error('bitmex交易报错,冻结交易资金 ')
            return enum.TransactionResult.Fail.value

        cumQty = result[u'cumQty']
        avgPx = result[u'avgPx']
        if tradeType == 1 or tradeType == 2:
            position.baseCoin -= cumQty * avgPx
            if tradeType == 1:
                position.tradeCoin += cumQty
            else:
                position.nullCoin += cumQty
        elif tradeType == 3 or tradeType == 4:
            position.baseCoin += cumQty * avgPx
            if tradeType == 3:
                position.tradeCoin -= cumQty
