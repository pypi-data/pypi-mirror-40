...
if sys.version_info[0] < 3:
  raise Exception("bb-unsucked requires Python 3.6+")

if sys.version_info[1] < 6:
  raise Exception("bb-unsucked requires Python 3.6+. You have python 3, but the minor version is < 6")
...
CONFIG_FILES = [
  ...
  f"{_HOME_DIR}/.bb-unsucked/config.ini",
]
...
CONFIG = {
  ...
  "COOKIES_FILE": f"{_HOME_DIR}/.bb-unsucked/cookies.bin",
  "AGRESSIVE_CACHING": False, # when True, trade slower beginning for faster future
  
  "COURSES_CACHE_FILE": f"{_HOME_DIR}/.bb-unsucked/courses-cache.bin",
  "COURSES_MAX_CACHE_S": 7 * 24 * 60 * 60,
  
  "ANNOUNCEMENTS_CACHE_FILE": f"{_HOME_DIR}/.bb-unsucked/announcements-cache.bin",
...
"""
  ...
  def get_cookies(self):
    ...
    if not os.path.exists(cookies_file):
      err_msg = f"""No authentication cookies found in {cookies_file}!
Please read in your blackboard session cookies by running 'bb-unsucked build-cache path/to/recent_request.har'"""
      raise RuntimeError(err_msg)
    return load_obj(cookies_file)
  
  def get_tab_action_url(self):
    base = self.config["BASE_DOMAIN"]
    return f"{base}/webapps/portal/execute/tabs/tabAction"
  ...
  def import_har(self, harfile: str):
    ...
    with open(harfile, 'r') as file_handle:
      ...
      for http_entry in har_json["log"]["entries"]:
        if http_entry["request"]["url"] == f"{base}/webapps/portal/execute/tabs/tabAction":
...
name must be of the form '201820_SPRING_MSIM495_29614: GAME PHYSICS' because it it parsed into components."""
  ...
  def file_name(self):
    ...
    return f"{lower_course_id}_{camels}"
