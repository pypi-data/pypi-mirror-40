...
def gui(model, data=None, target=None, name=None, class_names=None, class_colors=None, port=8080):
    ...
    server_process = multiprocessing.Process(name="RFVis Server", target=_start_server, args=(forest, port))
    server_process.start()
    return server_process


def _start_server(data, port=8080, debug=False, use_reloader=False, threaded=True, **kwargs):
    ...
    run_simple("localhost", port, app, use_reloader=use_reloader, use_debugger=debug, threaded=threaded, **kwargs)
