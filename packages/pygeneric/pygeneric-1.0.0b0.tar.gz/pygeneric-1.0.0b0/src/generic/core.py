...
cte = (lambda _cte: property(lambda self: _cte))
factory = (lambda func: property(lambda self: func()))
attr = (
    lambda _attr:
    property(lambda self: getattr(self, _attr))
        .setter(lambda self, value: setattr(self, _attr, value)))
...
class Generic(_generic_base):
    ...
    __annotations__ = factory(dict)
    __closure__ = cte(None)
    __code__ = cte(None)
    __defaults__ = cte(None)
    __globals__ = factory(lambda self: sys._getframe(1).f_locals)
    __name__ = attr('name')
    __doc__ = attr('doc')
    ...
    def _register_factory(self, argtypes, factory=None, restype=None):
        ...
        Every time the dispatcher reaches a method factory, it calls
...
def subtypes(typ, data):
    """Return all entries in data that are subtypes of typ."""

    subclass = subtypecheck
    return [s for s in data if subclass(s, typ)]


def supertypes(typ, data):
    """Return all entries in data that are supertypes of typ"""

    subclass = subtypecheck
    return [s for s in data if subclass(typ, s)]
...
def generic(*args, **kwargs):
    ...
    if args and callable(args[0]):
        ...
        if args or kwargs:
            result.overload(*args, **kwargs)(func)
        else:
        ...
        def decorator(func):
            if not callable(func):
                ...
                raise TypeError(msg)
            return generic(func, *args, **kwargs)
...
def _instance_check_factory(func, argtypes, restype, instancecheck=isinstance):
    ...
    def checked(*args, **kwds):
        ...
        for i, (x, typ) in enumerate(zip(args, argtypes)):
            if instancecheck(x, typ):
                fmt = i, tname(x), typ.__name__
