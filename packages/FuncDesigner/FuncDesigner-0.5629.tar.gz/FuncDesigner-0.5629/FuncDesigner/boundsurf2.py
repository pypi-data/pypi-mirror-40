...
#    domain[k][0][domain_ind], domain[k][1][domain_ind])*v for k, v in self.d.items())
    ...
    # TODO: mb get rid of domain_ind
        c = self.c
        oovars = set(self.d.keys()) | set(self.d2.keys())
        n = getattr(domain, 'nPoints',0)
        if n == 0: # TODO: get rid of it, always provide nPoints while domain creation
            Vals = domain.values()
        ...
        for k in oovars:
            _d = domain[k]
            l, u = _d[0][domain_ind], _d[1][domain_ind]
...
#            ind = where(ind)[0]
    ...
    __neg__ = lambda self: \
    surf2(dict((k, -v) for k, v in self.d2.items()), dict((k, -v) for k, v in self.d.items()), 
    -self.c)
...
#                self, other = other, self
            ...
            d = S.copy()
            d.update(O)
            similarKeys = set(S.keys()) & set(O.keys())
            ...
            if type(other) == surf2:
                ...
                d2 = self.d2#.copy()# TODO: mb copy() is not required here
