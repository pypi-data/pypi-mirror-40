...
def sum_derivative(r_, r0, INP, dep, point, fixedVarsScheduleID, Vars=None, fixedVars = None, useSparse = 'auto'):
    ...
    isSP = getattr(point, 'maxDistributionSize', 0) != 0
    ...
    # TODO: IS IT REQUIRED YET? MAY BE QUITE COSTLY SOMETIMES
...
def sum(inp, *args, **kwargs):
    if isinstance(inp, GeneratorType):
        inp = list(inp)
#        # can be numpy sum of objects here
#        return sum(list(inp), *args, **kwargs)
    ...
    # TODO: check it and mb modify it wrt sum(ooarray1,ooarray2,etc)
    ...
    if condIterableOfOOFuns:
        ...
        for elem in inp: # TODO: mb use reduce() or something like that
            if not isinstance(elem, (oofun, ooarray)): 
                ...
                # so it doesn't work for different sizes
                #TODO: check where asanyarray is required
                r0 = r0 + (elem if np.isscalar(elem) else np.asanyarray(elem))
                continue
            INP.append(elem)
        if len(INP) == 0:
            return r0
