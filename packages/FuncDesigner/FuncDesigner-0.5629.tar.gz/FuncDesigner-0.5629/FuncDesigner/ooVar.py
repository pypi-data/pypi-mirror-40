...
class oovar(oofun):
    ...
    has_implemented_lh = True # TODO: mb rework it by a check like oofun.lh is None
...
#
#            ind = isfinite(self.lb) & logical_not(isfinite(self.ub))
#            v0[ind] = self.lb[ind]
#
#            ind = logical_not(isfinite(self.lb)) & isfinite(self.ub)
#            v0[ind] = self.ub[ind]
#
#            self.v0 = v0
#            
...
#        ''')
    ...
    if size is not None:
        if len(args) > 0:
            raise FuncDesignerException('incorrect oovars usage')
        return oovars(size, *kw)
