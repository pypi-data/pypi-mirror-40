...
PythonSum, PythonAny, PythonMax, PythonAll = sum, any, max, all
PythonProd = lambda arg: functools.reduce(operator.mul, arg, 1)

#def PythonProd(arg):
#    print('asdf')
#    return functools.reduce(operator.mul, arg, 1)
from numpy import inf, asfarray, atleast_2d, zeros, dot, asarray, atleast_1d, \
...
from FuncDesigner.Interval import Interval, mul_interval, pow_const_interval, pow_oofun_interval, div_interval, \
...
#            return self.hasStochasticVariables
            ...
            return self.is_oovar and (self.domain in (bool, np.bool_))
        ...
        #TODO: mb add vectorization here
        self.size = r 
...
#            definiteRange = logical_and(definiteRange, self.getDefiniteRange(arg_infinum, arg_supremum))
    ...
    def _interval(self, domain, dtype, ia_surf_level = 0):
        ...
        # TODO: mb improve it, don't use tile each time
        if tmp is not None:
...
#            
        #TODO: new 
#        if v is not None: 
#            Dict = domain.localStoredIntervals
#        elif v is None or ((v not in self._getDep() or self.is_oovar) and self is not v):
#            Dict = domain.storedIntervals
#            
#        r = Dict.get(self, None)
        ...
        if newComputation:
            ...
            # TODO: mb rework the condition, handle only funcs involved in the prob, not all created
            if self._usedIn > 1:
                if v is None and domain.useSave:
        ...
        if type(r[0]) in (boundsurf, boundsurf2): 
            ...
            if newComputation:
                ...
                if len(Tmp):
                    ...
                    if self._usedIn > 1:
                        if v is None and domain.useSave:
                            domain.storedIntervals[self] = (R, definiteRange)
                        if v is not None:
                            domain.localStoredIntervals[self] =  (R, definiteRange)
...
#                using FuncDesigner.sum() instead is highly recommended''')              
        ...
        assert not isinstance(other, (list, tuple)), 'summation of oofun with list/tuple is forbidden, use arrays instead'
        
        if not isinstance(other, (oofun, ndarray)) and not isscalar(other):
...
#            return fd_prod(self._prod_elements[:-1]+[-self._prod_elements[-1]])
    ...
    def __div__(self, other):
        ...
        assert not isinstance(other, (list, tuple)), 'division of oofun with list/tuple is forbidden, use arrays instead'
...
#            if other.size == 1 or 'size' in self.__dict__ and self.size in (1, other.size):
            if other.size == 1:
                r._D = lambda *args, **kwargs:\
                    dict((key, value/other) for key, value in self._D(*args, **kwargs).items())
                r.d = raise_except
        ...
        div_expression(self, other, *args, **kw)
        return r
    ...
    def __rdiv__(self, other):
        ...
        other = array(other, 'float') # TODO: float128 and sparse matrices handling!
        if other.size == 1:
            other = other.item()
        ...
        r.expression = lambda *args, **kw:\
        rdiv_expression(self, other, *args, **kw)
        return r
    ...
    def __mul__(self, other):
        ...
        assert not isinstance(other, (list, tuple)), 'multiplication of oofun with list/tuple is forbidden, use arrays instead'
...
#            r = oofun(np.prod, INP, vectorized=True)
        ...
        r.expression = lambda *args, **kw:\
        mul_expression(self, other, *args, **kw)
    ...
    def __pow__(self, other):
        ...
        assert not isinstance(other, (list, tuple)), 'this operation of oofun with list/tuple is forbidden, use arrays instead'
...
#                        r.monotonities = (-1 if other%2 == 0 else 1, 1)
            ...
            r._interval_ = lambda *args, **kw:\
            pow_oofun_interval(self, other, *args, **kw)
...
#        r.isCostly = True
        ...
        r.expression = lambda *args, **kw:\
        pow_expression(self, other, *args, **kw)
        return r
    ...
    def __rpow__(self, other):
        ...
        r._interval_ = lambda *args, **kw:\
        rpow_interval(r, other, *args, **kw)

        r.expression = lambda *args, **kw:\
        rpow_expression(self, other, *args, **kw)
...
#            print x.shape
    ...
    def eq(self, other):
        ...
        if type(other) in (str, string_): 
            ...
            # TODO: CHECK IS 2ND CONDITION REQUIRED YET
            if self._is_categorical_field:
                Domain = self.aux_domain_dict
                _engine = self.numerical
            else:
                if 'aux_domain' not in self.__dict__: # TODO: check is it required yet?
                    ...
                    self.formAuxDomain()
                Domain = self.aux_domain
                
                # TODO: check is it correct or self.numerical is required instead
                _engine = self
...
#                raise FuncDesignerException('probably you have changed domain of categorical oovar, that is not allowed')
            ...
            ind = Domain.get(other, -1) 
            if ind == -1:
                raise FuncDesignerException('compared value %s is absent in oovar %s domain' %(other, self.name))
            
            #TODO: MB USE THIS ONE INSTEAD
            #r = Constraint(self, ub = ind, lb = ind, tol=0.5)
            r = Constraint(_engine-ind, ub = 0, lb = 0, tol=0.5, vectorized = True)
            
            if self.is_oovar: # or self._is_categorical_field: 
                r.nlh = lambda Lx, Ux, p, dataType: self.nlh(Lx, Ux, p, dataType, ind)
#            elif self._is_categorical_field:
                
            elif not self._is_categorical_field:
                raise_except('this case for string-return-type oofun comarison with string is not overloaded yet')
                
            Other = other
            
        elif 'startswith' in dir(other): # can't use __dict__ - it can be absent
            return False # TODO: check it - is it required yet?
        else:
            if type(other) == bool:
                other = int(other)
            elif type(other) == ndarray and other.dtype == bool:
                other = int(other.item()) if other.size == 1 else asarray(other, int)
            
            #TODO: MB USE THIS ONE INSTEAD
            #r = Constraint(self, ub = ind, lb = ind, tol=0.5)
            
            r = Constraint(self - other, ub = 0.0, lb = 0.0, vectorized = True) # do not perform check for other == 0, copy should be returned, not self!
#            assert 0
            if self.is_oovar and isscalar(other) and self.domain is not None:
                # perform some speedup here taking into account oovar func lh()
                # TODO: mb proper hanling of case "other is ndarray of size > 1" with oovar
                if self.domain is bool or self.domain is 'bool':
                    if other not in (0, 1, True, False):# and type(other) not in (int, int16, int32, int64):
                        raise FuncDesignerException('bool oovar can be compared with [0,1,True,False] only')
                    
                    # TODO: use self.lh for other cases where lh is defined for the oofun involved 
                    r.nlh = self.nlh if other in (True, 1.0)\
                    else self.lh if self.is_oovar \
                    else (~self).nlh
...
#        self.inputOOVarTotalLength = 0
    ...
    def _getDep(self):
        ...
        if 'dep' in self.__dict__:
            ...
            if len(r_oovars) + PythonSum(len(elem) for elem in r_oofuns) != len(r):
                isUncycled = False
