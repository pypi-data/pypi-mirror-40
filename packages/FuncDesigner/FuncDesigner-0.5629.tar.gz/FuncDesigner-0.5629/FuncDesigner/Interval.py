...
import numpy as np
from FDmisc import FuncDesignerException, update_mul_inf_zero, \
update_negative_int_pow_inf_zero, update_div_zero, where
...
# TODO: remove it (temporary for compatibility with interalgT.py)
adjust_lx_WithDiscreteDomain = adjust_ux_WithDiscreteDomain = None
...
def direct_div(lb1_ub1, lb2_ub2):
    ...
    if tmp.dtype == object:
        raise FuncDesignerException('this division is unimplemented yet')
...
def get_inv_b2_coeffs(ll, uu, dll, duu, c_l, c_u):
    ...
    ind_z3 = logical_or(ind_z, ind_z2)
    c[ind_z3] = 1.0/min_val[ind_z3]
