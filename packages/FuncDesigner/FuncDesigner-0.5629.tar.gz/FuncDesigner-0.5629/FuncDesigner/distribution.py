...
def mergeDistributions(d1, d2, operation):
    ...
    if not is_d1_stoch or not is_d2_stoch or cond_same_stoch:
        if not is_d2_stoch: # thus d1 is stoch
            ...
            or (hasattr(operator, 'div')  and operation == operator.div) \
            and not isinstance(d2, ooarray) else np.asanyarray(d2)
            ...
            or (hasattr(operator, 'div')  and operation == operator.div) \
            and not isinstance(d1, ooarray) else np.asanyarray(d1)
            ...
            vals1 = d1.values
            vals2 = d2.values
            distribType = d1.distribType if d1.distribType == d2.distribType \
            else 'undefined'
        ...
        case1 = np.all(d1.probabilities == d1.probabilities[0])
        case2 = np.all(d2.probabilities == d2.probabilities[0])
        
        #debug
#        Probabilities0 = \
#            (d1.probabilities.reshape(-1, 1) * d2.probabilities.reshape(1, -1)).flatten()
        ...
        if case1 and case2:
            Probabilities = \
            np.empty(d1.probabilities.size*d2.probabilities.size)
            Probabilities.fill(d1.probabilities[0] * d2.probabilities[0])
        elif case1:
            Probabilities = \
            np.tile(d1.probabilities[0]*d2.probabilities, d1.probabilities.size)
#            __rr = np.linalg.norm(Probabilities-Probabilities0)
#            if __rr > -1:
#                print ('1', __rr)
        elif case2:
            Probabilities = \
            np.tile(d2.probabilities[0]*d1.probabilities.reshape(-1, 1), d2.probabilities.size).flatten()
#            __rr = np.linalg.norm(Probabilities-Probabilities0)
#            if __rr > -1:
#                print ('2', __rr)
        else:
...
#    assert isinstance(distrib, stochasticDistribution), 'd_quantile works for stochasticDistribution input only'
    ...
    if interpolate not in (True, False)\
    and distrib.distribType == 'undefined'\
    and f_quantile.interpolate_w is False:
        print('''
        WARNING : you should provide parameter "interpolate" = True / False 
        into function P() for mixed discrete-continuous distributions; currently True will be used;
        for False only global solvers (http://openopt.org/GLP) are suitable
...
def P(c, interpolate = 'auto'):
    ...
    if np.isfinite(c.ub) and c.lb == -np.inf:
        r = oofun(lambda x: f_quantile(x, c.ub, interpolate), c.oofun, 
                  ...
                  engine_monotonity = -1, vectorized = True)
...
def std(arg, *args, **kw):
    if isinstance(arg, stochasticDistribution):
        ...
        if is_prod_scalar(arg):
            multiplier, tmp = arg._fixed_part, arg._unfixed_part
            return np.abs(multiplier) * std(tmp)
...
def var(arg, *args, **kw):
    if isinstance(arg, stochasticDistribution):
        ...
        r.getOrder = lambda *args, **kw: 2 * arg.getOrder(*args, **kw)
    else:
