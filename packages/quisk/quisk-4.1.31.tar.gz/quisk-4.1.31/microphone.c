...
#if DEBUG_IO || DEBUG
		x = fabs(dsample) / CLIP16;
		if (x > peakIn)
			peakIn = x;
#endif
		// FIR bandpass filter; separate into I and Q
		csample = quisk_dC_out(dsample, &filter1) * 1.95;	// tuned filter loss 0.5
		amplitude = cabs(csample);
#if DEBUG_IO || DEBUG
		if (amplitude / CLIP16 > peakOut2)
			peakOut2 = amplitude / CLIP16;
#endif
		if (amplitude > CLIP16)
			csample = csample / (amplitude / CLIP16);
		filtered[i] = csample * volume;
	}
	//quisk_calc_audio_graph(CLIP16, filtered, NULL, count, 0);
#if DEBUG_IO || DEBUG
	if (debug_timer == 0) {
		printf ("peakIn %10.6lf  peakOut2 %10.6lf  ratio %10.6lf", peakIn, peakOut2, peakOut2 / peakIn);
		if (peakOut2 > 1.0)
			printf ("  CLIP\n");
		else
			printf ("\n");
...
{	// Input samples are creal(filtered), output is filtered.
	...
	if (debug_timer == 0) {
		printf ("inMax  %10.6lf", inMax);
		printf ("    peakIn %10.6lf", peakIn);
		if (peakIn > 1.0)
			printf ("  CLIP");
		else if (peakIn > Xmin)
			printf ("  High");
			...
			printf ("      ");
		printf ("    peakOut2 %10.6lf", peakOut2);
		if (peakOut2 > 1.0)
			printf ("  CLIP\n");
		else
			printf ("\n");
...
// the output rate is MIC_OUT_RATE.
	...
	if (key_down && ! key_was_down)		// Key was pressed. Zero the first few samples to clear the buffers.
		key_down_counter = 4800;
