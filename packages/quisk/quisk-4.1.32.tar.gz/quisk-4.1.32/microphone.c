...
{  // Call to set the buffer size. Then call to initialize the structure.
	int i;

	if (size > 0)		// If size > 0, set the buffer size. Do not change the size once set.
		pt->buf_size = size;
	if (pt->buffer == NULL) {
		pt->buffer = (complex double *)malloc(pt->buf_size * sizeof(complex double));
	}
	pt->index = 0;
	pt->block_index = 0;
	pt->counter = 0;
...
static void process_alc(complex double * cSamples, int count, struct alc * pt)
{  // Automatic Level Control (ALC)
	...
	for (i = 0; i < count; i++) {
		...
		cSamples[i] = pt->buffer[pt->index] * pt->gain_now;		// remove sample from buffer and apply gain
#if DEBUG_LEVEL || DEBUG_IO || DEBUG
		magn = cabs(cSamples[i]);
		if (magn >= CLIP16)
			printf("ALC clip gain %9.6f level %9.6f\n", pt->gain_now, magn / CLIP16);
#endif
		pt->buffer[pt->index] = csamp;		// add new sample to buffer
		magn = cabs(csamp);			// measure new sample
		if (magn * (pt->gain_now + pt->gain_change * pt->buf_size) > (CLIP16 - 10)) {
			pt->gain_change = ((CLIP16 - 10) / magn - pt->gain_now) / pt->buf_size;
			pt->final_gain = pt->gain_now + pt->gain_change * pt->buf_size;
			pt->block_index = pt->index;
			pt->counter = 0;
			pt->fault = 0;
			pt->next_change = 1E10;
			//printf("ALC DEC: gain %9.6f change %9.6f final gain %9.6f\n", pt->gain_now, pt->gain_change, pt->final_gain);
		}
			...
			if (pt->final_gain > pt->gain_max) {
				pt->gain_change = (pt->gain_max - pt->gain_now) / pt->buf_size;
...
{	// Input samples are creal(filtered), output is filtered.
	...
	if (encode)
		for (i = 0; i < count; i++)
			filtered[i] *= 2.0;	// change the output level close to 1.0
	return count;
}
...
{
	int i, sample, maximum, interp, mic_interp, key_down, alc_active;
...
// the output rate is MIC_OUT_RATE.
	...
	if (key_down) {		// create transmit I/Q samples
		...
		if (alc_active)
			process_alc(cSamples, count, &tx_alc);
#if DEBUG_LEVEL || DEBUG_IO || DEBUG
		if ( ! alc_active) {
			for (i = 0; i < count; i++) {
				magn = cabs(cSamples[i]);
