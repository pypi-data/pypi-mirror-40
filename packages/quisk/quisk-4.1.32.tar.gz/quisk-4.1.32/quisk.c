	...
	if (rxMode == DGT_IQ) {
		...
		case 1:		// stereo, higher frequency is real
			if (quisk_tx_tune_freq < rx_tune_freq) {
				squelch_real = MeasureSquelch[0].squelch < squelch_level;
				squelch_imag = MeasureSquelch[1].squelch < squelch_level;
				for (i = 0; i < nSamples; i++)
					cSamples[i] = dsamples[i] + I * dsamples2[i];
			}
			else {
				squelch_real = MeasureSquelch[1].squelch < squelch_level;
				squelch_imag = MeasureSquelch[0].squelch < squelch_level;
				for (i = 0; i < nSamples; i++)
		...
		case 3:		// mono receive channel
			squelch_real = squelch_imag = MeasureSquelch[0].squelch < squelch_level;
			for (i = 0; i < nSamples; i++)
				cSamples[i] = dsamples[i] + I * dsamples[i];
			break;
		case 4:		// mono transmit channel
			squelch_real = squelch_imag = MeasureSquelch[1].squelch < squelch_level;
			for (i = 0; i < nSamples; i++)
...
start_agc:
	...
	else if (rxMode == FM) {		// mode is FM
		if (squelch_real && squelch_imag) {
			for (i = 0; i < nSamples; i++)
				cSamples[i] = 0;
		}
