...
def get_filter_tx(mode):	# Return the bandwidth, center of the Tx filters
  if mode in ('AM', 'DGT-IQ'):
    return 6000, 0
  if mode in ('FM', 'DGT-FM'):
    return 10000, 0
  if mode in ('CWL', 'CWU'):
    return 10, 0
  if mode in ('FDV-L', 'FDV-U'):
    return 2700, 1500
  return 2700, 1650
...
class GraphDisplay(wx.Window):
  ...
  def DrawFilter(self, dc):
    ...
    if self.tune_rx:
      ...
      dc.DrawLine(self.tune_rx + rit, 0, self.tune_rx + rit, self.height)
  ...
  def SetTuningLine(self, tune_tx, tune_rx):
    ...
    if self.tune_rx:
      dc.SetPen(self.tuningPenRx)
      dc.DrawLine(tune_rx + rit, 0, tune_rx + rit, self.height)
    dc.SetPen(self.tuningPenTx)
...
class WaterfallDisplay(wx.Window):
  ...
  def OnPaint(self, event):
    ...
    if self.tune_rx:
      dc.DrawLine(self.tune_rx + rit, self.margin, self.tune_rx + rit, self.height)
  def SetHeight(self, height):
    self.height = height
    self.SetSize((self.graph_width, height))
  def DrawFilter(self, dc):
    ...
    if self.tune_rx:
      dc.SetPen(self.tuningPenRx)
      dc.DrawLine(self.tune_rx + rit, 0, self.tune_rx + rit, self.margin)
    dc.SetPen(self.tuningPenTx)
  ...
  def SetTuningLine(self, tune_tx, tune_rx):
    ...
    if self.tune_rx:
      ...
      dc.DrawLine(tune_rx + rit, self.margin, tune_rx + rit, self.height)
...
The new code supports multiple corrections per band.""")
  ...
  def MakeButtons(self, frame, gbs):
    ...
    self.MakeAccel(b)
    left_row2.append(b)
    self.buttonAGC = QuiskCheckbutton(frame, self.OnBtnAGC, 'AGC')   # AGC and Squelch
    self.buttonAGC.char_shortcut = 'G'
    self.MakeAccel(self.buttonAGC)
    # This button can be labeled "Sqlch" so add another accelerator.
    rid = wx.NewId()
    self.main_frame.Bind(wx.EVT_MENU, self.buttonAGC.Shortcut, id=rid)
    self.accel_list.append(wx.AcceleratorEntry(wx.ACCEL_ALT, ord('q'), rid))
    self.BtnAGC = WrapSlider(self.buttonAGC, self.OnBtnAGC, slider_value=self.levelSpot, display=True)
  ...
  def OnChangeZoom(self, event):
    ...
    if x < 50:
      ...
      if not self.zooming:		# set deltaf when zoom mode starts
        ...
        if self.mode in ('CWL', 'LSB', 'DGT-L', 'FDV-L'):
          center = - center
  ...
  def OnBtnMode(self, event, mode=None):
    ...
    if mode == 'FM':
      ...
      self.buttonAGC.char_shortcut = 'q'
