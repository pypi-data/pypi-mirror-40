...
class QPowerMeterCalibration(wx.Frame):
  ...
  def MakeControls(self, panel):
    ...
    x = tab2 + charx * 20
    add = wx.lib.buttons.GenButton(panel, -1, 'Add to Table', pos=(x, y - h * 3 // 10))
    ww, hh = add.GetSize().Get()
    width = x + ww + tab1
    add.Bind(wx.EVT_BUTTON, self.OnBtnAdd)
    add.SetBezelWidth(2)
    add.SetUseFocusIndicator(False)
    y += h * 3
    # line 3
    sv = wx.lib.buttons.GenButton(panel, -1, 'Save')
    sv.Bind(wx.EVT_BUTTON, self.OnBtnSave)
    sv.SetBezelWidth(2)
    sv.SetUseFocusIndicator(False)
    cn = wx.lib.buttons.GenButton(panel, -1, 'Cancel')
    cn.Bind(wx.EVT_BUTTON, self.OnBtnCancel)
    cn.SetBezelWidth(2)
    cn.SetUseFocusIndicator(False)
    w, h = cn.GetSize().Get()
