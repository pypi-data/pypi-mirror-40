...
from constant import LOG_DIR, SERVICE_META_REPO_PATH, SERVICE_VERSION_CALCULATE_PATH

APSCLI_USED_CMDLINE_PARAMS = ('action', 'fqsn', 'version', 'ver_calc', 'debug', 'EM', 'wp', 'JOBID')

import aps_jobcontrol
from logging import DEBUG, getLogger, Formatter
from logging.handlers import TimedRotatingFileHandler
from JobNode import JobNode

file_handler = TimedRotatingFileHandler(os.path.join(LOG_DIR, 'apscli.log'), when='D', interval=1, backupCount=30)
...
def gen_wf_template_version(cmdline_param_dic):
    ...
    if 'version' in cmdline_param_dic and cmdline_param_dic['version'] is not None and isinstance(cmdline_param_dic['version'], Hashable):
        ...
        from wf_runtime import _run_cmd_in_thread
        ...
        #from op_control import version_from_pod_name_using_sra_master_config
...
def run():
    ...
    if action=='run':
        ...
        from wf_build import register_fqsn 
        register_fqsn(
            fqsn, 
        ...
        from wf_build import unregister_fqsn
        unregister_fqsn(
            fqsn, 
            version=cmdline_param_dic['version'],
            default_version=cmdline_param_dic['default_version']
        ...
        from wf_build import gen_service_category_dic
