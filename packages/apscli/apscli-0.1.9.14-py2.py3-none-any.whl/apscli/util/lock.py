...
class NFSLock(object):
    ...
    def __init__(self, path, mode, retry=0, interval_in_seconds=1, job_id='a', startpath=cf.get('Resource_Lock', 'absolute_prefix_path'), effective_in_seconds=cf.getint('Resource_Lock', 'max_effective_in_seconds')):
        ...
        assert type(effective_in_seconds) is int and 0<effective_in_seconds<=cf.getint('Resource_Lock', 'max_effective_in_seconds'), 'resource_lock should be effective within at most 12 hours'
        self.path = path
        self.mode = mode
        self.retry = retry
