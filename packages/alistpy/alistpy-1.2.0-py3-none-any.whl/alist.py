class AList(object):
    ...
    def __repr__(self):
        ...
        # print(AList)
        return 'AList{}'.format(self.__value.__repr__())

    def __contains__(self, key):
        """Check to see if a key is in the AList"""
        # For x in AList()
        for cell in self.__value:
            if cell[0] == key:
                return True
        return False
    ...
    def __next__(self):
        ...
        try:
            return self.__value[self._iterindex]
        except IndexError:
    ...
    def __setitem__(self, key, value):
        ...
        # AList['x'] = v
        tmp = list(self.__value)
        for cell in tmp:
            if cell[0] == key:
                new = [x for x in self.__value if x[0] != key]
                new.append((key, value))
                self.__value = tuple(new)
                return value
        tmp.append((key, value))
        self.__value = tuple(tmp)
        return value
    ...
    def __delitem__(self, key):
        ...
        for cell in self.__value:
            if cell[0] == key:
                new = tuple([x for x in self.__value if x[0] != key])
                self.__value = new
                return None
        raise KeyError
    ...
    def popitem(self):
        ...
        if len(self.__value) > 0:
            val = self.__value[0]
            new = tuple([x for x in self.__value if x[0] != val[0]])
            self.__value = new
            return val
        else:
    ...
    def __add__(self, alist2):
        """Like update, but returns a new instance of an AList"""
        tmp = AList()
        tmp.update(self)
        tmp.update(alist2)
        return tmp
