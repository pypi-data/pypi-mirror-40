...
class ora_db(object):
    ...
    def rows_as_dicts(self, cursor, dados):
    ...
    def execCommandParamReturnAsDict(self, _query, _params):
        self._cursor.prepare(_query)
        self._cursor.execute(None, _params)
        data = self._cursor.fetchall()
        data = self.rows_as_dicts(self._cursor, data)
        return data

    def execCommandReturnAsDict(self, _query):
        self._cursor.execute(_query)
        data = self._cursor.fetchall()
        data = self.rows_as_dicts(self._cursor, data)
        return data
