...
class PageObject(six.with_metaclass(ABCMeta, object)):
    ...
    * selib     a reference to SeleniumLibrary, Selenium2Library, or the
                selenium library specified during import
    * se2lib    a reference to an instance of Selenium2Library (deprecated)
    ...
    * _wait_for_page_refresh
    ...
    @property
    def selib(self):
        polib = self.builtin.get_variable_value("${PageObjectLibrary}")
        return polib.selib
    ...
    def _wait_for_page_refresh(self, timeout=10):
        ...
        self._wait_for_readystate("complete")

    def _wait_for_readystate(self, expected_state="complete", timeout="5 seconds"):
        '''Wait for document.readyState to change to the expected value'''
        seconds = robot.utils.timestr_to_secs(timeout)
        timeout = time.time() + seconds
        while True:
            ready_state = self.browser.execute_script("return document.readyState")
            if ready_state == expected_state:
                return
            elif time.time() < timeout:
                time.sleep(.25)
            else:
                break
        raise Exception("Timed out waiting for document.readyState. "
                        "Expected '%s' but it is '%s'" % (expected_state, ready_state))
