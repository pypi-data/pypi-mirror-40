...
class TestTool(TestCase):
    ...
    def setUp(self):
        self.context = ToolContext(MagicMock(), MagicMock(), MagicMock)
    ...
    def test_line_tool(self):
        # TODO: Add 'remove by ID' assertion
        self.context.tool_add_line()
        self.assertIsInstance(self.context._current_tool, LineTool)
        ...
        remove_by_id = Mock()
        self.context.model.remove_by_id = remove_by_id
        add_polyline = Mock()
        self.context.model.add_polyline = add_polyline

        sad_iterator = 3
        for p in points[2:]:
            self.context.canvas_click(p)
            self.assertIsInstance(self.context._current_tool, LineSegmentTool)
            remove_by_id.assert_called_once()
            remove_by_id.reset_mock()
            add_polyline.assert_called_once_with(points[:sad_iterator])
            add_polyline.reset_mock()
