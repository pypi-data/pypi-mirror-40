...
def load(schema:Union[str, TextIO, SchemaDefinition]=None) -> ToolkitGenerator:
    """
...
def ancestors(s:str) -> Optional[Element]:
    try:
        return load().ancestors(s.replace('_', ' '))
...
def children(s:str) -> List[Element]:
    return load().children.get(s.replace('_', ' '), [])

def parent(s:str) -> Optional[Element]:
    return load().parent.get(s.replace('_', ' '))
...
def get_element(name:str) -> Optional[Element]:
    return load().obj_for(name.replace('_', ' '))
...
def get_all_by_mapping(curie:str) -> Optional[List[str]]:
    ...
    """
    return list(load().mappings.get(curie, []))
