...
bool
cmSetTestsPropertiesCommand::InitialPass(std::vector<std::string> const& args,
                                         cmExecutionStatus&)
{
    if(args.empty())
    {
    ...
    {
        std::string errors;
        bool        ret = cmSetTestsPropertiesCommand::SetOneTest(
            args[i], propertyPairs, this->Makefile, errors);
        if(!ret)
        {
...
bool
cmSetTestsPropertiesCommand::SetOneTest(const std::string&        tname,
                                        std::vector<std::string>& propertyPairs,
