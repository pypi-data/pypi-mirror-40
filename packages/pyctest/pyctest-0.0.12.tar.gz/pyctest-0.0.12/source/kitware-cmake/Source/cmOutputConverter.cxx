...
std::string
cmOutputConverter::ConvertToOutputFormat(const std::string& source,
                                         OutputFormat       output) const
{
    ...
    if(output == SHELL || output == WATCOMQUOTE)
    {
        ...
        result =
            this->EscapeForShell(result, true, false, output == WATCOMQUOTE);
...
static bool
cmOutputConverterNotAbove(const char* a, const char* b)
{
...
bool
cmOutputConverter::ContainedInDirectory(std::string const&      local_path,
...
std::string
cmOutputConverter::ConvertToRelativePath(std::string const& local_path,
...
std::string
cmOutputConverter::ForceToRelativePath(std::string const& local_path,
                                       std::string const& remote_path)
{
...
static bool
cmOutputConverterIsShellOperator(const std::string& str)
{
...
std::string
cmOutputConverter::EscapeForShell(const std::string& str, bool makeVars,
                                  bool forEcho, bool useWatcomQuote) const
{
...
std::string
cmOutputConverter::EscapeForCMake(const std::string& str)
{
...
void
cmOutputConverter::SetLinkScriptShell(bool linkScriptShell)
{
    this->LinkScriptShell = linkScriptShell;
}

cmState*
cmOutputConverter::GetState() const
{
...
static int
Shell__CharIsWhitespace(char c)
{
    return ((c == ' ') || (c == '\t'));
}

static int
Shell__CharNeedsQuotesOnUnix(char c)
{
    return ((c == '\'') || (c == '`') || (c == ';') || (c == '#') ||
            (c == '&') || (c == '$') || (c == '(') || (c == ')') ||
            (c == '~') || (c == '<') || (c == '>') || (c == '|') ||
...
static int
Shell__CharNeedsQuotesOnWindows(char c)
{
    return ((c == '\'') || (c == '#') || (c == '&') || (c == '<') ||
            (c == '>') || (c == '|') || (c == '^'));
}
...
static int
Shell__CharIsMakeVariableName(char c)
{
    return c && (c == '_' || isalpha((static_cast<int>(c))));
}

int
cmOutputConverter::Shell__CharNeedsQuotes(char c, int flags)
{
...
const char*
cmOutputConverter::Shell__SkipMakeVariables(const char* c)
{
    while(*c == '$' && *(c + 1) == '(')
    {
        const char* skip = c + 2;
...
int
cmOutputConverter::Shell__ArgumentNeedsQuotes(const char* in, int flags)
{
...
std::string
cmOutputConverter::Shell__GetArgument(const char* in, int flags)
{
