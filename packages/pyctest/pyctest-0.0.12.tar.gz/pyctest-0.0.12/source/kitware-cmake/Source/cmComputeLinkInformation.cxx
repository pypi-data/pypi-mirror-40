...
, Config(config)
{
    ...
    this->OrderRuntimeSearchPath->SetImplicitDirectories(
        this->ImplicitLinkDirs);
    if(this->OrderDependentRPath)
    {
        this->OrderDependentRPath->SetImplicitDirectories(
            this->ImplicitLinkDirs);
        this->OrderDependentRPath->AddLanguageDirectories(
...
cmComputeLinkInformation::GetDirectories() const
{
...
std::vector<std::string> const&
cmComputeLinkInformation::GetDepends() const
{
    return this->Depends;
}

std::vector<std::string> const&
cmComputeLinkInformation::GetFrameworkPaths() const
{
    ...
    if(this->OldLinkDirMode)
    {
        ...
        for(cmGeneratorTarget const* tgt : wrongItems)
        {
            ...
            cmStateEnums::ArtifactType artifact =
                implib ? cmStateEnums::ImportLibraryArtifact
...
void
cmComputeLinkInformation::AddImplicitLinkInfo()
{
...
void
cmComputeLinkInformation::AddItem(std::string const&       item,
                                  cmGeneratorTarget const* tgt)
{
    ...
    if(tgt && tgt->IsLinkable())
    {
        ...
        if(impexe && this->LoaderFlag)
        {
            ...
            cmStateEnums::ArtifactType artifact =
                this->UseImportLibrary ? cmStateEnums::ImportLibraryArtifact
                                       : cmStateEnums::RuntimeBinaryArtifact;
        ...
        {
            // Add the interface library as an item so it can be considered as
            // part of COMPATIBLE_INTERFACE_ enforcement.  The generators will
            // ignore this for the actual link line.
            this->Items.emplace_back(std::string(), false, tgt);

            // Also add the item the interface specifies to be used in its
            // place.
...
void
cmComputeLinkInformation::AddSharedDepItem(std::string const&       item,
...
void
cmComputeLinkInformation::AddLinkPrefix(const char* p)
{
    if(p && *p)
    {
...
void
cmComputeLinkInformation::AddTargetItem(std::string const&       item,
...
void
cmComputeLinkInformation::AddFullItem(std::string const& item)
{
...
void
cmComputeLinkInformation::AddUserItem(std::string const& item,
                                      bool               pathNotKnown)
{
...
void
cmComputeLinkInformation::AddFrameworkPath(std::string const& p)
{
...
bool
cmComputeLinkInformation::CheckSharedLibNoSOName(std::string const& item)
{
...
void
cmComputeLinkInformation::AddSharedLibNoSOName(std::string const& item)
{
...
void
cmComputeLinkInformation::PrintLinkPolicyDiagnosis(std::ostream& os)
{
...
cmComputeLinkInformation::LoadImplicitLinkInfo()
{
    ...
    if(const char* implicitLibs = this->Makefile->GetDefinition(implicitLibVar))
    {
...
static void
cmCLI_ExpandListUnique(const char* str, std::vector<std::string>& out,
                       std::set<std::string>& emitted)
{
                                   ...
                                   bool                      for_install) const
{
    ...
    // present.  This is done even when skipping rpath support.
    {
        ...
        for(std::string const& li : lc->Languages)
        {
            ...
            {
                std::string dirVar =
                    "CMAKE_" + li + "_IMPLICIT_LINK_DIRECTORIES";
                if(const char* dirs = this->Makefile->GetDefinition(dirVar))
                {
...
std::string
cmComputeLinkInformation::GetRPathString(bool for_install) const
{
...
std::string
cmComputeLinkInformation::GetChrpathString() const
{
