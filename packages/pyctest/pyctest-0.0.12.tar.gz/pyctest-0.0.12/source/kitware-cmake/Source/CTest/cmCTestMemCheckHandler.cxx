...
static void
xmlReportError(int line, const char* msg, void* data)
{
...
void
cmCTestMemCheckHandler::Initialize()
{
...
int
cmCTestMemCheckHandler::PreProcessHandler()
{
...
int
cmCTestMemCheckHandler::PostProcessHandler()
{
...
void
cmCTestMemCheckHandler::GenerateTestCommand(std::vector<std::string>& args,
                                            int                       test)
{
    std::string        index;
...
void
cmCTestMemCheckHandler::InitializeResultsVectors()
{
    // fill these members
...
int
cmCTestMemCheckHandler::GetDefectCount()
{
...
cmCTestMemCheckHandler::InitializeMemoryChecking()
{
       ...
       "AddressSanitizer")
    {
        ...
        this->LogWithPID =
            true;  // even if we give the log file the pid is added
    }
    if(this->CTest->GetCTestConfiguration("MemoryCheckType") == "LeakSanitizer")
    {
        this->MemoryTester = this->CTest->GetCTestConfiguration("CMakeCommand");
    ...
    if(!this->CTest->GetCTestConfiguration("MemoryCheckCommandOptions").empty())
    {
        memoryTesterOptions =
    ...
    switch(this->MemoryTesterStyle)
    {
        case cmCTestMemCheckHandler::VALGRIND:
        {
                    ...
                    .empty())
            {
                ...
                this->MemoryTesterOptions.push_back(
                    "--suppressions=" + this->CTest->GetCTestConfiguration(
                                            "MemoryCheckSuppressionFile"));
            }
            this->MemoryTesterDynamicOptions.push_back(
                "--log-file=" + this->MemoryTesterOutputFile);
            break;
        }
        case cmCTestMemCheckHandler::PURIFY:
        {
                   ...
                   .size())
            {
                ...
                std::string filterFiles =
                    "/FilterFiles=" + this->CTest->GetCTestConfiguration(
                                          "MemoryCheckSuppressionFile");
        ...
        case cmCTestMemCheckHandler::UB_SANITIZER:
        {
            ...
            if(!this->CTest
                    ->GetCTestConfiguration("MemoryCheckSanitizerOptions")
                    .empty())
            {
                extraOptions = ":" + this->CTest->GetCTestConfiguration(
...
bool
cmCTestMemCheckHandler::ProcessMemCheckOutput(const std::string& str,
                                              std::string&       log,
...
cmCTestMemCheckHandler::FindOrAddWarning(const std::string& warning)
{
...
bool
cmCTestMemCheckHandler::ProcessMemCheckSanitizerOutput(const std::string& str,
                                                       std::string&       log,
    ...
    for(std::string const& l : lines)
    {
        ...
        if(!resultFound.empty())
        {
            std::vector<int>::size_type idx =
                this->FindOrAddWarning(resultFound);
...
bool
cmCTestMemCheckHandler::ProcessMemCheckPurifyOutput(const std::string& str,
                                                    std::string&       log,
                                                    std::vector<int>&  results)
{
    std::vector<std::string> lines;
...
bool
cmCTestMemCheckHandler::ProcessMemCheckValgrindOutput(const std::string& str,
                                                      std::string&       log,
                                                      std::vector<int>& results)
{
    std::vector<std::string> lines;
    ...
    cmCTestOptionalLog(this->CTest, DEBUG,
                       "Start test: " << lines.size() << std::endl,
                       this->Quiet);
...
void
cmCTestMemCheckHandler::PostProcessTest(cmCTestTestResult& res, int test)
{
    cmCTestOptionalLog(this->CTest, HANDLER_VERBOSE_OUTPUT,
...
void
cmCTestMemCheckHandler::TestOutputFileNames(int                       test,
                                            std::vector<std::string>& files)
{
    std::string        index;
    std::ostringstream stream;
