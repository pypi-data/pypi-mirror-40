...
bool
cmComputeTargetDepends::Compute()
{
...
void
cmComputeTargetDepends::CollectTargets()
{
...
void
cmComputeTargetDepends::CollectDepends()
{
...
void
cmComputeTargetDepends::CollectTargetDepends(int depender_index)
{
    // Get the depender.
            ...
            for(cmSourceFile const* o : objectFiles)
            {
                ...
                if(!objLib.empty())
                {
                    cmLinkItem const& objItem =
                        depender->ResolveLinkItem(objLib);
                           ...
                           depender->GetType() != cmStateEnums::OBJECT_LIBRARY)
                        {
                            this->GlobalGenerator->GetCMakeInstance()
                                ->IssueMessage(cmake::FATAL_ERROR,
                                               "Only executables and libraries "
                                               "may reference target objects.",
                                               depender->GetBacktrace());
                            return;
                        }
                        const_cast<cmGeneratorTarget*>(depender)
                            ->Target->AddUtility(objLib);
                    }
                }
            }
...
void
cmComputeTargetDepends::AddInterfaceDepends(int depender_index,
...
void
cmComputeTargetDepends::AddTargetDepend(int               depender_index,
                                        cmLinkItem const& dependee_name,
                                        bool              linking)
{
    // Get the depender.
                                        ...
                                        bool                     linking)
{
    ...
    } else
    {
        ...
        this->InitialGraph[depender_index].emplace_back(dependee_index,
                                                        !linking);
    }
}
    ...
    for(int depender_index = 0; depender_index < n; ++depender_index)
    {
        ...
        for(cmGraphEdge const& ni : nl)
        {
            ...
            fprintf(stderr, "  depends on target %d [%s] (%s)\n",
                    dependee_index, dependee->GetName().c_str(),
...
void
cmComputeTargetDepends::DisplayComponents(cmComputeComponentGraph const& ccg)
{
...
bool
cmComputeTargetDepends::CheckComponents(cmComputeComponentGraph const& ccg)
{
    ...
    cmComputeComponentGraph const& ccg, int c, bool strong)
{
    ...
    if(strong)
    {
        ...
        e << "The component contains at least one cycle consisting of strong "
          << "dependencies (created by add_dependencies) that cannot be "
             "broken.";
...
bool
cmComputeTargetDepends::IntraComponent(std::vector<int> const& cmap, int c,
                                       int i, int* head, std::set<int>& emitted,
                                       std::set<int>& visited)
{
...
bool
cmComputeTargetDepends::ComputeFinalDepends(cmComputeComponentGraph const& ccg)
{
        ...
        ++depender_component)
    {
        ...
        for(cmGraphEdge const& ni : nl)
        {
            ...
            int dependee_component_head =
                this->ComponentHead[dependee_component];
