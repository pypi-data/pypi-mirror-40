...
void
cmLocalVisualStudio7Generator::AddHelperCommands()
{
...
void
cmLocalVisualStudio7Generator::Generate()
{
...
void
cmLocalVisualStudio7Generator::FixGlobalTargets()
{
...
void
cmLocalVisualStudio7Generator::WriteProjectFiles()
{
...
void
cmLocalVisualStudio7Generator::WriteStampFiles()
{
...
void
cmLocalVisualStudio7Generator::CreateSingleVCProj(const std::string& lname,
...
{
    if(this->GlobalGenerator->GlobalSettingIsOn("CMAKE_SUPPRESS_REGENERATION"))
    {
        return nullptr;
    }
...
void
cmLocalVisualStudio7Generator::WriteConfigurations(
...
cmVS7FlagTable cmLocalVisualStudio7GeneratorFortranFlagTable[] = {
    ...
    { "OptimizeForProcessor", "QxP", "", "codeExclusivelyCodeNamedPrescott",
      0 },
...
cmVS7FlagTable cmLocalVisualStudio7GeneratorFlagTable[] = {
    ...
    { "FloatingPointModel", "fp:strict", "Use strict floating point model", "1",
      0 },
    { "FloatingPointModel", "fp:fast", "Use fast floating point model", "2",
      0 },
                                                  ...
                                                  cmGeneratorTarget* target)
{
  ...
  fout << "\t\t\tIntermediateDirectory=\""
    ...
    if(this->FortranProject)
    {
        ...
        if(target_mod_dir)
        {
            modDir = this->ConvertToRelativePath(
...
std::string
cmLocalVisualStudio7Generator::GetBuildTypeLinkerFlags(
                                               ...
                                               const Options&     targetOptions)
{
    ...
    switch(target->GetType())
    {
        ...
        case cmStateEnums::MODULE_LIBRARY:
        {
            ...
            const char* stackVal =
                this->Makefile->GetDefinition(stackVar.c_str());
            if(stackVal)
            {
        ...
        case cmStateEnums::EXECUTABLE:
        {
            ...
            if(this->WindowsCEProject)
            {
                ...
                if(!linkOptions.GetFlag("EntryPointSymbol"))
                {
                    const char* entryPointSymbol =
                        targetOptions.UsingUnicode()
                            ? (isWin32Executable ? "wWinMainCRTStartup"
                                                 : "mainWCRTStartup")
                            : (isWin32Executable ? "WinMainCRTStartup"
                                                 : "mainACRTStartup");
            ...
            {
                fout << "\t\t\t\tSubSystem=\""
                     << (isWin32Executable ? "subSystemWindows"
                                           : "subSystemConsole")
                     << "\"\n";
            } else
            {
...
static std::string
cmLocalVisualStudio7GeneratorEscapeForXML(const std::string& s)
{
    std::string ret = s;
...
static std::string
GetEscapedPropertyIfValueNotNULL(const char* propertyValue)
{
...
void
cmLocalVisualStudio7Generator::OutputDeploymentDebuggerTool(
...
void
cmLocalVisualStudio7Generator::WriteTargetVersionAttribute(
...
void
cmLocalVisualStudio7GeneratorInternals::OutputLibraries(std::ostream&     fout,
                                                        ItemVector const& libs)
{
       ...
       target->GetName() != CMAKE_CHECK_BUILD_SYSTEM_TARGET)
    {
        if(cmSourceFile const* sf = this->CreateVCProjBuildRule())
        {
            ...
            for(cmGeneratorTarget::AllConfigSource& si : sources.Sources)
            {
                ...
                {
                    haveCMakeLists = true;
                    // Replace the explicit source reference with our generated
                    // one.
                    si = acs;
                    break;
                }
    ...
    std::vector<std::string> const&           configs)
{
        ...
        i != configs.end(); ++i, ++ci)
    {
        ...
        std::string lang = lg->GlobalGenerator->GetLanguageFromExtension(
        ...
        {
            fc.CompileDefs =
                genexInterpreter.Evaluate(cdefs, COMPILE_DEFINITIONS);
            needfc = true;
        }
        ...
        {
            fc.IncludeDirs =
                genexInterpreter.Evaluate(cincs, INCLUDE_DIRECTORIES);
            needfc = true;
        }
        ...
        fc.ExcludedFromBuild = sf.GetPropertyAsBool("HEADER_FILE_ONLY") ||
                               std::find(acs.Configs.begin(), acs.Configs.end(),
                                         ci) == acs.Configs.end();
...
std::string
cmLocalVisualStudio7Generator::ComputeLongestObjectDirectory(
...
bool
cmLocalVisualStudio7Generator::WriteGroup(
            ...
            } else if(!fcinfo.FileConfigMap.empty())
            {
                    ...
                    fci != fcinfo.FileConfigMap.end(); ++fci)
                {
                       ...
                       !fc.CompileDefsConfig.empty() || !fc.IncludeDirs.empty())
                    {
                        ...
                        this->AppendIncludeDirectories(includes, fc.IncludeDirs,
                                                       **sf);
                        fileOptions.AddIncludes(includes);
                        fileOptions.OutputFlagMap(fout, 5);
                        fileOptions.OutputAdditionalIncludeDirectories(
                            fout, 5,
                            ppLang == "CXX" && this->FortranProject ? "Fortran"
                                                                    : ppLang);
                        fileOptions.OutputPreprocessorDefinitions(fout, 5,
                                                                  ppLang);
                    }
...
void
cmLocalVisualStudio7Generator::WriteCustomRule(
    std::ostream& fout, std::vector<std::string> const& configs,
    const char* source, const cmCustomCommand& command, FCInfo& fcinfo)
{
        ...
        i != configs.end(); ++i)
    {
        ...
        if(this->FortranProject)
        {
            cmSystemTools::ReplaceString(script, "$(Configuration)",
                                         i->c_str());
        }
        /* clang-format off */
    fout << "\t\t\t\t\t<Tool\n"
        ...
        } else
        {
            ...
            {
                // Get the real name of the dependency in case it is a CMake
                // target.
                std::string dep;
...
void
cmLocalVisualStudio7Generator::WriteVCProjBeginGroup(std::ostream& fout,
                                                     const char*   group,
                                                     const char*)
{
    /* clang-format off */
...
void
cmLocalVisualStudio7Generator::OutputTargetRules(std::ostream&      fout,
                                                 const std::string& configName,
                                                 cmGeneratorTarget* target,
                                                 const std::string& /*libName*/)
{
...
void
cmLocalVisualStudio7Generator::WriteProjectStartFortran(
...
void
cmLocalVisualStudio7Generator::WriteVCProjFooter(std::ostream&      fout,
                                                 cmGeneratorTarget* target)
{
    fout << "\t<Globals>\n";
...
std::string
cmLocalVisualStudio7Generator::EscapeForXML(const std::string& s)
{
    return cmLocalVisualStudio7GeneratorEscapeForXML(s);
}

std::string
cmLocalVisualStudio7Generator::ConvertToXMLOutputPath(const char* path)
{
    std::string ret =
...
std::string
cmLocalVisualStudio7Generator::ConvertToXMLOutputPathSingle(const char* path)
{
    std::string ret =
        this->ConvertToOutputFormat(path, cmOutputConverter::SHELL);
    cmSystemTools::ReplaceString(ret, "\"", "");
...
void
cmVS7GeneratorOptions::OutputFlag(std::ostream& fout, int indent,
                                  const char* flag, const std::string& content)
{
    fout.fill('\t');
    fout.width(indent);
                ...
                if(strcmp(atts[i], "ProjectGUID") == 0)
                {
                    if(atts[i + 1])
                    {
                        ...
                        if(this->GUID[0] == '{')
                        {
                            ...
                            this->GUID =
                                this->GUID.substr(1, this->GUID.size() - 2);
                        }
                    } else
                    {
                        this->GUID.clear();
...
void
cmLocalVisualStudio7Generator::ReadAndStoreExternalGUID(const std::string& name,
                                                        const char*        path)
{
...
std::string
cmLocalVisualStudio7Generator::GetTargetDirectory(
    cmGeneratorTarget const* target) const
{
    std::string dir;
    dir += target->GetName();
