...
# from queue import Queue  # works with Python 2.7
from sg_module_queues.myqueue import MyQueue  # should work with Python 3.6?
...
class ModuleQueues:
    ...
    def __init__(self, input_queue_key, processing_queue_key, output_queue_key):
        ...
        try:
            self.input_queue = MyQueue(REDIS_URL, input_queue_key)
            self.processing_queue = MyQueue(REDIS_URL, processing_queue_key)
            self.output_queue = MyQueue(REDIS_URL, output_queue_key)
        except:
