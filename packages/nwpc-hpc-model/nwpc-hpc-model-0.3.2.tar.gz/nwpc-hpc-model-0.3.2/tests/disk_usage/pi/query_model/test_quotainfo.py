...
from nwpc_hpc_model.disk_usage.pi import PiDiskUsageQueryModel, QueryCategory, \
    PiDiskUsageCategoryList, record_parser, value_saver
...
class TestQuotaInfo(object):
    def check_build_model(self, test_case):
        ...
        model = PiDiskUsageQueryModel.build_from_category_list(record, category_list)
    ...
    def test_build_model_using_config_file(self):
        ...
        config_file_path = Path(
            Path(__file__).parent,
            "../../../../nwpc_hpc_model/disk_usage/pi/conf/disk_usage.config.yml"
        )

        with open(config_file_path) as f:
            category_list_config = yaml.load(f)

        category_list = PiDiskUsageCategoryList.build_from_config(category_list_config['category_list'])

        test_case_list = list()

        nwp_output = Path(
            Path(__file__).parent,
            "../../data/pi/quotainfo_nwp_qu.txt"
        )
