...
class FutPaperTradeApi(ITradeApi):
    ...
    def SendOrder(self,clientOrderID, symbol, exchange, orderSide, price, quantity, orderType, offset):
        ...
        self.__wirteInfo("orderID:{0},symbol:{1},exchange:{2},orderSide:{3},price:{4},quantity:{5},orderType:{6},offset:{7}".format(clientOrderID, symbol, exchange, orderSide, price, quantity, orderType, offset))

        self.__executor.submit(self.__raiseOrderChanged,order)

        param = {'strategyID':self.__strategyID,'orderID':clientOrderID,'symbol':symbol,'exchange':int(exchange),'orderSide':int(orderSide),'price':price,'quantity':quantity,'orderType':int(orderType),'offset':int(offset)}
