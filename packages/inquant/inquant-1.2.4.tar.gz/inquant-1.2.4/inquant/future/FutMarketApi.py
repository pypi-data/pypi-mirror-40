...
from inquant.util.TripleDESCryptogram import *
...
class FutMarketApi(IMarketApi):
    ...
    def __OnRecv(self,msg):
        ...
        if arr[0] == '99':
            ...
            if not tick:
                return
            tickKey = '{0}.{1}.tick.0'.format(tick.Symbol,tick.Exchange)
            ...
            if len(tickList) > 100:
                tickList.pop(0)
            #缓存3天
            self.__cache.set(tickKey,tickList,60 * 60 * 24 * 3)
            ...
            if not bar:
                return
            barKey = '{0}.{1}.bar.{2}'.format(bar.Symbol,bar.Exchange,bar.BarType)
            ...
            if len(barList) > 100:
                barList.pop(0)
            #缓存3天
            self.__cache.set(barKey,barList,60 * 60 * 24 * 3)
    ...
    def GetHisBar(self,symbol, exchange, barType, startTime, count):
        ...
        if not klineType:
            ...
            return None
        contractId = self.__GetContractId(symbol)
        if not contractId:
            self.__wirteError("获取合约内部编号失败：" + symbol)
            return None
        url = 'https://hq.inquant.cn/hqfut/quant/getkline.ashx?contractID={0}&count={1}&type={2}&endTime={3}&isLastKLine=1'.format(contractId, count, klineType, startTime)
        resp = httpGet(url)
        if not resp:
    ...
    def __GetContractId(self,symbol):
        '''获取合约编号'''
        url = 'https://hq.inquant.cn/hqfut/MktData/contractDetailed.ashx?symbol=' + symbol
        cache = self.__cache.get(url)
        if cache:
            return cache
        resp = httpGet(url)
        if not resp:
        ...
        self.__cache.set(url,contractId,60 * 60 * 2)
        return contractId
