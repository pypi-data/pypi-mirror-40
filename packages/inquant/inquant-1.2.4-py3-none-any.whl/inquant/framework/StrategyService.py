...
class StrategyService(IStrategyService):
        ...
        while True:
            ...
            try:
                if self.__marketApi.GetStatus() != -1:
                    self.__wirteInfo('后台订阅:' + str(self.GetSubscribeList()))
                if len(self.__subscribeList) > 0:
    ...
    def GetSubscribeList(self):
        ...
        for x in self.__subscribeList:
            item = '{0}.{1}.{2}.{3}'.format(x.get('symbol'),str(x.get('exchnage')),x.get('marketType'),x.get('barType'))
            data.append(item)
        return data
    ...
    def GetLastTick(self,symbol, exchange, count):
        """获取最近几笔Tick数据"""
        return self.__marketApi.GetLastTick(symbol,exchange,count)

    def GetLastBar(self,symbol, exchange, barType, count):
        """获取最近几笔Bar数据"""
        return self.__marketApi.GetLastBar(symbol, exchange, barType, count)
    ...
    def GetContract(self,symbol, exchange):
        """获取合约信息"""
        return self.__baseDataApi.GetContract(symbol, exchange)

    def GetFutContracts(self,varietyCode, exchange, futType):
        """按品种代码获取期货合约"""
        return self.__baseDataApi.GetFutContracts(varietyCode, exchange, futType)
    ...
    def __wirteInfo(self,info):
        '''写日志'''
        self.WriteLog('StrategyService',info)
