...
# Django COS Imports
from django_cos.blocks.mixins import ButtonMixin
from django_cos.blocks.html_blocks import ButtonBlock, ImageBlock
from django_cos.blocks.base_blocks import BaseBlock, BaseLayoutBlock, CollectionChooserBlock
...
class CardBlock(BaseBlock):
    ...
    class Meta:
        template = 'django_cos/blocks/card_foot.html'
        icon = 'fa-list-alt'
        label = _('Card')
...
class CarouselBlock(BaseBlock):
    ...
    """
    carousel = SnippetChooserBlock('django_cos.Carousel')

    class Meta:
        icon = 'image'
        label = _('Carousel')
        template = 'django_cos/blocks/carousel_block.html'
...
class ModalBlock(ButtonMixin, BaseLayoutBlock):
    ...
    class Meta:
        template = 'django_cos/blocks/modal_block.html'
...
class NavExternalLinkBlock(NavBaseLinkBlock):
    ...
    class Meta:
        template = 'django_cos/blocks/external_link_block.html'
...
class NavPageLinkBlock(NavBaseLinkBlock):
    ...
    class Meta:
        template = 'django_cos/blocks/page_link_block.html'
        label = _('Page Link')
...
class NavDocumentLinkBlock(NavBaseLinkBlock):
    ...
    class Meta:
        template = 'django_cos/blocks/document_link_block.html'
...
class PageListBlock(BaseBlock):
    ...
    class Meta:
        template = 'django_cos/blocks/pagelist_block.html'
        icon = 'list-ul'
    ...
    def get_context(self, value, parent_context=None):
        ...
        if value['indexed_by']:
            ...
            # try to use the DjangoCOSPage `get_index_children()`,
            # but fall back to get_children if this is a non-DjangoCOSPage
            try:
...
class PriceListItemBlock(BaseBlock):
    ...
    class Meta:
        template = 'django_cos/blocks/pricelistitem_block.html'
        icon = 'fa-usd'
...
class PriceListBlock(BaseBlock):
    ...
    class Meta:
        template = 'django_cos/blocks/pricelist_block.html'
        icon = 'fa-usd'
        label = _('Price List')
...
class ContentWallBlock(BaseBlock):
    ...
    """
    content_wall = SnippetChooserBlock('django_cos.ContentWall')
    ...
    class Meta:
        ...
        template = 'django_cos/blocks/content_wall_block.html'
...
class ReusableContentBlock(BaseBlock):
    ...
    """
    content = SnippetChooserBlock('django_cos.ReusableContent')

    class Meta:
        icon = 'fa-recycle'
        label = _('Reusable Content')
        template = 'django_cos/blocks/reusable_content_block.html'
