...
# Django COS Imports
from django_cos import utils
from django_cos.forms import SearchForm
from django_cos.models import DjangoCOSPage, DjangoCOSEventPage, get_page_models, GeneralSettings
from django_cos.importexport import convert_csv_to_json, import_pages, ImportPagesFromCSVFileForm
...
def search(request):
    ...
    if search_form.is_valid():
        ...
        # This provides better search results than simply searching limited fields on DjangoCOSPage.
        db_models = []
        ...
        # Fallback for any other search backend
            ...
            if search_model:
                ...
                results = DjangoCOSPage.objects.live().search(search_query)
    ...
    # Render template
    return render(request, 'django_coscms/pages/search.html', {
        'request': request,
        'pagetypes': pagetypes,
        'form': search_form,
        'results': results,
...
def event_generate_single_ical_for_event(request):
    if request.method == "POST":
        ...
        event_page_models = DjangoCOSEventPage.__subclasses__()
...
def event_generate_ical_for_calendar(request):
    if request.method == "POST":
        try:
            page = DjangoCOSPage.objects.get(id=request.POST.get('page_id')).specific
            print(page)
        except ValueError:
            raise Http404
...
def event_get_calendar_events(request):
    if request.is_ajax():
        try:
            page = DjangoCOSPage.objects.get(id=request.GET.get('pid')).specific
        except ValueError:
            raise Http404
