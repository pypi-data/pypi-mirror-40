...
from django_cos.models import DjangoCosFormPage


@hooks.register('insert_global_admin_css')
def global_admin_css():
    return format_html('<link rel="stylesheet" type="text/css" href="{}">', static('css/django-cos-admin.css'))


@hooks.register('insert_editor_css')
def editor_css():
    return format_html('<link rel="stylesheet" type="text/css" href="{}">', static('css/django-cos-editor.css'))


@hooks.register('insert_editor_js')
def collapsible_js():
    return format_html('<script src="{}"></script>', static('js/django-cos-editor.js'))
...
def django_cos_forms(user, editable_forms):
    """
    Add our own DjangoCOSFormPage to editable_forms, since wagtail is unaware
    ...
    form_models = [
        ...
        if issubclass(model, (AbstractForm, DjangoCOSFormPage))
    ]
    form_types = list(
