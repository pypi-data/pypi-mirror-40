...
from dataclay.commonruntime.Initializer import init_prv_mgr
...
class ExecutionEnvironmentSrv(object):
    ...
    def start(self):
        ...
        init_prv_mgr("%s_%d_PY" % (settings.dataservice_name, settings.dataservice_port))
        self.execution_environment = ExecutionEnvironment(settings.dataservice_name)

        if TRACE_ENABLED:
            interceptor = ParaverServerInterceptor(settings.server_listen_addr, settings.server_listen_port)
            ...
            ee = DataServiceEE(self.execution_environment, interceptor)
