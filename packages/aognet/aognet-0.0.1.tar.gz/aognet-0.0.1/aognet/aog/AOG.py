...
class Node(object):
    ...
    def __init__(self, node_id=-1, node_type=NodeType.Unknow, rect_idx=-1, child_ids=None, parent_ids=None,
...
class AOGrid(object):
    ...
    def _BFS(self, id, q, visited):
        ...
        # for i in self.node_set[id].child_ids:
        #     if visited[i] < 2:
        #         q, visited = self._BFS(i, q, visited)
        ...
        #     if visited[q[i]] < 2 and  node.node_type != NodeType.TerminalNode:
        #         for j in node.child_ids:
        #             if j not in q:
        #                 q.append(j)
        #         visited[q[i]] = 2
    ...
    def _countPaths(self, s, t, npaths):
        if s.id == t.id:
            ...
            if not npaths[s.id]:
                ...
                parent_ids1 = set(s.parent_ids)
                for c in s.child_ids:
                    ch = self.node_set[c]
                    ch_rect = self.primitive_set[ch.rect_idx]
                    parent_ids2 = ch.parent_ids
                    
                    if s.node_type == NodeType.AndNode and \
                       rect.Width() == ch_rect.Width() and rect.Height() == ch_rect.Height() and \
                       len(parent_ids1.intersection(parent_ids2)) > 0:
                        continue
                    if s.node_type == NodeType.OrNode and \
                       ((ch.node_type == NodeType.OrNode and len(parent_ids1.intersection(parent_ids2)) > 0) or \
    ...
    def _add_tnode_topdown_connection(self):
        ...
        while i < len(tnode_queue):
            ...
            # prim_type = [rect.Height() - 1, rect.Width()]
            # ids = self.find_node_ids_with_given_prim_type(prim_type, rect)

            # prim_type = [rect.Height(), rect.Width() - 1]
            # ids += self.find_node_ids_with_given_prim_type(prim_type, rect)
    ...
    def _add_tnode_bottomup_connection(self):
        ...
        # not to use this lateral connection as alternative
        ...
        #     self.node_set[id_].node_type = NodeType.OrNode
    ...
    def _add_lateral_connection(self):
        ...
        #self._add_onode_topdown_connection()

        # lateral connections between children of root OR-nodes
        # ch_ids = self.node_set[self.BFS[0]].child_ids
        ...
        for node in self.node_set:
            ...
            #self.node_set[node.id].child_ids = [ch]

        # remove root OR-node from DFS and BFS                               
        #root_id = self.node_set[self.BFS[0]].child_ids[-1]        
        #return root_id
    ...
    def _add_node_bottomup_connection_layerwise(self, node_type=NodeType.TerminalNode, direction=0):
        ...
        for p in prim_types:
            ...
            for i in range(istart, iend, step):
                ...
                if chid not in self.node_set[id_].child_ids:
                    self.node_set[id_].child_ids.append(chid)
                # for k in range(1, len(ind)): # dense
                #     j = i-k if direction==0 else i+k
    ...
    def Create(self):
        ...
        while len(BFS) > 0:
            ...
            if curNode.node_type == NodeType.OrNode:
                ...
                #         break
                #     # if curId==0 and self.param.remove_symmetric_children_of_or_node and topHt > curHt//2:                    
                #     if self.param.remove_symmetric_children_of_or_node==1 and ((keepTop and topHt > curHt//2) or (not keepTop and topHt < curHt//2)):
                #         break
                #     elif self.param.remove_symmetric_children_of_or_node==2 and ((keepTop and topHt < curHt//2) or (not KeepTop and topHt > curHt//2)):
                #         continue
                    
                #     bottomHt = curHt - topHt
                #     if self.param.overlap_ratio > 0: #and self.param.max_children_kept_for_or == 0:
                ...
                #         break
                #     #if curId==0 and self.param.remove_symmetric_children_of_or_node and leftWd > curWd//2:
                #     if self.param.remove_symmetric_children_of_or_node==1 and ((keepLeft and leftWd > curWd//2) or (not keepLeft and leftWd < curWd//2)):
                #         break
                #     elif self.param.remove_symmetric_children_of_or_node==2 and ((keepLeft and leftWd < curWd//2) or (not keepLeft and leftWd > curWd//2)):
                #         continue
