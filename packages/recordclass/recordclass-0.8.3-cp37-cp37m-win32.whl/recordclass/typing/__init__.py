import collections
from recordclass.record import recordclass, structclass
...
def _make_structclass(name, types):
    ...
    struct_cls.__annotations__ = struct_cls._field_types = dict(types)
    try:
...
#         readonly = options.get('readonly', False)
        ...
        struct_cls = _make_structclass(typename, types.items(), **options)
        ...
        for key in ns:
            if key in _prohibited:
                raise AttributeError("Cannot overwrite RecordClass attribute " + key)
            elif key not in _special and key not in struct_cls._fields:
