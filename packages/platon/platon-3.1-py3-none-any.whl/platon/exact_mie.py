...
class TransitDepthCalculator:
    ...
    def _get_mie_scattering_absorption(self, P_cond,T_cond,ri,part_size,
        ...
        def integrand(y, wavelength):
            r = part_size * np.exp(np.sqrt(2) * sigma * y)
            x = 2*np.pi*r/wavelength
            if x < 1e-6: Qext = 0
            elif x > 1e4: Qext = 2
            else:
                #Qext = self._mie_cache.get(ri, [x])[0]
                #if np.isnan(Qext):
                Qext = mie_multi_x.get_Qext(ri, [x])[0]                    

            return np.exp(-y**2) * np.sqrt(np.pi) * part_size**2 * np.exp(2*np.sqrt(2)*sigma*y) * Qext

        '''for i, w in enumerate(self.lambda_grid):
            result, _ = scipy.integrate.quad(integrand, -np.inf, np.inf, args=(w))
            print(i, w, result)
            eff_cross_section[i] = result
            
        np.save("eff_cross_section.npy", eff_cross_section)
        plt.loglog(self.lambda_grid, eff_cross_section)
        plt.show()'''
        #points, weights = np.polynomial.hermite.hermgauss(quadrature_deg)
        z_scores = np.linspace(-5, 5, 100)
        integrand_values = []
        ...
        for i in range(absorption_coeff.shape[1]):
            plt.loglog(self.lambda_grid, absorption_coeff[:, i, 0])
        np.save("rough_absorption_coeff.npy", absorption_coeff)
        plt.show()
        return absorption_coeff
