...
class MotController:
    ...
    def setInverted_2(self, invertType: int) -> None:
    ...
    def getActiveTrajectoryPosition_3(self, pidIdx: int) -> int:
        assert isinstance(pidIdx, int)
        assert isinstance(retval, int)
        return retval

    def getActiveTrajectoryVelocity_3(self, pidIdx: int) -> int:
        assert isinstance(pidIdx, int)
        assert isinstance(retval, int)
        return retval

    def getActiveTrajectoryArbFeedFwd_3(self, pidIdx: int) -> float:
        assert isinstance(pidIdx, int)
        assert isinstance(retval, (float))
        return retval
    ...
    def getActiveTrajectoryAll_5(self, pidIdx: int) -> typing.Tuple[int, int, float]:
        assert isinstance(pidIdx, int)
        assert isinstance(retval, tuple) and len(retval) == 3
        assert isinstance(retval[0], int)
        assert isinstance(retval[1], int)
    ...
    def pushMotionProfileTrajectory_3(self, position: float, velocity: float, arbFeedFwd: float, auxiliaryPos: float, auxiliaryVel: float, auxiliaryArbFeedFwd: float, profileSlotSelect0: int, profileSlotSelect1: int, isLastPoint: bool, zeroPos0: bool, timeDur: int, useAuxPID: bool) -> ErrorCode:
        assert isinstance(position, (int, float))
        assert isinstance(velocity, (int, float))
        ...
        assert isinstance(profileSlotSelect0, int) and profileSlotSelect0 < 4294967296 and profileSlotSelect0 >= 0
        assert isinstance(profileSlotSelect1, int) and profileSlotSelect1 < 4294967296 and profileSlotSelect1 >= 0
        assert isinstance(isLastPoint, bool)
        assert isinstance(zeroPos0, bool)
        assert isinstance(timeDur, int) and timeDur < 4294967296 and timeDur >= 0
