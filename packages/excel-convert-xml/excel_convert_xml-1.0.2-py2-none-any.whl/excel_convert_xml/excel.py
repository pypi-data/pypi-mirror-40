...
class Config():
    def __init__(self):
        ...
        if os.path.exists(conf.get(sections[0], "excel_file_name")):
            if os.path.splitext(conf.get(sections[0], "excel_file_name"))[-1] == ".xls" or ".xlsx":
                self.excel_file_path = conf.get(sections[0], "excel_file_name")
            else:
                raise Exception("%s not excel file" % os.path.join(conf.get(sections[0], "excel_file_name")))
        else:
...
class Excel(xml_element):
    ...
    def _get_sheet(self, path, sheet_name):
        return self._open_excel(path=path).sheet_by_name(sheet_name=sheet_name)

    def _get_sheet_names(self, path):
        return self._open_excel(path=path).sheet_names()

    def _get_sheet_data(self, path, sheet_name):
        table = self._get_sheet(path, sheet_name)
        rows = table.nrows
        Priority_index = 0
        Test_Module_index = 0
        Summary_index = 0
        ...
        for row in range(1, rows):
            ...
            _Priority = table.cell(row, Priority_index).value.replace('"', "'")
            """ filter empty row """
            if _Test_Module == "" and _Test_Level == "" and _TestRun == "" \
                    and _Summary == "" and _ExpectedResults == "" \
                    and _Description == "" and _Priority == "":
                j = j + 1
            else:
                ...
                self.Priority.append(_Priority)
    ...
    def _add_test_case(self, case, SummaryValue, PriorityValue, TestRunValue, DescriptionValue, ExpectedResultsValue):
        ...
        if PriorityValue != "":
            self.create_sub_element_text(caseElement, "priority", "PriorityValue")
        else:
    ...
    def deal_sheet_data(self, test_sheet=False):
        ...
        for case, testPoint in zip(list_cases, self.TestPoint[:-1]):
            for SummaryValue, TestRunValue, DescriptionValue, PriorityValue, ExpectedResultsValue in zip(
                    ...
                    self.Priority[int(self.TestPoint[i]) + 1:int(self.TestPoint[i + 1])],
                    self.ExpectedResults[int(self.TestPoint[i]) + 1:int(self.TestPoint[i + 1])]):
                if SummaryValue != "":
                    self._add_test_case(case, SummaryValue, PriorityValue, TestRunValue, DescriptionValue, ExpectedResultsValue)
            i += 1

data = Excel()

def __cratet_xml_file(data, excel_file_path, sheets):
    for sheet_name in sheets:
        data._get_sheet_data(excel_file_path, sheet_name)
...
def excle_to_xml(excel_file_path, project_name, write_xml_file_path):
    ...
    sheets = []
    for sheet_name in data._get_sheet_names(excel_file_path):
        match = re.search("TestCase[_-]*", sheet_name)
        if match:
            logging.info(sheet_name)
            sheets.append(sheet_name)
    __cratet_xml_file(data, excel_file_path, sheets)
