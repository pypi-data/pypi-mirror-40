...
def setup(project_root):
    ...
    ignore = ['egg', 'self_replication', '__pycache__']

    for based, subds, files in os.walk(os.path.join(thispath)):
        # don't go into ignored dirs
        if any([item in based for item in ignore]):
            continue

        for subd in subds:
            if not any([item in subd for item in ignore]):
                try:
...
def main():
    ...
    if not os.path.isfile(os.path.join(xanity.paths.conda_env, 'bin', 'python')):
