...
from .common import digest_file, digest_string, pickle_dump, pickle_load, file2mod, list_modules_at_path
...
class ContextDecorator:
    ...
    def __call__(self, f):
        def f_wrapped(xanity, *args, **kwargs):
            ...
            from xanity.constants import ACTIVITIES

            if self.context == ACTIVITIES.EXTERNAL:
...
class Xanity(object):
    ...
    def find_data(self, experiment_name):
        return path.join(self._find_recent_run_path(experiment_name), experiment_name)
    ...
    def _populate_root_info(self, root_path):
        if path.isdir(path.join(root_path, '.xanity')):
    ...
    def _run_one_anal(self, analysis, experiment, analysis_ind=None, exp_ind=None):
        ...
        #               filename=path.join(self.analyses[analysis].subexp_dirs[index],
        #               analysis + '.log'))
        #            tfh.setFormatter(logging.Formatter('%(asctime)s :%(levelname)s: %(message)s'))
        #            tfh.setLevel(logging.DEBUG)
        #            self.logger.addHandler(tfh)
    ...
    def _hash_conda_env(self):
        ...
        try:
            ...
            conda_env_contents = ''.join(sorted(str(subprocess.check_output([
                'bash', '-lc', 'conda list -p {}'.format(conda_env_path)  # xanity(bash) makes sure conda is on path :)
            ]).decode()).replace(' ', '').split()))
    ...
    def load_checkpoint(self, checkpoints):
        ...
        cp_dirs = [path.join(self.paths.checkpoints, self.status.focus.name, var) for var in checkpoints]
        cp_files = [path.join(self.paths.checkpoints, self.status.focus.name, var + '.pkl') for var in checkpoints]

        if all([path.isdir(cp_dir) for cp_dir in cp_dirs]):
            for cp_dir in cp_dirs:
                for item in os.listdir(cp_dir):
        ...
        elif all([path.isfile(cp_file) for cp_file in cp_files]):
            vars = []
            for cp_file in cp_files:
                vars.append(pickle_load(cp_file))

            return tuple(vars) if 'flat' not in locals() else vars[0]
    ...
    @ContextDecorator(ACTIVITIES.EXTERNAL)
    def save_checkpoint(self, checkpoint_names, variables=None):
        if not self.args.savecp:
            return False

        checkpoint_names = [checkpoint_names] if isinstance(checkpoint_names, str) else checkpoint_names

        if variables is None:

            # saving runpath in file-system
            cp_dirs = [path.join(self.paths.checkpoints, self.status.focus.name, cp_name) for cp_name in checkpoint_names]
            ...
            for cp_dir in cp_dirs:
                if not path.isdir(cp_dir):
                    ...
                    for item in os.listdir(self.status.data_dir):
            ...
            assert len(variables) == len(checkpoint_names)

            cp_files = [path.join(self.paths.checkpoints, self.status.focus.name, cp_name + '.pkl')
    ...
    def save_variable(self, value):
        ...
        name = inspect.stack(1)[3][4][0].split('(')[1].split(')')[0]
