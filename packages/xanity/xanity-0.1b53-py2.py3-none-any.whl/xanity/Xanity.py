...
class Xanity(object):
    ...
    @ContextDecorator(ACTIVITIES.EXTERNAL)
    def load_checkpoint(self, checkpoint_name, variables=None):
        ...
        if os.path.isdir(cp_dir):
            ...
            for root, dirs, files in os.walk(cp_dir):
                ...
                for f in files:
                    ...
                    # if not os.path.isfile(d) or overwrite:
                    os.link(s, d)
            ...
            return rvars if rvars else True

        else:
            return [False]*len(variables) if variables is not None else False
    ...
    def persistent(self, name, value=None):
        if ACTIVITIES.EXP in self.status.act_tags or ACTIVITIES.ANAL in self.status.act_tags:
            ...
            if value and not os.path.isfile(filename):
