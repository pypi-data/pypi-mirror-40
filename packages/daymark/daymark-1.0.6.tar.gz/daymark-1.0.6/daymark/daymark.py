...
class daymark():
	...
	def init(redis_end_point):
		...
		if redisEndPoint:
			...
			rmaster = Redis(host = redisEndPoint)
	...
	def logError(errorMsg, envVar):
		...
		if errorMsg:
			...
			streamMsg = '{"id": ' + '"' + envVar + '"' + ', "status": "FAILED"}'
	...
	def logSuccessful(successMsg, envVar):
		...
		if successMsg:
			...
			#The message that will be streamed in redis
			streamMsg = '{"id": ' + '"' + envVar + '"' + ', "status": "COMPLETED"}'
			rmaster.publish('jobs', streamMsg)
			sys.exit(successMsg)
		else:
	...
	def setProgress(key, value):
		...
		streamMsg = '{"id": ' + '"' + envVar + '"' + ', "progress": ' +  '"' + percentage + '"}'
