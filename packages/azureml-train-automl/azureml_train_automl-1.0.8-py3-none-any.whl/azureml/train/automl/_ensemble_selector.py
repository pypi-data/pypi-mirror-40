...
class EnsembleSelector(object):
    ...
    def __init__(self,
                 ...
                 iterations: int=10,
                 y_transformer=None):
        """Create EnsembleSelector used for choosing the pipelines that should be part of en ensemble.
    ...
    def select(self):
        ...
        if self.metric_objective == constants.OptimizerObjectives.MAXIMIZE:
            ...
            if single_model_scores[0] > 0:
                ...
                threshold_score = (1 + EnsembleSelector.SELECTION_GREEDY_SCORE_THRESHOLD) * single_model_scores[0]
            count = 0
            for score in single_model_scores:
                if score < threshold_score:
                    break
                else:
                    count += 1
            initial_size = min(count, initial_size)
            # because sorting is done in ASC order, we'll need to reverse the result first
            ensemble = np.argsort(single_model_scores)[::-1][:initial_size].tolist()
        else:
            # compute the threshold score based on the best individual score
            if single_model_scores[0] > 0:
                threshold_score = (1 + EnsembleSelector.SELECTION_GREEDY_SCORE_THRESHOLD) * single_model_scores[0]
        ...
        ensemble_predictions = self._get_ensemble_predictions(ensemble)
        best_ensemble_score = self._get_model_score(ensemble_predictions)
        ...
        for iteration in range(number_of_models_needed):
            ...
            for j, _ in enumerate(self.fitted_models):
                ...
                scores[j] = self._get_model_score(ensemble_predictions)
    ...
    def _compute_models_predictions(self):
        ...
        if self.training_type == constants.TrainingType.TrainAndValidation:
            ...
            split_index = 0
            for _, _, _, _, y_valid_fold, sample_wt_valid_fold in self.dataset.get_CV_splits():
                if split_index == 0:
                    cross_validate_indices = list()
                    cross_validate_indices.append((0, len(y_valid_fold)))
                    y_valid = y_valid_fold
                    sample_weight_valid = sample_wt_valid_fold
                else:
                    prev_length = len(y_valid)
                    cross_validate_indices.append((prev_length, prev_length + len(y_valid_fold)))
            ...
            predictions = np.zeros((y_valid.shape[0], self.num_classes, self.model_count))

            for index, model_tuple in enumerate(self.fitted_models):
                split_index = 0
                temp = None
                for _, _, _, X_valid_fold, y_valid_fold,\
                        sample_wt_valid_fold in self.dataset.get_CV_splits():
    ...
    def _get_model_predictions(self, model, validation_set, trained_class_labels):
        if hasattr(model, 'predict_proba'):
            ...
            result = metrics.add_padding_for_missing_class_labels(pred_proba_result=result,
    ...
    def _get_ensemble_predictions(self, ensemble_list: list):
    ...
    def _get_ensemble_predictions_simple(self, predictions: np.array):
    ...
    def _get_model_score(self, model_predictions: np.array):
