...
from automl.client.core.common import metrics, pipeline_spec, utilities, model_wrappers
from automl.client.core.common.exceptions import (DataException,
                                                  ServiceException)
from automl.client.core.common.utilities import (_y_nan_check,
...
from ._logging import cleanup_log_map, get_logger, _blacklist_logging_keys, TELEMETRY_AUTOML_COMPONENT_KEY
...
class AzureAutoMLClient(object):
    ...
    def __init__(self,
        ...
        if self.automl_settings.data_script is not None:
            ...
            try:
                ...
                # log_utils.log_traceback(e, self.logger)
                raise DataException(
    ...
    def _init_adb_driver_run(self,
        ...
        try:
            ...
            log_utils.log_traceback(ex, self.logger)
            raise
    ...
    def _populate_fit_iteration_parameters_dict(self):
        """
        Populate the fit parameters dictionary with the raw data.

        :return: dictionary containing the raw data
        """
        fit_iteration_parameters_dict = dict()
        fit_iteration_parameters_dict['X'] = self.input_data.get("X")
        fit_iteration_parameters_dict['y'] = self.input_data.get("y")
        fit_iteration_parameters_dict['X_valid'] = self.input_data.get("X_valid")
        fit_iteration_parameters_dict['y_valid'] = self.input_data.get("y_valid")
        fit_iteration_parameters_dict['sample_weight'] = self.input_data.get("sample_weight")
        fit_iteration_parameters_dict['sample_weight_valid'] = self.input_data.get("sample_weight_valid")
        fit_iteration_parameters_dict['cv_splits_indices'] = self.input_data.get("cv_splits_indices")
        fit_iteration_parameters_dict['x_raw_column_names'] = self.input_data.get("x_raw_column_names")
        return fit_iteration_parameters_dict

    def _repopulate_transformed_context(self, transformed_data_context):
        """
        Read and recover the transformed data from the cache store.

        If the read from cache is not successful, then populate a dictionary so
        that data transformation can happen again.

        :return: transformed data context, dictionary containing raw data
        """
        if transformed_data_context is not None:
            # Attempt to read transformed data from the cache store if pre-processing is turned on
            try:
                if self.automl_settings.preprocess:
                    start = time.time()
                    transformed_data_context._load_from_cache()
                    end = time.time()
                    self.logger.info("time taken for loading from cache {}".format(end - start))

                # Since the read from cache is successful, set fit_iteration_parameters_dict to None
                fit_iteration_parameters_dict = None

            except Exception as e:
                self.logger.warning("Error while loading from cache, defaulting to redo {}".format(e))

                # Since the read from cache is unsuccessful, populate fit_iteration_parameters_dict
                transformed_data_context = None
                fit_iteration_parameters_dict = self._populate_fit_iteration_parameters_dict()
        else:
            fit_iteration_parameters_dict = self._populate_fit_iteration_parameters_dict()

        return transformed_data_context, fit_iteration_parameters_dict
    ...
    def _fit_local(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None, sample_weight_valid=None,
        ...
        if not existing_run:
            ...
            subsampling = self.automl_settings.enable_subsampling and \
                utilities.subsampling_recommended(
        ...
        # release the memory associalted with transformed data in case pre-processing is turned on
        if self.automl_settings.preprocess:
        ...
        try:
            ...
            while self.current_iter < self.automl_settings.iterations:
                ...
                transformed_data_context, fit_iteration_parameters_dict = self._repopulate_transformed_context(
