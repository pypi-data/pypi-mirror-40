...
def bin_bp_dense(M, coords_bp, bin_len=10000):
    ...
    coords_bp : numpy array of int
        First column is the basepair start of fragments bins
        and second col, the index of their chromosome.
    Returns
    -------
    ...
    list :
        List of contigs
    """
    # Get fragments where new chromosome starts
    chromstart = np.where(coords_bp == 0)[0]
    chromend = chromstart[1:]
    chromend = np.append(chromend, M.shape[0])
    ...
    coords_bp = coords_bp // bin_len
    frags = np.array([chroms, coords_bp])
    # Keep track of index fragments
    frag_idx = range(frags.shape[0])
    # Number of bins to create
    n_bins = np.unique(frags, axis=1).shape[0]
    # Initialise output structures
    out_M, out_frags = np.zeros((n_bins, n_bins)), np.zeros((n_bins, 2))
    ...
    for coords, bin_frags in itertools.groupby(frag_idx, lambda x: tuple(frags[x, :])):
        ...
        # Fill output matrix line by line
        out_M[coords[1]] = np.array([M[bin_frags, bin_frags].sum()])
        # Write coordinates to output bin list (restore coord to basepairs)
        out_frags[coords[1], :] = coords[0], coords[1] * bin_len
