...
def df2pntshp(df, filename, datatype = ogr.OFTString):
    ...
    for i in range(lons.shape[0]):
        ...
        for fieldName in fieldnames:
            ...
            outFeature.SetField(str(fieldName),np.float64(fieldValue))
...
def zonal_stats_raster(path_raster, path_shp, attr_name = "NAME_1", mode = "m"):
    ...
    for i in range(feat_num):
        ...
        if mode == "m":
            ...
            data_dict[feat.GetField(attr_name)] = np.percentile(zoneraster, 25)
        elif mode == "p75":
            data_dict[feat.GetField(attr_name)] = np.percentile(zoneraster, 75)
