...
class Loop(ABC):
    ...
    def iteration_wrapper(self):
        self.iteration_log = []  # reset iteration log
    ...
    def collect_log(self, message):
        ...
        if len(self.iteration_log) > ITERATION_LOG_MAX_LEN:  # to be safe from memory leak
            self.iteration_log = self.iteration_log[-ITERATION_LOG_MAX_LEN:0]
