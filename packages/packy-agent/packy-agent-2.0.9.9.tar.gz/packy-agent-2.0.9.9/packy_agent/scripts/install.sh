...
log() {
    message=$1
    echo $message
    if [ "$PACKY_SERVER_BASE_URL" != "" -a "$PACKY_TOKEN" != "" ]; then
        curl -s -H 'Content-Type: application/octet-stream' -H "Authorization: Bearer $PACKY_TOKEN" --data-binary "$message" "${PACKY_SERVER_BASE_URL%/}/api/v2/agent/log/" > /dev/null || :
    fi
}
