...
class PeriodicLoop(CallableLoop):
    ...
    def __init__(self, period, callable_=None, args=(), kwargs=None,
                 is_logged_iteration=True, jitter_factor=None, main_loop=None, **mkwargs):
    ...
    def iteration_wrapper(self):
        ...
        if elapsed < period:
            ...
            global_switch_log = {}
            # if self.main_loop:
            #     for loop in self.main_loop.loops:
            #         loop_greenlet = loop.get_greenlet()
            #         if not loop_greenlet:
            #             continue
            #
            #         global_switch_log[f'{loop.description}-{id(loop_greenlet)}'] = list(
            #             loop_greenlet.switch_log)
            ...
            logger.warning(f'NOT WAITING {description} for next iteration: '
                           ...
                           extra={'iteration_log': list(self.iteration_log),
                                  'switch_log': list(self.get_greenlet().switch_log),
                                  'global_switch_log': global_switch_log})
