		...
		for action in actions:
			try:
				...
				if len(items) > 2 and items[2].isdigit() and items[2] == '18' and len(items) >= 7:
					dataDict[str(startTime) + '$' + str(aIndex)] = {'time': es.timeToEsLocalTime(float(items[0])), 'uid': uid, 'type': items[5], 'delay': long(items[6]), 'vid': items[4], 'domain': items[3].replace('https://', '')}
			except Exception as error:
