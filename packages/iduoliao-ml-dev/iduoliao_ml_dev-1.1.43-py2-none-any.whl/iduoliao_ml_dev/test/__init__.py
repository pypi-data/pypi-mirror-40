...
def saveFile():
	...
	start = 1546531200000
	#start = 1547740800000
	while start < 1547827200000:
		for hit in es.scrollSearch('clientlog_read', 'watch', {"size": 10000, "query": {"bool": {"must": [{"range": {"create_time": {"gte": start,"lt": start + 12 * 60 * 60 * 1000}}}, {"term": {"type": {"value": "avplay"}}}]}}}):
			source = hit['_source']
			...
			for item in body.split('\n'):
				...
				try:
					...
					count += 1
				lines.append('\t'.join([str(int(es.esTimeToTime(source['endtime']))), data['uid'], data['vid'], data['ym_version'], data['wx_version'], data['model'], data['networktype'], data['platform'], data['sdkversion'], str(data['width'] if data.has_key('width') else -1), str(data['height'] if data.has_key('height') else -1), data['errmsg']]))
