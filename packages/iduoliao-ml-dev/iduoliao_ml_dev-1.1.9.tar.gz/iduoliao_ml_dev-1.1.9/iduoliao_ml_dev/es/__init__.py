...
def updateStatisticsDocResolve(index, doc):
	...
	success = esUpdateStatisticsDocResolve(localEsclient, index, doc)
	print(success)
...
def esUpdateStatisticsDocResolve(es, index, doc, doc_type='data'):
	print(es, index, doc_type)
	print(es.bulk(index='statistics_' + index + '_write', doc_type=doc_type, body=doc)['errors'])
	time.sleep(10)
	        ...
	        },
	        {
	          "range": {
	            "create_time": {
	              ...
	              "lte": timeToEsTime(min(lastPersonasTime + 60 * 60, int(time.time()) - 90))
	            }
	          }
	        }
	      ]
...
def getEsclient(fullIndex=None):
	return testEsclient if isTest else relEsclient if fullIndex == 'works_video_read' else localEsclient if fullIndex == 'statistics_works_kartuns_write' else dataEsclient
