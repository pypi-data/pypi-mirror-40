...
def list(env, format, filter, json_flag):
  ...
  resp = requests.get('{}/auth/v1/me/companies?sort=name&{}'.format(app_name, filter), headers=headers)
...
def provision(name, type, format, json_flag):
  ...
  resp = check_response(requests.get(build_url('{app}/auth/v1/context'), headers=get_headers())).json()
  ...
  resp = check_response(requests.post(build_url('{app}/auth/v1/admin/companies'), json=data, headers=get_headers()))
