...
class AllPairs(object):
    def __init__(self, parameters, filter_func=lambda x: True, previously_tested=None, n=2):
        ...
        print("----------")
        print(self.__max_unique_pairs_expected)
        for v in self.__working_item_matrix:
            print(v)
        print("----------")
    ...
    def __next__(self):
        ...
        while -1 < i < len(self.__working_item_matrix):
            ...
            vv = self.__get_values(chosen_item_list[: i + 1])
            if vv[0] == "1.13":
                pass
                # """
                print("!", vv, i, indexes[i])
                for v in self.__working_item_matrix:
                    print(self.__get_values(v))
                print()
                # """
        ...
        print("# ", self.__get_values(chosen_item_list))
        ...
        if len(self.__pairs) == previous_unique_pairs_count:
            ...
            print("!! could not find new unique pairs - stop")
            raise StopIteration()
    ...
    def __resort_working_array(self, chosen_item_list, num):
        for item in self.__working_item_matrix[num]:
            ...
            new_combs = []
            for i in range(0, self.__n):
                s = set([key(z) for z in combinations(chosen_item_list + [item], i + 1)]) - self.__pairs.get_combs()[i]
                if s:
                    #print(s)
                    pass
                new_combs.append(s)
