...
class AnsiStr:
    ...
    def wrap( self, width: int, pad:bool=False ) -> Iterator[_AnsiStr_]:
        return cast( Iterator[AnsiStr], string_helper.wrap( self._str, width, pad ) )
...
def wrap( text: str, width: int, pad:bool=False ) -> Iterator[str]:
    return (str( x ) for x in AnsiStr( text ).wrap( width, pad ))
