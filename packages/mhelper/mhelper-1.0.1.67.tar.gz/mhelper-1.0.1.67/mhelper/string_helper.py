...
from typing import Callable, Iterable, Iterator, List, Optional, Union, cast, Sequence
...
def timedelta_to_string( delta : datetime.timedelta ) -> str:
    if delta.seconds < 0:
        return "(-{})".format( timedelta_to_string( -delta ) )
...
def to_int( x, default = None ) -> Optional[int]:
    if not x:
        return default
...
def to_bool( x, default = NOT_PROVIDED ):
    try:
        y = float( x )
        return y != 0
    except:
...
def find( *,
    ...
    if not case:
        namer = (lambda old_namer: (lambda x: [__insensitive( y ) for y in old_namer( x )]))( namer )
        search = __insensitive( search )
