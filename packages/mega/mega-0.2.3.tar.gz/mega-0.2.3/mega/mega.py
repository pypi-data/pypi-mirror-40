...
class Mega():
    ...
    Attributes:
        Pads
            height:         (int)    Length of terminal window.
            width:          (int)    Height of terminal window.
...
async def parse_args(rc, mega):
    ...
    if mega.height < 25:
        fatal('terminal height too short')
    if mega.width < 37:
        fatal('terminal width too short')
...
# if __name__ == '__main__':
    # mega_dir  = os.path.dirname(os.path.realpath(__file__))
    # mega_file = os.path.join(mega_dir, '.megarc')
    # try:
        # main(mega_file)
