...
def beaut_plot(size_x=12, size_y=12, linestyle='-.'):
    ''' To make plot more beautifull;
        takes as arguments size_x, size_y, linestyle
        code:
...
def MNK(x,y):
    ...
    d_c = ((1/n + x_ser**2/D)*(sum([(y[i]-b*x[i]-c)**2 for i in range(n)]))/(n-2))**(0.5)
    return b, c, d_b, d_b/b, d_c, d_c/c


def f_MNK(x,y):
    ''' BETA VERSION Input format: x,y\nx - n by m matrix (values of each variable
        in own LINE(!!!). Note, it should be 2-d array. If you have only 1-d,
        print double "[[]]"\ny - n by 1 matrix one LINE(!!!)\nOutput format: b0 (free member), b1, b2, ... , bn
        This indices are in b, type(b) = np.array
    '''
...
def random_error(array, t_stud):
    ''' Input array (list type), t_stud - Student's coeffitient (float type);
        return error, mean
    '''
    len_ = len(array)
    mean = sum(array) / len_
