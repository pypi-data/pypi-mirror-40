...
class ComnameCutter():
    ...
    def cut(self,com_name):
        ...
        name_segs = braced_strs.extend(name_segs)
        if self.method == "FSM":
    ...
    def docut1(self,name_segs):
        ...
        for seg in reversed(name_segs):
            if seg in self.TAILS or seg in self.SUBTAILS:
...
def main1():
    ...
    test_names = jsonfiler.load("json/com_names.json")[6000000:7000000]
    cutter = ComnameCutter("FSM")
    ok_count = 0
    ...
    for test_name in tqdm(test_names):
        ...
        if name_cuts["ok"]:
            ...
            pass
    print("完成度:"+str(ok_count)+"/"+str(all_count))    
    jsonfiler.dump(name_cuts_error, "error.json", indent=4)
    jsonfiler.dump(new_types, "new_types.json", indent=4)

def main():
    #test_names = jsonfiler.load("json/com_names.json")[6000000:6100000]
    test_names = jsonfiler.load("json/com_names.json")
 
    # com_sub_tail的库跟tail的库是同一个
    path_types = os.path.join("user_dict","user_dict_com_tails.dat")
    with open(path_types, "r", encoding="utf8") as f:
        SUBTAILS = f.readlines()
    TAILS = [x.strip() for x in SUBTAILS]
    TAILS_SET = set(TAILS)
    ...
    for test_name in tqdm(test_names):
        ...
        try:
            ...
            empty_set = set()
            share_set = set(name_segs) & TAILS_SET
            shere_list= list(share_set)
            if share_set != empty_set:
                alert_names.append(test_name+":"+str(name_segs))
                prev_seg = ""
                for seg in name_segs:
                    if seg in share_set:
                        learn_type = prev_seg+seg+"="+prev_seg+"+"+seg
                        if len(prev_seg+seg) == 3: learn_types_s3.append(learn_type)
                        if len(prev_seg+seg) == 4: learn_types_s4.append(learn_type)
                        if len(prev_seg+seg) == 5: learn_types_s5.append(learn_type)
                        if len(prev_seg+seg) == 6: learn_types_s6.append(learn_type)
                        if len(prev_seg+seg) == 7: learn_types_s7.append(learn_type)
                        if len(prev_seg+seg) == 8: learn_types_s8.append(learn_type)
                        if len(prev_seg+seg) == 9: learn_types_s9.append(learn_type)
                    prev_seg = seg
            else:
                ok_count+=1
    ...
    jsonfiler.dump(name_cuts_error, "error.json", indent=4)
    jsonfiler.dump(new_types, "new_types.json", indent=4)
    jsonfiler.dump(alert_names, "alert_names.json", indent=4)
    jsonfiler.dump(learn_types_s3, "learn_types_s3.json", indent=4)
    jsonfiler.dump(learn_types_s4, "learn_types_s4.json", indent=4)
    jsonfiler.dump(learn_types_s5, "learn_types_s5.json", indent=4)
    jsonfiler.dump(learn_types_s6, "learn_types_s6.json", indent=4)
    jsonfiler.dump(learn_types_s7, "learn_types_s7.json", indent=4)
    jsonfiler.dump(learn_types_s8, "learn_types_s8.json", indent=4)
    jsonfiler.dump(learn_types_s9, "learn_types_s9.json", indent=4)
