...
class DomesticHotWater(Heated):
    minTemp: float = 35
    maxTemp: float = 70
    availableOperationModes = ['AUTO', 'ON', 'OFF']

    def get_active_mode(self):
        if self.operationMode == 'ON':
            return TimeProgramDaySetting(str(0), self.configuredTemperature, "ON")
        elif self.operationMode == 'OFF':
            return TimeProgramDaySetting(str(0), 35, "OFF")
        else:
            return super().get_active_mode()
