...
def process_mroot(child_list, insertion_list):
    # Syntax: `<mroot> base index </mroot>.`
    # 親タグ要素の塊がある場合
    if insertion_list:
        if len(insertion_list) == 2:
            # Both base and index are `Parent like` block
            mroot = r'\sqrt [{{{}}}] {{{{}}}}'.format(insertion_list[0], insertion_list[1])
        else:
            # `mroot` の直下が子タグ要素だった場合、`child_list` は要素を持つはず
            if child_list:
                # index is `parent like` block, but base is not.
                base = ''.join(child_list)
                mroot = r'\sqrt [{{{}}}] {{{}}}'.format(base, insertion_list[0])
            # `mroot` の直下が`parent like`な要素だった場合、`child_list` は要素を持たない
            else:
                # base is `parent like` block, but index is not.
                index = ''.join(child_list)
                mroot = r'\sqrt [{{{}}}] {{{}}}'.format(insertion_list[0], index)
    # 親タグ要素の塊がない場合
    else:
