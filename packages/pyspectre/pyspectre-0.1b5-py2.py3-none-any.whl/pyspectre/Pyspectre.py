...
from . import pyspectre_path, pyspectre_ahdl_lib, pyspectre_ckt_dir, pyspectre_sim_dir
...
def hash_stims(stimuli, time_vec):
    return filehashfn(
        str(tuple(np.concatenate([time_vec.ravel(), stimuli.ravel()]))).encode('utf-8')
    ).hexdigest()[:16]


def batch_hashes(stimsets, time_vect):
    ids = []
    for set in stimsets:
        ids.append(hash_stims(set, time_vect))
    return ids


def run_batch(circuit, stimuli, time_vec, signals):
    print('running_batch of {} simulations.'.format(len(stimuli)))
    results = circuit.run_stimuli(stimuli, time_vec, signals)
    return results
...
class Circuit(object):
    ...
    def run_stimuli(self, stimuli, time_vec, signals):
        with Pyspectre(circuit=self, fork_repl=True) as pyspectre:
            results = pyspectre.run_stimuli(list_of_stimuli=stimuli, time_vec=time_vec, signames=signals)
        return results
...
class Pyspectre(object):
    ...
    def _start_binary(self):
        ...
        os.makedirs(pyspectre_ahdl_lib, exist_ok=True)
        ...
        # setup working_path
        self.working_path = tempfile.mkdtemp(dir=pyspectre_sim_dir, prefix='')
        ...
        for item in items:
            if os.path.isdir(item):
                shutil.copytree(item, os.path.join(self.working_path,os.path.split(item)[-1]))
            else:
        ...
        # determine state of any compilation libraries
        #
        ...
        os.environ['CDS_AHDLCMI_SHIPDB_DIR'] = pyspectre_ahdl_lib
        os.environ['CDS_AHDLCMI_SHIPDB_COPY'] = 'YES'
        ...
        if _rcfile is not None:
            invocation = 'bash --noprofile --norc -c \'stty -icanon && source {} && '.format(_rcfile)
        else:
            invocation = 'bash --noprofile --norc -c \'stty -icanon && '
        ...
        self.cmd = 'cd ' + self.working_path \
                   + ' && ' + _spectre_binary + ' ' + ' '.join(cmd_args) \
                   + ' -raw=\"{}\"'.format('raw') \
                   + ' -outdir=\"{}\"'.format(self.working_path)

        self.cmd = self.cmd + ' ' + self.spicefilepath

        full_cmd = invocation + self.cmd + '\''

        # self._load_relevant_lib_items()
        scllogfile = os.path.join(cache_target, '.' + os.path.split(self.spicefilepath)[1].split('.scs')[0] + '.scllog')
        wait_ct = 0
        while os.path.isfile(scllogfile):
            time.sleep(0.1)
            wait_ct += 1
            if wait_ct > 50:
                raise RuntimeError('running pyspectre process hasn\'t cleaned up it\'s scllog file: {}'.format(scllogfile))
        ...
        print('started spectre in {:6.1f}ms'.format((time.clock()-otime)*1000))
    ...
    def _cache_local_ship_db(self):
        ...
        path_rep = self.working_path.replace('/', '_')
        id_rep = '_' + self.circuit.id

        local_lib_contents = os.listdir(local_ship_db)
        library_contents = os.listdir(pyspectre_ahdl_lib)
        equiv_lib_targs = [re.sub(path_rep, id_rep, item) for item in local_lib_contents]

        for local_item, lib_item in zip(local_lib_contents, equiv_lib_targs):
            if lib_item in library_contents:
                continue

            s = os.path.join(local_ship_db, local_item)
            d = os.path.join(pyspectre_ahdl_lib, lib_item)
            if os.path.isdir(s):
                shutil.copytree(s, d)
            elif os.path.isfile(s):
                shutil.copy(s, d)

                if any([re.match(pattern, local_item) for pattern in _get_patterns_for_internal_replacement(path_rep)]):
                    contents = open(d, mode='rb').read()
                    contents = contents.replace(path_rep.encode(), id_rep.encode())
    ...
    def _read_file(self, filename, signames):
    ...
    def setpwl(self, name, time_vec, volts):
        ...
        assert time_vec.shape == volts.shape, 'incompatible time vector'
    ...
    def read_results(self, signal_names, analysis_name=None, analysis_type=None):
        if analysis_name is None:
            if self.analysis_name is not None:
                analysis_name = self.analysis_name
            else:
    ...
    def run_stimuli(self, list_of_stimuli, time_vec, signames):
        times = []
        signals = []
        return_type = 'float32'
