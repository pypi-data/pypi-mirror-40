...
def run_batch(circuit, stimuli, time_vec, signals):
    return circuit.run_stimuli(stimuli, time_vec, signals)
...
class Circuit(object):
    ...
    def run_stimuli(self, stimuli, time_vec, signals):
        pyspectre = Pyspectre(circuit=self, fork_repl=True)
        return pyspectre.run_stimuli(list_of_stimuli=stimuli, time=time_vec, signames=signals)
    ...
    def run_stimuli(self, list_of_stimuli, time_vec, signames):
        ...
        for stimulus in list_of_stimuli:
            ...
            for i, input in enumerate(self.pwl_inputs):
                self.setpwl(input, time=time_vec, volts=stimulus[i])

            self.setparams(self.analysis_name, {'stop': time[-1]})
            self.runanalysis()
        ...
        if all([np.all(t == times[0]) for t in times]):
            times = times[0].reshape((1, -1))
