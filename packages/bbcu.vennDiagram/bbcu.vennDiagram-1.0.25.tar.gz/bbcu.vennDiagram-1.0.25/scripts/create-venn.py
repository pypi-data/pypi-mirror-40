...
class VennDiagram(object):
    def __init__(self, input_dir, output_dir, min_log_fc, max_log_fc, min_pv, max_pv, total_gene_numbers, fast_run, package_dir,
        ...
        self.filtered_genes_gene = {}  # key is sample gene, value dict {sample: tuple ((Atnum,pv,log2FC))
        self.samples_list = {}
        self.summary_json = []
        self.summary = {}
                ...
                for row in self.read_file(input_file):
                    ...
                    if row['pv'] <= self.max_pv and row['pv'] >= self.min_pv and row['log2FC'] >= self.min_log_fc and \
                        ...
                        if not file_name in self.filtered_genes:
                            self.filtered_genes[file_name] = [(gene_name, row['pv'], row['log2FC'])]
                        else:
                            self.filtered_genes[file_name].append((gene_name, row['pv'], row['log2FC']))
                        if row['Atnum'] not in self.filtered_genes_gene:
                            self.filtered_genes_gene[gene_name] = {}# key is sample gene, value dict {sample: tuple ((Atnum,pv,log2FC))
                        self.filtered_genes_gene[gene_name][file_name] = (gene_name, row['pv'], row['log2FC'])
        ...
        for i in range(2, len(self.samples_list.keys()) + 1):
            for samples in itertools.combinations(self.samples_list.keys(), i):
                ...
                if not self.fast_run:
                    intersected = sorted(intersected)
                    ...
                    if self.fast_run:
                        ...
                        for gene in intersected:
                            ...
                            for sample in samples:
                                if sample in self.filtered_genes_gene[gene]:
                                    details = self.filtered_genes_gene[gene][sample]
                                    intersected_file.write(','.join([str(details[1]),str(details[2])]))
                                    intersected_file.write(',')
                                intersected_file.write('\n')
                        ...
                        #             i=-1
                        #             for details in self.filtered_genes[sample][sample_last_index[sample]:]:
                        #                 i+=1
                        #                 if details[0] == gene:
