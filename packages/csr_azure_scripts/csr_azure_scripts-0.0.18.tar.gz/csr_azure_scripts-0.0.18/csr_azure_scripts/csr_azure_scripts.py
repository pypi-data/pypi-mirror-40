...
class cas():
    def __init__(self):
        ...
        if self.storage_acct_name != "" and self.storage_acct_key != "":
            self.fileutils = file_utils.StorageFileUtils(self.storage_acct_name, self.storage_acct_key, self.cloud)
            self.blobutils = blob_utils.BlobUtils(self.storage_acct_name, self.storage_acct_key)
        self.logger = self.setup_logging('autoscaler')

    def create_file_utils(self):
        if self.storage_acct_key == "":
            print("Storage account key is not provided. Download file api may fail.")
        if self.storage_acct_name == "":
    ...
    def load_env(self):
        ...
        if not os.path.exists(filename):
            ...
            if self.storage_acct_name:
                self.fileutils = file_utils.StorageFileUtils(self.storage_acct_name,
                                                             self.storage_acct_key, os.getenv('CLOUD', 'azure'))
