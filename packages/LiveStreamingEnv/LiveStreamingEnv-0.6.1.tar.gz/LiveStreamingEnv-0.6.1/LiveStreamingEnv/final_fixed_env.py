...
class Environment:
    ...
    def get_video_frame(self, bit_rate, target_buffer):
        ...
        if self.time < self.cdn_arrive_time[quality][self.video_chunk_counter[quality]] and not end_of_video: # CDN can't get the frame
            ...
            cdn_has_frame = []
            for bitrate in range(BITRATE_LEVELS):
                cdn_has_frame.append(self.video_size[bitrate][self.video_chunk_counter[bitrate] : self.newest_frame[bitrate]])
            for bitrate in range(BITRATE_LEVELS):
                cdn_has_frame.append(self.gop_flag[bitrate][self.video_chunk_counter[bitrate]:self.newest_frame[bitrate]])
            # Return the loop
