...
class jspaperAxes(matplotlib.axes.Axes):
    ...
    def Yaxis(self,min=None,max=None,label=None,scale=None,size=None,charsize=None,tick=None,ticklabel=None,**kwargs):
        """
        Set xaxis
        ...
        if scale is not None:
            if scale[0]=='l':
                self.set_yscale(value='log', nonposx='clip', subsy=[2, 3, 4, 5, 6, 7, 8, 9])
            else :
                self.set_yscale(value='linear')

        self.figure.show()
        
    def Xaxis(self,min=None,max=None,label=None,scale=None,size=None,charsize=None,tick=None,ticklabel=None,**kwargs):
        """
        Set xaxis
        ...
        if scale is not None:
            if scale[0]=='l':
                self.set_xscale(value='log', nonposx='clip', subsy=[2, 3, 4, 5, 6, 7, 8, 9])
            else:
...
def surface(x,y,z,xdim=None,levels=8, colorMap='jet',lineMap=None,alpha=0.7):
    ...
    ::
     ...
     ffs=js.sf.orientedLatticeStructureFactor(qxyz,sclattice,orientation=[1,0,0],domainsize=ds,rmsd=0.1,hklmax=2)
...
def contourImage(x,y=None,z=None,levels=8,fontsize=10,colorMap='jet',lineMap=None,title=None,
    ...
    --------
     - Create log scale image for maskedArray (sasImage) ::
        ...
        xyz=js.dA(np.c_[x.flatten(),z.flatten(),0.3*np.sin(x*z/np.pi).flatten()+0.01*np.random.randn(len(x.flatten()))].T)
        xyz.setColumnIndex(ix=0,iy=2,iz=1)
        js.mpl.contourImage(xyz)
    ...
    if np.ndim(x)<2 or hasattr(x,'_isdataArray'):
        ...
        if np.unique(x).shape[0]*np.unique(y)[0]==np.shape(z)[0]:
            ...
            if isinstance(levels, (int, float)):
                levels = np.r_[np.min(zz):np.max(zz):levels * 1j]
            im       = ax.imshow(zz, cmap=cmap, extent=extend, origin=origin)
            ...
            if isinstance(levels, (int, float)):
                levels = np.r_[np.min(z):np.max(z):levels * 1j]
            im=ax.tricontour(x, y, z, levels=levels, linewidths=1, cmap=lmap,extent=extend,origin=origin)
            im2=ax.tricontourf(x, y, z, levels=levels, cmap=cmap,extent=extend,origin=origin)
        ...
        # image array
        z=x
        if isinstance(levels,(int,float)):levels=np.r_[np.min(z):np.max(z):levels*1j]
        try:
            if axis!='pixel':
            ...
            extend=None
        im      = ax.imshow(z, cmap=cmap, extent=extend, origin=origin)
        im.cset = ax.contour(z,levels=levels,linewidths=1,cmap=lmap,extent=extend,origin=origin)
        im.labels=ax.clabel(im.cset,inline=True,fmt='%1.1f',fontsize=fontsize)
        fig.colorbar(im) # note that colorbar is a method of the figure, not the axes
