...
For convenience we use Ipython (colors highligthing, history, ...), start it typing "ipython" in your shell.
...
Models are described in formfactor. pDA integrates over a Gaussian parameter.
...
^^^^^^^^^^^^^^^^^^^^^^^
Small angle scattering data relate to **spherical particles in solvent**
with a polydispersity in radius ::
 ...
 # use Guinier model as first size estimate in limited range q<1/Rg
 # fit it (with start values) and show result
 data.fit(js.ff.guinier,freepar={'Rg':5,'A':1},
                        fixpar={},
                        mapNames={'q':'X'},condition=lambda a:a.X<1/7.)

 data.showlastErrPlot(yscale='log',fitlinecolor=5) # reused in next fits
 ...
 def sphere(q,R,A,bgr):
    return A*js.ff.sphere(q=q,radius=R).Y+bgr
 ...
 psphere= lambda q,R,A,bgr,dR:A*js.formel.pDA(js.ff.sphere,dR,'radius',q=q,radius=R).Y+bgr
 # use a monodisperse sphere model
...
- dataList  (js.dL) reads **all** datasets from one/multiple files (may differ in shape) ::
   ...
   data.append(dat)                               # apend single dataArray read from above
  ...
  **Read all, filter later**
  Create dataList from multiple files (uses `glob <https://docs.python.org/2/library/glob.html#module-glob>`_ patterns) ::
   ...
   for dat in data:
      ...
      dat.pressure=dat.comment[1].split()[1]     # if second commnt line contains 'pressure 1000 mbar'
...
* The filename is **always** stored in attribut ``.name``
...
- **Data** need to be read and prepared.
 ...
 - 2D data (e.g. a detector image with 2 dimensions) need to be transfomed to coordinates
   *.X*,*.Z* with values in *.Y*. This also gives pixels coordinates in an image a physical
   ...
   See last example in :ref:`Some Sinusoidal fits with different kinds to use data attributes`
