...
class GraceGraph:
    ...
    def gen_axis(self, axis_prefix, ax_min=None, ax_max=None,
        ...
        if scale is not None:
            ...
            elif scale[0] in ('r','R'):  scale='reciprocal'
            else:scale='normal'
        ...
        if isinstance(ticklabel,(float,int)):
            ...
            ticklabel+=[None]*4
            commands +=TickLabel(format=ticklabel[0],prec=ticklabel[1],charsize=ticklabel[2],placeon=ticklabel[3]).output(axname)
        ...
        if isinstance(tick,list):
            ...
            if tick[4] is not None and tick[4][0] in ('n', 'b', 'o'):
                ...
                placeon=None
            commands +=Tick(major=tick[0], minorticks=tick[1],majorsize=tick[2],minorsize=tick[3],placeon=placeon).output(axname)
    ...
    def plot(self, *datasets,**kwargs ):
        ...
        for dataset in datasets:
            ...
            if 'errorbar' in kwargs or 'er' in kwargs:
                ...
                #avoid color of 0
                errorbar=Errorbar(True,None,color=(self.lastsymbol[2] if self.lastsymbol[2] else self.lastline[3]),size=0.5)
            #now the datasets
            if isinstance(dataset, Data):
                ...
                if hasattr(dataset,'Y') and (not hasattr(dataset,'eY') or dataset.eY is None):
                ...
                elif hasattr(dataset,'Y') and hasattr(dataset,'eY') and (not hasattr(dataset,'eX') or dataset.eX is None):
                    ...
                    raise Exception('values not specified ix=%s, iy=%s, iey=%s' %(dataset._ix,dataset._iy,dataset._iey,))
...
class GracePlot:
    ...
    def stacked(self, number, hshift, vshift, frame=None, yaxis='off', yaxisnumber=0, frametype=0, framepattern=1, ):
        ...
        if vshift<0:
            frameymin=frameymin+abs(vshift*number)
        if hshift<0:
            framexmin=framexmin+abs(hshift*number)
...
class Data:
    ...
    def output(self,graceGraph,count):
        ...
        strlist += ['s%d.y%d[%d]=%g' % (count, dyidx+1, idx, yy) for (dyidx, dy) in enumerate(self.dylist) for (idx,yy) in enumerate(dy) ]
