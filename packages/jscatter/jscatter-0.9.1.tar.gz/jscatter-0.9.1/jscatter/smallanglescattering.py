...
def desmear(Ios, beamProfile, NIterations=-15, windowsize=4, qmax=4, **kwargs):
    ...
    while True:
        ...
        print('%3.i %8.5g %8.5g ' % (len(Idesmeared), meangamma, Idesmeared.convergence.array.min(),),
              Idesmeared[-1].decreasing)
        if len(Idesmeared) >= abs(NIterations):
            print('len(Idesmeared) = NIterations', len(Idesmeared))
            return Idesmeared[-1]
        elif (NIterations < 0) and (Idesmeared[-1].decreasing is False) and (Idesmeared[-2].decreasing is False):
...
def getBeamWidth(empty, minmax='auto', show=False):
    ...
    try:
        ...
        pass
    primarybeam.fit(_gauss, {'mean': empty.Y.argmax(), 'sigma': 0.015, 'bgr': 0, 'A': 1}, {}, {'x': 'X'})
