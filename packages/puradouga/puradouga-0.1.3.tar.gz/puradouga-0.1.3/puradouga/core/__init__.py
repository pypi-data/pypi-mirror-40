...
from puradouga.core.helper import args_kwargs_decorator, order_by_list
from . import loader
...
class SystemFilter:
    ...
    def eval_ordered(self, method: Callable, eval_function: Callable, max_results: int = 1, copy: bool = True,
        ...
        results = self._call_plugins(method, eval_function, args, kwargs, copy, order=order_by_list(plugin_name_order))
        results = filter(lambda a: a[0], results)
        results = itertools.islice(results, 0, max_results)
        for result in results:
            yield result
    ...
    def _call_plugins(self, method: Callable, eval_function: Callable, args: list, kwargs: dict,
                      copy: bool = True, order: Optional[Callable] = None):

        plugins = self.list()
        if order:
            plugins = sorted(plugins, key=order, reverse=True)
