            ...
            while running_job_ids:
                ...
                for job_id in list(running_job_ids):
                    ...
                    if status == drmaa.JobState.DONE:
                        ...
                        raise ruffus.ruffus_exceptions.JobFailed("job {} failed".format(job_id))
                    status_summary.append((job_id, status))

                if self.pipeline.is_job_suspended():
                    for job_id, status in status_summary:
                        self.logger.debug("job with job {} will be suspended".format(job_id))
                        if status == drmaa.JobState.RUNNING:
                            self.session.control(job_id, drmaa.JobControlAction.SUSPEND)
                        elif status == drmaa.JobState.QUEUED_ACTIVE:
                            self.session.control(job_id, drmaa.JobControlAction.HOLD)

                    while self.pipeline.is_job_suspended():
                        gevent.sleep(GEVENT_TIMEOUT_WAIT)

                    for job_id, status in status_summary:
                        self.logger.debug("job with job {} will be resumed".format(job_id))
                        # collect status again as things might have changed
                        status = self.session.jobStatus(job_id)
                        if status == drmaa.JobState.USER_SUSPENDED:
                            self.session.control(job_id, drmaa.JobControlAction.RESUME)
                        elif status == drmaa.JobState.USER_ON_HOLD:
                            self.session.control(job_id, drmaa.JobControlAction.RELEASE)

                gevent.sleep(GEVENT_TIMEOUT_WAIT)

        except ruffus.JobSignalledBreak:
            for job_id in running_job_ids:
                self.logger.debug("job with job {} will be terminated".format(job_id))
                self.session.control(job_id, drmaa.JobControlAction.TERMINATE)
            raise
...
def run(statement, **kwargs):
    ...
    options["pipeline"] = ruffus.task.lookup_pipeline(pipeline=None)
