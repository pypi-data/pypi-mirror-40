...
from . import service
from . import xbus_nrpc, xbus_pb2
from . import envelope_writer
            ...
            if not apc.detached:
                if apc.error:
                    yield make_actor_processing_state(
                        request.context, xbus_pb2.ActorProcessingState.ERROR,
                        str(apc.error))
                else:
                    yield make_actor_processing_state(
                        request.context,
...
class Actor(object):
    ...
    async def processing_success(self, context):
        await self.processing_end(context,
                                  xbus_pb2.ActorProcessingEndRequest.SUCCESS)
    ...
    def listactors(self):
        ...
        for key, value in actor_dict.items():
            try:
                ...
                if name != key:
                    raise ValueError("Inconsistent key/name: Key=%s, name=%s" %
                                     (key, name))
                value['name'] = name
...
class Client(object):
    ...
    def load_tls_conf(self, confdir):
        ...
        if conf['tls.certdir'] and name:
            if conf['tls.private-key'] is None and conf['tls.private-key-file'] is None:
                conf['tls.private-key-file'] = os.path.join(
                    conf['tls.certdir'], name + '.key')
            if conf['tls.certificate'] is None and conf['tls.certificate-file'] is None:
                conf['tls.certificate-file'] = os.path.join(
                    conf['tls.certdir'], name + '.crt')
            if conf['tls.csr'] is None and conf['tls.csr-file'] is None:
                conf['tls.csr-file'] = os.path.join(conf['tls.certdir'],
                                                    name + '.csr')
        ...
        for entry in ('tls.private-key', 'tls.certificate', 'tls.csr',
                      'tls.rootca'):
    ...
    async def connect(self, loop):
        ...
        with pemfiles(self.conf['tls.certificate'],
                      self.conf['tls.private-key']) as (certfile, keyfile):
    ...
    async def whoami(self):
        ...
        whoami_client = xbus_nrpc.WhoAmIClient(
            conn.FixedInboxConn(
                self.nc,
                "xbus.client.whoami.%s.reply" % self.conf['account-name']))
    ...
    async def load_actor_defs(self):
        ...
        clientapi = xbus_nrpc.ClientAPIClient(self.clientconn(),
                                              self.conf['account-id'])
