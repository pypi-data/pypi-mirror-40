#!/usr/bin/env python
"""
Module for vasp CHGCAR file I/O
"""
...
import os
import numpy as np
from ..core import Xstructure, util

def read(f, reportlevel=0):

    molecule = Molecule.Molecule()

    line = util.read_next_line(f)
    ...
    molecule.unitcell = _read_unitcell(f)
    typelist, nalist = _read_typelist(f)
    molecule.atomlist, molecule.atomcoord = _read_atomlist(f, typelist, nalist)
    molecule.finalize()
    # volumetric data
    # molecule.volumedata = _read_volume_data(f)
