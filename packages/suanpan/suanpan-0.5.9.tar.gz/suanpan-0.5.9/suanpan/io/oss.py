...
class OssStorage(object):
    ...
    def downloadFolder(
        ...
        # Download from oss
        with tqdm.tqdm(
            total=len(downloads), bar_format=self.PBAR_FORMAT, desc="Downloading"
        ) as pbar:
            _run = self.pbarRunner(pbar)(self.downloadFile)
            with asyncio.multiThread(workers) as pool:
                for file, filePath in downloads.items():
                    pool.apply_async(
                        _run,
                        args=(file, filePath),
                        kwds={
        ...
        for root, _, files in os.walk(folderPath):
            for file in files:
                ...
                if filePath not in downloads.values():
                    path.remove(filePath)
                    logger.info("Removed rest file: {}".format(filePath))
    ...
    def uploadFolder(
        ...
        # Upload files to oss
        with tqdm.tqdm(
            total=len(uploads), bar_format=self.PBAR_FORMAT, desc="Uploading"
        ) as pbar:
            _run = self.pbarRunner(pbar)(self.uploadFile)
            with asyncio.multiThread(workers) as pool:
                for filePath, objectName in uploads.items():
                    pool.apply_async(
                        _run,
        ...
        for file in remoteFiles - localFiles:
            objectName = self.ossPathJoin(folderName, file)
            self.remove(objectName)
            logger.info(
                "Removed rest file: {}".format(self.storageUrl(bucket, objectName))
            )
        # End
    ...
    def copyFolder(
        ...
        with tqdm.tqdm(
            total=len(copies), bar_format=self.PBAR_FORMAT, desc="Copying"
        ) as pbar:
            _run = self.pbarRunner(pbar)(self.copyFile)
            with asyncio.multiThread(workers) as pool:
                for file in copies:
                    ...
                    pool.apply_async(
                        _run,
                        args=(file, dist),
                        kwds={"bucketOrBucketName": bucket, "quiet": True},
                    )
