...
class Mesh(object):
    ...
    def save_model(self, mem=None):
        filepath = None
        if conlib.is_str(mem):
            filepath = mem

        if not isinstance(mem, numpy.ndarray):
...
def test():
    m = Mesh()
    m = Mesh()
    assert m.__version__.startswith(m.info('version'))
