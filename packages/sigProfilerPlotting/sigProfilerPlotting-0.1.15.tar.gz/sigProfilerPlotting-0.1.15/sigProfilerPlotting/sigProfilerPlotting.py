...
def plotID(matrix_path, output_path, project, plot_type, percentage=False):
	...
	if plot_type == '94' or plot_type == 'ID94' or plot_type == '94ID':
		...
		for sample in mutations:
			...
			if sig_probs:
				plt.text(0.13, 0.85, sample, fontsize=30, weight='bold', color='black', fontname= "Arial", transform=plt.gcf().transFigure)
			else:
				plt.text(0.13, 0.85, sample + ": " + "{:,}".format(int(total_count)) + " transcribed indels", fontsize=30, weight='bold', color='black', fontname= "Arial", transform=plt.gcf().transFigure)
		...
		for sample in mutations:
			...
			for i in range(0, 4, 1):
				panel1.add_patch(plt.Rectangle((x,y_top), .154, .05, facecolor=colors[i], clip_on=False, transform=plt.gcf().transFigure)) 
				panel1.add_patch(plt.Rectangle((x,y_bottom), .154, .05, facecolor=colors[i], clip_on=False, transform=plt.gcf().transFigure)) 
				x += .1715

			x -= .001
			panel1.add_patch(plt.Rectangle((x,y_top), .098, .05, facecolor=colors[i+1], clip_on=False, transform=plt.gcf().transFigure)) 
			panel1.add_patch(plt.Rectangle((x,y_bottom), .098, .05, facecolor=colors[i+1], clip_on=False, transform=plt.gcf().transFigure)) 
...
def main():
	...
	plotID("/Users/ebergstr/Desktop/test/output/INDEL/test2.INDEL28.all", "/Users/ebergstr/Desktop/", "test", 'simple_ID', False)
