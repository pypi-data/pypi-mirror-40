...
<xsl:stylesheet
    ...
    <xsl:template match="ref" mode="unestructured_formated"><refbiblio id="{@id}"><xsl:apply-templates select="element-citation/*" mode="unestructured_formated_elements"></xsl:apply-templates></refbiblio></xsl:template>
