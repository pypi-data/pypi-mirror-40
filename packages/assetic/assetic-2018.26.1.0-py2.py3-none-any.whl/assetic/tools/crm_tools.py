...
class CRMTools(object):
    ...
    def process_new_crms(self):
        ...
        for wr_crm_obj in crms:
            ...
            new_note_list = [
                WrNote(note=self.wr_status_note["New"]
                       , context="Status"
    ...
    def process_changed_work_requests(self, check_all_active_crms=False):
        ...
        for wr in wrs:
            ...
            if wr["CreatedDateTime"] != wr["LastModified"]:
                # something has changed in this work request since last check.
                # Don't know if status has changed, but can ignore 'New'
                if wr["WorkRequestStatus"] != "New":
                    new_status = wr["WorkRequestStatus"]

                enddate = maxdate
                if wr["WorkRequestStatus"] in self._close_crm_statuses:
            ...
            if len(new_note_list) > 0 or bclose_crm_task:
                ...
                if wr["Id"] in active_crm_wr_guid_list:
                    ...
                    crm = self.crm_tools.get_crm_request_for_wr(wr["Id"])
                if crm:
                    ...
                    if bclose_crm_task:
                        crm_code_obj = self.crm_tools. \
                            get_resolution_code_for_status(
                                wr["WorkRequestStatus"], crm.crm_representation)
                        close_date = wr["LastModified"]
                        ...
                        if wr["WorkRequestStatus"] == "Cancelled":
    ...
    def apply_wo_status_comment(self, crms, active_crm_wr_guid_list
        ...
        for wo in wos:
            ...
            for wr in wo["LinkedWorkRequests"]:
                ...
                target_note = None
                if self._wo_target_dates_to_crm and wo["Status"] in \
                        ["PREP","WAPR","WMATL"]:
                    target_start = None
                    target_end = None
