...
def clean(source, search, no_action, v, vv, empty,
    ...
    * If a source and search path are specified, any duplicate files
    ...
    search_pth = Path(search) if search is not None else Path('/')
    ...
    if no_action:
        flist = [str(search_fmap['folders'][fid]['full_path']) for fid in fold_del]
        dlist = [str(search_fmap['files'][fid]['full_path']) for fid in file_del]
        pydoc.pager("""Directories to recursively delete
    {}
Files to delete
    {}""".format(
        "\n    ".join(flist),
