...
def find_main_window_hwnd(title=""):
    if platform.system() == 'Windows':
        import win32gui
...
def open_new_window(url, title=default_window_title, payload=None, maximized=False, minimized=False, cid='',
                    width=default_window_width, height=default_window_height, enable_max=True, inject_obj={}):
...
def create_browser_view(uid, title="", url=None, width=default_window_width, height=default_window_height,
                        ...
                        minimized=False, cid='', call_back=None, enable_max=True, inject_obj={}):
    browser = BrowserView(uid, title, url, width, height, resizable, full_screen, min_size,
                          background_color, web_view_ready, cid=cid, enable_max=enable_max, inject_obj=inject_obj)
    if maximized:
    ...
    set_javascript_bindings(uid, inject_obj=inject_obj)
...
def launch_main_window(uid, title, url, width, height, resizable, full_screen, min_size,
                       ...
                       app_name="Qt Application Made By Burgeon Developer", inject_obj={}):
    hwnd = find_main_window_hwnd(title)
...
def set_javascript_bindings(uid, inject_obj={}):
