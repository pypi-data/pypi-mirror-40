...
class _ConnectionBase:
    ...
    def recv(self):
        ...
        while True:
            try:
                self._recv_bytes()
                objects.append(pickle.loads(self._rbuf.getbuffer()))
                self._rbuf = io.BytesIO()
...
def NonBlockingPipe(duplex=True):
    '''
    ...
    if duplex:
        ...
        c1 = _Connection(s1.detach())
        c2 = _Connection(s2.detach())
    else:
        fd1, fd2 = os.pipe()
        _set_non_blocking_flag(fd1)
        _set_non_blocking_flag(fd2)
        c1 = _Connection(fd1, writable=False)
        c2 = _Connection(fd2, readable=False)
