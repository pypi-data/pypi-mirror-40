...
class Executable(AnyEntity):
    ...
    def add_input(self, name, value_type, **kwargs):
        return self._cw.create_entity(
            'ParameterDefinition', param_type=u'input', parameter_of=self,
            value_type=value_type, name=name, **kwargs)

    def add_output(self, name, value_type, **kwargs):
        return self._cw.create_entity(
    ...
    def wiring_language(self):
        ...
        for exe in self.uses_executable:
            ...
            for pdef, def_kind, other_kind in chain(
                zip(exe.input_defs.values(), repeat('input'), repeat('output')),
                zip(exe.output_defs.values(), repeat('output'), repeat('input'))):
                fields.append({
