...
class BaseParser(object):
    ...
    def _regular_term_op(cls):
        return lambda line, col: (lambda left, right: ast.BinOp(left=left,
                                                                op=cls(lineno=line,
                                                                       col_offset=col),
                                                                right=right,
                                                                lineno=line,
    ...
    def _handle_div(line, col):
        return lambda left, right: ast.Call(func=ast.Name(id='__tako_div__',
                                      ctx=ast.Load(),
                                      lineno=line,
                                      col_offset=col),
                        args = [left, right],
    ...
    def _handle_floordiv(line, col):
        return lambda left, right: ast.Call(func=ast.Name(id='__tako_floordiv__',
                                      ctx=ast.Load(),
                                      lineno=line,
                                      col_offset=col),
                        args = [left, right],
    ...
    _term_binops = {
        '+': _regular_term_op(ast.Add),
        '-': _regular_term_op(ast.Sub),
        '*': _regular_term_op(ast.Mult),
        '@': _regular_term_op(ast.MatMult),
        '/': _handle_div,
        '%': _regular_term_op(ast.Mod),
        '//': _handle_floordiv,
    }
    ...
    def p_pm_term(self, p):
        ...
        """
        p1 = p[1]
        op = self._term_binops[p1.value](p1.lineno, p1.lexpos)
        p[0] = [op, p[2]]
    ...
    def p_op_factor(self, p):
        ...
        if op is None:
            self._parse_error('operation {0!r} not supported'.format(p1),
                              self.currloc(lineno=p.lineno, column=p.lexpos))
        p[0] = [op(p1.lineno, p1.lexpos), p[2]]
    ...
    def p_number(self, p):
        """number : number_tok"""
        p1 = p[1]
        p[0] = ast.Call(func=ast.Name(id='__tako_num__',
                                      ctx=ast.Load(),
                                      lineno=p1.lineno,
                                      col_offset=p1.lexpos),
                        args = [ast.Str(s=p1.value,
                                        lineno=p1.lineno,
                                        col_offset=p1.lexpos)],
                        keywords=[],
                        starargs=None,
