...
class RealtimeStatus(PingsPane):
    ...
    def __init__(self, *args):
        ...
        if self.realtime_data.count(-1) == 1:
            status_string += '1 packet dropped of %s'.center(self.columns) % len(self.realtime_data)
        elif self.realtime_data.count(-1) > 1:
            status_string += '%s packets dropped of %s'.center(self.columns) % (self.realtime_data.count(-1), len(self.realtime_data))
        else:
            status_string += 'Displaying %s total packets from %s'.center(self.columns) % (len(self.realtime_data), self.address)
...
class AverageStatus(PingsPane):
    ...
    def __init__(self, *args):
        ...
        if sum(self.average_data) / len(self.average_data) < 0:
            ...
            print('Displaying the average of %s total packets from %s'.center(self.columns)
...
class RealtimePane(PlotPane):
    ...
    def __init__(self, *args):
        super(RealtimePane, self).__init__(*args)
        self.rows = args[1] - 1
        RealtimePlot(*args)
        RealtimeStatus(*args)
...
class AveragePane(PlotPane):
    ...
    def __init__(self, *args):
        super(AveragePane, self).__init__(*args)
        self.rows = args[1] - 1
        AveragePlot(*args)
        AverageStatus(*args)
        ...
        try:
            for ping in pings:
                ...
                if num_plots < 1:
                    ...
                    plot_rows = (rows / num_plots) - num_status
