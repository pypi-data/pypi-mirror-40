...
class File:
    ...
    def get_filt_list(self, sample_type, basi_path, f, pre, basi_dir=""):
        file_list = []
        if pre is not None:
            for dirpath, dirnames, filenames in os.walk(os.path.join(basi_dir, pre)):
                if dirpath != os.path.join(basi_path, pre):
                    break
                for dir_name in dirnames:
                    file_dict = self.get_file_item(basi_dir, dirpath, dir_name, f=='/')
                    file_dict["type"] = "文件夹"
                    file_dict["size"] = None
                    file_list.append(file_dict)
                for file_name in filenames:
                    file_type = self.get_file_type(sample_type, file_name)
                    file_dict = self.get_file_item(basi_dir, dirpath, file_name, f=='/')
                    file_dict["type"] = file_type
                    try:
                        file_size = os.path.getsize(file_dict['full_path'])
                        file_dict["size"] = get_file_size(file_size)
                    except Exception, e:
                        file_dict["size"] = ''
                        pass
                        # print file_path
