...
__all__ = ["OrderedSet", "Container", "Grid", "NoveltyArchive"]
...
class OrderedSet(MutableSet[T], Sequence[T]):
    ...
    def update(self, iterable: Iterable) -> None:
        ...
        If ``ignoreExceptions`` is True, ignore exceptions resulting for the addition of the items,
        but print a warning when this happens.
        """
        try:
...
class Container(Sequence):
    ...
    depot_type: bool or Backend (MutableSet or MutableSequence)
        TODO
    fitness_getter: Optional[FitnessGetter]
        TODO
    fitness_domain: Sequence[DomainLike] (sequence of 2-tuple of numbers)
        TODO
    features_getter: Optional[FeaturesGetter]
        TODO
    ...
    fitness_getter: Optional[FitnessGetter] # XXX
    fitness_domain: Sequence[DomainLike]
    features_getter: Optional[FeaturesGetter] # XXX
    features_domain: Sequence[DomainLike]
    _capacity: float
    _size: int = 0
    # TODO : pareto front, or remove ?
    _best: Optional[IndividualLike] = None
    _best_fitness: Optional[FitnessLike] = None
    _best_features: Optional[FeaturesLike] = None

    def __init__(self, iterable: Optional[Iterable] = None,
            storage_type: Type[BackendLike] = list, depot_type: Union[bool, Type[BackendLike]] = False,
            fitness_getter: Optional[FitnessGetter] = None, fitness_domain: Sequence = ((0., 1.),),
            features_getter: Optional[FeaturesGetter] = None, features_domain: Sequence = ((0., 1.),),
...
#    def _get_fitness_from_ind(self, individual: T) -> FitnessLike:
#        """Return the fitness values from ``individual``.
#        Depending on the type of ``individual``, a ValueError can be raised if the individual is not valid."""
#        if self.fitness_getter is not None:
#            return self.fitness_getter(individual)
#        elif isinstance(individual, DEAPStyleIndividualLike):
#            if individual.fitness.valid:
#                return individual.fitness.values
#            else:
#                raise ValueError("Fitness of ``individual`` is not valid")
#        elif isinstance(individual, QDPYIndividualLike):
#            return individual.fitness
#        #elif isinstance(individual, FitnessLike):
#        #    return individual
#        else:
#            raise ValueError("Unknown type of ``individual``")
#
#    def _get_features_from_ind(self, individual: T) -> FeaturesLike:
#        """Return the features values from ``individual``.
#        Depending on the type of ``individual``, a ValueError can be raised if the individual is not valid."""
#        if self.features_getter is not None:
#            return self.features_getter(individual)
#        elif isinstance(individual, QDPYIndividualLike):
#            return individual.features
#        elif isinstance(individual, DEAPStyleIndividualLike):
#            return individual.features
#        elif isinstance(individual, FeaturesLike):
#            return individual # Take the genome as features... XXX Remove ?
#        else:
#            raise ValueError("Unknown type of ``individual``")


#    def _dominates(self, ind0: T, ind1: T) -> bool:
#        """Return true if ``ind0`` dominates ``ind1``.
#        Depending on the type of ``individual``, a ValueError can be raised if the individual is not valid."""
#        assert type(ind0) == type(ind1), f"``ind0`` and ``ind1`` must be of the same type."
#        if isinstance(ind0, DEAPStyleIndividualLike) and isinstance(ind1, DEAPStyleIndividualLike):
#            if ind0.fitness.valid and ind1.fitness.valid:
#                return ind0.fitness.dominates(ind1.fitness)
#            else:
#                raise ValueError("Fitness of ``individual`` is not valid")
#        elif isinstance(ind0, QDPYIndividualLike) and isinstance(ind1, QDPYIndividualLike):
#            return all((a <= b) for a, b in zip(ind0.fitness, ind1.fitness)) # Minimization by default
#        #elif isinstance(ind0, FitnessLike) and isinstance(ind1, FitnessLike):
#        #    return all((a <= b) for a, b in zip(ind0, ind1)) # Minimization by default
#        else:
#            raise ValueError("Unknown type of individuals")
    ...
    def update(self, iterable: Iterable, ignore_exceptions: bool = False, issue_warning: bool = True) -> int:
    ...
    def summary(self) -> str:
        """Return a summary description of the container."""
        res: str = f"Summary {self.__class__.__name__}:\n"
        res += f"  size: {self.size_str()}\n"
        if self.depot is not None:
            res += f"  depot_size: {len(self.depot)}\n"
        res += f"  best: {self.best}\n"
        res += f"  best_fitness: {self.best_fitness}\n"
        res += f"  best_features: {self.best_features}"
        return res
...
class Grid(Container):
    ...
    def _init_grid(self) -> None:
        ...
        self._items_per_bin = {x: 0 for x in self._index_grid_iterator()}
    ...
    def summary(self) -> str:
        ...
        res += f"  best_fitness: {self.best_fitness}\n"
        res += f"  best_features: {self.best_features}\n"
        res += f"  best_index: {self.best_index}\n"
        res += f"  shape: {self.shape}\n"
        res += f"  max_items_per_bin: {self.max_items_per_bin}\n"
        res += f"  filled_bins: {self.filled_bins}\n"
        res += f"  bins_size: {self._bins_size}"
        return res
...
class NoveltyArchive(Container):
    ...
    def summary(self) -> str:
        ...
        res += f"  best_fitness: {self.best_fitness}\n"
        res += f"  best_features: {self.best_features}\n"
        res += f"  k: {self.k}   threshold_novelty: {self.threshold_novelty}"
        return res
