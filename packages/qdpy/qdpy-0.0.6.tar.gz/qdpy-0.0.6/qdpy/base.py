...
class Factory(dict):
    ...
    def build(self, config: Optional[Mapping[str, Any]] = None, **kwargs: Any) -> Any:
        ...
        :param kwargs: Any
            Additional configuration entries to send to the `from_config` class method.
        """
        if "name" in config and config["name"] in self:
            return self[config["name"]]
        final_config: MutableMapping[str, Any] = dict(copy.deepcopy(config))
        return self._build_internal(final_config, {}, **kwargs)
