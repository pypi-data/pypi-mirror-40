...
class AlgorithmLogger(Saveable):
    ...
    def evals(self) -> pd.DataFrame:
        return pd.DataFrame(self._evals_data)

    @property
    def iterations(self) -> pd.DataFrame:
        return pd.DataFrame(self._iterations_data)
    ...
    def _init_cols(self, content: Sequence[Any]) -> None:
        ...
        self._cols_size = [max(a, b) for a, b in zip(self._cols_size, c_size)]
    ...
    def _tell(self, algo: QDAlgorithmLike, ind: IndividualLike) -> None:
        ...
        self._evals_data['max'].append(list(algo.container.best_fitness))
...
try:
    ...
    class TQDMAlgorithmLogger(AlgorithmLogger):
        ...
        def _tell(self, algo: QDAlgorithmLike, ind: IndividualLike) -> None:
            ...
            self._tqdm_pbar.set_postfix(iteration=algo.current_iter)
