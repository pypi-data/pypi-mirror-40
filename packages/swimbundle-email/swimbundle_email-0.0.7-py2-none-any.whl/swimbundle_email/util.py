    ...
    def parse(self):
        ...
        if self.exclude_attachment_extensions:
            ...
            attachments_data = filter(
                lambda attachment: not bool(unicode(attachment["filename"]).lower().endswith(self.exclude_attachment_extensions)),
                attachments_data
            )
        ...
        result["sender"] = sender if self._get_clean_emails(sender) else self._get_authenticated_sender(result['headers'])
        result["valid_sender_email"] = self._get_clean_emails(sender) or self._get_authenticated_sender(result['headers'])
...
class EmailUtil(object):
    ...
    @staticmethod
    def parse_email_set(iterable_emails, parser_cls, parser_options, email_hook=None):
        """
        ...
        for email in iterable_emails:
            ...
            try:
                if email_hook:  # Hook function for changing the email if needed
                    email = email_hook(email)
