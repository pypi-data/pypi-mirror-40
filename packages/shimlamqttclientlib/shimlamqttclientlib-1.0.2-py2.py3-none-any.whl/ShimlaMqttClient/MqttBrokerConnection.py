...
class Manager:
    ...
    def _connect_to_broker(self):
        try:
            ...
            self.mqtt_client = mqtt.Client(client_id=config['mqtt_client']['client_id'], clean_session=False,
                                           protocol=mqtt.MQTTv311, transport="tcp")
            self.mqtt_client.username_pw_set(config['mqtt_client']['username'],
                                             config['mqtt_client']['password'])
            ...
            self.mqtt_client.connect(self.current_mqtt_host, config['mqtt_broker']['port'],
        ...
        while True:
            if self.connected_to_broker:
                ...
                if not self.connected_to_broker:
                    # Try the other host.
                    if (self.current_mqtt_host == config['mqtt_broker']['host']):
                        self.current_mqtt_host = config['mqtt_broker']['fallback']
                    else:
                        self.current_mqtt_host = config['mqtt_broker']['host']
    ...
    def _update_connection_status(self, status):
        if status:
            ...
            if config['pi']['print_debug_messages']:
    ...
    def on_message(self, client, userdata, msg):
        print('{} payload = {}'.format(msg.topic, str(msg.payload)))
        self.publish_callback_handler.handle(msg)

    def on_publish(self, client, userdata, mid):
        logging.debug("mid: " + str(mid))

    def publish(self, msg, topic, qos, retain):
        ...
        with self.publish_lock:
            ...
            # print('Publish msg: {} to topic: {} complete'.format(str(msg), topic))
...
class PeriodicPublish:
    ...
    def run(self):
        while True:
            ...
            time.sleep(config['pi']['timers']['mqtt_publish_rate'])
