...
class Manager:
    ...
    def _connect_to_broker(self):
        try:
            ...
            logging.error('Failed to connect to {}'.format(self.current_mqtt_host))
        ...
        while True:
            if self.connected_to_broker:
                ...
                while (not self.connected_to_broker) and timeout_secs < max_timeout_secs:
                    logging.debug('current_mqtt_host = {}. Waiting {} of {} seconds ...'.format(self.current_mqtt_host,
    ...
    def on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            logging.info('Successfully connected to {}'.format(self.current_mqtt_host))
            ...
            logging.error('Broker connection refused. Code = {}'.format(rc))
            self.mqtt_client.loop_stop()

    def on_disconnect(self, client, userdata, rc):
        self._update_connection_status(False)
        logging.info('Broker disconnected. Code = {}'.format(rc))
        self.mqtt_client.loop_stop()
