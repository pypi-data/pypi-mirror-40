# https://github.com/tomdee/president
...
from mcts.ismcts import GameState, Card, ismcts, Deck
            ...
            if not self.on_the_table:
                ...
                if self.consecutive_mode:
                    if self.straight_length > 0:
                        ...
                        candidate_cards = [card for card in hand if
                                           card.rank >= minimum_rank and card.rank < minimum_rank + self.straight_length]
                    else:
                ...
                if not self.on_the_table or self.combo_size > 0 or self.straight_length > 0:
                    ...
                    for next_cards_index, next_card in enumerate(next_cards):
                        ...
                        if not self.on_the_table or self.straight_length > 0:
                            ...
                            if straight_found[-1].rank + 1 == next_card.rank and straight_found[
                                -1].suit == next_card.suit:
...
def test_iters():
    ...
    for iterations in (1, 10, 100, 1000, 10000):
        ...
        term.clear_eol()
        print
        "Iteration %s - %s" % (iterations, win_counts)
