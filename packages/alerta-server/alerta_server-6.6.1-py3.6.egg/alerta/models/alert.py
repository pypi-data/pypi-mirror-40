...
class Alert:
    ...
    def get_status_and_value(self):
        """
        Get most recent history entry that isn't a blackout status.
        """
        for h in self.get_alert_history(self, page=1, page_size=10):
            if h.status != 'blackout':
                return h.status, h.value

    # de-duplicate an alert
    def deduplicate(self) -> 'Alert':
        now = datetime.utcnow()

        previous_status, previous_value = self.get_status_and_value()
    ...
    def update(self) -> 'Alert':
        ...
        previous_status, _ = self.get_status_and_value()
