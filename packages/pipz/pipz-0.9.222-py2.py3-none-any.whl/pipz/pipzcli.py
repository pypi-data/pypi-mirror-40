...
import re
from ctxlib.path import splitall
from ctxlib.processing import run
...
@click.pass_context
def docker_tree(ctx, build, tag, dryrun, rebuild):
    import glob
            ...
            if n is not None:
                ...
                for x in n.upstream_nodes():
                    if '/' in x.value:
                        ...
                        if user == 'jmulla':
                            ...
                            run(f'docker build git@gitlab.com:autox/swarm.git#master:images/{image} -t {user}/{image}:{tag} -t {user}/{image}:latest', dry=dryrun)
                            run(f'pipz update-image-ifrequired {x.dockerfile} --tag {tag}', dry=dryrun)
                ...
                for x in n.downstream_nodes():
                    if '/' in x.value:
                        user, image = x.value.split('/')
                        if user == 'jmulla':
                            run(f'docker build {downstram_processing} git@gitlab.com:autox/swarm.git#master:images/{image} -t {user}/{image}:{tag} -t {user}/{image}:latest', dry=dryrun)
                            run(f'pipz update-image-ifrequired {x.dockerfile} --tag {tag}', dry=dryrun)
...
def run_test(ctx):
    run('dir')
    run('docker build  git@gitlab.com:autox/swarm.git#master:images/airf -t jmulla/airf:110 -t jmulla/airf:latest')
