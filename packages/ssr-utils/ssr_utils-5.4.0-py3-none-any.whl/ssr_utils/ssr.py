...
class SSR:
    def __init__(self):
        self._config = profig.Config('config.ini')
        self._config.init('path.python', '/usr/bin/python3')
        self._config.init('path.python_ssr', '/data/repo/shadowsocksr/shadowsocks/local.py')
        self._config.init('path.proxychains4', '/usr/local/bin/porxychains4')
        self._config.init('proxy.http', '')
        self._config.init('proxy.https', '')
        self._config.init('ssr_utils.proxychains4_cache_time', 300)
        self._config.sync()
    ...
    def pc4_conf_file(self):
        ...
        if os.path.exists(local_proxy_list_file):
            ...
            if os.path.exists(path_to_pc4_conf_file) and \
                    ...
                    < self._config['ssr_utils.proxychains4_cache_time']:
    ...
    def get_available(self):
        ...
        if pc4_conf_file:
            self._cmd = '{path_to_pc4} -q -f {pc4_conf_file} '.format(
                path_to_pc4=self._config['path.proxychains4'],
        ...
        self._cmd += '{python} {python_ssr} -c {path_to_config}'.format(
            python=self._config['path.python'],
            python_ssr=self._config['path.python_ssr'],
