...
class AltElement(object):
    ...
    def mobile_dragToElement(self, other_element, durationIndSeconds=600):
...
class AltrunUnityDriver(object):
    ...
    def wait_for_current_scene_to_be(self, scene_name, timeout=70, interval=1):
        t = 0
        current_scene = ''
        while (t <= timeout):
    ...
    def wait_for_element(self, name,camera_name='', timeout=30, interval=0.5):
        t = 0
        alt_element = None
        while (t <= timeout):
            try:
