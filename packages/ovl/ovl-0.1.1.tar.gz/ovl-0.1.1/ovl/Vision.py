...
class Vision(object):
    ...
    def get_contours(self, img, color=None, save=True):
        ...
        if color is not None:
            ...
            hsv_low_limit, hsv_high_limit = color.low, color.high
        else:
            hsv_low_limit, hsv_high_limit = self.color.low, self.color.high
    ...
    def apply_sample(self, **kwargs):
        ...
        """
        def cam_port_activate(port_val, mul_mode, color, wid, hgt):
            if self.camera is None:
                self.camera_setup(port_val, img_width=wid, img_height=hgt)
            return_val, image = self.camera.read()
            if return_val:
                if mul_mode:
                    self.contours = color.get_contours(image, self)
        ...
        def image_activate(image_loc, mul_mode, color):
            if type(image_loc) is str:
                image = cv2.imread(image_loc, cv2.IMREAD_COLOR)
            else:
                image = image_loc
            if mul_mode:
                self.contours = color.get_contours(image)
                self.apply_all_filters()
            else:
                self.contours = self.get_contours(image, color)
                self.apply_all_filters()
        ...
        if "camera_port" in kwargs:
            ...
            contours, img = cam_port_activate(port, multi_mode, color_object, self.width, self.height)
        elif "port" in kwargs:
            port = kwargs["port"]
            contours, img = cam_port_activate(port, multi_mode, color_object, self.width, self.height)
        elif "image" in kwargs:
            contours, img = image_activate(kwargs["image"], multi_mode, color_object)
        elif "img" in kwargs:
            contours, img = image_activate(kwargs["img"], multi_mode, color_object)
        elif "pic" in kwargs:
            contours, img = image_activate(kwargs["pic"], multi_mode, color_object)
        elif "picture" in kwargs:
            contours, img = image_activate(kwargs["picture"], multi_mode, color_object)
        else:
