...
class Vision(object):
    ...
    def get_contours_mask(self, mask, save=True):
        if cv2.__version__.startswith("3."):
            found_contours = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[1]
            if save:
            ...
            found_contours = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[0]
            if save:
                self.contours = found_contours
            return found_contours
        else:
    ...
    def apply_image_filters(self, img):
        for idx, img_filter in enumerate(self.image_filters):
            params = (img, ) + self.image_params[idx]
            img = img_filter(*params)
        return img
