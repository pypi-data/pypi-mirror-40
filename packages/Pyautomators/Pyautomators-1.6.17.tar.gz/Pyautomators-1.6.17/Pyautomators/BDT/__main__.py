...
import sys
import six
from Pyautomators.BDT import __version__
from Pyautomators.BDT.configuration import Configuration, ConfigError
from Pyautomators.BDT.parser import ParserError
from Pyautomators.BDT.runner import Runner
from Pyautomators.BDT.runner_util import print_undefined_step_snippets, reset_runtime, \
    InvalidFileLocationError, InvalidFilenameError, FileNotFoundError
from Pyautomators.BDT.textutil import compute_words_maxsize, text as _text
import shutil
...
def run_behave(config, runner_class=None):
    ...
    :param config:          Configuration object for Pyautomators.BDT.
    ...
    if config.lang_list:
        from Pyautomators.BDT.i18n import languages
        stdout = sys.stdout
        if six.PY2:
...
def print_formatters(title=None, stream=None):
    ...
    """
    from Pyautomators.BDT.formatter._registry  import format_items
...
def main(args=None):
    ...
    for arg in args:
        import re
        result=re.search('-o=\w{1,}.json', arg)
    ...
    if(value is not None):
        from TESTE.Documentacao import tranforma_cucumber
        try:
            tranforma_cucumber(value)
