...
from Pyautomators.BDT.model_core import \
        Status, BasicStatement, TagAndStatusStatement, TagStatement, Replayable
from Pyautomators.BDT.matchers import NoMatch
from Pyautomators.BDT.textutil import text as _text
if six.PY2:
...
class Step(BasicStatement, Replayable):
    ...
    def run(self, runner, quiet=False, capture=True):
        ...
        if not skip_step_untested:
            try:
                ...
                if self.status == Status.untested:
                    ...
                    # pylint: disable=redefined-variable-type
                    logging.info("...Step:   {}:{}:...Sucess!".format(str(self.keyword),self.name))
        ...
        if self.status == Status.failed:
            ...
            logging.info("...Step:   {}:{}:...Erro!\nmsg.:{}".format(self.keyword,self.name,self.error_message))
...
class Row(object):
    ...
    **indexed access**
    ...
    def as_dict(self):
        ...
        """
        from Pyautomators.BDT.compat.collections import OrderedDict
