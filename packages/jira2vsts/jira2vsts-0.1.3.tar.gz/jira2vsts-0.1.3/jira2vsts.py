...
@click.option('--send', '-s', 'issue_to_send', type=click.STRING, default=False, help="Send one jira by code")
@click.pass_context
...
def __process_project(logger, config, data, wit_client, jira, jira_project, PROJECTS, STATES, JIRA_BROWSE,
                      issue_to_send):
    start = time.time()
    ...
    issue_to_send, vsts_destination_issue = issue_to_send.split(',', 2)
    if vsts_destination_issue != vsts_project:
        return
    nbr_issues = 0
    ...
    if issue_to_send:
        ...
        jira_issues = jira.search_issues(
            'project=%s AND updated > "%s" ORDER BY updated ASC, createdDate ASC' % (jira_project, last_sync_formatted))
    else:
    ...
    {f.description}
        ...
        try:
            ...
            for work_item in work_items:
                ...
                if workitem_name and workitem_name.startswith(jira_item) and work_item.fields[
...
def _main(ctx, validate, logger, config, issue_to_send):
    ...
    for jira_project in JIRA_PROJECTS:
        t = Thread(target=__process_project,
                   args=(
                       logger, config, data, wit_client, jira, jira_project, PROJECTS, STATES, JIRA_BROWSE,
                       issue_to_send))
        t.setDaemon(True)
        t.start()
        threads.append(t)
