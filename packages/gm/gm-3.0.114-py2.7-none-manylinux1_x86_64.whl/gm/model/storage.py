...
from gm.csdk.c_sdk import BarLikeDict2, TickLikeDict2, gmi_now, py_gmi_get_cash, py_gmi_get_positions, \
    py_gmi_get_accounts, py_gmi_get_account_connections, py_gmi_set_timer
...
from gm.utils import adjust_frequency, gmsdklogger
...
class Context(object):
    ...
    def _add_bar2bar_data_cache(self, k, bar):
        ...
        if kk in self.bar_data_set:
            gmsdklogger.info("bar data %s 已存在, 跳过不加入", kk)
        else:
        ...
        if frequency == DATA_TYPE_TICK:
            if symbol in context.tick_data_cache:
                ...
                if count > len(symbol_tick_queue):
                    ...
                    for bd in tick_datas:
                        bd['created_at'] = bd['created_at'].replace(tzinfo=None)
            ...
            if k in context.bar_data_cache:
                ...
                if count > len(symbol_bar_queue):
                    ...
                    for bd in bar_datas:
                        bd['eob'] = bd['eob'].replace(tzinfo=None)
                        bd['bob'] = bd['bob'].replace(tzinfo=None)
