...
from gm.constant import CALLBACK_TYPE_TICK, CALLBACK_TYPE_BAR, \
    ...
    CALLBACK_TYPE_INIT
from gm.csdk.c_sdk import BarLikeDict2, TickLikeDict2, gmi_now
...
from gm.model.storage import context, BarWaitgroupInfo
...
from gm.utils import gmsdklogger, beijing_tzinfo
...
def bar_callback(data):
    ...
    if not context.has_wait_group:
        context.on_bar_fun(context, data)
        return
    ...
    if context.mode == MODE_BACKTEST:  # 处理回测模式下, wait_group = True
        ...
        if pre_eob_in_backtest is None:
            context._temporary_now = datetime.datetime.fromtimestamp(gmi_now()).replace(tzinfo=beijing_tzinfo)
    ...
    if context.mode == MODE_LIVE:  # 处理实时模式下, wait_group = True
        ...
        # 以eob做为key值, tick做为value值. 二级dict
        eob_tick_dict = bars_in_waitgroup_live[frequency]  # type: Dict[datetime.datetime, List[BarLikeDict2]]
        if eob not in eob_tick_dict:
            eob_tick_dict[eob] = [bar]
        else:
            eob_tick_dict[eob].append(bar)

        # 检查一下是否全部都到了. 到了的话触发一下
        if len(barwaitgroupinfo) == len(eob_tick_dict[eob]):
            gmsdklogger.info("实时模式下, wait_group的tick都到齐了, 触发on_bar. eob=%s", eob)
            context.on_bar_fun(context, eob_tick_dict[eob])
            del eob_tick_dict[eob]
            context.bar_data_cache[k].appendleft(bar)
        else:
            context.bar_data_cache[k].appendleft(bar)
...
def parameters_callback(data):
    if _or_not_none(context.on_parameter_fun, context.on_parameter_fun_v2):
        ...
        parameters = [protobuf_to_dict(p, including_default_value_fields=True, dcls=DictLikeParameter) for p in parameters.parameters]
    ...
    if (context.on_bar_fun is not None) and context.has_wait_group and context.is_live_model():
        ...
        for frequency, eob_tick_dict in six.iteritems(bars_in_waitgroup_live):
            ...
            if barwaitgroupinfo is not None:
                ...
                gmsdklogger.info("frequency =%s 已全部退订", frequency)
...
def trade_disconnected_callback():
    if context.on_trade_data_disconnected is not None:
        context.on_trade_data_disconnected(context)
