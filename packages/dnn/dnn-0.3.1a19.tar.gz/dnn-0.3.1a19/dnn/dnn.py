...
from . import overfit, optimizers, predutil, result, normalizer
import sys
...
class DNN:
    ...
    def set_train_dir (self, path, reset = False, auto_save = True, improve_metric = "cost", performance_threshold = 0.0):
        ...
        if not (improve_metric == "cost" or improve_metric.startswith ("performance")): 
            raise ValueError ("improve_metric should be one of cost|performance[:index]")                
        self.metric = improve_metric
        if self.metric.startswith ("performance"):
            try: 
                self.metric = int (self.metric [12:])
            except ValueError:
                self.metric = -1
    ...
    def logp (self, name, val):
        self.log (name, val, self.is_validating and "valid" or "resub")
    ...
    def restore (self, for_train  = False):     
        ...
        if for_train:
            with open (os.path.join (self.train_dir, "objects"), "rb") as f:
                global_step, self.max_performance, self.overfitwatch, self.cherry = pickle.load (f)
            self.create_trainables (global_step)            
        with self.graph.as_default ():
            self.init_session()
        with self.graph.as_default ():            
    ...
    def save (self, filename = None):
        ...
        with open (os.path.join (self.train_dir, "objects"), "wb") as f:
            pickle.dump ((self.eval (self.global_step), self.max_performance, self.overfitwatch, self.cherry), f)
    ...
    def maybe_save_checkpoint (self, performance):
        if isinstance (performance, (tuple, list)):
            performance = performance [self.metric == "cost" and -1 or self.metric]
    ...
    def add_performance (self):
        ...
        try:
            self.performance = self.performance_metric ()
        except TypeError: # OK Implemented
            self.performance = tf.constant (0.0)
            result.Result._perfmetric_func = self.performance_metric
        except NotImplementedError:
            self.performance = tf.constant (0.0)
    
    def create_trainables (self, global_step = 0):
        if not self.summaries_dir:
    ...
    def evaluate (self, x, y, is_training, ops, *args, **kargs):
        ...
        performances = []
        opsres = []
        ops = [self.logit, self.cost, self.performance] + ops
        ...
        r = result.Result (
            ...
            np.mean (costs), np.mean (performances),
            np.concatenate(opsres, 0), kargs, 
            self.epoch, global_step,            
            is_training, self.on_ftest
        )
        if not self.on_ftest:
    ...
    def batch (self, x, y, ops = None, **kargs):
        ...
        return result.Result (
            ...
            self.epoch,  global_step, True, False
        )
    ...
    def valid (self, x, y, ops = [], **kargs):
        ...
        self.maybe_save_checkpoint (r.performance)
    ...
    def lstm (
            self, n_input, hidden_size, lstm_layers = 1, 
        ...
        for i in range (lstm_layers):            
            lstm = tf.nn.rnn_cell.LSTMCell (hidden_size, activation = activation)
            if dropout:
