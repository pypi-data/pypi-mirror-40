...
class DNN:
    ...
    def set_train_dir (self, path, reset = False, auto_save = True, improve_metric = "cost", accuracy_threshold = 0.0):
    ...
    def restore (self, for_train  = False):     
        ...
        if for_train:
            with open (os.path.join (self.train_dir, "objects"), "rb") as f:
                global_step, self.max_acc, self.overfitwatch, self.cherry = pickle.load (f)
    ...
    def save (self, filename = None):
        ...
        with open (os.path.join (self.train_dir, "objects"), "wb") as f:
            pickle.dump ((self.eval (self.global_step), self.max_acc, self.overfitwatch, self.cherry), f)
    ...
    def add_accuracy (self):
        try:
            ...
            result.Result._accfunc = self.custom_accuracy
    ...
    # custom accuracy handler ----------------------------------------------    
    def custom_accuracy (self, r):
        return self.calculate_accuracy (r)
