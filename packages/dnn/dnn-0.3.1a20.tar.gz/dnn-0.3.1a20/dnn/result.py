...
class Result:
    ...
    def __init__ (self, xs, ys, logits, cost, ops, args, epoch, global_step, is_validating, is_evaluating, on_ftest):
        self.xs = xs
        self.ys = ys
        self.logits = logits
        self.cost = cost        
        ...
        if self.is_evaluating and self._perfmetric_func: # no run within batch steps
            ...
            for p in performance:
                if isinstance (p, (list, tuple)):
                    ...
                    self.performance_names.append (None)
    ...
    def __make_summary (self, d = {}):
        ...
        if len (self.performance) == 1:        
            d ["eval/perf"] = self.performance [0]
        else:      
            for idx, val in enumerate (self.performance):
                try: 
                    name = self.performance_names [idx]
                except TypeError:
                    try:
