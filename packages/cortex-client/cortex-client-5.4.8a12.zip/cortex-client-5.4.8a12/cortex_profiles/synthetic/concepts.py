import itertools
from typing import Set, List, Mapping, Optional
...
class CortexConceptsProvider(BaseProviderWithDependencies):
    ...
    def set_of_concepts(self, concept_limits:Mapping[str,int]) -> Set[Link]:
        """
        ...
        max_number_of_choosable_concepts_per_type = {
            concept_type: min(concept_limits.get(concept_type, 0), len(concepts))
        ...
        concept_choices = [
            list(self.fake.random.choice(list(
                itertools.combinations(
                    concepts,
                    self.fake.random.choice(range(0, max_number_of_choosable_concepts_per_type[concept_type] + 1))
                )
            )))
            for concept_type, concepts in self.indexed_concepts_to_choose_from.items()
        ]
