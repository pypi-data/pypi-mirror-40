...
from cortex_profiles.types.profiles import ProfileCommit, Profile, ProfileSnapshot
...
class InternalProfilesClient(_Client):
    ...
    def __init__(self, url:str, version:str, token:str, environmentId:str="cortex/default"):
        ...
        self.mongoclient = pymongo.MongoClient(secret["mongo_uri"])
        self.mongodb = self.mongoclient["cortex-graph"]
        ...
        # self.profiles_collection = self.mongodb["profiles"]
        # self.locks_collection = self.mongodb["locks"]
    ...
    def flush_profiles(self):
        ...
        # self.flush_collection(self.profiles_collectio)
        # self.flush_collection(self.locks_collectio)
    ...
    def list_profiles(self, query:Optional[dict]=None) -> List[dict]:
        return list(map(
            lambda x: x["_id"],
            self.attributes_collection.aggregate(
                [
    ...
    def find_latest_snapshot_for_profile(self, profileId: str) -> ProfileSnapshot:
        ...
        snapshot = utils.drop_from_dict(self.snapshots_collection.find_one(query), ["_id"])
