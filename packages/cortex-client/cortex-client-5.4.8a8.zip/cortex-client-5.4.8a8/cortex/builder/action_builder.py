...
class ActionBuilder:
    ...
    def _build_job(self, dry_run=False):
        if self._code_s:
            if not dry_run:
                # Build new docker image with the given action source code
                self._deploy_job_image(self._code_s, 'main', '')
        elif self._model:
            # Deploy from ML model object
            self._deploy_model(dry_run)
        else:
    ...
    @staticmethod
    def _docker_build_and_push(build_dir, repository, auth_config=None):
    ...
    def _deploy_daemon_image(self, source, func_name, global_code):
        ...
        try:
            ...
            self._docker_build_and_push(temp_dir, self._get_docker_repository(), self._get_docker_auth())
    ...
    def _deploy_job_image(self, source, func_name, global_code):
        temp_dir = tempfile.mkdtemp()
        try:
            ...
            # Deploy the Cortex Action
            # TODO: pass down vcpus and memory params
            self._client.deploy_action(self._name, self._kind, self._get_docker_repository(use_prefix=True),
                                       action_type=_action_job)
        finally:
            # cleanup temp
    ...
    def _deploy_model(self, dry_run=False):
        ...
        if self._action_type == _action_daemon:
            ...
            self._deploy_job_image(_predict_source, 'predict', _globals)
        else:
            if dry_run:
