...
class retry():
    ...
    def _decorator(self, *args, **kw):
        attempt_count = self.max_attempt_count
        while True:
            try:
                result = self.func(*args, **kw)
                if self.result_retry_flag is not None and result is self.result_retry_flag:
                    raise result_is_retry_flag_err('返回值是重试标记')
                return result
            except:
                if self.error_callback is not None:
                    self.error_callback(self.func)
