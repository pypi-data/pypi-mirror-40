...
def shift_image(image1, image2, accuracy = 100):
    """
    Laterally shifts the second image to match with the 1st image. 
    ...
    back_image = fourier_shift(numpy.fft.fftn(image2), shift)
    back_image = numpy.fft.ifftn(back_image).real
    return back_image


def shift_image_CTF(image1, image2, beta_delta, fresnelNumber1, fresnelNumber2, accuracy=100, zero_compensation=0.01):
    """
    Laterally shifts the second image to match with the 1st image. The shift is calculated after the CTF_retrieval. Returns the second image
    ...
    shift, error, diffphase = register_translation(image1_CTF, image2_CTF, accuracy)
    back_image = fourier_shift(numpy.fft.fftn(image2), shift)
    back_image = numpy.fft.ifftn(back_image).real
...
def shift_one_imageset(image, beta_delta, fresnelN, accuracy, zero_compensation):
    #shifts images for the image-set
    out = numpy.zeros(image.shape, dtype = image.dtype)
    for i in numpy.arange(out.shape[0]):
        out[i] = shift_image_CTF(image[0], image[i], beta_delta, fresnelN[0], fresnelN[i], accuracy, zero_compensation)
...
def multi_distance_CTF(image, beta_delta, fresnelNumber, zero_compensation):
    ...
    for i in arange(image.shape[0]):
        nominator, denominator, f = ctf_mono((image.shape[1],image.shape[2]), fresnelNumber[i], fresnelNumber[i], beta_delta, zero_compensation)
...
# =============================================================================
#     в этих модулях выше получается не очень понятно как считать cutoff и f для нескольких имиджей - fresnelx же меняется если несколько имиджей
# =============================================================================
