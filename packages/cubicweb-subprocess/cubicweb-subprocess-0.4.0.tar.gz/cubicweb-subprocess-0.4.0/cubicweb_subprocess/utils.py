...
def communicate(process, chunk_size=4096):
    ...
    the outputs of the ``process``. So one gets stdout and stderr from process
    while it writes to them without blocking the main process.
    The reason for not using `select` is to ensure Windows portability.
