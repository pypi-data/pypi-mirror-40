...
class Schema(object):
    ...
    def study_id_order(self, study_id_order=None):
        if study_id_order:
            study_id_difference = (set(study_id_order) - set(self.studies['study_id']))
            if study_id_difference != set():
                warn("study_id %s is not in the studies table" % (study_id_difference))

            study_id_difference = (set(self.studies['study_id']) - set(study_id_order))
            if study_id_difference != set():
                warn("study_id %s is in the studies table, but not provided in study_id_order" % (study_id_difference))

            study_id_order = study_id_order
        else:
            study_id_order = self.studies.sort_values('start_date', ascending=False).study_id.tolist()
    ...
    def question_id_order(self, study_id_order=None, question_id_order=None):
        ...
        if question_id_order:
            question_id_difference = (set(question_id_order) - set(self.questions['question_id']))
            if question_id_difference != set():
                warn("question_id %s is not in the questions table.\
                     Will use default question_id_order instead." % (question_id_difference))
                question_id_order = None
            else:

                question_id_difference = (set(self.questions['question_id']) - set(question_id_order))
                if question_id_difference != set():
            ...
            study_id_order_sorter = self.study_id_order_sorter(study_id_order=study_id_order)

            _question_orders = self.question_orders.copy()
            _question_orders['study_id_order'] = _question_orders['study_id'].map(study_id_order_sorter)
            question_id_order = _question_orders.sort_values(['study_id_order','order'])\
                                .drop_duplicates(subset=['question_id']).question_id.tolist()
        return question_id_order

    def question_id_order_sorter(self,study_id_order=None,question_id_order=None):
        if question_id_order:
            question_id_order = question_id_order
        else:
    ...
    def codebook_for_dataset(self, question_id_order_sorter=None):
        ...
        codebook = self.response_choices[['question_id','value','full_label','display_label']]\
                       .rename(columns={'value': 'value_code'})
        codebook['question_id_order'] = codebook['question_id'].map(question_id_order_sorter)
    ...
    def question_metadata_for_dataset(self, question_id_order_sorter=None):
        ...
        question_metadata = self.questions[['question_id','full_text','display_text','category',
                                            'subcategory','topic','subtopic','type_of_response',
                                            'scale_id']]\
                                .merge(scales_info,how='left',on='scale_id')
        question_metadata['question_id_order'] = question_metadata['question_id'].map(question_id_order_sorter)
        question_metadata = question_metadata.sort_values('question_id_order')\
                                             .drop('question_id_order', axis=1)\
                                             .reset_index(drop=True)
    ...
    def data_for_dataset(self, study_id_order=None, question_id_order=None):
        ...
        pivoted_closed_ended_responses = self.closed_ended_responses.pivot_table(
            values='value',index=['study_id','respondent_id'],columns='question_id')
        pivoted_open_ended_responses = self.open_ended_responses.pivot_table(
            values='text_response',index=['study_id','respondent_id'],columns='question_id',aggfunc= lambda x: x)

        data = pivoted_closed_ended_responses\
                    .merge(pivoted_open_ended_responses,how='outer',on=['study_id','respondent_id'])\
                    .reset_index()
        data = data\
                    .merge(self.studies[['study_id','start_date']],how='left',on='study_id')\
                    .rename(columns={'start_date': 'study_start_date'})
        data = data[['study_id','study_start_date','respondent_id']+\
            self.question_id_order(study_id_order=study_id_order, question_id_order=question_id_order)]
    ...
    def to_dataset(self, study_id_order=None, question_id_order=None):
        study_id_order_for_dataset=self.study_id_order(study_id_order=study_id_order)
        ...
        data = self.data_for_dataset(study_id_order=study_id_order_for_dataset, question_id_order=question_id_order_for_dataset)
        codebook = self.codebook_for_dataset(question_id_order_sorter=question_id_order_sorter)
        question_metadata = self.question_metadata_for_dataset(question_id_order_sorter=question_id_order_sorter)
