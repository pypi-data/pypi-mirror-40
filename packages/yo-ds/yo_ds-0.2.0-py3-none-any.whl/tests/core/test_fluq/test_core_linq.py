...
class GeneralSelectorsTests(LinqTestBase):
    ...
    def test_select(self):
        self.check(Query.args(1, 2).select(str), '1', '2')

    def test_select_many(self):
        self.check(Query.args(1, 2, 3).select_many(lambda z: range(z)), 0, 0, 1, 0, 1, 2)

    def test_distinct(self):
        self.check(Query.args(1,2,2,3,3).distinct(),1,2,3)

    def test_distinct_1(self):
        self.check(Query.args('aa','ab','b').distinct(lambda z: z[0]),'aa','b')

    def test_where(self):
        self.check(Query.args(1, 2, 3, 4).where(lambda z: z % 2 == 0), 2, 4)
    ...
    def test_cast_2(self):
        self.check(Query.args(1,2,3).cast(int),1,2,3)
