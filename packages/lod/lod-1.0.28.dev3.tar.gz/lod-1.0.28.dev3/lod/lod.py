...
def add_output_file(file_name):
    ...
    Note that newly created objects can only be found with the object_manager after running :meth:`lod.lod.make_output_available`. This is because objects can be created via method-chaining, so it is not clear when the object is fully created.
    """
    _initialize_if_not_done_yet()
...
def display_message():
    """
    Returns a :class:`lod.basics.MessageBuilder`, which is used for building a :class:`lod.basics.Message` to display to the user,
...
def make_output_available():
    """
    Finalizes all previously added objects and makes them available to the object_manager.
    """
    global _output_objects
    global _finalized_output_objects
    _initialize_if_not_done_yet()
    ...
    # Finalize all output objects
    for obj in _output_objects:
        if isinstance(obj, basics.TagBuilder):
            obj_dict = obj.content.to_json()
        else:
            obj_dict = obj.to_json()
        print(obj_dict)
        print(type(obj_dict))
        if 'argument_identifiers' in obj_dict:
            for a in obj_dict['argument_identifiers']:
                print(a)
                print(type(a))
        cleaned_object = basics.parse_object_according_to_type(obj_dict['identifier']['type'], obj_dict)
        _finalized_output_objects.append(cleaned_object)
    _output_objects = []
...
def save_output_to_file():
    ...
    Note that this calls :meth:`lod.lod.make_output_available` as a subroutine.
    """
    global _finalized_output_objects
    make_output_available()
    # add all the output objects to the file
    res = {
        'output_objects' : [a.to_json() for a in _finalized_output_objects],
    }
    # write to file
