...
def add_output_file(file_name):
    ...
    Note that newly created objects can only be found with the object_manager after running :meth:`lod.make_output_available`. This is because objects can be created via method-chaining, so it is not clear when the object is fully created.
    """
    _initialize_if_not_done_yet()
