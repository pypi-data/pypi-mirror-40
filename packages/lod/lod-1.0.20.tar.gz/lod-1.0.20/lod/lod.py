...
def _add_output_object(obj):
    ...
    The object (1) is added to the object_manager and therefore becomes available right away and (2) is added to a list of output objects to be verified by the server. This means that the object added to the object_manager immediately is not the same as the one that will be added later on the server to replace it. In particular, it does not get parsed and its missing fields filled out.
    """
