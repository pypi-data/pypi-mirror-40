...
def _add_output_object(obj):
    ...
    _message_being_built_check(0)
    _output_objects.append(obj)
    get_object_manager().add_object(cleaned_object)


_message_builder_counter = 0
def _message_being_built_check(c):
    """
    ...
    """
    global _message_builder_counter
    _message_builder_counter += c
    if (c == 0 and _message_builder_counter != 0) or (c == 1 and _message_builder_counter != 1):
...
def tag(symbol, comment=None, weight=None, arguments=None, also_create_signal=False):
    ...
    if arguments is None:
        arguments = []
    tag = basics.Tag(create_preliminary_identifier('tag'), arguments, symbol_name=symbol, comment=comment, weight=weight)
...
def start_message():
    ...
    def callback(message_builder):
        ...
        message_builder = basics.parse_message(message_builder.to_json())
        _message_being_built_check(-1)
        _add_output_object(message_builder)
        return request
    _message_being_built_check(1)
    res = basics.MessageBuilder(callback=callback)


def option(name, description, trigger, display, actions, existing_variables=None):
    """
