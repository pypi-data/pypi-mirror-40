...
def simulate_one_term(qubit_list, hamiltonian_term, coef, t):
    ...
    hamiltonian_term=PauliOperator.parse_pauli(hamiltonian_term)
    ...
    for single_term in hamiltonian_term:        
        if single_term[0] is 'X':            
            transform.insert(H(qubit_list[single_term[1]]))            
            actual_qlist.append(qubit_list[single_term[1]])
        elif single_term[0] is 'Y':
            transform.insert(RX(qubit_list[single_term[1]],pi/2))            
            actual_qlist.append(qubit_list[single_term[1]])
        elif single_term[0] is 'Z':               
            actual_qlist.append(qubit_list[single_term[1]])
    prog.insert(transform)\
        .insert(simulate_z_term(actual_qlist, coef, t))\
...
def simulate_hamiltonian(qubit_list,pauliOperator,t,slices=3):
    ...
    for i in range(slices):
        for op in pauliOperator.ops:
            prog.insert(simulate_one_term(qubit_list,op,
                pauliOperator.ops[op],t/slices
            ))
