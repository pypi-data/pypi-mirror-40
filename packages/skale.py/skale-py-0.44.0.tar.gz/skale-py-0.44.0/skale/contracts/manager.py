...
class Manager(BaseContract):
    ...
    def create_node(self, ip, port, name, wallet):
        ...
        transaction_data = self.create_node_data_to_bytes(ip, port, name, pk, skale_nonce)
    ...
    def create_node_data_to_bytes(self, ip, port, name, pk, nonce):
        pk_fix = str(pk)[2:]
        ...
        data_bytes = type_bytes + port_bytes + nonce_bytes + ip_bytes + pk_bytes + name_bytes
    ...
    def create_schain(self, lifetime, type_of_nodes, deposit, name, wallet):
        ...
        token = self.skale.get_contract_by_name('token')
        skale_nonce = Helper.generate_nonce()
        transaction_data = self.create_schain_data_to_bytes(lifetime,
    ...
    def create_schain_data_to_bytes(self, lifetime, type_of_nodes, name, nonce):
