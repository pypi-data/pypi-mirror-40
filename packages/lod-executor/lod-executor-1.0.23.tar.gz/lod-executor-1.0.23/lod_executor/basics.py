...
class Tag(object):
    ...
    """
    def __init__(self, own_identifier, arguments, symbol_name=None, symbol_identifier=None, comment=None, weight=None, creation_trigger=None, creator_id=None):
...
class TagBuilder():
    ...
    def signal(self):
        ...
        The !set_signal_weight Tag will copy the weight and comment of this Tag.
        """
...
class ObjectManager(object):
    ...
    def find_latest_tag(self, symbol_name, min_weight=None, max_weight=None, comment=None, step=None, arguments=[], not_nullified=True):
        """
        Returns the latest Tag that matches the criteria, or None if no match can be found.
        Can perform an exact match on the comment, check if the weight is within some bounds, check if the step in which the Tag was created equals some specified step, and check if an argument is equal to a required argument.
        ...
        for candidate_identifier in reversed(self._identifiers):
            ...
            if comment is not None and candidate_tag.comment != comment:
                continue
            if step is not None and self.get_step_number_of_addition(candidate_identifier) != step:
                continue
    ...
    def is_reserved(self, obj_or_identifier, reserve_tag_symbol=None):
        """
        Checks an object to find out if there is a reserve_ Tag targeting it in its first position.
        reserve_ Tags with !nullify on them don't count.
        Optionally, you can specify a particular reserve_ Tag instead of any arbitrary one.
        """
    ...
    def task_is_finished(self, task_tag_or_identifier):
        """
        Checks a task\_ Tag or a !task Tag to find out if the task is finished.
        ...
        Can raise an InvalidParamsException.
        """
        # if this is the !task tag, get the corresponding task_ tag.
        ...
        if isinstance(task_tag_or_identifier, Tag) and task_tag_or_identifier.symbol_name == '!task':
            task_tag_or_identifier = self.get_object_for_identifier(task_tag_or_identifier.argument_identifiers[0])
        ...
        if not isinstance(task_tag_or_identifier, Tag) or not task_tag_or_identifier.symbol_name.startswith('task_'):
            raise InvalidParamsException("The argument must be a !task tag or a tag beginning with the prefix 'task_'.")
