...
class DatabricksStep(PipelineStep):
    ...
    :type compute_target:
    str
    azureml.core.compute.databricks.DatabricksCompute
    ...
    def _get_libraries_from_runconfig(runconfig):
        """Extracts libraries from runconfig
        :param runconfig: Runconfig to use
        :type runconfig: azureml.core.runconfig.RunConfiguration
    ...
    def _serialize_maven_libraries(maven_libraries):
        return ";".join('coordinates={0}|repo={1}|exclusions={2}'
                        .format(m.coordinates, m.repo, ",".join(m.exclusions)) for m in maven_libraries)

    @staticmethod
    def _serialize_pypi_libraries(pypi_libraries):
        return ";".join('package={0}|repo={1}'.format(p.package, p.repo) for p in pypi_libraries)

    @staticmethod
    def _serialize_rcran_libraries(rcran_libraries):
        return ";".join('package={0}|repo={1}'.format(r.package, r.repo) for r in rcran_libraries)
