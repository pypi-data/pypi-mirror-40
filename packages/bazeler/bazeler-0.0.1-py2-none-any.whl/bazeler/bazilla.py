#!/usr/bin/env python
'''
Bazilla: A bazel management tool. This writes WORKSPACE files for each
        ...
        with open(fragment, 'r') as handle:
            ...
            if build_file:
                ...
                if not path.exists(BUILD):
                    # assert path.exists(BUILD), '[{}] does not exist.'.format(BUILD)
...
def sources(root):
    ...
    for file in files:
        with open(file, 'r') as handle:
            ...
            manifests[entry['name']] = LOCAL_REPOSITORY_TEMPLATE.format(entry['name'], path.dirname(file))
    return manifests
...
def write(target, manifests, permissive=False, ignore=[]):
    assert target
    ...
    for dependency in dependencies:
        if dependency in set(ignore):
            logging.info('Ignoring: [{}]'.format(dependency))
            continue
...
def bazilla(root, offline=False, search_paths=None, verbose=False, permissive=False, ignore=''):
    if verbose:
    ...
    targets_to_ignore = []
    if (ignore):
        targets_to_ignore = ignore.split(',')
    ...
    if not outputs:
        ...
        return 1
    for sink in outputs:
        write(sink, manifests, permissive, targets_to_ignore)
