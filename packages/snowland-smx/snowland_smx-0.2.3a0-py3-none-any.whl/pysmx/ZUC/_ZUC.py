...
def addition_uint31(a, b):
    c = np.uint32(a) + np.uint32(b)
    return np.uint32(c & 0x7FFFFFFF) + np.uint32(c >> 31)
...
class ZUC(object):
    def __init__(self, key, iv):
        ...
        self.x = np.zeros(4, dtype=np.uint32)
    ...
    def zuc_init(self, key, iv):
        # Expand key.
        self.lfsr = npa([make_uint31(key[i], D[i], iv[i]) for i in range(16)], dtype=np.uint32)
        self.r = [0, 0]
    ...
    def zuc_encrypt(self, input):
        ...
        if isinstance(input, list):
            input = npa(input, dtype=np.uint32)
        length = len(input)
        key_stream = self.zuc_generate_keystream(length)
        return list(input ^ key_stream)
