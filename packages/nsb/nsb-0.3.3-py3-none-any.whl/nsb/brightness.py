...
class Brightness():
    ...
    def calculate(self):
        ...
        ax.scatter(np.array(sourcealt), np.array(separation), bright)
        #plt.scatter(bright, separation, s=1, label='Separation')
       # plt.scatter(bright, moonalt, s=1, label='Moon alt')
       # plt.scatter(bright, sourcealt, s=1, label='Source alt')
        ax.set_xlabel('Source alt')
        ax.set_ylabel('Separation')
        ax.set_zlabel('Brightness')
        plt.legend()
        plt.draw()
        m = np.array(moonphase)
        b = np.array(bright)
        s = np.array(separation)
        ma = np.array(moonalt)
        sa = np.array(sourcealt)
        #print(m[(m > 16) & (m < 17)])
        fig = plt.figure()

        #select = (np.where(np.array(moonphase) > 40 and np.array(moonphase) < 40))
        plt.scatter(s[(m > 40) & (m < 60) & (ma > 40) & (ma < 60) & (sa > 40) & (sa < 60)],
                    b[(m > 40) & (m < 60) & (ma > 40) & (ma < 60) & (sa > 40) & (sa < 60)], label='Phase 50')
        plt.scatter(s[(m > 60) & (m < 80) & (ma > 40) & (ma < 60) & (sa > 40) & (sa < 60)],
                    b[(m > 60) & (m < 80) & (ma > 40) & (ma < 60) & (sa > 40) & (sa < 60)], label='Phase 70')
        plt.scatter(s[(m > 80) & (m < 100) & (ma > 40) & (ma < 60) & (sa > 40) & (sa < 60)],
                    b[(m > 80) & (m < 100) & (ma > 40) & (ma < 60) & (sa > 40) & (sa < 60)], label='Phase 90')
