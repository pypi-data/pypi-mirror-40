...
@click.option("--name", "-n", help="Name which the downloaded file will be saved under.")
...
def executeDownload(name, verbose, download_url):
    ...
    run = tachydownload(downloadUrl=download_url, fileName=filename, fileSize=filesize, verboseState=verbose)
    try:
        run.main()
    except Exception:
        print("Error: Connection was broken by the host server.")
        run.cleanHiddenFiles()
        sys.exit()
