...
class BlockFactory:
    ...
    def __call__(self, input_size: int, in_channels: int, out_channels: int, stride: int):
        if self.skip:
            return self.skip_block_func(
                input_size=input_size, in_channels=in_channels, out_channels=out_channels,
                stride=stride, batch_norm_2d=self.batch_norm_2d)
        else:
            return self.mobile_block_func(
                input_size=input_size, in_channels=in_channels, out_channels=out_channels,
                stride=stride, expansion=self.expansion, kernel=self.kernel, groups=self.groups,
                batch_norm_2d=self.batch_norm_2d, relu=self.relu, residual=False)
    ...
    def factory(cls, block_name: str,
        ...
        if block_name == 'SkipBlock':
            return BlockFactory(skip=True, batch_norm_2d=batch_norm_2d, relu=relu,
