...
def routes(origen, destino, fecha=None, hini='00:00', hfin='23:59'):
    ...
    k_station_orig, v_station_orig = key_station(origen)
    k_station_dest, v_station_dest = key_station(destino)
    ...
    route['fromStationText'] = v_station_orig
    route['toStationText'] = v_station_dest
    ...
    for transbordo in transbordos:
        for r in re.findall('De (.*) a (.*)<', str(transbordo)):
            journey = dict()
            journey['journeyId'] = j
            k_station, v_station = key_station(r[0])
            journey['journeyFromStation'] = k_station
            k_station, v_station = key_station(r[1])
            journey['journeyToStation'] = k_station
...
def plan(origen, destino, fecha=None, hora='00:00', tipohora='S'):
    ...
    p['fromStationText'] = v_station_orig
    p['toStationText'] = v_station_dest
    p['zoneTickets'] = ''
    p['journey'] = list()
    p['hour'] = hora
    ...
    for i in range(1, len(steps) - 1):
        ...
        for r in re.findall(rx, str(steps[i])):
            k_station, v_station = key_station(str(r[2]))
            journey['journeyFromStation'] = k_station
