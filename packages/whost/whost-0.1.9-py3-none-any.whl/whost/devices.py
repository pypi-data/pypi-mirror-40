...
def get_writer(slot, writer_conf):
    """ device, name, slot combination. Updates conf if missing. """
    device = get_block_name(Path(writer_conf["device_path"]))
    if device is None:
        return None
    writer_conf.update(
        {"device": device, "name": get_display_name(device), "slot": slot}
    )
    return writer_conf
