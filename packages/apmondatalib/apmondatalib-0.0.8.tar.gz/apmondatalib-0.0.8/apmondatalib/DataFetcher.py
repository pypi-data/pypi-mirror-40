import sys
from apmondatalib.common import Connection
from apmondatalib.common import Constants
import datetime
import pymongo
...
class SensorRawDataFetcher:
    ...
    def __init__(self, host, port, database, sensor_id, default_page_size, time_offset):
        ...
        self.mongo = Connection.create_mongodb_connection(host, port)
    ...
    def read_humidity(self, page_number, page_size=None):
        ...
        return self.read(Constants.SensorType.Humidity, page_number, page_size)

    def read_temperature(self, page_number, page_size=None):
        if page_size is None:
            page_size = self.default_page_size

        return self.read(Constants.SensorType.Temperature, page_number, page_size)

    def read_light(self, page_number, page_size=None):
        if page_size is None:
            page_size = self.default_page_size

        return self.read(Constants.SensorType.Light, page_number, page_size)

    def read_motion(self, page_number, page_size=None):
        if page_size is None:
            page_size = self.default_page_size

        return self.read(Constants.SensorType.Motion, page_number, page_size)

    def count_humidity(self):
        return self.count(Constants.SensorType.Humidity)

    def count_temperature(self):
        return self.count(Constants.SensorType.Temperature)

    def count_light(self):
        return self.count(Constants.SensorType.Light)

    def count_motion(self):
        return self.count(Constants.SensorType.Motion)

    def count_total_humidity_pages(self, page_size):
        return self.count_total_pages(Constants.SensorType.Humidity, page_size)

    def count_total_temperature_pages(self, page_size):
        return self.count_total_pages(Constants.SensorType.Temperature, page_size)

    def count_total_light_pages(self, page_size):
        return self.count_total_pages(Constants.SensorType.Light, page_size)

    def count_total_motion_pages(self, page_size):
        return self.count_total_pages(Constants.SensorType.Motion, page_size)
    ...
    def read_humidity_in_range(self, from_date, to_date, page_number, page_size=None):
        ...
        return self.read_in_range(Constants.SensorType.Humidity, from_date, to_date, page_number, page_size)

    def read_temperature_in_range(self, from_date, to_date, page_number, page_size=None):
        if page_size is None:
            page_size = self.default_page_size

        return self.read_in_range(Constants.SensorType.Temperature, from_date, to_date, page_number, page_size)

    def read_light_in_range(self, from_date, to_date, page_number, page_size):
        if page_size is None:
            page_size = self.default_page_size

        return self.read_in_range(Constants.SensorType.Light, from_date, to_date, page_number, page_size)

    def read_motion_in_range(self, from_date, to_date, page_number, page_size):
        if page_size is None:
            page_size = self.default_page_size

        return self.read_in_range(Constants.SensorType.Motion, from_date, to_date, page_number, page_size)
...
class DailyDataFetcher:
    ...
    def append_prediction(self, sensor_type, year, month, day, prediction_set):
        result = self.collection.update_one({"sensorId": self.sensor_id,
                                    ...
                                    "type": sensor_type},
                                   {"$set": {"predictionSet": prediction_set}})
