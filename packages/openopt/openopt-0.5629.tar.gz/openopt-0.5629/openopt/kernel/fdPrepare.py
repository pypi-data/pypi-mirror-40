...
#                p.x0 = dict([(key, p.x0[key]) for key in D2])
    ...
    # TODO: get rid of atleast_1d in future, currently "iteration over a 0-d array" bug is triggered otherwise
    f_elems = [F_] if isinstance(F_, oofun) else atleast_1d(F_.view(ndarray)) if isinstance(F_, ooarray) else F 
