    ...
    if fv is None or (type(fv) in [list, tuple] and (len(fv)==0 or fv[0] is None)):
        ...
        for func in fv:
            if isinstance(func, (list, tuple, set, ndarray)):
                for elem in func:
                    number += asarray(getVal(elem, X, p, args, kwargs)).size
            else:
                number += asarray(getVal(func, X, p, args, kwargs)).size
            arr.append(number)
        ...
        FV = fv[0] if type(fv) in [list, tuple] else fv
        tmp = getVal(FV, X, p, args, kwargs)
