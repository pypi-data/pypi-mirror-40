...
#        if self._immutable: self.err('openopt TSP instances are immutable, arguments should pass to constructor or solve()')
...
#            graph = nx.MultiDiGraph(graph) #if init_graph_is_directed else nx.MultiGraph(graph)
        #new
        if not init_graph_is_multigraph:
#            if init_graph_is_directed:
        ...
        '''                  Define node2index dict                  '''
        node2index = dict((node, i) for i, node in enumerate(nodes))
        ################################################################
        
        
        '''              Define edge_ind2x_ind_val dict              '''
        # used by interalg only, but present in encode() args
        edge_ind2x_ind_val = {}
        ################################################################
        ...
        # list with elems defining how many edges leads from a node[i]
        node_out_edges_num = []
...
#            raise 0
            ...
            #TODO: CHECK THE CONDITION(S)
#            if init_graph_is_multigraph and not isMOP\
#            and type(mainCr) in [str, np.str_]:

            # TODO: (fix it!)
            # handle isMOP here
            if not isMOP and init_graph_is_multigraph\
            and type(mainCr) in (str, np.str_):
                
                # This part of code removes edges 
                # except of the cheapest one
                # (graph may contain several edges for a pair of nodes {node_i, node_j})
                
                # TODO: check why type(mainCr) in [str, np.str_] cond was added, mb rework/remove it
...
#                raise 0
                ...
                # TODO: MB replace hstack by other func
        ...
        if interalg_gdp: 
            ...
            # TODO: mb remove it when proper enum implementation in FD engine will be done
            for i in range(n):
...
#        if init_graph_is_directed:
        ...
        for node, edges_inds in dictTo.items():
            ...
            if interalg_gdp:
                x_inds, local_edge_nums = [], []
#                print('----=======-----')
                for elem in edges_inds:
                    x_ind, local_edge_num = edge_ind2x_ind_val[elem]
                    x_inds.append(x_ind)
                    local_edge_nums.append(local_edge_num)
#                    print(x_ind, x_val)
                
                c = engine((x[x_ind] == local_edge_num)(tol = 0.5) \
                for x_ind, local_edge_num in zip(x_inds, local_edge_nums))
...
#                raise 0
        ...
        for optCrName in usedValues:
            ...
            if len(tmp) == 0:
                ...
                self.err("for optimization creterion '%s' at least one edge doesn't have this attribute" % optCrName)
            
            if interalg_gdp:
                F = []
                lc = 0
                for v in x:
                    domain = v._init_domain
                    vals = [tmp[i] for i in range(lc, lc + domain.size)]
                    lc += domain.size
                    #F = sum(x)
                    #F.append(fd.interpolator(domain, vals, k=1, s=0.00000001)(v))
                    
                    # FIX it!
                    #F.append(fd.interpolator(domain, vals, k=1)(v))
                    F.append(fd.interpolator(domain, vals, k=1)(v))
                    #F.append(v[vals])
        ...
        for param in ('start', 'returnToStart'):
            KW.pop(param, None)
        p.finish = lambda r, p: encode(P, p, r, oo, interalg_gdp, edge_ind2x_ind_val, n, x, EdgesCoords, 
           EdgesDescriptors, m, self)
...
def encode(P, p, r, oo, interalg_gdp, edge_ind2x_ind_val, n, x, EdgesCoords, 
           EdgesDescriptors, m, self):
    if P != oo.MOP:
        r.ff = p.ff
        if interalg_gdp:
