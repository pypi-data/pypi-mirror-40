...
#                yy2 = yy2.tolist()+[yy2[-1]]
            if isNewTrajectory:
                ...
                if ind==0:
                    if isfinite(p.ylim[0]): pylab.plot([xx[0]],  [p.ylim[0]],  
                    color='w')
                    if isfinite(p.ylim[1]): pylab.plot([xx[0]],  [p.ylim[1]],  
                    color='w')
                    if p.probType in ('NLSP', 'SNLE'): 
                        pylab.plot([xx[0]], [p.ftol / self.REDUCE],  color='w')
            if ind == 1:
                horz_line_value = 1 if p.useScaledResidualOutput else p.primalConTol
                pylab.plot([xx[0], xx[-1]], [horz_line_value, horz_line_value], 
                ls = self.axLineStyle, linewidth = self.axLineWidth, color='g',
                marker = self.axMarker, ms = self.axMarkerSize, 
                mew = self.markerEdgeWidth, mec = self.axMarkerEdgeColor, 
                mfc = self.axFaceColor)
            elif p.probType in ('NLSP', 'SNLE'):
                pylab.plot([xx[0], xx[-1]], [p.ftol, p.ftol], ls = self.axLineStyle, 
                linewidth = self.axLineWidth, color='g',
                marker = self.axMarker, ms = self.axMarkerSize, 
                mew = self.markerEdgeWidth, mec = self.axMarkerEdgeColor, 
                mfc = self.axFaceColor)
            if isNewTrajectory:
                pylab.plot([xx[0]], [yy2[0]], color = color, 
                marker = self.specifierStart,  markersize = self.markerSize)
                p3 = pylab.plot([xx[0], xx[0]+1e-50], [yy2[0], yy2[0]], 
                color = color, markersize = self.markerSize)
                p._handler_for_legend = p3
                if p.legend == '': 
                    ...
                    pylab.legend([p3[0]], [p.legend], shadow = True)
                pylab.plot(xx[1:], yy2[1:], color,  marker = usualMarker, 
                markersize = self.markerSize//3)
            else:
                pylab.plot(xx, ravel(yy2), color + specifier, marker = usualMarker, markersize = self.iterMarkerSize)

            if p.isFinished:
                pylab.legend([p._handler_for_legend[0]], [p.solver.__name__], shadow = True, loc=0)
                #xMin, xMax = [], []
                if p.istop<0:
                ...
                if ymax - ymin > 25 * (yy2[-1] - ymin):
                    if ind == 0:
                        ...
                        bounds_y = [ymin-delta, ymax+delta]
                    elif ind == 1:
                        bounds_y = [ymin/0.8, ymax*1.5]
                    else:
                        assert 0, 'implemented for obj and cons only for now'

                    pylab.scatter([(xmin+xmax)/2,  (xmin+xmax)/2], 
                    bounds_y, 
                    s=1, c='w', edgecolors='none',  marker='o')
#                    print('>>>>', [ymin-delta, ymax+delta])
#                    pylab.draw()
