...
class baseProblem(oomatrix, residuals, ooTextOutput):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #used in interalg
        self._lin_ineq_ids = dict()
        self._lin_eq_ids = dict()
        
        if self.allowedGoals is not None: # None in EIG
            if 'min' in self.allowedGoals:
                self.minimize = lambda *args, **kwargs: \
                minimize(self, *args, **kwargs)
            if 'max' in self.allowedGoals:
                self.maximize = lambda *args, **kwargs: \
...
#            maxViolation = kwargs['maxViolation']
        maxViolation = kwargs.get('maxViolation', 0.01)
