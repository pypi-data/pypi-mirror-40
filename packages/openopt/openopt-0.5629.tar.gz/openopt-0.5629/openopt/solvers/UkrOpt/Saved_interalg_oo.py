...
class interalg(baseSolver):
    ...
    def __solver__(self, p):
        ...
        if isMOP:
            from interalgMOP import processBoxesMOP
...
#            p.err("the solver can't handle problems of type " + p.probType)
        ...
        ooVars = list(p._freeVarsList)
        
        #TODO: create it in openopt kernel, not in interalg kernel
        p._freeVarsDict = dict((v, i) for i, v in enumerate(ooVars))
        
        
        x0 = dict((v, p._x0[v]) for v in ooVars)
        domain = oopoint(((v, [atleast_1d(p.lb[i]), atleast_1d(p.ub[i])]) for i,  v in enumerate(ooVars)), skipArrayCast = True)
...
#                could speedup the solver %s''' % self.__name__)
        ...
        if not isIP and not isODE:
            from interalgLLR import adjustCentersWithDiscreteVariables
            adjustCentersWithDiscreteVariables(xRecord.reshape(1, -1), p)

        bestKnownObjValueWithPoint = np.inf
        
        Lx = lb.reshape(1, -1)
        Ux = ub.reshape(1, -1)
        bestKnownObjValueWithoutPoint = np.inf
        ...
        if isSNLE:
            bestKnownObjValueWithoutPoint = 0.0
#            fd_obj = None
            eqs = [fd_abs(elem) for elem in p.user.f]
            fd_obj = fd_sum(eqs)
            
            # temporary change, TODO: rework it
            fd_obj.resolveSchedule = fd_obj._getDep()
            p._objDep = fd_obj.Dep & p.freeVarsSet
            ...
            fd_obj = p.user.f
            p._objDep = set()
            p._objDep.update(*[_f.Dep for _f in fd_obj])
            ...
            if fStart is not None and fStart < bestKnownObjValueWithPoint: 
                bestKnownObjValueWithoutPoint = fStart
                
            for X0 in [point(xRecord), point(p.x0)]:
                if X0.isFeas(altLinInEq=False) and X0.f() < bestKnownObjValueWithPoint:
                    bestKnownObjValueWithPoint = X0.f()

            if p.isFeas(p.x0):
                tmp = fd_obj(p._x0)
                if  tmp < bestKnownObjValueWithoutPoint:
                    bestKnownObjValueWithoutPoint = tmp
                
            if p.fOpt is not None:
                if p.fOpt > bestKnownObjValueWithoutPoint:
                    p.warn('user-provided fOpt seems to be incorrect, ')
                bestKnownObjValueWithoutPoint = p.fOpt
        else:# ODE
            # TODO: implement p._objDep = fd_obj.Dep & p.freeVarsSet here
...
#            self.dataHandling ='sorted'
        ...
        if self.dataHandling == 'auto':
            if isIP or isODE:
                ...
                if fd_obj.isUncycled and p._isOnlyBoxBounded and np.all(np.isfinite(p.user.f[0].interval(domain).lb)):
                    #maxNodes = 1
                    self.dataHandling = 'sorted'
                if p.iprint >= 0:
...
#        if not use_saved:
        ...
        # TODO: further develoopment (add triggering cons data while point/centers computation)
...
#            p.iprint=1
            ...
            if len(C0) != 0: # SNLE also can have constraints
                if use_saved:
                    Lx, Ux, nlhc, residual, definiteRange, indT, \
                    semiinvariant =\
                    processConstraints(_C0, Lx, Ux, semiinvariant, p, dataType)
                else:
                    Lx, Ux, nlhc, residual, definiteRange, indT, \
                    semiinvariant, activeCons =\
                    processConstraints(C, C0, Lx, Ux, semiinvariant, inactiveNodes, p, dataType, activeCons)
            else:
                residual, definiteRange, indT = None, True, None
                nlhc = np.empty((Lx.shape[0], 2*Lx.shape[1]), np.float64)
...
#            print('-', p.iter, hasPoint(Lx, Ux, a), pointInd(Lx, Ux, a))

            assert len(activeCons) == semiinvariant.size == Lx.shape[0]
            
            if Lx.size != 0:
                AllNodes, cutLevel, threshold, semiinvariant, Solutions, xRecord, bestKnownObjValueWithoutPoint, bestKnownObjValueWithPoint = \
                pb(p, nlhc, residual, definiteRange, Lx, Ux, ooVars, fd_obj, C, bestKnownObjValueWithPoint, cutLevel, \
                             nNodes, bestKnownObjValueWithoutPoint, fTol, Solutions, varTols, inactiveNodes, \
                             dataType, maxNodes, semiinvariant, indT, xRecord, activeCons)
                if semiinvariant is None:
                    break 
                
            else:
                AllNodes = inactiveNodes
                # TODO: mb remove threshold recalculation from here
                threshold = 0.0 if isSNLE or isMOP else \
                PythonMin(bestKnownObjValueWithoutPoint, \
                          bestKnownObjValueWithPoint - (fTol if Solutions.maxNum == 1 else 0.0))
            # TODO: check it                            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            
            peak_nodes_number = PythonMax(peak_nodes_number, 
                                    len(np.atleast_1d(AllNodes) if type(AllNodes) == np.ndarray else AllNodes))
            
#            assert len(activeCons) == semiinvariant.size == Lx.shape[0]
            
            
            Lx, Ux, inactiveNodes, semiinvariant, activeCons = \
                splitNodes(AllNodes, self.maxActiveNodes, p, Solutions, ooVars, varTols, np.inf if isIP else threshold)
            
            nActiveNodes.append(Lx.shape[0]//2)
            if Lx.size == 0: 
        ...
        if not isSNLE and not isIP and not isMOP:
            ...
            if isscalar(bestKnownObjValueWithPoint):
                p.extras['bestKnownObjValueWithPoint'] = bestKnownObjValueWithPoint
            if isscalar(bestKnownObjValueWithoutPoint):
                p.extras['bestKnownObjValueWithoutPoint'] = bestKnownObjValueWithoutPoint
            if isscalar(cutLevel):
                p.extras['cutLevel'] = cutLevel
...
#        p.xk = p._bestPoint.x
        ...
        if isSNLE or isMOP:
            ...
            lf = array([t.key for t in AllNodes])#.flatten()
            if lf.size != 0:
                cutLevel = nanmin((nanmin(lf), cutLevel))

        if not isMOP:
            AbsVal = PythonMin(abs(ff), abs(cutLevel))
            cond_rTol = ff - cutLevel <= p.rTol * AbsVal
            rtol_accuracy = (ff - cutLevel) / AbsVal 
            if AbsVal == 0: rtol_accuracy = 100.0 
            
            cond_fTol = ff - cutLevel < fTol
            
            p.extras['isRequiredPrecisionReached'] = \
            True if(cond_fTol or cond_rTol) and isFeas else False
            # and (PointsLeft is False or (isSNLE and (p._nObtainedSolutions >= maxSolutions or maxSolutions==1))) 
        ...
        # TODO: simplify it
        if not isMOP:
            tmp = [nanmin(np.hstack((ff, cutLevel, lf))), np.asscalar(np.array(ff))]
