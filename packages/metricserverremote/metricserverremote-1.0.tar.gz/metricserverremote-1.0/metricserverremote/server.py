...
class MetricServer:
    ...
    def create_service(self, type):
        ...
        client = zeep.Client(wsdl=wsdlpath, transport=zeep.Transport(timeout=10))
        return self.get_service(client=client)

    def logon(self, server, user, password):
        self.remoteServer = server
        self.userservice = self.create_service('users')
        self.licenseservice = self.create_service('license')
        self.tagsservice = self.create_service('tags')
        self.resultservice = self.create_service('result')
        self.processservice = self.create_service('processing')
        self.metricsservice = self.create_service('metrics')
        self.outputservice = self.create_service('outputconfiguration')
        self.userName = user
