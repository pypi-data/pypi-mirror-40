...
@click.option('-p', '--project_uid', type=str, help='What interval the timer sends a request')
@click.option('-a', '--project_account_uid', type=str, help='What interval the timer sends a request')
...
@click.pass_context
@login_required
def timer_sync_output(ctx, job_uid, project_uid, project_account_uid, delay, verbose):
    if delay is None:
        delay = 5.0
    ...
    path = '/onepanel/.onepanel/jobs/{}'.format(job_uid)
    ...
    project_vc.project_account_uid = project_account_uid
    project_vc.project_uid = project_uid
    path = os.getcwd()
    ...
    def onepanel_push_wrapper():
        ...
        jobs.upload_output_with_parameters(ctx, job_uid, project_vc, project_account_uid)
