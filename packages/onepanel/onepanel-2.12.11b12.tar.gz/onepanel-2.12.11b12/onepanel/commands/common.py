...
@click.option('-t', '--path', type=str, help='Path to sync files from')
@click.option('-v', '--verbose', type=bool, help='If command should log what it is doing')
@click.pass_context
@login_required
def timer_sync_output(ctx, job_uid, project_uid, project_account_uid, path, delay, verbose):
    if delay is None:
        delay = 5.0
    ...
    def onepanel_push_wrapper():
        ...
        jobs.upload_output_with_parameters(ctx, job_uid, project_uid, project_account_uid, source_directory=path)
