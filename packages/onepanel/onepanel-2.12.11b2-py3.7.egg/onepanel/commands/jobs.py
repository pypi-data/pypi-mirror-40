...
@jobs.command('timer-sync-output', help='Syncs job output and logs repeated on a timer', hidden=True)
@click.argument('path', type=str)
@click.option('-d', '--delay', type=float, help='What interval the timer sends a request')
@click.option('-v', '--verbose', type=bool, help='If command should log what it is doing')
@click.pass_context
@login_required
def timer_sync_output(ctx, path, delay, verbose):
    if delay is None:
        delay = 5.0
    ...
    def onepanel_push_wrapper():
        if not project_vc.exists_local(path):
        ...
        logging.getLogger(JOB_LOGGER).info('Doing command')
        subprocess.Popen(['onepanel', 'push'], cwd=path, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
