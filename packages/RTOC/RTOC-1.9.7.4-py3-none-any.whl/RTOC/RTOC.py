...
if os.name == 'nt':
    import ctypes
    myappid = 'RTOC.1.9.7.4'  # arbitrary string
...
class RTOC(QtWidgets.QMainWindow, Actions):
    ...
    def initDeviceWidget(self):
        ...
        for i in reversed(range(self.deviceLayout.count())):
            if not self.deviceLayout.itemAt(i).widget().isChecked():
        ...
        if self.systemTrayAction.isChecked() and not self.forceQuit:
            ...
            if ok is not None:
                if ok == True:
                    ...
                    self.logger.exportJSON(self.config['documentfolder']+"/restore.json", scripts, True)
                elif ok == False:
