...
def prepare_platform(prep_env):
    ...
    subprocess.Popen(['Xvfb', ':1', '-screen', '0', '1025x768x16'], env=prep_env)
    time.sleep(5)
    logger.info('starting dbus')
    subprocess.Popen(['/etc/init.d/dbus', 'start'], env=prep_env)
    time.sleep(5)
    logger.info('starting pulseaudio')
    subprocess.Popen(['pulseaudio'], env=prep_env)
    time.sleep(5)
