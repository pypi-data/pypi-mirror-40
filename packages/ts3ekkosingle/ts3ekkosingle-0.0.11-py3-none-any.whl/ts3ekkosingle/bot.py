...
try:
    ...
    from ts3ekkosingle.models import Invoker, startup
    from ts3ekkosingle.permission import InvokerCtx
    ...
    from .permission import InvokerCtx
...
class EkkoBot(TS3Ekko):
    ...
    def _can(self, action: str, context: InvokerCtx) -> bool:
        """
    ...
    def can(self, permission: str, event: ts3.response.TS3Event, quiet=False,
        ...
        if not self._can(permission, invoker_ctx):
    ...
    def determine_invoker(self, event):
        ...
        """
        return Invoker(event['invokeruid'], event['invokername'])
