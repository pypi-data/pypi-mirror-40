...
class MockConfigReader(ConfigReader):
    def _check_config(self):
        try:
            ...
            raise Exception("No string_field in configuration")

        try:
            self.__list_field = self.configuration["list_field"]
        except KeyError:
            raise Exception("No list_field in configuration")

        try:
            self.__dictionary_field = self.configuration["dictionary_field"]
        except KeyError:
            raise Exception("No directory_field in configuration")
        ...
        try:
            ...
            raise Exception("no key2 key in dictionary_field")
