...
class Bucket:
    ...
    def _get(self, key=""):
        ...
        try:
            ...
            if code == "AccessDenied" and err_msg == "Access Denied":
                msg = f"Received access denied when trying to access S3 object: s3://{self.bucket_name}/{k}"
    ...
    def _get_latest_ver_of_obj(self, key=""):
        ...
        # print(obj)
        if not obj:
            msg = f"Latest version of object was not found for s3://{self.bucket_name}/{key}"
            raise ClientError(msg)
        ...
        try:
            ...
            code = err["Code"]
            err_msg = err["Message"]
            # TODO: Convert to logger.error print(code, err_msg)
            if (
