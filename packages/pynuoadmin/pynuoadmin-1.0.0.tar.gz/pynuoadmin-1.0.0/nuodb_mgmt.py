...
def _get_authorized_session(address, db_password, service, timeout=5.0):
    # `address` is of the form [<hostname>/]<address>:<port>, if hostname and
    # address differ
    address = address.split('/')[-1]
    session = Session(address, service=service, timeout=timeout)
...
def disable_ssl_warnings():
    try:
        if hasattr(requests.packages, 'urllib3'):
            ...
            import urllib3
        # suppress SubjectAltNameWarning: Certificate for localhost has no `subjectAltName`  # noqa
        urllib3.disable_warnings(urllib3.exceptions.SubjectAltNameWarning)
        # suppress InsecureRequestWarning: Unverified HTTPS request is being made. Adding certificate verification is strongly advised.  # noqa
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    except Exception:
        pass


# unless NUOCMD_DISABLE_SSL_WARNINGS=0 is explicitly set, disable SSL warnings
if os.environ.get('NUOCMD_DISABLE_SSL_WARNINGS') != '0':
...
class AdminConnection(object):
    ...
    def __init__(self, url_base, client_key=None, server_cert=None,
                 response_timeout=20):
        """  # noqa
        ...
        :param server_cert: the path to the server's TLS certificate to verify
        """
        ...
        self.server_cert = server_cert if server_cert is not None else False
    ...
    def _send_request(self, method, *resource_path, **kwargs):
        ...
        if self.server_cert is not None:
            request_kwargs['verify'] = self.server_cert
    ...
    def _get(self, *resource_path, **kwargs):
        return self._send_request(requests.get, *resource_path, **kwargs)

    def _post(self, *resource_path, **kwargs):
        return self._send_request(requests.post, *resource_path, **kwargs)

    def _put(self, *resource_path, **kwargs):
        return self._send_request(requests.put, *resource_path, **kwargs)

    def _delete(self, *resource_path, **kwargs):
        return self._send_request(requests.delete, *resource_path, **kwargs)
    ...
    def hotcopy_database(self, db_name, type, shared=False,
        ...
        resp = self._post('databases', db_name, 'hotCopy', json=payload,
                          timeout=response_timeout)
        self._raise_for_status(resp)
        return HotCopyResponse(resp.json())
    ...
    def get_admin_config(self, server_id=None):
        ...
        resp = self._get(*resource_path)
        self._raise_for_status(resp)
        return AdminServerConfig(resp.json())
    ...
    def _get_db_password(self, db_name):
        ...
        self._raise_for_status(resp)
        return resp.json().get('dbPassword')
