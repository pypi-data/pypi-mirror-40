...
class CommandProcessor(object):
    ...
    def _get_admin_conn(cls, parsed_args):
        ...
        if (not api_server.startswith('http://') and
            ...
            if (client_key is None and server_cert is None and
                'NUOCMD_CLIENT_KEY' not in os.environ and  # noqa
                'NUOCMD_SERVER_CERT' not in os.environ):  # noqa
        ...
        def func(prefix, parsed_args, **kwargs):
            try:
                ...
                if tokens is not None and len(tokens) % 2 == 1:
                    ...
                    return value_completer(prefix, parsed_args,
                                           **kwargs)
                # use key completer if even
    ...
    def __init__(self):
        ...
        self._add_argument('--server-cert', required=False,
                           default=EnvironmentalDefault(),
                           help='server certificate to verify when using HTTPS')  # noqa
        ...
        if 'func' in args:
            ...
            kwargs = dict((k, v) for k, v in args._get_kwargs()
                          if k not in ['api_server', 'show_json',
                                       'show_json_fields', 'debug',
                                       'func', 'client_key', 'server_cert'])
            try:
...
class AdminCommands(object):
    ...
    COMMON_PROCESS_FMT = '[{engine_type}] {address::<UNKNOWN ADDRESS>} ({region_name}) [start_id = {start_id}] [server_id = {server_id}] [pid = {pid::}] [node_id = {node_id::}]'  # noqa
    ...
    def show_archives(self, db_name=None, removed=False,
        ...
        for archive in archives:
            ...
            running = self.conn.get_processes(db_name=db_name,
            ...
            if len(running) != 0:
                ...
                # last one
                exited = self.conn.get_exited_processes(
                    db_name=db_name, **{'process.archiveId': str(archive.id)})
    ...
    def get_core_file(self, start_id, output=CORE_FILE_OUTPUT_FMT,
        ...
        stream = self.conn.stream_core_file(start_id)
        filename = get_formatted(output, process)
        if output_dir is not None:
            filename = os.path.join(output_dir, filename)
        self._download_core_file(stream, filename)

    def _download_core_file(self, stream, filename):
    ...
    def _get_admin_diagnose_info(self, server, admin_dir):
        ...
        conn = nuodb_mgmt.AdminConnection(
            url_base, self.conn.client_key, self.conn.server_cert)
    ...
    def _get_engine_diagnose_info(self, process, engine_dir, descriptor,
                                  include_metadata, include_cores):
        ...
        if include_cores:
            # download core file
            stream = self.conn.stream_core_file(process.start_id)
            self._print('Downloading core file for {} ...', descriptor)
            self._download_core_file(stream,
                                     os.path.join(directory, 'core.gz'))
    ...
    DEFAULT_DNAME = 'CN={}, OU=Eng, O=NuoDB, L=Boston, ST=MA, C=US'.format(HOSTNAME)  # noqa
    ...
    @Argument('--sub-altnames', nargs='*',
              help='list of subjectAltNames for the certificate')
    @Argument('--resolve-san', action='store_true',
              help='whether to resolve subjectAltName using DNS')
    @Argument('--algorithm', default='RSA', choices=['RSA', 'DSA'],
    ...
    def create_keypair(self, keystore, store_password, key_password=None,
        ...
        if sub_altnames is not None:
            for san in sub_altnames:
                args += ['-ext', 'san=' + san]
        if resolve_san:
            args += ['-ext', 'san=dns:' + self.HOSTNAME]
        if ca:
            args += ['-ext', 'bc:c']
    ...
    def _get_signed_cert(self, csr, ca_keystore, ca_store_password,
                         ca_store_type=None, ca_alias=None):
        ...
        return self._check_output(args, csr)
    ...
    @Subcommand('sign', 'certificate',
                help='sign a certificate in a keystore with a root CA certificate')  # noqa
    @Argument('--keystore', required=True,
    ...
    @Argument('--ca-keystore', required=True,
              help='the keystore contaiing the root CA certificate')
