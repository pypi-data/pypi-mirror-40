...
class simulation:
    def __init__(self,u,duration,SNpath,vnrpath,vnrnumber):  #5/100 unit time
        self.resource_set=""
        self.VNRS = []
        self.sn = []
        self.timeframe = []
        ...
        self.suipianrate = []
        self.expdistr = np.around(np.random.exponential(1/u, 2000) * 100) + 1
    ...
    def simulation_run(self,mapfunction_handle,functionname_str):
        ...
        print("total time unit:", sum(self.expdistr))
        vn = len(self.VNRS)
        self.VNRS[0].time = 0
    ...
    def savevarible(self):
        pickle.dump(self.sn, open('.\\sn\\sn.data', 'wb'))
        pickle.dump(self.VNRS, open('.\\sn\\vnrs.data', 'wb'))
    def loadvarible(self):
        self.sn=pickle.load(open('.\\sn\\sn.data', 'rb'))
        self.VNRS=pickle.load(open('.\\sn\\vnrs.data', 'rb'))
    def changedistrbution(self,scpu=[],sbanwidth=[],vcpu=[],vbandwidth=[]):
        self.resource_set ="scpu:"+str(scpu)+" sbw:"+str(sbanwidth)+" vcpu:"+str(vcpu)+" vbw:"+str(vbandwidth)
        self.sn.changedistribution(scpu,sbanwidth)
        n=len(self.VNRS)
        for i in range(n):
