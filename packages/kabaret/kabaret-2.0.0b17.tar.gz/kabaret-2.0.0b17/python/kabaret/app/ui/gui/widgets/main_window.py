...
class MainWindowManager(object):
    ...
    def register_view_type(self, Type):
        vtn = Type.view_type_name()
        self._view_types[vtn] = Type
        return vtn

    def _create_dock_widget(self, dock_closed_callback, title, parent, view):
        dock = DockWidget(dock_closed_callback, title, parent)
    ...
    def add_view(self, type_name, area=None, visible=True, *view_args, **view_kwargs):
        try:
            Type = self._view_types[type_name]
        except:
            raise ValueError('Unknown view type %r (has %r)' %
                             (type_name, self._view_types.keys()))

        view = Type(self.session, self, self.main_window,
                    *view_args, **view_kwargs)

        dock = self._create_dock_widget(self.dock_closed, view.view_title(), self.main_window, view)
        ...
        tab_target = self.find_view(type_name, create=False, area=area)

        self.main_window.addDockWidget(area, dock)

        if tab_target is not None:
            self.main_window.tabifyDockWidget(tab_target.dock_widget(), dock)
