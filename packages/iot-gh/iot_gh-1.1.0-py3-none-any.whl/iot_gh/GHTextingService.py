...
class GHTextingService(threading.Thread):
    #GH_COMMANDS = ["about", "help", "help_verbose", "o", "open", "c", "close", "f", "fan", "l", "lamp", "b", "beep"]
    ...
    def _get_next_commands(self):
        commands = None
        params = {"token": self._access_token}
        params["after_id"] = self._last_message_id  
        headers = {"content-type": "application/json"}
        end_point = "/groups/%s/messages" % self._group_id
        url = "%s%s" % (self.BASE_URL, end_point)
        r = requests.get(url, headers=headers, params=params)
        if r.status_code != 200:
            ...
            count = len(messages)
            if count > 0:
                self._last_message_id = messages[count-1]["id"]
    ...
    def _execute_commands(self, commands):
        ...
        if self._valid_commands(cmds):
            ...
            #if "about" in cmds:
            #    m = "The IoT Greenhouse™ is a learning system developed to teach the " \
            #        "fundamentals of Python programming and Internet of Things (IoT). " \
            #        "It removes barriers of existing IoT training solutions by abstracting " \
            #        "both hardware and software components. Students interact with hardware " \
            #        "without challenges that building prototype circuits create. Additionally, " \
            #        "students quickly create robust Python code to control the system using " \
            #        "APIs provided by the IoT Greenhouse service software."
            #    self._send_message(m)
            #elif "help" in cmds:
            #    self._send_message("Valid IoT Greenhouse commands are: #%s" % " #".join(self.GH_COMMANDS ))
            #elif "help_verbose" in cmds:
            #    m = "Valid IoT Greenhouse commands are:\n\n" \
            #        "#about - provides an overview of the IoT Greenhouse™ system.\n\n" \
            #        "#help - lists valid IoT Greenhouse commands.\n\n" \
            #        "#help_verbose - detailed help on using the IoT Greenhouse GroupMe command interface.\n\n" \
            #        "#o or #open - open greenhouse louvers.\n\n" \
            #        "#c or #close - close greenhosue louvers.\n\n" \
            #        "#f or #fan - toggle fan on and off.\n\n" \
            #        "#l or #lamp - toggle white lamp on and off.\n\n"  \
            #        "#b or #beep - sound beep.\n"
            #    self._send_message(m)
            #else:
            #    if "o"  in cmds or "open" in cmds:
            #        self._gh_service.servo.move(1)
            #        self._send_message("Louvers opened.")
            #    elif "c"  in cmds or "close" in cmds:
            #        self._gh_service.servo.move(0)
            #        self._send_message("Louvers closed.")
            #    if "f" in cmds or "fan" in cmds:
            #        self._gh_service.fan.toggle()
            #        self._send_message("Fan is %s." % self._gh_service.fan.get_status())
            #    if "l" in cmds or "lamp" in cmds:
            #        self._gh_service.lamps.white.toggle()
            #        self._send_message("Lamp is %s." % self.lamps.white.get_status())
            #    if "b" in cmds or "beep" in cmds:
            #        self._gh_service.buzzer.beep()
            #        self._send_message("Buzzer was beeped.")
...
if __name__ == "__main__":
    ...
    gms = GHTextingService(ACCESS_TOKEN, ghs)
