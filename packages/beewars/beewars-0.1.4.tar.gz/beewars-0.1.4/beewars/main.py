...
for i in sys.path:
    print(i)
    if os.path.exists(i+"/beewars/"):
        fpref = i+"/beewars/"
        break
fpref += "sym/"
print(fpref)
HEX = pygame.image.load(fpref+"hex.png")
...
for key in FILENAMES:
    try:
        im = pygame.image.load(fpref+"hex.png")
        img = pygame.image.load(fpref+FILENAMES[key])
...
def draw():
    ...
    for i in range(len(board)):
        ...
        if board[i]>=0:
            filename = fpref+FILENAMES[board[i]]
...
def draw_scores():
    global board, disp
    pygame.font.init()
    fontname = fpref+"../assets/digital-7.mono.ttf"
