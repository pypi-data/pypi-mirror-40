...
HEX = get_pygame_image(map_code_to_image(0))
...
i = 0
for key in (1,2,3,10,11,12,13):
    try:
        im = get_pygame_image(map_code_to_image(0))
        img = get_pygame_image(map_code_to_image(key))
...
def draw():
    ...
    for i in range(len(board)):
        ...
        if board[i]>=0:
            im = get_pygame_image(map_code_to_image(board[i]))
