...
def deploy(ctx, config_file, env, region):
    ...
    # spinner.start clobbers user input, the prompt for JWT token stuff
    spinner.start("Uploading artifact to deployment service ...")
    #    print("Uploading artifact to deployment service ...")
    ...
    #     print("Examining file system in df deploy command")
    #     os.system("ls -alh")
    #     os.system("pwd")
    #     os.system("cat ./df.config")
    ...
    response = c.auth_post(murl, data = json.dumps(post), headers={'Content-Type':'application/json'})
    ### auth_post throws an exception on non 200 responses
