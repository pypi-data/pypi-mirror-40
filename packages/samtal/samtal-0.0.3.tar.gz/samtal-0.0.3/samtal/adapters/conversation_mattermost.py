...
class MattermostConversation(Conversation):
    @property
    def to_user_id(self):
        return self.to.mattermost.split('/')[-1]

    def _create_channel(self):
        rv = self.driver.channels.create_direct_message_channel([self.user_id, self.to_user_id])
        return rv['id']
    ...
    def make_message(self, post: dict) -> Message:
        is_bluepill = post['user_id'] == self.user_id
