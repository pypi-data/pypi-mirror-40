...
from samtal.core.test_helpers import thomas_dupond, sample_questions, question_review
...
def test_should_ask_a_question():
    ...
    bot = Bot(AnswerRepositoryMock(), conversation_provider)
...
def test_should_read_another_answer():
    question = Question("devops", "code health", "code review", "Team does pair programming")
    answer_repository = AnswerRepositoryMock()
    bot = Bot(answer_repository)
...
def test_sould_run_the_bot_for_one_iteration():
    repository = AnswerRepositoryMock()
...
def test_sould_run_the_bot_longer():
    repository = AnswerRepositoryMock()
    bot = Bot(repository, AnsweringConversationProviderMock(), members=sample_members(), questions=sample_questions())
...
def test_sould_run_the_bot_for_two_questions():
    repository = AnswerRepositoryMock()
