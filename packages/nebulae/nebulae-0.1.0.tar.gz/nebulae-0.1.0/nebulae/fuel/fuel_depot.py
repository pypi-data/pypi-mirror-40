...
class FuelDepot(object):
    ...
    def loadFuel(self, config=None, name=None, batch_size=None, if_shuffle=True,
                 ...
                 height=0, width=0, channel=3, frame=0, resol_ratio=1,
        ...
        if not os.path.exists(data_path):
            raise ValueError('There is no such file or directory named %s.' % data_path)
    ...
    def milesPerEpoch(self, name):
        return ceil(self.dataset[name].nsample/self.dataset[name].param['batch_size'])

    def tankVolume(self, name):
        return self.dataset[name].nsample
