...
class FuelGenerator(object):
    ...
    def __init__(self, config=None, file_dir=None, file_list=None, dtype=None,
                 height=0, width=0, channel=1, encode='jpeg', is_seq=False):
        self.param = {}
        self.data = {}
        ...
        if config:
            ...
            self.param['is_seq'] = config.get('is_seq', encode)
        else:
                    ...
                    for k, key in enumerate(self.info_keys):
                        if k == 0: # dealing with raw data
                            if self.param['is_seq']:
                                csl = line[k].split(',')  # comma separated line
                                max_frames = len(csl) if len(csl)>max_frames else max_frames
                                temp_data = []
                                for f in csl:
                                    temp_data.append(self._preProcess(os.path.join(self.param['file_dir'], f)))
                                self.data[key].append(temp_data)
                            else:
